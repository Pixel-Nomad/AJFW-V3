...
local function LoadPhone()
    ...
    AJFW.Functions.TriggerCallback('aj-phone:server:GetPhoneData', function(pData)
        ...
        PhoneData.PlayerData = AJFW.Functions.GetPlayerData()
...
local function OpenPhone()
    AJFW.Functions.TriggerCallback('aj-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            PhoneData.PlayerData = AJFW.Functions.GetPlayerData()
            ...
            AJFW.Functions.TriggerCallback('aj-garages:server:GetPlayerVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            AJFW.Functions.Notify("You don't have a phone", 'error')
        end
    end)
end
...
local function CancelCall()
    TriggerServerEvent('aj-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('aj-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('aj-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('aj-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('aj-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNUICallback('ClearRecentAlerts', function(_, cb)
    TriggerServerEvent('aj-phone:server:SetPhoneAlerts', 'phone', 0)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('aj-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('aj-phone:server:RemoveMail', MailId)
    cb('ok')
end)
...
RegisterNUICallback('AcceptMailButton', function(data, cb)
    ...
    TriggerServerEvent('aj-phone:server:ClearButtonData', data.mailId)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('aj-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    AJFW.Functions.TriggerCallback('aj-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('PostAdvert', function(data, cb)
    TriggerServerEvent('aj-phone:server:AddAdvert', data.message, data.url)
    cb('ok')
end)

RegisterNUICallback('DeleteAdvert', function(_, cb)
    TriggerServerEvent('aj-phone:server:DeleteAdvert')
    cb('ok')
end)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('aj-phone:server:SetPhoneAlerts', 'whatsapp', newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    AJFW.Functions.TriggerCallback('aj-phone:server:PayInvoice', function(CanPay, Invoices)
        if CanPay then PhoneData.Invoices = Invoices end
        cb(CanPay)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('aj-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    local amount = data.amount
    local invoiceId = data.invoiceId
    AJFW.Functions.TriggerCallback('aj-phone:server:DeclineInvoice', function(_, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
    end, society, amount, invoiceId)
    TriggerServerEvent('aj-phone:server:BillingEmail', data, false)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('aj-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
    ...
    if #Hashtag <= 3 then
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                TriggerServerEvent('aj-phone:server:UpdateHashtags', Handle, TweetMessage)
            end
        end
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                if (Firstname ~= nil and Firstname ~= '') and (Lastname ~= nil and Lastname ~= '') then
                    if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                        TriggerServerEvent('aj-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                    end
                end
            end
        end
        ...
        TriggerServerEvent('aj-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
    else
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNUICallback('DeleteTweet', function(data, cb)
    TriggerServerEvent('aj-phone:server:DeleteTweet', data.id)
    cb('ok')
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('aj-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('aj-phone:server:RemoveInstallation', data.app)
    cb('ok')
end)

RegisterNUICallback('GetTruckerData', function(_, cb)
    local TruckerMeta = AJFW.Functions.GetPlayerData().metadata['jobrep']['trucker']
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('aj-phone:server:RemoveContact', Name, Number)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data, cb)
    ...
    AJFW.Functions.Notify('GPS Location set: ' .. data.alert.title)
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    ...
    AJFW.Functions.TriggerCallback('aj-phone:server:ScanPlate', function(result)
        AJFW.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            ...
            if AJFW.Shared.Vehicles[vehname] ~= nil then
                result.label = AJFW.Shared.Vehicles[vehname]['name']
            else
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            AJFW.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    AJFW.Functions.Notify('GPS has been set to ' .. data.HouseData.HouseData.adress .. '!', 'success')
    cb('ok')
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    AJFW.Functions.Notify('GPS has been set!', 'success')
    cb('ok')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(_, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('ClearMentions', function(_, cb)
    ...
    TriggerServerEvent('aj-phone:server:SetPhoneAlerts', 'twitter', 0)
    SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
    cb('ok')
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data, cb)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('aj-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
        cb('ok')
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('aj-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('GetWhatsappChats', function(_, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:GetCallState', function(CanCall, IsOnline, _)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('aj-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('aj-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    AJFW.Functions.TriggerCallback('aj-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = 'UpdateChat',
            chatData = Chat,
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            AJFW.Functions.TriggerCallback('aj-phone:server:GetWebhook', function(hook)
                if not hook then
                    AJFW.Functions.Notify('Camera not setup', 'error')
                    return
                end
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        AJFW.Functions.Notify('You need to input a Player ID', 'error')
    else
        TriggerServerEvent('aj-phone:server:sendPing', args[1])
    end
end, false)
...
RegisterNetEvent('aj-phone:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
    SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { title = 'AJank', text = '&#36;' .. amount .. ' has been added to your account!', icon = 'fas fa-university', color = '#8c7ae6', }, })
    SendNUIMessage({ action = 'UpdateBank', NewBalance = PhoneData.PlayerData.money.bank })
end)

-- RegisterNetEvent('aj-phone:client:UpdateTweetsDel', function(source, Tweets)
...
RegisterNetEvent('aj-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
    PhoneData.Tweets = Tweets
    local MyPlayerId = PhoneData.PlayerData.source
...
RegisterNetEvent('aj-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('aj-phone:server:SetPhoneAlerts', 'phone')
...
RegisterNetEvent('aj-phone-new:client:BankNotify', function(text)
    SendNUIMessage({
        action = 'PhoneNotification',
        NotifyData = {
            title = 'Bank',
...
RegisterNetEvent('aj-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = 'Mail',
    ...
    TriggerServerEvent('aj-phone:server:SetPhoneAlerts', 'mail')
end)

RegisterNetEvent('aj-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = 'UpdateMails',
        Mails = NewMails
    })
...
RegisterNetEvent('aj-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('aj-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
...
RegisterNetEvent('aj-phone:client:BillingEmail', function(data, paid, name)
    if paid then
...
RegisterNetEvent('aj-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == 'ongoing' then
        SendNUIMessage({
...
RegisterNetEvent('aj-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
...
RegisterNetEvent('aj-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            TriggerServerEvent('aj-phone:server:SetPhoneAlerts', 'whatsapp')
        end
    else
...
RegisterNetEvent('aj-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNetEvent('aj-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNetEvent('aj-phone:client:AddTransaction', function(_, _, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('aj-phone:server:AddTransaction', Data)
end)

RegisterNetEvent('aj-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications['phone'].Alerts = Config.PhoneApplications['phone'].Alerts + 1
    TriggerServerEvent('aj-phone:server:SetPhoneAlerts', 'phone', Config.PhoneApplications['phone'].Alerts)
end)

RegisterNetEvent('aj-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNetEvent('aj-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('aj-phone:client:CustomNotification', function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = title,
