...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://aj-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    AJ.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});
...
AJ.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = AJ.Phone.Functions.FormatTwitterMessage(clean);
...
$(document).on('click','#twt-delete-click',function(e){
    ...
    $.post('https://aj-phone/DeleteTweet', JSON.stringify({id: source}))
})
...
AJ.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    $('#tweet-new-url').val("");
    if (Tweets.length > 0) {
...
AJ.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        AJ.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://aj-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            AJ.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
AJ.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
AJ.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    AJ.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
