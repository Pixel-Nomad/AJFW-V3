        ...
        while true do
            ...
            if isDriver then
                if IsControlJustPressed(0, 155) then
                    ...
                    TriggerServerEvent('aj-mechanicjob:server:syncNitrousFlames', netId, true)
                    nitrousActive = true
                end
                ...
                if IsControlJustReleased(0, 155) or nitrousVehicles[plate].level <= 0 then
                    ...
                    TriggerServerEvent('aj-mechanicjob:server:syncNitrousFlames', netId, false)
                    if nitrousVehicles[plate].level <= 0 then
                        nitrousVehicles[plate].hasnitro = false
                        TriggerServerEvent('aj-mechanicjob:server:syncNitrous', plate, false)
                        ...
                        TriggerServerEvent('aj-mechanicjob:server:syncNitrous', plate, true, nitrousVehicles[plate].level)
                    end
                end
            else
                plate = nil
...
AddEventHandler('gameEventTriggered', function(event)
    if event == 'CEventNetworkPlayerEnteredVehicle' then
        ...
        AJFW.Functions.TriggerCallback('aj-mechanicjob:server:getnitrousVehicles', function(vehs)
...
RegisterNetEvent('aj-mechanicjob:client:syncNitrousFlames', function(net, toggle)
...
RegisterNetEvent('aj-mechanicjob:client:installNitrous', function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then return end
    local closestVehicle, distance = AJFW.Functions.GetClosestVehicle()
    ...
    local vehiclePlate = AJFW.Functions.GetPlate(closestVehicle)
    if not vehiclePlate then return end
    if not IsNearBone(closestVehicle, 'engine') then return end
    ToggleHood(closestVehicle)
    AJFW.Functions.Progressbar('use_nos', Lang:t('progress.nitrous'), 5000, false, true, {
        ...
        ToggleHood(closestVehicle)
        TriggerServerEvent('aj-mechanicjob:server:removeItem', 'nitrous')
        TriggerServerEvent('aj-mechanicjob:server:syncNitrous', vehiclePlate, true, 100)
