local function GetArmor(vehicle)
    ...
    for i = -1, GetNumVehicleMods(vehicle, 16) - 1 do
        ...
        local armorItem = {
            ...
            disabled = disabled,
            params = {
                event = 'aj-mechanicjob:client:install',
                args = {
                    upgradeType = 'armor',
                    modType = 16,
                    upgradeIndex = i,
    ...
    exports['aj-menu']:openMenu(armorMenu, true)
end
...
local function GetBrakes(vehicle)
    ...
    exports['aj-menu']:openMenu(brakesMenu, true)
end
...
local function GetEngine(vehicle)
    ...
    exports['aj-menu']:openMenu(engineMenu, true)
end
...
local function GetSuspension(vehicle)
    ...
    exports['aj-menu']:openMenu(suspensionMenu, true)
end
...
local function GetTransmission(vehicle)
    ...
    exports['aj-menu']:openMenu(transmissionMenu, true)
end
...
local function GetTurbo(vehicle)
    ...
    exports['aj-menu']:openMenu(turboMenu, true)
end

-- Events

RegisterNetEvent('aj-mechanicjob:client:install', function(data)
    ...
    AJFW.Functions.Progressbar('mechanic_install', string.format(Lang:t('progress.installing'), partName), 5000, false, true, {
        ...
        TriggerServerEvent('aj-mechanicjob:server:removeItem', 'veh_' .. partName)
        AJFW.Functions.Notify(string.format(Lang:t('success.installed'), partName), 'success')
    end, function()
        if shouldToggleHood then ToggleHood(data.vehicle) end
    end)
end)

RegisterNetEvent('aj-mechanicjob:client:installPart', function(item)
