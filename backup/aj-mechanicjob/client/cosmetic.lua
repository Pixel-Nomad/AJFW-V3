...
local function PaintList(category)
    ...
    if dialog.primarypaint and dialog.secondarypaint and dialog.pearlescentpaint and dialog.wheelpaint then
        ...
        TriggerServerEvent('aj-mechanicjob:server:sprayVehicle', netId, dialog.primarypaint, dialog.secondarypaint, dialog.pearlescentpaint, dialog.wheelpaint, colors)
    end
end
...
local function CustomColor()
    ...
    if (dialog.hex or dialog.colorpicker) and dialog.section then
        ...
        local netId = NetworkGetNetworkIdFromEntity(vehicle)
        if isPainting then return end
        TriggerServerEvent('aj-mechanicjob:server:sprayVehicleCustom', netId, dialog.section, dialog.paintType, HexToRGB(color))
    end
end
...
function PaintCategories()
    ...
    exports['aj-menu']:openMenu(Paints)
end
...
local function WindowTint(vehicle)
    ...
    exports['aj-menu']:openMenu(tints)
end
...
local function PlateIndex(vehicle)
    ...
    exports['aj-menu']:openMenu(plates)
end

-- Events

RegisterNetEvent('aj-mechanicjob:client:vehicleSetColors', function(netId, section, colorIndex)
...
RegisterNetEvent('aj-mechanicjob:client:startParticles', function(netId, color)
    isPainting = true
    local vehicle = NetworkGetEntityFromNetworkId(netId)
...
RegisterNetEvent('aj-mechanicjob:client:stopParticles', function()
    isPainting = false
...
RegisterNetEvent('aj-mechanicjob:client:installCosmetic', function(item)
    local vehicle, distance = AJFW.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    ...
    if item == 'veh_interior' then
        OpenInteriors(vehicle)
        TriggerServerEvent('aj-mechanicjob:server:removeItem', item)
