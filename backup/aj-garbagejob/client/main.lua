...
local function DeliverAnim()
    ...
    if Config.UseTarget and hasBag then
        ...
        exports['aj-target']:RemoveTargetEntity(garbageVehicle)
        if (amountOfBags - 1) <= 0 then
            AJFW.Functions.TriggerCallback('garbagejob:server:NextStop', function(hasMoreStops, nextStop, newBagAmount)
                if hasMoreStops and nextStop ~= 0 then
                    ...
                    SetGarbageRoute()
                    AJFW.Functions.Notify(Lang:t("info.all_bags"))
                    SetVehicleDoorShut(garbageVehicle, 5, false)
                else
                    if hasMoreStops and nextStop == currentStop then
                        AJFW.Functions.Notify(Lang:t("info.depot_issue"))
                        amountOfBags = 0
                    else
                        -- You are done with work here.
                        AJFW.Functions.Notify(Lang:t("info.done_working"))
                        SetVehicleDoorShut(garbageVehicle, 5, false)
                        RemoveBlip(deliveryBlip)
                        SetRouteBack()
                        amountOfBags = 0
            ...
            if amountOfBags > 1 then
                AJFW.Functions.Notify(Lang:t("info.bags_left", { value = amountOfBags }))
            else
                AJFW.Functions.Notify(Lang:t("info.bags_still", { value = amountOfBags }))
            end
            exports['aj-target']:AddCircleZone('garbagebin', vector3(CL.coords.x, CL.coords.y, CL.coords.z), 2.0,{
...
function TakeAnim()
    local ped = PlayerPedId()
    AJFW.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        if Config.UseTarget and not hasBag then
            hasBag = true
            exports['aj-target']:RemoveZone("garbagebin")
            exports['aj-target']:AddTargetEntity(garbageVehicle, {
            options = {
        ...
        StopAnimTask(PlayerPedId(), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        AJFW.Functions.Notify(Lang:t("error.cancled"), "error")
    end)
end
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            exports['aj-base']:DrawText(Lang:t("info.grab_garbage"), 'left')
                        end
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            if not TrucText then
                                TrucText = true
                                exports['aj-base']:DrawText(Lang:t("info.dispose_garbage"), 'left')
                            end
                            if IsControlJustPressed(0, 51) and hasBag then
                                StopAnimTask(PlayerPedId(), 'missfbi4prepp1', '_bag_walk_garbage_man', 1.0)
                                DeliverAnim()
                                AJFW.Functions.Progressbar("deliverbag", Lang:t("info.progressbar"), 2000, false, true, {
                                        disableMovement = true,
                                        disableCarMovement = true,
                                        disableMouse = false,
                                        disableCombat = true,
                        ...
                        AJFW.Functions.Notify(Lang:t("error.no_truck"), "error")
                        hasBag = false
                    end
                end
            end
    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("aj-garbagejob:client:MainMenu")
            end
            Wait(1)
        end
    end)
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['aj-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "aj-garbagejob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle', job = "garbage",}},
                distance = 2.0
            })
        else
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            exports['aj-base']:DrawText(Lang:t("info.talk"), 'left')
                            Listen4Control()
                        else
                            ControlListen = false
                            exports['aj-base']:HideText()
...
RegisterNetEvent('aj-garbagejob:client:RequestRoute', function()
    if garbageVehicle then continueworking = true TriggerServerEvent('garbagejob:server:PayShift', continueworking) end
    AJFW.Functions.TriggerCallback('garbagejob:server:NewShift', function(shouldContinue, firstStop, totalBags)
        if shouldContinue then
            if not garbageVehicle then
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        AJFW.Functions.TriggerCallback('AJFW:Server:SpawnVehicle', function(netId)
                            ...
                            garbageVehicle = veh
                            SetVehicleNumberPlateText(veh, "AJ-" .. tostring(math.random(1000, 9999)))
                            ...
                            AJFW.Functions.Notify(Lang:t("info.deposit_paid", { value = Config.TruckPrice }))
                            AJFW.Functions.Notify(Lang:t("info.started"))
                            TriggerServerEvent("aj-garbagejob:server:payDeposit")
                ...
                if occupied then
                    AJFW.Functions.Notify(Lang:t("error.all_occupied"))
                end
            end
            currentStop = firstStop
            currentStopNum = 1
            ...
            AJFW.Functions.Notify(Lang:t("info.not_enough", { value = Config.TruckPrice }))
        end
    end, continueworking)
end)

RegisterNetEvent('aj-garbagejob:client:RequestPaycheck', function()
    if garbageVehicle then
        BringBackCar()
        AJFW.Functions.Notify(Lang:t("info.truck_returned"))
    end
    TriggerServerEvent('garbagejob:server:PayShift')
end)

RegisterNetEvent('aj-garbagejob:client:MainMenu', function()
    if playerJob.name == Config.Jobname then
        local MainMenu = {}
        MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'aj-garbagejob:client:RequestPaycheck',}}
        if not garbageVehicle or finished then
            MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'aj-garbagejob:client:RequestRoute',}}
        end
        exports['aj-menu']:openMenu(MainMenu)
    else
        AJFW.Functions.Notify(Lang:t("error.job"))
    end
end)
