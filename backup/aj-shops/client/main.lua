...
local function openShop(shop, data)
    AJFW.Functions.TriggerCallback('aj-shops:server:SetShopInv', function(shopInvJson)
                ...
                if curProduct.requiredLicense and not hasLicense(curProduct.requiredLicense, PlayerData.metadata["licences"]) then
                    ...
                    for _, license in ipairs(curProduct.requiredLicense) do
                        if not PlayerData.metadata["licences"][license] and not notifiedLicenses[license] then
                            AJFW.Functions.Notify(string.format(Lang:t("error.missing_license"), license), "error")
        ...
        while listen do
            if IsControlJustPressed(0, 38) then -- E
                ...
                if inChips then
                    ...
                    TriggerServerEvent("aj-shops:server:sellChips")
                else
                    exports["aj-base"]:KeyPressed()
...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if Config.UseTarget then
            exports['aj-target']:AddTargetEntity(ShopPed[k], {
                options = {
                    {
                        label = v["targetLabel"],
                        icon = v["targetIcon"],
    ...
    if Config.UseTarget then
        exports['aj-target']:AddTargetEntity(ShopPed["casino"], {
            options = {
                {
                    label = 'Sell Chips',
...
-- Events
RegisterNetEvent("aj-shops:client:UpdateShop", function(shop, itemData, amount)
    TriggerServerEvent("aj-shops:server:UpdateShopItems", shop, itemData, amount)
end)

RegisterNetEvent("aj-shops:client:SetShopItems", function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent("aj-shops:client:RestockShopItems", function(shop, amount)
    ...
    CreateThread(function()
        ...
        combo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                exports["aj-base"]:DrawText(Lang:t("info.open_shop"))
                listenForControl()
            else
        ...
        sellChips:onPlayerInOut(function(isPointInside)
            if isPointInside then
                inChips = true
                exports["aj-base"]:DrawText(Lang:t("info.sell_chips"))
                listenForControl()
            else
                inChips = false
                exports["aj-base"]:HideText()
