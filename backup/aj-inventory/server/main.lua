...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	--	print('inventory:server:OpenInventory is deprecated use exports[\'aj-inventory\']:OpenInventory() instead.')
	local src = source
	local ply = Player(src)
	local Player = AJFW.Functions.GetPlayer(src)
	if ply.state.inv_busy then
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif AJFW.Shared.SplitStr(fromInventory, '-')[1] == 'traphouse' then
		local traphouseId = AJFW.Shared.SplitStr(fromInventory, '-')[2]
...
RegisterServerEvent('inventory:server:GiveItem', function(target, name, amount, slot)
	...
	if not item then
		AJFW.Functions.Notify(src, Lang:t('notify.infound')); return
	end
	if item.name ~= name then
		AJFW.Functions.Notify(src, Lang:t('notify.iifound')); return
	end
...
AJFW.Functions.CreateCallback('AJFW:HasItem', function(source, cb, items, amount)
	print('^3AJCore:HasItem is deprecated, please use AJFW.Functions.HasItem, it can be used on both server- and client-side and uses the same arguments.^0')
	local retval = false
	local Player = AJFW.Functions.GetPlayer(source)
