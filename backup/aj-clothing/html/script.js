...
AJClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://aj-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://aj-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://aj-clothing/setCurrentPed', JSON.stringify({ ped: newValue }), function(model) {
                    $("#current-model").html("<p>" + model + "</p>")
                });
                canChange = true;
                AJClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    AJClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function() {
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://aj-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e) {
    ...
    if (selectedCam == null) {
        ...
        $.post('https://aj-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch (event.keyCode) {
        case 68: // D
            $.post('https://aj-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://aj-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
AJClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        ...
        switch (event.data.action) {
            case "open":
                AJClothing.Open(event.data);
                break;
            case "close":
                AJClothing.Close();
                break;
            case "updateMax":
                AJClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                AJClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                AJClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                AJClothing.ResetValues();
                break;
...
AJClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e) {
    e.preventDefault();
    AJClothing.Close();
    $.post('https://aj-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e) {
    e.preventDefault();
    AJClothing.Close();
    $.post('https://aj-clothing/resetOutfit');
});

AJClothing.SetCurrentValues = function(clothingValues) {
...
AJClothing.Open = function(data) {
    ...
    AJClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    AJClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e) {
    ...
    $.post('https://aj-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e) {
    ...
    $.post('https://aj-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
AJClothing.Close = function() {
    $.post('https://aj-clothing/close');
...
AJClothing.SetMaxValues = function(maxValues) {
...
AJClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e) {
    ...
    $.post('https://aj-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e) {
    ...
    $.post('https://aj-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
