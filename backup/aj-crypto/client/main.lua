...
local requiredItems = {[1] = {name = AJFW.Shared.Items["cryptostick"]["name"], image = AJFW.Shared.Items["cryptostick"]["image"]}}
...
local function ExchangeSuccess()
	TriggerServerEvent('aj-crypto:server:ExchangeSuccess', math.random(1, 10))
end
...
local function ExchangeFail()
	...
	if RemoveChance == LosingNumber then
		TriggerServerEvent('aj-crypto:server:ExchangeFail')
		TriggerServerEvent('aj-crypto:server:SyncReboot')
	end
end
	...
	CreateThread(function()
		while Crypto.Exchange.RebootInfo.state do
			if (Crypto.Exchange.RebootInfo.percentage + 1) <= 100 then
				...
				TriggerServerEvent('aj-crypto:server:Rebooting', true, Crypto.Exchange.RebootInfo.percentage)
			else
				Crypto.Exchange.RebootInfo.percentage = 0
				Crypto.Exchange.RebootInfo.state = false
				TriggerServerEvent('aj-crypto:server:Rebooting', false, 0)
			end
			Wait(1200)
		end
	end)
				...
				if dist < 1.5 then
					if not Crypto.Exchange.RebootInfo.state then
						...
						if IsControlJustPressed(0, 38) then
							AJFW.Functions.TriggerCallback('aj-crypto:server:HasSticky', function(HasItem)
								if HasItem then
									TriggerEvent("mhacking:show")
									TriggerEvent("mhacking:start", math.random(4, 6), 45, HackingSuccess)
								else
									AJFW.Functions.Notify(Lang:t('error.you_dont_have_a_cryptostick'), 'error')
								end
							end)
						end
					else
...
RegisterNetEvent('aj-crypto:client:SyncReboot', function()
	Crypto.Exchange.RebootInfo.state = true
	SystemCrashCooldown()
end)

RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
	TriggerServerEvent('aj-crypto:server:FetchWorth')
	TriggerServerEvent('aj-crypto:server:GetRebootState')
end)

RegisterNetEvent('aj-crypto:client:UpdateCryptoWorth', function(crypto, amount, history)
...
RegisterNetEvent('aj-crypto:client:GetRebootState', function(RebootInfo)
