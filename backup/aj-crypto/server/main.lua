...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent('aj-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, json.decode(result[1].history))
        else
            TriggerClientEvent('aj-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, nil)
        end
    end
end
...
AJFW.Commands.Add('setcryptoworth', 'Set crypto value', { { name = 'crypto', help = 'Name of the crypto currency' }, { name = 'Value', help = 'New value of the crypto currency' } }, false, function(source, args)
    local src = source
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('AJFW:Notify', src, 'You have changed the value of ' .. Crypto.Labels[crypto] .. ' from: $' .. Crypto.Worth[crypto] .. ' to: $' .. NewWorth .. ' (' .. ChangeLabel .. ' ' .. PercentageChange .. '%)')
                Crypto.Worth[crypto] = NewWorth
                TriggerClientEvent('aj-crypto:client:UpdateCryptoWorth', -1, crypto, NewWorth)
                ...
                TriggerClientEvent('AJFW:Notify', src, Lang:t('text.you_have_not_given_a_new_value', { crypto = Crypto.Worth[crypto] }))
            end
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('text.this_crypto_does_not_exist'))
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('text.you_have_not_provided_crypto_available_ajit'))
    end
end, 'admin')

AJFW.Commands.Add('checkcryptoworth', '', {}, false, function(source)
    local src = source
    TriggerClientEvent('AJFW:Notify', src, Lang:t('text.the_ajit_has_a_value_of', { crypto = Crypto.Worth['ajit'] }))
end)

AJFW.Commands.Add('crypto', '', {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth['ajit'])

    TriggerClientEvent('AJFW:Notify', src, Lang:t('text.you_have_with_a_value_of', { playerPlayerDataMoneyCrypto = Player.PlayerData.money.crypto, mypocket = MyPocket }))
end)

-- Events

RegisterServerEvent('aj-crypto:server:FetchWorth', function()
    for name, _ in pairs(Crypto.Worth) do
        ...
        if result[1] ~= nil then
            ...
            if result[1].history ~= nil then
                ...
                TriggerClientEvent('aj-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, json.decode(result[1].history))
            else
                TriggerClientEvent('aj-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, nil)
            end
        end
    end
end)

RegisterServerEvent('aj-crypto:server:ExchangeFail', function()
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.cryptostick_malfunctioned'), 'error')
    end
end)

RegisterServerEvent('aj-crypto:server:Rebooting', function(state, percentage)
    Crypto.Exchange.RebootInfo.state = state
    Crypto.Exchange.RebootInfo.percentage = percentage
end)

RegisterServerEvent('aj-crypto:server:GetRebootState', function()
    local src = source
    TriggerClientEvent('aj-crypto:client:GetRebootState', src, Crypto.Exchange.RebootInfo)
end)

RegisterServerEvent('aj-crypto:server:SyncReboot', function()
    TriggerClientEvent('aj-crypto:client:SyncReboot', -1)
end)

RegisterServerEvent('aj-crypto:server:ExchangeSuccess', function(LuckChance)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName('cryptostick')
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t('success.you_have_exchanged_your_cryptostick_for', { amount = Amount }), 'success', 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('aj-phone:client:AddTransaction', src, Player, {}, Lang:t('credit.there_are_amount_credited', { amount = Amount }), 'Credit')
    end
end)

-- Callbacks

AJFW.Functions.CreateCallback('aj-crypto:server:HasSticky', function(source, cb)
...
AJFW.Functions.CreateCallback('aj-crypto:server:GetCryptoData', function(source, cb, name)
    local Player = AJFW.Functions.GetPlayer(source)
    local CryptoData = {
...
AJFW.Functions.CreateCallback('aj-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = AJFW.Functions.GetPlayer(source)
    local total_price = math.floor(tonumber(data.Coins) * tonumber(Crypto.Worth['ajit']))
    if Player.PlayerData.money.bank >= total_price then
        local CryptoData = {
            History = Crypto.History['ajit'],
            Worth = Crypto.Worth['ajit'],
            Portfolio = Player.PlayerData.money.crypto + tonumber(data.Coins),
            WalletId = Player.PlayerData.metadata['walletid'],
        }
        Player.Functions.RemoveMoney('bank', total_price, 'bought crypto')
        TriggerClientEvent('aj-phone:client:AddTransaction', source, Player, data, Lang:t('credit.you_have_ajit_purchased', { dataCoins = tonumber(data.Coins) }), 'Credit')
...
AJFW.Functions.CreateCallback('aj-crypto:server:SellCrypto', function(source, cb, data)
    local Player = AJFW.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        local amount = math.floor(tonumber(data.Coins) * tonumber(Crypto.Worth['ajit']))
        TriggerClientEvent('aj-phone:client:AddTransaction', source, Player, data, Lang:t('debit.you_have_sold', { dataCoins = tonumber(data.Coins) }), 'Debit')
...
AJFW.Functions.CreateCallback('aj-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            TriggerClientEvent('aj-phone:client:AddTransaction', source, Player, data, 'You have ' .. tonumber(data.Coins) .. " Ajit('s) transferred!", 'Debit')
            local Target = AJFW.Functions.GetPlayerByCitizenId(result[1].citizenid)

            if Target ~= nil then
                Target.Functions.AddMoney('crypto', tonumber(data.Coins), 'transfer crypto')
                TriggerClientEvent('aj-phone:client:AddTransaction', Target.PlayerData.source, Player, data, 'There are ' .. tonumber(data.Coins) .. " Ajit('s) credited!", 'Credit')
            else
    ...
    Citizen.CreateThread(function()
        ...
        while (true) do
            ...
            if type(get_coin_price) == 'number' then
                Crypto.Worth['ajit'] = get_coin_price
            else
