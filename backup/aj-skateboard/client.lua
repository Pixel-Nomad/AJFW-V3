...
RegisterNetEvent("aj-skateboard:PickPlace", function() local Ped = PlayerPedId()
	if not IsPedSittingInAnyVehicle(Ped) then
		if DoesEntityExist(skateboard.Bike) then
			...
			local options = {
				{ action = function() TriggerEvent("aj-skateboard:GetOn", { board = skateboard.Skate }) end,
					icon = "fas fa-car", label = "Get on", board = skateboard.Skate },
				{ action = function() TriggerEvent("aj-skateboard:PickPlace", { board = skateboard.Skate }) end,
...
RegisterNetEvent("aj-skateboard:GetOn", function() local Ped = PlayerPedId()
	if GetEntityModel(Ped) == `a_c_cat_01` then
