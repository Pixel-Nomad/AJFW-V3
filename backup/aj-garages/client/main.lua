...
local function OpenGarageMenu(data)
    AJFW.Functions.TriggerCallback('aj-garages:server:GetGarageVehicles', function(result)
        if result == nil then return AJFW.Functions.Notify(Lang:t('error.no_vehicles'), 'error', 5000) end
        ...
        for _, v in pairs(result) do
            ...
            local vname = nil
            pcall(function()
                vname = AJFW.Shared.Vehicles[v.vehicle].name
            end)
...
local function DepositVehicle(veh, data)
    ...
    AJFW.Functions.TriggerCallback('aj-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            ...
            TriggerServerEvent('aj-garages:server:updateVehicleStats', plate, totalFuel, engineDamage, bodyDamage)
            CheckPlayers(veh)
            if plate then TriggerServerEvent('aj-garages:server:UpdateOutsideVehicle', plate, nil) end
            AJFW.Functions.Notify(Lang:t('success.vehicle_parked'), 'primary', 4500)
        else
...
local function CreateBlipsZones()
    ...
    comboZone:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
                            ...
                            if not IsVehicleAllowed(zone.data.category, currentVehicle) then
                                AJFW.Functions.Notify(Lang:t('error.not_correct_type'), 'error', 3500)
                                return
                            end
            ...
            exports['aj-base']:DrawText(displayText, 'left')
        else
            listenForKey = false
...
function GetSpawnPoint(garage)
    ...
    if not location then
        AJFW.Functions.Notify(Lang:t('error.vehicle_occupied'), 'error')
    end
    return location
end
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    TriggerEvent('aj-garages:client:takeOutGarage', data)
    cb('ok')
end)

RegisterNUICallback('trackVehicle', function(plate, cb)
    TriggerServerEvent('aj-garages:server:trackVehicle', plate)
    cb('ok')
end)
...
RegisterNUICallback('takeOutDepo', function(data, cb)
    ...
    if depotPrice ~= 0 then
        TriggerServerEvent('aj-garages:server:PayDepotPrice', data)
    else
        TriggerEvent('aj-garages:client:takeOutGarage', data)
    end
    cb('ok')
...
RegisterNetEvent('aj-garages:client:trackVehicle', function(coords)
...
RegisterNetEvent('aj-garages:client:takeOutGarage', function(data)
    AJFW.Functions.TriggerCallback('aj-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            local location = GetSpawnPoint(data.garage)
            if not location then return end
            AJFW.Functions.TriggerCallback('aj-garages:server:spawnvehicle', function(netId, properties, vehPlate)
                ...
                TriggerServerEvent('aj-garages:server:updateVehicleState', 0, vehPlate)
            ...
            AJFW.Functions.Notify(Lang:t('error.not_depot'), 'error', 5000)
        end
...
local function CreateHouseZone(index, garage, zoneType)
    ...
    houseComboZones:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            exports['aj-base']:DrawText(Lang:t('info.house_garage'), 'left')
        else
...
RegisterNetEvent('aj-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if Config.Garages[formattedHouseName] then
        ...
        AJFW.Functions.TriggerCallback('aj-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
            ...
            TriggerServerEvent('aj-garages:server:syncGarage', Config.Garages)
        end, house)
    end
end)

RegisterNetEvent('aj-garages:client:houseGarageConfig', function(houseGarages)
...
RegisterNetEvent('aj-garages:client:addHouseGarage', function(house, garageInfo) -- event from housing on garage creation
    local formattedHouseName = string.gsub(string.lower(house), ' ', '')
    Config.Garages[formattedHouseName] = {
        houseName = house,
