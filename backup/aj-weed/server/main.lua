...
AJFW.Functions.CreateCallback('aj-weed:server:getBuildingPlants', function(_, cb, building)
...
RegisterNetEvent('aj-weed:server:placePlant', function(coords, sort, currentHouse)
    ...
    TriggerClientEvent('aj-weed:client:refreshHousePlants', -1, currentHouse)
end)

RegisterNetEvent('aj-weed:server:removeDeathPlant', function(building, plantId)
    MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?', {plantId, building})
    TriggerClientEvent('aj-weed:client:refreshHousePlants', -1, building)
end)

RegisterServerEvent('aj-weed:server:removeSeed', function(itemslot, seed)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    Player.Functions.RemoveItem(seed, 1, itemslot)
end)

RegisterNetEvent('aj-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if weedBag ~= nil and weedBag.amount >= sndAmount then
        if house ~= nil then
            ...
            if result[1] ~= nil then
                ...
                TriggerClientEvent('AJFW:Notify', src, Lang:t('text.the_plant_has_been_harvested'), 'success', 3500)
                TriggerClientEvent('aj-weed:client:refreshHousePlants', -1, house)
            else
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.this_plant_no_longer_exists'), 'error', 3500)
            end
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.house_not_found'), 'error', 3500)
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.you_dont_have_enough_resealable_bags'), 'error', 3500)
    end
end)

RegisterNetEvent('aj-weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('AJFW:Notify', src, AJWeed.Plants[plantName]["label"] ..
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    for plantName, _ in pairs(AJWeed.Plants) do
        AJFW.Functions.CreateUseableItem("weed_" .. plantName .. "_seed", function(source, item)
            TriggerClientEvent('aj-weed:client:placePlant', source, plantName, item)
        end)
    end

    AJFW.Functions.CreateUseableItem("weed_nutrition", function(source, item)
        TriggerClientEvent('aj-weed:client:foodPlant', source, item)
    end)
end)
    ...
    while true do
        ...
        for k, plant in pairs(housePlants) do
            if housePlants[k].health > 50 then
                local Grow = math.random(AJWeed.Progress.min, AJWeed.Progress.max)
                ...
                elseif housePlants[k].progress + Grow >= 100 then
                    if housePlants[k].stage ~= AJWeed.Plants[housePlants[k].sort]["highestStage"] then
            ...
            if healthTick then
                local plantFood = math.max(0, plant.food - AJWeed.FoodUsage)
        ...
        TriggerClientEvent('aj-weed:client:refreshHousePlants', -1)
        healthTick = not healthTick
        Wait((60 * 1000) * AJWeed.GrowthTick)
    end
end)
