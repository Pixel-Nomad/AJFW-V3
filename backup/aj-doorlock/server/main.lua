...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and AJFW.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
AJFW.Functions.CreateCallback('aj-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

AJFW.Functions.CreateCallback('aj-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('aj-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('aj-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('aj-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('aj-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not AJFW.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = AJFW.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('aj-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('aj-doorlock:server:removeLockpick', function(type)
	local Player = AJFW.Functions.GetPlayer(source)
...
AJFW.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('aj-doorlock:client:addNewDoor', source)
end, Config.CommandPermission)

AJFW.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('aj-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)
