...
function TakeOutVehicle(vehicleInfo)
    ...
    AJFW.Functions.TriggerCallback('AJFW:Server:SpawnVehicle', function(netId)
        ...
        TriggerServerEvent("inventory:server:addTrunkItems", AJFW.Functions.GetPlate(veh), Config.VehicleItems)
...
function MenuGarage()
    ...
    local Vehicles = Config.Vehicles[AJFW.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "aj-newsjob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function MenuHeliGarage()
    ...
    local Helicopters = Config.Helicopters[AJFW.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Helicopters) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "aj-newsjob:client:TakeOutHelicopters",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('aj-newsjob:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('aj-newsjob:client:TakeOutHelicopters', function(data)
    local vehicle = data.vehicle
