...
CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			TriggerEvent('aj-multicharacter:client:chooseChar')
			return
		end
	end
end)
...
local function initializePedModel(model, data)
    CreateThread(function()
        ...
        if data then
            TriggerEvent('aj-clothing:client:loadPlayerClothing', data, charPed)
        end
    end)
end
...
local function openCharMenu(bool)
    AJFW.Functions.TriggerCallback("aj-multicharacter:server:GetNumberOfCharacters", function(result)
        local translations = {}
...
RegisterNetEvent('aj-multicharacter:client:closeNUI', function()
    DeleteEntity(charPed)
    SetNuiFocus(false, false)
end)
    ...
    AJFW.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            ...
            if insideMeta.house then
                TriggerEvent('aj-houses:client:LastLocationHouse', insideMeta.house)
            elseif insideMeta.apartment.apartmentType and insideMeta.apartment.apartmentId then
                TriggerEvent('aj-apartments:client:LastLocationHouse', insideMeta.apartment.apartmentType, insideMeta.apartment.apartmentId)
            else
                SetEntityCoords(ped, coords.x, coords.y, coords.z)
                SetEntityHeading(ped, coords.w)
                FreezeEntityPosition(ped, false)
...
RegisterNUICallback('closeUI', function(_, cb)
    ...
    DoScreenFadeOut(10)
    TriggerServerEvent('aj-multicharacter:server:loadUserData', cData)
    openCharMenu(false)
...
RegisterNUICallback('disconnectButton', function(_, cb)
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
    TriggerServerEvent('aj-multicharacter:server:disconnect')
    cb("ok")
end)
...
RegisterNUICallback('cDataPed', function(nData, cb)
    ...
    if cData ~= nil then
        if not cached_player_skins[cData.citizenid] then
            ...
            AJFW.Functions.TriggerCallback('aj-multicharacter:server:getSkin', function(model, data)
...
RegisterNUICallback('setupCharacters', function(_, cb)
    AJFW.Functions.TriggerCallback("aj-multicharacter:server:setupCharacters", function(result)
	cached_player_skins = {}
        SendNUIMessage({
...
RegisterNUICallback('createNewCharacter', function(data, cb)
    ...
    TriggerServerEvent('aj-multicharacter:server:createCharacter', cData)
    Wait(500)
    cb("ok")
end)

RegisterNUICallback('removeCharacter', function(data, cb)
    TriggerServerEvent('aj-multicharacter:server:deleteCharacter', data.citizenid)
    DeletePed(charPed)
    TriggerEvent('aj-multicharacter:client:chooseChar')
    cb("ok")
end)
