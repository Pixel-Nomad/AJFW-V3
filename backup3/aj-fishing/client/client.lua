...
RegisterNetEvent('aj-fishing:startFishing', function()
    if IsPedInAnyVehicle(cache.ped) or IsPedSwimming(cache.ped) then
        TriggerEvent('aj-fishing:notify', Strings.cannot_perform, Strings.cannot_perform_desc, 'error')
        return
    end
    local hasItem = lib.callback.await('aj-fishing:checkItem', 100, Config.bait.itemName)
    if hasItem then
        ...
        if water then
            if not fishing then
                ...
                while fishing do
                    ...
                    if IsDisabledControlJustReleased(0, 24) then
                        TaskPlayAnim(cache.ped, 'mini@tennis', 'forehand_ts_md_far', 1.0, -1.0, 1.0, 48, 0, 0, 0, 0)
                        TriggerEvent('aj-fishing:notify', Strings.waiting_bite, Strings.waiting_bite_desc, 'inform')
                        Wait(math.random(Config.timeForBite.min, Config.timeForBite.max))
                        TriggerEvent('aj-fishing:notify', Strings.got_bite, Strings.got_bite_desc, 'inform')
                        Wait(1000)
                        local fishData = lib.callback.await('aj-fishing:getFishData', 100)
                        if lib.skillCheck(fishData.difficulty) then
                            ...
                            if breakChance < Config.fishingRod.breakChance then
                                TriggerServerEvent('aj-fishing:rodBroke')
                                TriggerEvent('aj-fishing:notify', Strings.rod_broke, Strings.rod_broke_desc, 'error')
                                ClearPedTasks(cache.ped)
                                fishing = false
                                break
                            end
                            TriggerEvent('aj-fishing:notify', Strings.failed_fish, Strings.failed_fish_desc, 'error')
                        end
            ...
            TriggerEvent('aj-fishing:notify', Strings.no_water, Strings.no_water_desc, 'error')
        end
    else
        TriggerEvent('aj-fishing:notify', Strings.no_bait, Strings.no_bait_desc, 'error')
    end
end)

RegisterNetEvent('aj-fishing:interupt', function()
    fishing = false
    ClearPedTasks(cache.ped)
end)
