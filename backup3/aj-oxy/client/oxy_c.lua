...
local pillWorker = nil

Citizen.CreateThread(function()
	while AJFW == nil do Wait(100) end
    AJFW.Functions.TriggerCallback('jacob:get:oxy', function(carspawn, pillWorkers)
		carspawns   = carspawn
        pillWorker  = pillWorkers
	end)
end)
...
function CreateOxyVehicle()
    ...
    local plt = AJFW.Functions.GetPlate(oxyVehicle)
...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			elseif OxyChance <= Config.BigRewarditemChance2 then
				TriggerServerEvent("oxydelivery:receiveBigRewarditem2")
			else
				TriggerServerEvent("oxydelivery:receivemoneyyy")
			end

			Citizen.Wait(2000)
			TriggerEvent('AJFW:Notify', "The delivery was on point, your pager will be updated with the next drop off")
		else
			TriggerEvent('AJFW:Notify',"The drop off failed",2)
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			TriggerEvent('AJFW:Notify',"You are close to the drop off")
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			TriggerEvent('AJFW:Notify',"You are no longer selling Oxy due to taking too long to drop off", 2)
		end
...
Citizen.CreateThread(function()
	while pillWorker == nil do Wait(100)end
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			if IsControlJustReleased(0,38) then
				AJFW.Functions.TriggerCallback('aj-oxy:server:start', function(HasItem, type)
					if HasItem then
    ...
    while true do
		...
		elseif OxyRun then
			...
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				TriggerEvent('AJFW:Notify', "The dealer isn't giving you anymore locations due to the state of his car", 2)
			else
				if tasking then
			        Citizen.Wait(30000)
			    else
				...
				if dstcheck < 5.0 then
					...
					if job ~= "police" then
				    	...
				    	if IsControlJustReleased(0,38) then
							...
							if not daytime then
		            			TriggerEvent('AJFW:Notify',"Its too late - noones buying shit!",2)
		            		else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true
	TriggerEvent('AJFW:Notify',"A car has been left outside for you. Your pager will be updated with locations soon")
	CreateOxyVehicle()
end)