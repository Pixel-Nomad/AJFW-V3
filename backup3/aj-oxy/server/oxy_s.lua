...
AJFW.Functions.CreateCallback('jacob:get:oxy', function(source, cb)
...
AddEventHandler('oxydelivery:server', function()
    ...
    if Player.Functions.RemoveMoney('cash', Config.DefaultPrice, "Oxy-run") then
		...
		TriggerClientEvent("inventory:client:ItemBox", source, AJFW.Shared.Items['lotto'], "remove")
        TriggerClientEvent('oxydelivery:startDealing', source)
 
    else
        TriggerClientEvent('AJFW:Notify', source, "You do not have enough money", "error")
    end
end)		
...
AddEventHandler('oxydelivery:receiveBigRewarditem', function()
	...
	TriggerClientEvent('inventory:client:ItemBox', source, AJFW.Shared.Items[Config.BigRewarditem], "add")
end)
...
AddEventHandler('oxydelivery:receiveBigRewarditem2', function()
	...
	TriggerClientEvent('inventory:client:ItemBox', source, AJFW.Shared.Items[Config.BigRewarditem2], "add")
end)
...
AJFW.Functions.CreateCallback('aj-oxy:server:start', function(source, cb)
    local src = source
