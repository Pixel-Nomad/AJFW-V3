...
-- 				--AJFW.Functions.Draw2DText('Press ~g~[ E ]~w~ to Process Cannabis')
-- 			end

-- 			if IsControlJustReleased(0, 38) and not isProcessing then
-- 				TriggerServerEvent('aj-weed:processCannabis')
-- 				-- 	if result then
-- 				-- 		ProcessWeed()
-- 				-- 	else
-- 				-- 		AJFW.Functions.Notify('You do not have enough Cannabis to process..', 'error')
-- 				-- 	end
-- 				-- end, 'cannabis')
-- 			end
-- 		else
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				AJFW.Functions.TriggerCallback('weed:ingredient', function(HasItem, type)
					if HasItem then
						ProcessWeed()
					else
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing2.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				AJFW.Functions.TriggerCallback('weed:ingredient2', function(HasItem, type)
					if HasItem then
	...
	AJFW.Functions.Progressbar("search_register", "Processing..",'red', 15000, false, true, {
		...
		while timeLeft > 0 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.WeedProcessing.coords, false) > 4 then
				TriggerServerEvent('aj-weed:cancelProcessing')
				break
			end
		end
...
function ProcessWeed2()
	...
	AJFW.Functions.Progressbar("search_register", "Processing..",'red', 15000, false, true, {
		...
		TriggerServerEvent("aj-weed:server:process")
		--TriggerServerEvent('aj-weed:processCannabis2')
		local timeLeft = Config.Delays.WeedProcessing / 1000
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				AJFW.Functions.Progressbar("search_register", "Picking up Cannabis..",'red', 10000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					...
					TriggerServerEvent('aj-weed:pickedUpCannabis')
					TriggerServerEvent('aj-weed:weed')
...
function SpawnWeedPlants()
	--[[while spawnedWeeds < 25 do
		...
		AJFW.Functions.SpawnLocalObject('prop_weed_02', weedCoords, function(obj)
    ...
    while b < random do
        ...
        local E = CreateObject(-305885281, D.x + math.random(-30,30), D.y + math.random(-30,30), DD, false, false, true)
...
function GenerateWeedCoords()
	while true do
		...
		weedCoordX = Config.CircleZones.WeedField.coords.x + 10
		weedCoordY = Config.CircleZones.WeedField.coords.y + 10
