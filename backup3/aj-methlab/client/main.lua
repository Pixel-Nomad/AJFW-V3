                                ...
                                --StartHackingm()
                                    if result then
                                        if iteminfo.info.uses >= 1 then
                                            ...
                                            AJFW.Functions.Progressbar("starting", "Hacking..",'green', 10000, false, true, {}, {}, {}, {}, function() -- Done
                                                local infos = {}
                            ...
                            if(GetDistanceBetweenCoords(pos, Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, true) < 20)then
                                ...
                                if(GetDistanceBetweenCoords(pos, Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, true) < 1)then
                                    if CarryPackage == nil then
                                        ...
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
                                            AJFW.Functions.Progressbar("pickup_reycle_package", "Grabbing ingredients..",'red', 5000, false, true, {
...
function processmeth()
	AJFW.Functions.TriggerCallback('aj-methlab:server:ingredient', function(HasItem, type)
        if HasItem then
			...
			AJFW.Functions.Progressbar("process-", "Processing..",'red', 18000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
...
function packmeth()
	AJFW.Functions.TriggerCallback('aj-methlab:server:packing', function(HasItem, type)
        if HasItem then
			...
			AJFW.Functions.Progressbar("process-", "Packing..",'orange', 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
    ...
    while complete do
        ...
        if am > 0 then
            ...
            if done then
                ...
                if first then
                    AJFW.Functions.Notify("wait for few seconds for location", "red")
                    first = false
                else
                    AJFW.Functions.Notify("wait for few seconds for next location", "red")
                end
                -- AddCancelBlip()
                ...
                if dit2 <= 2 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if firstplate == secondplate then
                            ...
                            AJFW.Functions.Notify("bewaqoof samjha he kiya same start car return karo warna chatko", "black")
                        end
                    end
                ...
                if dist <= 2 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                            ...
                            AJFW.Functions.Progressbar("pickss", "Picking..",'white', 10000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
