...
RegisterNetEvent('AJFW:Client:SetDuty', function(duty) if Config.JobRole then if PlayerJob.name == Config.JobRole then onDuty = duty end end end)
...
CreateThread(function()
		...
		if loc.Enable then
			...
			JobLocation:onPlayerInOut(function(isPointInside)
				if not isPointInside then
					...
					if loc.Job then
						if onDuty then TriggerServerEvent("AJFW:ToggleDuty") end
			...
			Targets[nameEnter] =
				exports['aj-target']:AddBoxZone(nameEnter, jobLoc.Enter.coords.xyz, jobLoc.Enter.w, jobLoc.Enter.d, { name=nameEnter, heading = jobLoc.Enter.coords.w, debugPoly=Config.Debug, minZ=jobLoc.Enter.coords.z-1, maxZ=jobLoc.Enter.coords.z+2 },
					{ options = { { event = "aj-recycle:TeleWareHouse", icon = "fas fa-recycle", label = Loc[Config.Lan].target["enter"]..(Config.PayAtDoor and " ($"..Config.PayAtDoor..")" or ""), tele = jobLoc.Enter.tele, job = loc.Job, enter = true }, },
					distance = 1.5 })

			local nameExit = "RecycleExit"..location
			Targets[nameExit] =
				exports['aj-target']:AddBoxZone(nameExit, jobLoc.Exit.coords.xyz, jobLoc.Exit.w, jobLoc.Exit.d, { name=nameExit, heading = jobLoc.Exit.coords.w, debugPoly=Config.Debug, minZ=jobLoc.Exit.coords.z-1, maxZ=jobLoc.Exit.coords.z+2, },
					{ options = { { event = "aj-recycle:TeleWareHouse", icon = "fas fa-recycle", label = Loc[Config.Lan].target["exit"], tele = jobLoc.Exit.tele }, },
					distance = 1.5 })

			local nameDuty = "RecycleDuty"..location
			Targets[nameDuty] =
				exports['aj-target']:AddBoxZone(nameDuty, jobLoc.Duty.coords.xyz, jobLoc.Duty.w, jobLoc.Duty.d, { name=nameDuty, heading = jobLoc.Duty.coords.w, debugPoly=Config.Debug, minZ=jobLoc.Duty.coords.z-0.5, maxZ=jobLoc.Duty.coords.z+0.5, },
					{ options = { { event = "aj-recycle:dutytoggle", icon = "fas fa-hard-hat", label = Loc[Config.Lan].target["duty"], job = loc.Job, Trolly = jobLoc.Trolly }, },
					distance = 1.5 })
			...
			if jobLoc.Trade then
				for i = 1, #jobLoc.Trade do
					...
					Targets[nameTrade] =
						exports['aj-target']:AddBoxZone(nameTrade, vec3(jobLoc.Trade[i].coords.x, jobLoc.Trade[i].coords.y, jobLoc.Trade[i].coords.z-1), jobLoc.Trade[i].w, jobLoc.Trade[i].d, { name=nameTrade, heading = jobLoc.Trade[i].coords.w, debugPoly=Config.Debug, minZ= jobLoc.Trade[i].coords.z-1, maxZ=jobLoc.Trade[i].coords.z+1 },
							{ options = { { event = "aj-recycle:Trade:Menu", icon = "fas fa-box", label = Loc[Config.Lan].target["trade"], job = loc.Job, Ped = Peds[nameTrade] }, },
							distance = 1.5 })
				end
			end
		end
	...
	for i = 1, #Config.Locations["Recycle"] do local loc = Config.Locations["Recycle"][i]
		...
		Targets[nameSell] =
			exports['aj-target']:AddBoxZone(nameSell, vec3(loc.coords.x, loc.coords.y, loc.coords.z-1), 1.0, 1.0, { name=nameSell, heading = loc.coords.w, debugPoly = Config.Debug, minZ = loc.coords.z-1, maxZ=loc.coords.z+1 },
				{ options = { { event = "aj-recycle:Selling:Menu", icon = "fas fa-box", label = Loc[Config.Lan].target["sell"], Ped = Peds[nameSell], }, },
				distance = 2.5 })
	end
	...
	for i = 1, #Config.Locations["BottleBanks"] do local loc = Config.Locations["BottleBanks"][i]
		...
		if loc.Blip.blipEnable then makeBlip({ coords = loc.coords, sprite = loc.Blip.sprite, col = loc.Blip.col, name = loc.Blip.name } ) end
		Targets[nameBank] =
			exports['aj-target']:AddBoxZone(nameBank, vec3(loc.coords.x, loc.coords.y, loc.coords.z-1), 1.0, 1.0, { name=nameBank, heading = loc.coords.w, debugPoly = Config.Debug, minZ = loc.coords.z-1, maxZ=loc.coords.z+1 },
				{ options = { { event = "aj-recycle:Bottle:Menu", icon = "fas fa-certificate", label = Loc[Config.Lan].target["sell_bottles"], job = Config.JobRole, Ped = Peds[nameBank] }, },
				distance = 1.5 })
	end
end)
...
function EndJob()
	if Targets["Package"] then exports["aj-target"]:RemoveTargetEntity(randPackage) end
...
function ClearProps()
	...
	if Targets["DropOff"] then exports["aj-target"]:RemoveTargetEntity(TrollyProp) end
...
function PickRandomPackage(Trolly)
	...
	if Targets["Package"] then exports["aj-target"]:RemoveTargetEntity(randPackage, "Search") end
	...
	Targets["Package"] =
		exports['aj-target']:AddTargetEntity(randPackage,
			{ options = { { event = "aj-recycle:PickupPackage:Start", icon = 'fas fa-magnifying-glass', label = Loc[Config.Lan].target["search"], Trolly = Trolly} },
			distance = 2.5,	})
end

--Event to enter and exit warehouse
RegisterNetEvent("aj-recycle:TeleWareHouse", function(data) local Ped = PlayerPedId()
	if data.enter then
		...
		-- 				else
		-- 					local p = promise.new()	Core.Functions.TriggerCallback("aj-recycle:GetCash", function(cb) p:resolve(cb) end)
		-- 					cash = Citizen.Await(p)
		-- 				end
		-- 				if cash >= Config.PayAtDoor then TriggerServerEvent("aj-recycle:DoorCharge")
		-- 				else triggerNotify(nil, Loc[Config.Lan].error["no_money"], "error") return end
		-- 			end
		-- 		end
		-- 	else
		...
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do Wait(10) end
		if onDuty then TriggerEvent('aj-recycle:dutytoggle') end
		SetEntityCoords(Ped, data.tele.xyz)
		DoScreenFadeIn(500)
	end
end)

RegisterNetEvent("aj-recycle:PickupPackage:Start", function(data) local Ped = PlayerPedId()
	...
	if progressBar({label = Loc[Config.Lan].progressbar["search"], time = 5000, cancel = true, icon = "fas fa-magnifying-glass"}) then
		...
		TriggerEvent("aj-recycle:PickupPackage:Hold", data)
	end
end)

RegisterNetEvent("aj-recycle:PickupPackage:Hold", function(data) local Ped = PlayerPedId()
	--Clear current target info
	exports["aj-target"]:RemoveTargetEntity(randPackage, "Search")
	...
	Targets["DropOff"] =
		exports['aj-target']:AddTargetEntity(TrollyProp,
			{ options = { { event = "aj-recycle:PickupPackage:Finish", icon = 'fas fa-recycle', label = Loc[Config.Lan].target["drop_off"], Trolly = data.Trolly } },
			distance = 2.5,	})
end)

RegisterNetEvent("aj-recycle:PickupPackage:Finish", function(data) local Ped = PlayerPedId()
	--Once this is triggered it can't be stopped, so remove the target and prop
	if Targets["DropOff"] then exports["aj-target"]:RemoveTargetEntity(TrollyProp, Loc[Config.Lan].target["drop_off"]) Targets["DropOff"] = nil end
	destroyProp(TrollyProp) SetEntityDrawOutline(TrollyProp, false) TrollyProp = nil
	--Remove target and the whole prop, seen as how no ones aj-target works and its my fault 😊
...
RegisterNetEvent('aj-recycle:dutytoggle', function(data)
...
local Selling = false
RegisterNetEvent('aj-recycle:SellAnim', function(data) local Ped = PlayerPedId()
	...
	for k in pairs(Config.Prices) do
		if k == data.item then TriggerServerEvent('aj-recycle:Selling:Mat', {item = data.item, Ped = data.Ped }) Selling = false lockInv(false) return end
	end
	TriggerServerEvent("aj-recycle:TradeItems", { item = data.item, amount = data.amount })
	Selling = false
	lockInv(false)
end)

RegisterNetEvent('aj-recycle:Selling:Menu', function(data)
	...
	if Config.Menu == "aj" then
		...
		sellMenu[#sellMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "aj-recycle:CloseMenu" } }
	end
	for item, price in pairsByKeys(Config.Prices) do
		sellMenu[#sellMenu+1] = {
			...
			params = { event = "aj-recycle:SellAnim", args = { Ped = data.Ped, item = item } },
			title = Core.Shared.Items[item].label, description = Loc[Config.Lan].menu["sell_all"]..price..Loc[Config.Lan].menu["each"],
			event = "aj-recycle:SellAnim", args = { Ped = data.Ped, item = item },
		}
	end
...
--Recyclable Trader
RegisterNetEvent('aj-recycle:Trade:Menu', function(data)
	if Selling then return end
	local tradeMenu = {}
	...
	if Config.Menu == "aj" then
		...
		tradeMenu[#tradeMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "aj-recycle:CloseMenu" } }
	end
	local tradeTable = {}
	...
	for _, v in pairs(Config.RecycleAmounts["Trade"]) do
		tradeMenu[#tradeMenu+1] = {
			...
			params = { event = "aj-recycle:SellAnim", args = { item = "recyclablematerial", amount = v.amount, Ped = data.Ped } },
			event = "aj-recycle:SellAnim", args = { item = "recyclablematerial", amount = v.amount, Ped = data.Ped }
		}
		Wait(0)
	end
	...
	elseif Config.Menu == "aj" then exports['aj-menu']:openMenu(tradeMenu) end
	lookEnt(data.Ped)
end)

--Recyclable Trader
RegisterNetEvent('aj-recycle:Bottle:Menu', function(data)
	if Selling then return end
	local tradeMenu = {}
	if Config.Menu == "aj" then
