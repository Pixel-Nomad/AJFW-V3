...
AddEventHandler('AJFW:Client:OnPlayerLoaded', function()
    ...
    AJFW.Functions.TriggerCallback('weapons:server:GetConfig', function(RepairPoints)
...
RegisterNetEvent('weapons:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData then
        if AJFW.Shared.Weapons[weapon]['name'] ~= 'weapon_unarmed' and AJFW.Shared.Weapons[weapon]['ammotype'] == type:upper() then
            local total = GetAmmoInPedWeapon(ped, weapon)
            local _, maxAmmo = GetMaxAmmo(ped, weapon)
            if total < maxAmmo then
                AJFW.Functions.Progressbar('taking_bullets', Lang:t('info.loading_bullets'), Config.ReloadTime, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    if AJFW.Shared.Weapons[weapon] then
                        ...
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[itemData.name], 'remove')
                        TriggerEvent('AJFW:Notify', Lang:t('success.reloaded'), 'success')
                    end
                end, function()
                    AJFW.Functions.Notify(Lang:t('error.canceled'), 'error')
                end)
            else
                AJFW.Functions.Notify(Lang:t('error.max_ammo'), 'error')
            end
        else
            AJFW.Functions.Notify(Lang:t('error.no_weapon'), 'error')
        end
    else
        AJFW.Functions.Notify(Lang:t('error.no_weapon'), 'error')
    end
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if CurrentWeaponData and next(CurrentWeaponData) then
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    local weapon = GetSelectedPedWeapon(ped)
                    if CanShoot then
                        if weapon and weapon ~= 0 and AJFW.Shared.Weapons[weapon] then
                            AJFW.Functions.TriggerCallback('prison:server:checkThrowable', function(result)
                        ...
                        if weapon ~= `WEAPON_UNARMED` then
                            TriggerEvent('inventory:client:CheckWeapon', AJFW.Shared.Weapons[weapon]['name'])
                            AJFW.Functions.Notify(Lang:t('error.weapon_broken'), 'error')
                            MultiplierAmount = 0
                        end
                    end
                end
                ...
                if distance < 10 then
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            ...
                            if CurrentWeaponData and next(CurrentWeaponData) then
                                if not data.RepairingData.Ready then
                                    local WeaponData = AJFW.Shared.Weapons[GetHashKey(CurrentWeaponData.name)]
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        AJFW.Functions.TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
