...
Citizen.CreateThread(function()
	for k, v in pairs(PedList) do
		if v.clickable then
			exports['aj-target']:AddBoxZone("CatPat"..k, vector3(v.coords.x, v.coords.y, v.coords.z-0.3), 0.7, 0.7, { name="CatPat"..k, heading = 0, debugPoly=Config.Debug, minZ= v.coords.z-1.5, maxZ=v.coords.z-0.5 }, 
...
RegisterNetEvent('jim-catcafe:CatPat')
AddEventHandler('jim-catcafe:CatPat', function()
	...
	for k,v in pairs (shopPeds) do
		...
		if dist < 2 then
			...
			--TriggerEvent("AJFW:Notify", ppCoords.z, "error")
...
PedList = { 
	...
	{ coords = vector3(-577.14, -1069.22, 22.99), heading = 0, animDict = "creatures@cat@amb@world_cat_sleeping_ground@base", animName = "base", clickable = false, frozen = true },
	...
	{ coords = vector3(-583.32, -1069.32, 22.99), heading = 90.0, animDict = "creatures@cat@amb@world_cat_sleeping_ground@base", animName = "base", clickable = false, frozen = true },
...
AddEventHandler('onResourceStop', function(r) if r == GetCurrentResourceName() then for k, v in pairs(PedList) do exports['aj-target']:RemoveZone("CatPat"..k) end end end)