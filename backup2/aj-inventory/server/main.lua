...
local function DecayItem(source, item, amount)
...
local function AddItem(source, item, amount, slot, info, forceUpdate, created)
    ...
    if fromInventory == "player" or fromInventory == "hotbar" then
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            elseif AJFW.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
                ...
                if toItemData ~= nil then
                    ...
                    if itemDataTest.amount >= toAmount then
                        if toItemData.name ~= fromItemData.name then
                            ...
                            Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info, true)
                ...
                local itemInfo = AJFW.Shared.Items[fromItemData.name:lower()]
                AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info, true, itemInfo["created"])
    ...
    elseif AJFW.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
                    ...
                    if toItemData.amount >= toAmount then
                        if toItemData.name ~= fromItemData.name then
                            ...
                            OtherPlayer.Functions.AddItem(itemInfo["name"], toAmount, fromSlot, toItemData.info, true)
                ...
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, true, fromItemData["created"])
            else
                local toItemData = OtherPlayer.PlayerData.items[toSlot]
                local itemDataTest = OtherPlayer.Functions.GetItemBySlot(toSlot)
                RemoveItem(playerId, itemInfo["name"], fromAmount, fromSlot, true)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot, true) then
			if AddItem(target, item.name, amount, false, item.info, true, item.created) then
				...
				AddItem(src, item.name, amount, item.slot, item.info, true, item.created)
...
function ConvertQuality(item)
	...
	local finalPercentDone = percentDone - item.info.quality
	if finalPercentDone < 0 then
        finalPercentDone = 0
    end
    return finalPercentDone
end
