...
local QBCore = exports["aj-base"]:GetCoreObject()
...
function Process(action, start, tick, finish)
	...
	if Action.icon then
		if QBCore.Shared.Items[tostring(Action.icon)] then
			local img = "aj-framework-assets/data/inventory/" -- default qb-core inventory link
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("AJFW:Notify", "You are already doing something!", "error")
        end
    else
        TriggerEvent("AJFW:Notify", "Cant do that action!", "error")
    end
end
