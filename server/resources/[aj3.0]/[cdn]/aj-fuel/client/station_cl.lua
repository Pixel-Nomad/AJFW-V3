if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    local function SpawnGasStationPeds()
        ...
        for i = 1, #Config.GasStations do
            ...
            exports['aj-target']:AddTargetEntity(ped, {
                options = {
                    {
                        type = "client",
                        ...
                        action = function()
                            TriggerEvent('aj-fuel:stations:openmenu', CurrentLocation)
                        end,
                    },
                },
                distance = 2.0
    ...
    local function SpawnPickupVehicles()
        ...
        if truckToSpawn then
            ...
            if spawnedDeliveryTruck ~= 0 and spawnedTankerTrailer ~= 0 then
                TriggerEvent("vehiclekeys:client:SetOwner", AJFW.Functions.GetPlate(spawnedDeliveryTruck))
                return true
            else
                return false
            end
    ...
    -- Events
    RegisterNetEvent('aj-fuel:stations:updatelocation', function(updatedlocation)
    ...
    RegisterNetEvent('aj-fuel:stations:client:buyreserves', function(data)
        local location = data.location
        local price = data.price
        local amount = data.amount
        TriggerServerEvent('aj-fuel:stations:server:buyreserves', location, price, amount)
    ...
    RegisterNetEvent('aj-fuel:station:client:initiatefuelpickup', function(amountBought, finalReserveAmountAfterPurchase, location)
        if amountBought and finalReserveAmountAfterPurchase and location then
            ...
            if SpawnPickupVehicles() then
                AJFW.Functions.Notify(Lang:t("fuel_order_ready"), 'success')
                                    ...
                                    if GetVehiclePedIsIn(ped, false) == spawnedDeliveryTruck then
                                        ...
                                        if not alreadyHasTruck then
                                            ...
                                            if not CreatedEventHandler then
                                                            ...
                                                            if IsControlJustReleased(2, 38) then
                                                                ...
                                                                if distBetweenTruckAndTrailer > 10.0 then
                                                                    ...
                                                                    if not stopNotifyTemp then
                                                                        AJFW.Functions.Notify(Lang:t("trailer_too_far"), 'error', 7500)
                                                                    end
                                                                    stopNotifyTemp = true
                                                                    Wait(1000)
                                                                    ...
                                                                    TriggerServerEvent('aj-fuel:station:server:fuelpickup:finished', ReservePickupData.location)
                                                                    -- Remove Handler
                                                ...
                                                locationSwapHandler = AddEventHandler('aj-fuel:stations:updatelocation', function(location)
                ...
                TriggerServerEvent('aj-fuel:station:server:fuelpickup:failed', location)
            end
        else
            if Config.FuelDebug then
                print("An error has occurred. The amountBought / finalReserveAmountAfterPurchase / location is nil: `aj-fuel:station:client:initiatefuelpickup`")
            end
        end
    end)

    RegisterNetEvent('aj-fuel:stations:client:purchaselocation', function(data)
        local location = data.location
        local CitizenID = AJFW.Functions.GetPlayerData().citizenid
        CanOpen = false
        Wait(5)
        AJFW.Functions.TriggerCallback('aj-fuel:server:locationpurchased', function(result)
            if result then
        ...
        if not IsOwned then
            TriggerServerEvent('aj-fuel:server:buyStation', location, CitizenID)
        elseif IsOwned then
            AJFW.Functions.Notify(Lang:t("station_already_owned"), 'error', 7500)
        end
    end)

    RegisterNetEvent('aj-fuel:stations:client:sellstation', function(data)
        local location = data.location
        local SalePrice = data.SalePrice
        local CitizenID = AJFW.Functions.GetPlayerData().citizenid
        CanSell = false
        Wait(5)
        AJFW.Functions.TriggerCallback('aj-fuel:server:isowner', function(result)
            if result then
                if Config.FuelDebug then print("The Location: "..location.." is owned by ID: "..CitizenID) end
                CanSell = true
            else
                AJFW.Functions.Notify(Lang:t("station_not_owner"), 'error', 7500)
        ...
        if CanSell then
            ...
            TriggerServerEvent('aj-fuel:stations:server:sellstation', location)
            if Config.FuelDebug then print("Event Triggered") end
        else
            AJFW.Functions.Notify(Lang:t("station_cannot_sell"), 'error', 7500)
        end
    end)

    RegisterNetEvent('aj-fuel:stations:client:purchasereserves:final', function(location, price, amount) -- Menu, seens after selecting the "purchase reserves" option.
                    ...
                    options = {
                        {
                            ...
                            event = 'aj-fuel:stations:client:buyreserves',
                            args = {
                                location = location,
                                price = price,
                                amount = amount,
    ...
    RegisterNetEvent('aj-fuel:stations:client:purchasereserves', function(data)
        local CanOpen = false
        local location = data.location
        AJFW.Functions.TriggerCallback('aj-fuel:server:isowner', function(result)
            local CitizenID = AJFW.Functions.GetPlayerData().citizenid
        ...
        Wait(Config.WaitTime)
        if CanOpen then
            local bankmoney = AJFW.Functions.GetPlayerData().money['bank']
            ...
            if Config.Ox.Input then
                ...
                if reserves then
                    ...
                    if not reservesAmount then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    Reservebuyamount = tonumber(reservesAmount)
                    if Reservebuyamount < 1 then AJFW.Functions.Notify(Lang:t("station_more_than_one"), 'error', 7500) return end
                    if (Reservebuyamount + Currentreserveamount) > Config.MaxFuelReserves then
                        AJFW.Functions.Notify(Lang:t("station_reserve_cannot_fit"), "error")
                    else
                        if math.ceil(GlobalTax(Reservebuyamount * Config.FuelReservesPrice) + (Reservebuyamount * Config.FuelReservesPrice)) <= bankmoney then
                            ...
                            TriggerEvent("aj-fuel:stations:client:purchasereserves:final", location, price, amount)

                        else
                            AJFW.Functions.Notify(Lang:t("not_enough_money_in_bank"), 'error', 7500)
                        end
                    end
                end
            else
                local reserves = exports['aj-input']:ShowInput({
                ...
                if reserves then
                    ...
                    if not reserves.amount then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
    ...
    RegisterNetEvent('aj-fuel:stations:client:changefuelprice', function(data)
        CanOpen = false
        local location = data.location
        AJFW.Functions.TriggerCallback('aj-fuel:server:isowner', function(result)
            local CitizenID = AJFW.Functions.GetPlayerData().citizenid
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if fuelprice then
                    ...
                    Wait(100)
                    if not fuelPrice then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    NewFuelPrice = tonumber(fuelPrice)
                    if NewFuelPrice < Config.MinimumFuelPrice then AJFW.Functions.Notify(Lang:t("station_price_too_low"), 'error', 7500) return end
                    if NewFuelPrice > Config.MaxFuelPrice then
                        AJFW.Functions.Notify(Lang:t("station_price_too_high"), "error")
                    else
                        TriggerServerEvent("aj-fuel:station:server:updatefuelprice", NewFuelPrice, CurrentLocation)
                    end
                end
            else
                local fuelprice = exports['aj-input']:ShowInput({
                ...
                if fuelprice then
                    if Config.FuelDebug then print("Attempting to change fuel price!") end
                    Wait(100)
                    if not fuelprice.price then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
    ...
    RegisterNetEvent('aj-fuel:stations:client:sellstation:menu', function(data) -- Menu, seen after selecting the Sell this Location option.
        local location = data.location
        local CitizenID = AJFW.Functions.GetPlayerData().citizenid
        AJFW.Functions.TriggerCallback('aj-fuel:server:isowner', function(result)
            if result then
        ...
        if CanOpen then
            ...
            if Config.Ox.Menu then
                    ...
                    options = {
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:sellstation',
                            args = {
                                location = location,
                                SalePrice = SalePrice,
                            }
                ...
                TriggerServerEvent("aj-fuel:stations:server:stationsold", location)
            else
                exports['aj-menu']:openMenu({
                    {
    ...
    RegisterNetEvent('aj-fuel:stations:client:changestationname', function() -- Menu for changing the label of the owned station.
        CanOpen = false
        AJFW.Functions.TriggerCallback('aj-fuel:server:isowner', function(result)
            local CitizenID = AJFW.Functions.GetPlayerData().citizenid
            if result then
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if NewName then
                    ...
                    if not NewNameName then AJFW.Functions.Notify(Lang:t("station_name_invalid"), 'error', 7500) return end
                    NewName = NewNameName
                    if type(NewName) ~= "string" then AJFW.Functions.Notify(Lang:t("station_name_invalid"), 'error') return end
                    if Config.ProfanityList[NewName] then AJFW.Functions.Notify(Lang:t("station_name_invalid"), 'error', 7500)
                        -- You can add logs for people that put prohibited words into the name changer if wanted, and here is where you would do it.
                        return
                    end
                    if string.len(NewName) > Config.NameChangeMaxChar then AJFW.Functions.Notify(Lang:t("station_name_too_long"), 'error') return end
                    if string.len(NewName) < Config.NameChangeMinChar then AJFW.Functions.Notify(Lang:t("station_name_too_short"), 'error') return end
                    Wait(100)
                    TriggerServerEvent("aj-fuel:station:server:updatelocationname", NewName, CurrentLocation)
                end
            else
                local NewName = exports['aj-input']:ShowInput({
                ...
                if NewName then
                    if Config.FuelDebug then print("Attempting to alter stations name!") end
                    if not NewName.newname then AJFW.Functions.Notify(Lang:t("station_name_invalid"), 'error', 7500) return end
    ...
    RegisterNetEvent('aj-fuel:stations:client:managemenu', function(location) -- Menu, seen after selecting the Manage this Location Option.
                    ...
                    options = {
                        {
                            ...
                            event = 'aj-fuel:stations:client:purchasereserves',
                            args = {
                                location = location,
                            },
                            metadata = {
                        ...
                        },
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:changefuelprice',
                            args = {
                                location = location,
                            },
                            metadata = {
                        ...
                        },
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:managefunds'
                        },
                        {
                            ...
                            icon = "fas fa-pen",
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:changestationname',
                        ...
                        },
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:sellstation:menu',
                            args = {
                                location = location,
                            },
                        },
    ...
    RegisterNetEvent('aj-fuel:stations:client:managefunds', function(location) -- Menu, seen after selecting the Manage this Location Option.
        AJFW.Functions.TriggerCallback('aj-fuel:server:isowner', function(result)
            local CitizenID = AJFW.Functions.GetPlayerData().citizenid
            if result then
                if Config.FuelDebug then print("The Location: "..CurrentLocation.." is owned by ID: "..CitizenID) end
                    ...
                    options = {
                        {
                            ...
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:WithdrawFunds',
                            args = {
                                location = location,
                            }
                        },
                        {
                            ...
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:DepositFunds',
                            args = {
                                location = location,
                            }
                        },
                        {
                            ...
                            arrow = false, -- puts arrow to the right
                            event = 'aj-fuel:stations:client:managemenu',
                            args = {
                                location = location,
                            }
                        },
    ...
    RegisterNetEvent('aj-fuel:stations:client:WithdrawFunds', function(data)
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Withdraw then
                    ...
                    if not WithdrawAmounts then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then AJFW.Functions.Notify(Lang:t("station_withdraw_too_little"), 'error', 7500) return end
                    if amount > StationBalance then AJFW.Functions.Notify(Lang:t("station_withdraw_too_much"), 'error', 7500) return end
                    WithdrawAmount = tonumber(amount)
                    if (StationBalance - WithdrawAmount) < 0 then
                        AJFW.Functions.Notify(Lang:t("station_withdraw_too_much"), 'error', 7500)
                    else
                        TriggerServerEvent('aj-fuel:station:server:Withdraw', amount, location, StationBalance)
                    end
                end
            else
                ...
                if Withdraw then
                    ...
                    if not Withdraw.amount then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then AJFW.Functions.Notify(Lang:t("station_withdraw_too_little"), 'error', 7500) return end
                    if amount > StationBalance then AJFW.Functions.Notify(Lang:t("station_withdraw_too_much"), 'error', 7500) return end
                    WithdrawAmount = tonumber(amount)
                    if (StationBalance - WithdrawAmount) < 0 then
    ...
    RegisterNetEvent('aj-fuel:stations:client:DepositFunds', function(data)
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Deposit then
                    ...
                    if not DepositAmounts then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then AJFW.Functions.Notify(Lang:t("station_deposit_too_little"), 'error', 7500) return end
                    DepositAmount = tonumber(amount)
                    if (DepositAmount) > bankmoney then
                        AJFW.Functions.Notify(Lang:t("station_deposity_too_much"), "error")
                    else
                        TriggerServerEvent('aj-fuel:station:server:Deposit', amount, location, StationBalance)
                    end
                end
            else
                ...
                if Deposit then
                    ...
                    if not Deposit.amount then AJFW.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then AJFW.Functions.Notify(Lang:t("station_deposit_too_little"), 'error', 7500) return end
                    DepositAmount = tonumber(amount)
                    if (DepositAmount) > bankmoney then
                        AJFW.Functions.Notify(Lang:t("station_deposity_too_much"), "error")
    ...
    RegisterNetEvent('aj-fuel:stations:client:Shutoff', function(location)
        TriggerServerEvent("aj-fuel:stations:server:Shutoff", location)
    end)

    RegisterNetEvent('aj-fuel:stations:client:purchasemenu', function(location) -- Menu, seen after selecting the purchase this location option.
        local bankmoney = AJFW.Functions.GetPlayerData().money['bank']
        ...
        if Config.OneStationPerPerson == true then
            AJFW.Functions.TriggerCallback('aj-fuel:server:doesPlayerOwnStation', function(result)
                if result then
            ...
            if PlayerOwnsAStation == true then
                AJFW.Functions.Notify('You can only buy one station, and you already own one!', 'error')
                return
            end
        end
        ...
        if bankmoney < costofstation then
            AJFW.Functions.Notify(Lang:t("not_enough_money_in_bank").." $"..costofstation, 'error', 7500) return
        end
                ...
                options = {
                    {
                        ...
                        icon = "fas fa-usd",
                        arrow = true, -- puts arrow to the right
                        event = 'aj-fuel:stations:client:purchaselocation',
                        args = {
                            location = location,
                        },
                        metadata = {
    ...
    RegisterNetEvent('aj-fuel:stations:openmenu', function() -- Menu #1, the first menu you see.
            ...
            lib.registerContext({
                ...
                options = {
                    ...
                    },
                    {
                        ...
                        icon = "fas fa-usd",
                        arrow = false, -- puts arrow to the right
                        event = 'aj-fuel:stations:client:purchasemenu',
                        args = CurrentLocation,
                    ...
                    },
                    {
                        ...
                        icon = "fas fa-gas-pump",
                        arrow = false, -- puts arrow to the right
                        event = 'aj-fuel:stations:client:Shutoff',
                        args = CurrentLocation,
