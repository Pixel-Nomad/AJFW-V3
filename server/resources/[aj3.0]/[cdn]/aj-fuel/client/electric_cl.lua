if Config.ElectricVehicleCharging then
    ...
    function SetElectricNozzle(state)
        ...
        elseif state == "pickup" then    
            TriggerEvent('aj-fuel:client:grabelectricnozzle')
                ...
                options = {
                    {
                        ...
                        event = 'aj-fuel:client:electric:ChargeVehicle',
                        args = {
    ...
    RegisterNetEvent('aj-fuel:client:electric:FinalMenu', function(purchasetype)
        local money = nil
        ...
        if Config.Ox.Input then
            ...
            if Electricity then
                ...
                if not HoldingElectricNozzle then AJFW.Functions.Notify(Lang:t("electric_no_nozzle"), 'error', 7500) return end
                if (ElectricityAmount + finalfuel) >= 100 then
                    ...
                    if GlobalTax(ElectricityAmount * FuelPrice) + (ElectricityAmount * FuelPrice) <= money then
                        TriggerServerEvent('aj-fuel:server:electric:OpenMenu', ElectricityAmount, IsInGasStation(), false, purchasetype, FuelPrice)
                    else
            ...
            Electricity = exports['aj-input']:ShowInput({
            ...
            if Electricity then
                ...
                if (Electricity.amount + finalfuel) >= 100 then
                    ...
                    if GlobalTax(Electricity.amount * FuelPrice) + (Electricity.amount * FuelPrice) <= money then
                        TriggerServerEvent('aj-fuel:server:electric:OpenMenu', Electricity.amount, IsInGasStation(), false, purchasetype, FuelPrice)
                    else
                        AJFW.Functions.Notify(Lang:t("not_enough_money"), 'error', 7500)
                    end
                end
    ...
    RegisterNetEvent('aj-fuel:client:electric:SendMenuToServer', function()
        local vehicle = GetClosestVehicle()
        ...
        if not AwaitingElectricCheck and FoundElectricVehicle then
            ...
            if not IsHoldingElectricNozzle() then AJFW.Functions.Notify(Lang:t("electric_no_nozzle"), 'error', 7500)  return end
            if CurFuel < 95 then
                if Config.Ox.Menu then
                    lib.registerContext({
                        id = 'electricmenu',
                        ...
                        options = {
                            {
                                ...
                                icon = "fas fa-usd",
                                arrow = false, -- puts arrow to the right
                                event = "aj-fuel:client:electric:FinalMenu",
                                args = 'cash',
                            },
                            {
            ...
            if AwaitingElectricCheck then
                ...
                AJFW.Functions.Notify(Lang:t("electric_vehicle_not_electric"), 'error', 7500)
            end
        end
    end)

    RegisterNetEvent('aj-fuel:client:electric:ChargeVehicle', function(data)
                ...
                if Config.Ox.Progress then
                    if lib.progressCircle({
                        ...
                        if purchasetype == "cash" then
                            TriggerServerEvent('aj-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice, true)
                    ...
                    AJFW.Functions.Progressbar("charge-car", Lang:t("prog_electric_charging"), time, false, true, {
                        ...
                        refueling = false
                        if not Config.AJPhonePayment or purchasetype == 'cash' then TriggerServerEvent('aj-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice, true) end
                        local curfuel = GetFuel(vehicle)
                        local finalfuel = (curfuel + fuelamount)
                        if finalfuel > 99 and finalfuel < 100 then
                            SetFuel(vehicle, 100)
    ...
    RegisterNetEvent('aj-fuel:client:grabelectricnozzle', function()
        local ped = PlayerPedId()
    ...
    RegisterNetEvent('aj-fuel:client:electric:RefuelMenu', function()
        if Config.AJPhonePayment then
            if not RefuelPossible then 
                TriggerEvent('aj-fuel:client:electric:SendMenuToServer')
            else 
                if Config.AJPhonePayment then
                    if not Cancelledrefuel and not RefuelCancelled then
                        if RefuelPossibleAmount then
                            ...
                            TriggerEvent('aj-fuel:client:electric:ChargeVehicle', purchasetype, fuelamounttotal)
                        else
                            AJFW.Functions.Notify(Lang:t("electric_more_than_zero"), 'error', 7500)
                        end
                    end
                end
            end
    ...
    if Config.AJPhonePayment then
        RegisterNetEvent('aj-fuel:client:electric:phone:PayForFuel', function(amount)
            ...
            local success = exports['aj-phone']:PhoneNotification(Lang:t("electric_phone_header"), Lang:t("electric_phone_notification")..total, 'fas fa-bolt', '#9f0e63', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
            if success then
                if AJFW.Functions.GetPlayerData().money['bank'] <= (GlobalTax(amount) + amount) then
                    AJFW.Functions.Notify(Lang:t("not_enough_money_in_bank"), "error")
                else
                    TriggerServerEvent('aj-fuel:server:PayForFuel', total, "bank", FuelPrice, true)
                    RefuelPossible = true
        ...
        options = {
            {
                num = 1,
                type = "client",
                event = "aj-fuel:client:grabelectricnozzle",
                icon = "fas fa-bolt",
