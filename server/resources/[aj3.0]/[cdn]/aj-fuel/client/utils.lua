...
function SetFuel(vehicle, fuel)
	if fuel <= 5 then
		-- if not Config.Blacklist[GetEntityModel(vehicle)] then
			if not GetVehicleHandbrake(vehicle) then
				if math.ceil(GetEntitySpeed(vehicle) * 2.23694 ) <= 5 then
	...
	if vehicle == 0 or vehicle == nil then 
		SetVehicleFuelLevel(GetVehiclePedIsIn(PlayerPedId(), false), fuel)
		DecorSetFloat(GetVehiclePedIsIn(PlayerPedId(), false), Config.FuelDecor, fuel + 0.0)
		 return 
	end
