...
if Config.FuelDebug then
	RegisterCommand('setfuel', function(source, args)
		...
		AJFW.Functions.Notify(Lang:t("set_fuel_debug")..' '..args[1]..'L', 'success')
	end, false)
...
local function FetchStationInfo(info)
	...
	AJFW.Functions.TriggerCallback('aj-fuel:server:fetchinfo', function(result)
		if result then
...
local function CanAfford(price, purchasetype)
	...
	if purchasetype == "bank" then Money = AJFW.Functions.GetPlayerData().money['bank'] elseif purchasetype == 'cash' then Money = AJFW.Functions.GetPlayerData().money['cash'] end
	if Money < price then
		return false
	else
		return true
...
if Config.ShowNearestGasStationOnly then
	RegisterNetEvent('aj-fuel:client:updatestationlabels', function(location, newLabel)
	...
	CreateThread(function()
		if Config.PlayerOwnedGasStationsEnabled then
			TriggerServerEvent('aj-fuel:server:updatelocationlabels')
		end
		Wait(1000)
	...
	for station_id = 1, #Config.GasStations, 1 do
		...
		Stations[station_id]:onPlayerInOut(function(isPointInside)
			if isPointInside then
				...
				if Config.PlayerOwnedGasStationsEnabled then
					TriggerEvent('aj-fuel:stations:updatelocation', station_id)
				end
			else
				TriggerEvent('aj-fuel:stations:updatelocation', nil)
				inGasStation = false
			end
		end)
	end
...
-- Client Events
if Config.AJPhonePayment then
	RegisterNetEvent('aj-fuel:client:phone:PayForFuel', function(amount)
		...
		if Config.EmergencyServicesDiscount['enabled'] == true then
            ...
            if shouldRecieveDiscount == true and not AJFW.Functions.GetPlayerData().job.onduty and Config.EmergencyServicesDiscount['ondutyonly'] then
                AJFW.Functions.Notify(Lang:t("you_are_discount_eligible"), 'primary', 7500)
		...
		local success = exports['aj-phone']:PhoneNotification(Lang:t("fuel_phone_header"), Lang:t("phone_notification")..total, 'fas fa-gas-pump', '#9f0e63', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
		if success then
			if AJFW.Functions.GetPlayerData().money['bank'] <= total then
				AJFW.Functions.Notify(Lang:t("not_enough_money"), "error")
			else
				TriggerServerEvent('aj-fuel:server:PayForFuel', total, "bank", FuelPrice, false, CachedFuelPrice)
				RefuelPossible = true
...
if Config.Ox.Menu then
	RegisterNetEvent('aj-fuel:client:OpenContextMenu', function(total, fuelamounttotal, purchasetype)
			...
			options = {
				{
					...
					event = 'aj-fuel:client:RefuelVehicle',
					args = {
...
RegisterNetEvent('aj-fuel:client:RefuelMenu', function(type)
	if Config.FuelDebug then print("aj-fuel:client:refuelmenu") end
	if not type then type = nil end
	if Config.AJPhonePayment then
		if not RefuelPossible then 
			TriggerEvent('aj-fuel:client:SendMenuToServer', type)
		else
			if not Cancelledrefuel and not RefuelCancelled then
				if RefuelPossibleAmount then
					...
					TriggerEvent('aj-fuel:client:RefuelVehicle', purchasetype, fuelamounttotal) 
				else
					if Config.FuelDebug then
						print("RefuelMenu: MORE THAN ZERO!")
					end
					AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error', 7500)
				end
			end
		end
	else
...
RegisterNetEvent('aj-fuel:client:grabnozzle', function()
	if Config.PlayerOwnedGasStationsEnabled then
		ShutOff = false
		Wait(50)
		AJFW.Functions.TriggerCallback('aj-fuel:server:checkshutoff', function(result)
			if result == true then
				AJFW.Functions.Notify(Lang:t("emergency_shutoff_active"), 'error', 7500) ShutOff = true return
			else
				ShutOff = false
			end
		end, CurrentLocation)
		...
		CreateThread(function()
			while holdingnozzle do
				...
				if dist > 7.5 then
					...
					AJFW.Functions.Notify(Lang:t("nozzle_cannot_reach"), 'error')
...
RegisterNetEvent('aj-fuel:client:returnnozzle', function()
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		if Config.TargetResource == 'ox_target' then
			exports.ox_target:removeGlobalVehicle('aj-fuel:options:1')
			exports.ox_target:removeGlobalVehicle('aj-fuel:options:2')
		end
...
RegisterNetEvent('aj-fuel:client:FinalMenu', function(purchasetype)
	if Config.FuelDebug then
		print('aj-fuel:client:FinalMenu', purchasetype)
	end
	if RefuelingType == nil then
		...
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
			if ReserveLevels < 1 then
				AJFW.Functions.Notify(Lang:t("station_no_fuel"), 'error', 7500) return
			end
		end
		if Config.PlayerOwnedGasStationsEnabled then
	...
	if Config.Ox.Input then
		...
		if fuel then
			...
			if not holdingnozzle and RefuelingType ~= 'special' then AJFW.Functions.Notify(Lang:t("no_nozzle"), 'error') return end
			if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == "special" then
				if tonumber(fuelAmount) > tonumber(ReserveLevels) then
					AJFW.Functions.Notify(Lang:t("station_not_enough_fuel"), "error") return
				end
			end
			if (fuelAmount + finalfuel) >= 100 then
				AJFW.Functions.Notify(Lang:t("tank_cannot_fit"), "error")
			else
				if GlobalTax(fuelAmount * FuelPrice) + (fuelAmount * FuelPrice) <= money then
					TriggerServerEvent('aj-fuel:server:OpenMenu', fuelAmount, inGasStation, false, purchasetype, tonumber(FuelPrice))
				else
					AJFW.Functions.Notify(Lang:t("not_enough_money"), 'error', 7500)
				end
			end
		else
			if Config.FuelDebug then
		...
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			if ReserveLevels < maxfuel then
				...
				fuel = exports['aj-input']:ShowInput({
		...
		if fuel then
			...
			if not holdingnozzle and RefuelingType ~= 'special' then AJFW.Functions.Notify(Lang:t("no_nozzle")) return end
			if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			...
			if (fuel.amount + finalfuel) >= 100 then
				...
				if GlobalTax(fuel.amount * FuelPrice) + (fuel.amount * FuelPrice) <= money then
					...
					TriggerServerEvent('aj-fuel:server:OpenMenu', fuel.amount, inGasStation, false, purchasetype, tonumber(FuelPrice))
				else
					AJFW.Functions.Notify(Lang:t("not_enough_money"), 'error', 7500)
				end
			end
...
RegisterNetEvent('aj-fuel:client:SendMenuToServer', function(type)
	local vehicle = GetClosestVehicle()
	local NotElectric = false
	if Config.ElectricVehicleCharging then
		local isElectric = GetCurrentVehicleType(vehicle)
		if isElectric == 'electricvehicle' then
			AJFW.Functions.Notify(Lang:t("need_electric_charger"), 'error', 7500) return 
		end
		NotElectric = true
	else
		NotElectric = true
	...
	if NotElectric then
		...
		local playercashamount = AJFW.Functions.GetPlayerData().money['cash']
		...
		if CurFuel < 95 then
			if Config.Ox.Menu then
					...
					options = {
						{
							...
							arrow = false, -- puts arrow to the right
							onSelect = function ()
								TriggerEvent('aj-fuel:client:FinalMenu', 'cash')
							end,
						},
						{
							...
							arrow = false, -- puts arrow to the right
							onSelect = function ()
								TriggerEvent('aj-fuel:client:FinalMenu', 'bank')
							end,
						},
						{
							title = Lang:t("menu_header_close"),
				...
				exports['aj-menu']:openMenu({
					...
					},
					{
						...
						icon = "fas fa-usd",
						params = {
							event = "aj-fuel:client:FinalMenu",
							args = 'cash',
						}
					},
					{
			...
			AJFW.Functions.Notify(Lang:t("tank_already_full"), 'error')
		end
	else
		AJFW.Functions.Notify(Lang:t("need_electric_charger"), 'error', 7500)
	end
end)

RegisterNetEvent('aj-fuel:client:RefuelVehicle', function(data)
	if RefuelingType == nil then
		FetchStationInfo("all")
		Wait(100)
	end
	local purchasetype, amount, fuelamount
	if not Config.AJPhonePayment then
			...
			CreateThread(function()
				while refueling do
					...
					if Cancelledrefuel then
						...
						if Config.AJPhonePayment and purchasetype == "bank" then
							...
							TriggerServerEvent("aj-fuel:server:phone:givebackmoney", MoneyToGiveBack)
							CachedFuelPrice = nil
						else
							TriggerServerEvent('aj-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice, false, CachedFuelPrice)
							CachedFuelPrice = nil
						end
						if RefuelingType == nil then
							if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
								TriggerServerEvent('aj-fuel:station:server:updatereserves', "remove", finalrefuelamount, ReserveLevels, CurrentLocation)
								if CachedFuelPrice ~= nil then
									if Config.FuelDebug then
										print("We have a cached price: $"..CachedFuelPrice..", we will credit this to the gas station.")
									end
									TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", finalrefuelamount, StationBalance, CurrentLocation, CachedFuelPrice)
									CachedFuelPrice = nil
								else
									TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", finalrefuelamount, StationBalance, CurrentLocation, FuelPrice)
								end
							end
						end
						local curfuel = GetFuel(vehicle)
			...
			if Config.Ox.Progress then
				if lib.progressCircle({
					...
					elseif purchasetype == "bank" then
						if not Config.AJPhonePayment or purchasetype == "cash" then 
							TriggerServerEvent('aj-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice, false, CachedFuelPrice)
						end
					end
					local curfuel = GetFuel(vehicle)
					...
					if RefuelingType == nil then
						if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
							TriggerServerEvent('aj-fuel:station:server:updatereserves', "remove", fuelamount, ReserveLevels, CurrentLocation)
							if CachedFuelPrice ~= nil then
								if Config.FuelDebug then
									print("We have a cached price: $"..CachedFuelPrice..", we will credit this to the gas station.")
								end
								TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", fuelamount, StationBalance, CurrentLocation, CachedFuelPrice)
								CachedFuelPrice = nil
							else
								TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", fuelamount, StationBalance, CurrentLocation, FuelPrice)
							end
						else
				...
				AJFW.Functions.Progressbar("refuel-car", Lang:t("prog_refueling_vehicle"), time, false, true, {
					...
					refueling = false
					if not Config.AJPhonePayment or purchasetype == "cash" then
						TriggerServerEvent('aj-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice)
					end
					local curfuel = GetFuel(vehicle)
					local finalfuel = (curfuel + fuelamount)
					if finalfuel > 99 and finalfuel < 100 then
...
-- Jerry Can --
RegisterNetEvent('aj-fuel:jerrycan:refuelmenu', function(itemData)
	if IsPedInAnyVehicle(PlayerPedId(), false) then AJFW.Functions.Notify(Lang:t("cannot_refuel_inside"), 'error') return end
	if Config.FuelDebug then print("Item Data: " .. json.encode(itemData)) end
	local vehicle = GetClosestVehicle()
	local vehiclecoords = GetEntityCoords(vehicle)
	local pedcoords = GetEntityCoords(PlayerPedId())
	if GetVehicleBodyHealth(vehicle) < 100 then AJFW.Functions.Notify(Lang:t("vehicle_is_damaged"), 'error') return end
				...
				options = {
					{
						...
						event = 'aj-fuel:jerrycan:refueljerrycan',
				...
				options = {
					{
						...
						event = 'aj-fuel:jerrycan:refuelvehicle',
						args = {itemData = itemData},
						disabled = nogas,
					},
				},
...
RegisterNetEvent('aj-fuel:client:jerrycanfinalmenu', function(purchasetype)
	Moneyamount = nil
	if purchasetype == 'bank' then
		Moneyamount = AJFW.Functions.GetPlayerData().money['bank']
	elseif purchasetype == 'cash' then
		Moneyamount = AJFW.Functions.GetPlayerData().money['cash']
	end
	if Moneyamount > math.ceil(Config.JerryCanPrice + GlobalTax(Config.JerryCanPrice)) then
		TriggerServerEvent('aj-fuel:server:purchase:jerrycan', purchasetype)
	else
		if purchasetype == 'bank' then AJFW.Functions.Notify(Lang:t("not_enough_money_in_bank"), 'error') end
		if purchasetype == "cash" then AJFW.Functions.Notify(Lang:t("not_enough_money_in_cash"), 'error') end
	end
end)

RegisterNetEvent('aj-fuel:client:purchasejerrycan', function()
	local playercashamount = AJFW.Functions.GetPlayerData().money['cash']
	if Config.Ox.Menu then
		lib.registerContext({
			id = 'purchasejerrycan',
			...
			options = {
				{
					...
					description = Lang:t("menu_pay_with_cash") .. playercashamount,
					icon = "fas fa-usd",
					event = 'aj-fuel:client:jerrycanfinalmenu',
					args = 'cash',
				},
				{
					title = Lang:t("menu_header_bank"),
...
RegisterNetEvent('aj-fuel:jerrycan:refuelvehicle', function(data)
	local ped = PlayerPedId()
	local vehicle = GetClosestVehicle()
	...
	if NotElectric then
		...
		if Config.Ox.Input then
			...
			if refuel and refuelAmount then
				if tonumber(refuelAmount) == 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuelAmount) < 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return end
				if tonumber(refuelAmount) > jerrycanfuelamount then AJFW.Functions.Notify(Lang:t("jerry_can_not_enough_fuel"), 'error') return end
				...
				if vehfuel + tonumber(refuelAmount) > 100 then AJFW.Functions.Notify(Lang:t("tank_cannot_fit"), 'error') return end
				...
				if Config.Ox.Progress then
					if lib.progressCircle({
						...
						AJFW.Functions.Notify(Lang:t("jerry_can_success_vehicle"), 'success')
						local JerryCanItemData = data.itemData
						local srcPlayerData = AJFW.Functions.GetPlayerData()
						TriggerServerEvent('aj-fuel:info', "remove", tonumber(refuelAmount), srcPlayerData, JerryCanItemData)
						SetFuel(vehicle, (vehfuel + refuelAmount))
					else 
						DeleteObject(JerrycanProp)
						StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
						AJFW.Functions.Notify(Lang:t("cancelled"), 'error')
					end
				else
					AJFW.Functions.Progressbar('refuel_gas', Lang:t("prog_refueling_vehicle"), refueltimer, false, true, { -- Name | Label | Time | useWhileDead | canCancel
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
			...
			local refuel = exports['aj-input']:ShowInput({
			...
			if refuel then
				if tonumber(refuel.amount) == 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuel.amount) < 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return end
				if tonumber(refuel.amount) > jerrycanfuelamount then AJFW.Functions.Notify(Lang:t("jerry_can_not_enough_fuel"), 'error') return end
				...
				if vehfuel + tonumber(refuel.amount) > 100 then AJFW.Functions.Notify(Lang:t("tank_cannot_fit"), 'error') return end
				JerrycanProp = CreateObject(joaat('w_am_jerrycan'), 1.0, 1.0, 1.0, true, true, false)
				local lefthand = GetPedBoneIndex(ped, 18905)
				AttachEntityToEntity(JerrycanProp, ped, lefthand, 0.11 --[[Left - Right (Kind of)]] , 0.0 --[[Up - Down]], 0.25 --[[Forward - Backward]], 15.0, 170.0, 90.42, 0, 1, 0, 1, 0, 1)
				if Config.Ox.Progress then
					if lib.progressCircle({
						...
						local JerryCanItemData = data.itemData
						local srcPlayerData = AJFW.Functions.GetPlayerData()
						TriggerServerEvent('aj-fuel:info', "remove", tonumber(refuel.amount), srcPlayerData, JerryCanItemData)
...
RegisterNetEvent('aj-fuel:jerrycan:refueljerrycan', function(data)
	FetchStationInfo('all')
	Wait(100)
	if Config.PlayerOwnedGasStationsEnabled then
		FuelPrice = (1 * StationFuelPrice)
	...
	if Config.Ox.Input then
		...
		if refuel then
			if tonumber(refuelAmount) == 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuelAmount) < 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return end
			if tonumber(refuelAmount) + tonumber(jerrycanfuelamount) > Config.JerryCanCap then AJFW.Functions.Notify(Lang:t("jerry_can_not_fit_fuel"), 'error') return end
			if tonumber(refuelAmount) > Config.JerryCanCap then AJFW.Functions.Notify(Lang:t("jerry_can_not_fit_fuel"), 'error') return end
			local refueltimer = Config.RefuelTime * tonumber(refuelAmount)
			if tonumber(refuelAmount) < 10 then refueltimer = Config.RefuelTime * 10 end
			local price = (tonumber(refuelAmount) * FuelPrice) + GlobalTax(tonumber(refuelAmount) * FuelPrice)
			if not CanAfford(price, "cash") then AJFW.Functions.Notify(Lang:t("not_enough_money_in_cash"), 'error') return end
			...
			if lib.progressCircle({
				...
				DeleteObject(JerrycanProp)
				StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
				AJFW.Functions.Notify(Lang:t("jerry_can_success"), 'success')
				local srcPlayerData = AJFW.Functions.GetPlayerData()
				if Config.Ox.Inventory then
					TriggerServerEvent('aj-fuel:info', "add", tonumber(refuelAmount), srcPlayerData, itemData)
				else
					TriggerServerEvent('aj-fuel:info', "add", tonumber(refuelAmount), srcPlayerData, itemData)
				end
				
				if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
					TriggerServerEvent('aj-fuel:station:server:updatereserves', "remove", tonumber(refuelAmount), ReserveLevels, CurrentLocation)
					if CachedFuelPrice ~= nil then
						TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", tonumber(refuelAmount), StationBalance, CurrentLocation, CachedFuelPrice)
					else
						TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", tonumber(refuelAmount), StationBalance, CurrentLocation, FuelPrice)
					end
				else
					if Config.FuelDebug then print("Config.PlayerOwnedGasStationsEnabled == false or Config.UnlimitedFuel == true, this means reserves will not be changed.") end
				end
				local total = (tonumber(refuelAmount) * FuelPrice) + GlobalTax(tonumber(refuelAmount) * FuelPrice)
				TriggerServerEvent('aj-fuel:server:PayForFuel', total, "cash", FuelPrice)
			else 
				SetEntityVisible(fuelnozzle, true, 0)
				DeleteObject(JerrycanProp)
				StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
		...
		if refuel then
			if tonumber(refuel.amount) == 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuel.amount) < 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return end
			if tonumber(refuel.amount) + tonumber(jerrycanfuelamount) > Config.JerryCanCap then AJFW.Functions.Notify(Lang:t("jerry_can_not_fit_fuel"), 'error') return end
			if tonumber(refuel.amount) > Config.JerryCanCap then AJFW.Functions.Notify(Lang:t("jerry_can_not_fit_fuel"), 'error') return end
			local refueltimer = Config.RefuelTime * tonumber(refuel.amount)
			if tonumber(refuel.amount) < 10 then refueltimer = Config.RefuelTime * 10 end
			...
			AttachEntityToEntity(JerrycanProp, ped, lefthand, 0.11 --[[Left - Right]] , 0.05 --[[Up - Down]], 0.27 --[[Forward - Backward]], -15.0, 170.0, -90.42, 0, 1, 0, 1, 0, 1)
			SetEntityVisible(fuelnozzle, false, 0)
			AJFW.Functions.Progressbar('refuel_gas', Lang:t("prog_jerry_can_refuel"), refueltimer, false,true, { -- Name | Label | Time | useWhileDead | canCancel
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
				...
				if Config.Ox.Inventory then
					TriggerServerEvent('aj-fuel:info', "add", tonumber(refuelAmount), srcPlayerData, jerryCanData)
				else
					TriggerServerEvent('aj-fuel:info', "add", tonumber(refuelAmount), srcPlayerData, jerryCanData)
				end
				if RefuelingType == nil then	
					if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
						TriggerServerEvent('aj-fuel:station:server:updatereserves', "remove", tonumber(refuel.amount), ReserveLevels, CurrentLocation)
						if CachedFuelPrice ~= nil then
							TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", tonumber(refuel.amount), StationBalance, CurrentLocation, CachedFuelPrice)
						else
							TriggerServerEvent('aj-fuel:station:server:updatebalance', "add", tonumber(refuel.amount), StationBalance, CurrentLocation, FuelPrice)
						end
				...
				TriggerServerEvent('aj-fuel:server:PayForFuel', total, "cash", FuelPrice, false, CachedFuelPrice)
			end, function() -- Play When Cancel
				SetEntityVisible(fuelnozzle, true, 0)
				DeleteObject(JerrycanProp)
				StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
...
RegisterNetEvent('cdn-syphoning:syphon:menu', function(itemData)
	if IsPedInAnyVehicle(PlayerPedId(), false) then AJFW.Functions.Notify(Lang:t("syphon_inside_vehicle"), 'error') return end
	...
	if Config.ElectricVehicleCharging then
		...
		if Config.ElectricVehicles[vehiclename] and Config.ElectricVehicles[vehiclename].isElectric then
			...
			AJFW.Functions.Notify(Lang:t("syphon_electric_vehicle"), 'error', 7500) return
		end
	else
		NotElectric = true
	end
		...
		if HasSyphon then
			...
			if reason == "syphon" then
				if Config.Ox.Input then
					...
					if syphon then
						...
						if tonumber(syphonAmount) < 0 then AJFW.Functions.Notify(Lang:t("syphon_more_than_zero"), 'error') return end
						if tonumber(syphonAmount) == 0 then AJFW.Functions.Notify(Lang:t("syphon_more_than_zero"), 'error') return end
						if tonumber(syphonAmount) > maxsyphon then AJFW.Functions.Notify(Lang:t("syphon_kit_cannot_fit_1").. fitamount .. Lang:t("syphon_kit_cannot_fit_2"), 'error') return end
						if currentsyphonamount + syphonAmount > Config.SyphonKitCap then AJFW.Functions.Notify(Lang:t("syphon_kit_cannot_fit_1").. fitamount .. Lang:t("syphon_kit_cannot_fit_2"), 'error') return end
						if (tonumber(syphonAmount) <= tonumber(cargasamount)) then
							...
							if lib.progressCircle({
								...
								if GetFuel(vehicle) >= syphonAmount then
									...
									AJFW.Functions.Notify(Lang:t("syphon_success"), 'success')
									SetFuel(vehicle, removeamount)
									local syphonData = data.itemData
									local srcPlayerData = AJFW.Functions.GetPlayerData()
									TriggerServerEvent('aj-fuel:info', "add", tonumber(syphonAmount), srcPlayerData, syphonData)
								else
									AJFW.Functions.Notify(Lang:t("menu_syphon_vehicle_empty"), 'error')
								end
							else
								PoliceAlert(GetEntityCoords(ped))
								StopAnimTask(ped, Config.StealAnimDict, Config.StealAnim, 1.0)
					...
					local syphon = exports['aj-input']:ShowInput({
					...
					if syphon then
						...
						if tonumber(syphon.amount) < 0 then AJFW.Functions.Notify(Lang:t("syphon_more_than_zero"), 'error') return end
						if tonumber(syphon.amount) == 0 then AJFW.Functions.Notify(Lang:t("syphon_more_than_zero"), 'error') return end
						if tonumber(syphon.amount) > maxsyphon then AJFW.Functions.Notify(Lang:t("syphon_kit_cannot_fit_1").. fitamount .. Lang:t("syphon_kit_cannot_fit_2"), 'error') return end
						if currentsyphonamount + syphon.amount > Config.SyphonKitCap then AJFW.Functions.Notify(Lang:t("syphon_kit_cannot_fit_1").. fitamount .. Lang:t("syphon_kit_cannot_fit_2"), 'error') return end
						if (tonumber(syphon.amount) <= tonumber(cargasamount)) then
							...
							AJFW.Functions.Progressbar('syphon_gas', Lang:t("prog_syphoning"), syphontimer, false, true, { -- Name | Label | Time | useWhileDead | canCancel
								disableMovement = true,
								disableCarMovement = true,
								disableMouse = false,
								disableCombat = true,
								...
								if GetFuel(vehicle) >= tonumber(syphon.amount) then
									...
									local syphonData = data.itemData
									local srcPlayerData = AJFW.Functions.GetPlayerData()
									TriggerServerEvent('aj-fuel:info', "add", tonumber(syphon.amount), srcPlayerData, syphonData)
									StopAnimTask(ped, Config.StealAnimDict, Config.StealAnim, 1.0)
								else
									AJFW.Functions.Notify(Lang:t("menu_syphon_vehicle_empty"), 'error')
								end
			...
			elseif reason == "refuel" then
				...
				if Config.Ox.Input then
					...
					if refuel then
						if tonumber(refuelAmount) == 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuelAmount) < 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuelAmount) > 100 then AJFW.Functions.Notify("You can't refuel more than 100L!", 'error') return end
						if tonumber(refuelAmount) > tonumber(currentsyphonamount) then AJFW.Functions.Notify(Lang:t("syphon_not_enough_gas"), 'error') return end
						if tonumber(refuelAmount) + tonumber(cargasamount) > 100 then AJFW.Functions.Notify(Lang:t("tank_cannot_fit"), 'error') return end
						local refueltimer = Config.RefuelTime * tonumber(refuelAmount)
						if tonumber(refuelAmount) < 10 then refueltimer = Config.RefuelTime * 10 end
						if lib.progressCircle({
							duration = refueltimer,
							...
							StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
							AJFW.Functions.Notify(Lang:t("syphon_success_vehicle"), 'success')
							SetFuel(vehicle, cargasamount + tonumber(refuelAmount))
							local syphonData = data.itemData
							local srcPlayerData = AJFW.Functions.GetPlayerData()
							TriggerServerEvent('aj-fuel:info', "remove", tonumber(refuelAmount), srcPlayerData, syphonData)
						else
							StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
							AJFW.Functions.Notify(Lang:t("cancelled"), 'error')
						end
					...
					if refuel then
						if tonumber(refuel.amount) == 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuel.amount) < 0 then AJFW.Functions.Notify(Lang:t("more_than_zero"), 'error') return elseif tonumber(refuel.amount) > 100 then AJFW.Functions.Notify("You can't refuel more than 100L!", 'error') return end
						if tonumber(refuel.amount) > tonumber(currentsyphonamount) then AJFW.Functions.Notify(Lang:t("syphon_not_enough_gas"), 'error') return end
						if tonumber(refuel.amount) + tonumber(cargasamount) > 100 then AJFW.Functions.Notify(Lang:t("tank_cannot_fit"), 'error') return end
						local refueltimer = Config.RefuelTime * tonumber(refuel.amount)
						if tonumber(refuel.amount) < 10 then refueltimer = Config.RefuelTime * 10 end
						AJFW.Functions.Progressbar('refuel_gas', Lang:t("prog_refueling_vehicle"), refueltimer, false, true, { -- Name | Label | Time | useWhileDead | canCancel
							disableMovement = true,
							...
							local syphonData = data.itemData
							local srcPlayerData = AJFW.Functions.GetPlayerData()
							TriggerServerEvent('aj-fuel:info', "remove", tonumber(refuel.amount), srcPlayerData, syphonData)
						end, function() -- Play When Cancel
							StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
							AJFW.Functions.Notify(Lang:t("cancelled"), 'error')
						end, "syphoningkit")
			...
			AJFW.Functions.Notify(Lang:t("syphon_no_syphon_kit"), 'error', 7500)
		end
	else
		AJFW.Functions.Notify(Lang:t("need_electric_charger"), 'error', 7500) return 
	end
...
-- Helicopter Fueling --
RegisterNetEvent('aj-fuel:client:grabnozzle:special', function()
	local ped = PlayerPedId()
	...
	AJFW.Functions.Notify(Lang:t("show_input_key_special"))
	if Config.PumpHose then
		local pumpCoords, pump = GetClosestPump(grabbednozzlecoords)
		-- Load Rope Textures
		RopeLoadTextures()
...
RegisterNetEvent('aj-fuel:client:returnnozzle:special', function()
	HoldingSpecialNozzle = false
	TriggerServerEvent("InteractSound_SV:PlayOnSource", "putbacknozzle", 0.4)
	Wait(250)
	DeleteObject(SpecialFuelNozzleObj)
							...
							while PlayerInSpecialFuelZone do
								...
								if IsControlJustReleased(0, Config.AirAndWaterVehicleFueling['refuel_button']) --[[ Control in Config ]] then
									...
									if not HoldingSpecialNozzle then
										AJFW.Functions.Notify(Lang:t("no_nozzle"), 'error', 1250)
									elseif dist > 4.5 then
										AJFW.Functions.Notify(Lang:t("vehicle_too_far"), 'error', 1250)
									elseif IsPedInAnyVehicle(PlayerPedId(), true) then 
										AJFW.Functions.Notify(Lang:t("inside_vehicle"), 'error', 1250)
									else
										if Config.FuelDebug then print("Attempting to Open Fuel menu for special vehicles.") end
										TriggerEvent('aj-fuel:client:RefuelMenu', 'special')
									end
								end
							end
						end)
...
AddEventHandler("AJFW:Client:OnPlayerUnload", function()
	...
	if Config.TargetResource == 'ox_target' then
		local options = {
			[1] = {
				...
				event = 'aj-fuel:client:RefuelMenu'
			},
			[2] = {
				name = 'aj-fuel:options:2',
				icon = "fas fa-bolt",
				...
				event = "aj-fuel:client:electric:RefuelMenu",
			}
		}
		...
		local modelOptions = {
			[1] = {
				name = "aj-fuel:modelOptions:option_1",
				num = 1,
				type = "client",
				event = "aj-fuel:client:grabnozzle",
				icon = "fas fa-gas-pump",
			...
			[2] = {
				name = "aj-fuel:modelOptions:option_2",
				num = 2,
				type = "client",
				event = "aj-fuel:client:purchasejerrycan",
				icon = "fas fa-fire-flame-simple",
			...
			[3] = {
				name = "aj-fuel:modelOptions:option_3",
				num = 3,
				type = "client",
				event = "aj-fuel:client:returnnozzle",
				icon = "fas fa-hand",
			...
			[4] = {
				name = "aj-fuel:modelOptions:option_4",
				num = 4,
				type = "client",
				event = "aj-fuel:client:grabnozzle:special",
				icon = "fas fa-gas-pump",
			...
			[5] = {
				name = "aj-fuel:modelOptions:option_5",
				num = 5,
				type = "client",
				event = "aj-fuel:client:returnnozzle:special",
				icon = "fas fa-hand",
			...
			options = {
				{
					type = "client",
					action = function ()
						TriggerEvent('aj-fuel:client:RefuelMenu')
					end,
					icon = "fas fa-gas-pump",
				...
				},
				{
					type = "client",
					action = function()
						TriggerEvent('aj-fuel:client:electric:RefuelMenu')
					end,
					icon = "fas fa-bolt",
				...
				if not IsVehicleBlacklisted(vehPedIsIn) then
					...
					if vehFuelLevel <= vehFuelShutoffLevel then
						if GetIsVehicleEngineRunning(vehPedIsIn) then
							...
							AJFW.Functions.Notify(Lang:t("no_fuel"), 'error', 3500)
