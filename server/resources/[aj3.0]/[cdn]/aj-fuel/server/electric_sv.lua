...
-- Events
RegisterNetEvent("aj-fuel:server:electric:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype, FuelPrice)
	local src = source
	...
	if not amount then if Config.FuelDebug then print("Electric Recharge Amount is invalid!") end TriggerClientEvent('AJFW:Notify', src, Lang:t("electric_more_than_zero"), 'error') return end
	Wait(50)
	if inGasStation and not hasWeapon then
		if Config.AJPhonePayment and purchasetype == "bank" then
			TriggerClientEvent("aj-fuel:client:electric:phone:PayForFuel", src, amount)
		else
