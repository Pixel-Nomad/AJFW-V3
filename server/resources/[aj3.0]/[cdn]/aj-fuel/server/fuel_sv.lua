...
if Config.AJPhonePayment then
	RegisterNetEvent('aj-fuel:server:phone:givebackmoney', function(amount)
		local src = source
...
RegisterNetEvent("aj-fuel:server:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype, FuelPrice)
	local src = source
	if not src then return end
	local player = AJFW.Functions.GetPlayer(src)
	if not player then return end
	if not amount then if Config.FuelDebug then print("Amount is invalid!") end TriggerClientEvent('AJFW:Notify', src, Lang:t("more_than_zero"), 'error') return end
	...
	if inGasStation == true and not hasWeapon then
		if Config.AJPhonePayment and purchasetype == "bank" then
			TriggerClientEvent("aj-fuel:client:phone:PayForFuel", src, amount)
		else
			if Config.Ox.Menu then
				if Config.FuelDebug then print("going to open the context menu (OX)") end
				TriggerClientEvent('aj-fuel:client:OpenContextMenu', src, total, amount, purchasetype)
			else
				TriggerClientEvent('aj-menu:client:openMenu', src, {
					{
...
RegisterNetEvent("aj-fuel:server:PayForFuel", function(amount, purchasetype, FuelPrice, electric)
	local src = source
	if not src then return end
	local Player = AJFW.Functions.GetPlayer(src)
	if not Player then return end
...
RegisterNetEvent("aj-fuel:server:purchase:jerrycan", function(purchasetype)
	local src = source if not src then return end
	local Player = AJFW.Functions.GetPlayer(src) if not Player then return end
	...
	if Config.Ox.Inventory then
		...
		if Player.Functions.AddItem("jerrycan", 1, false, info) then -- Dont remove money if AddItem() not possible!
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['jerrycan'], "add")
			Player.Functions.RemoveMoney(moneyremovetype, total, Lang:t("jerry_can_payment_label"))
		end
	end
end)
...
if Config.UseJerryCan then
	AJFW.Functions.CreateUseableItem("jerrycan", function(source, item)
		local src = source
		TriggerClientEvent('aj-fuel:jerrycan:refuelmenu', src, item)
	end)
end

--- Syphoning
if Config.UseSyphoning then
	AJFW.Functions.CreateUseableItem("syphoningkit", function(source, item)
		local src = source
		if Config.Ox.Inventory then
...
RegisterNetEvent('aj-fuel:info', function(type, amount, srcPlayerData, itemdata)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
CreateThread(function()
	updatePath = "/CodineDev/aj-fuel"
	resourceName = "aj-fuel ("..GetCurrentResourceName()..")"
