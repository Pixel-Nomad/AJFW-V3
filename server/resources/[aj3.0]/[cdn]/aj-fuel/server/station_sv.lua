if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
        ...
        if not newLabel or newLabel == nil then
            ...
            MySQL.Async.fetchAll('SELECT label FROM fuel_stations WHERE location = ?', {location}, function(result)
                if result then
                    ...
                    TriggerClientEvent('aj-fuel:client:updatestationlabels', -1, location, newLabel)
                else
            ...
            if src then
                TriggerClientEvent('aj-fuel:client:updatestationlabels', src, location, newLabel)
            else
                TriggerClientEvent('aj-fuel:client:updatestationlabels', -1, location, newLabel)
            end
        end
    ...
    -- Events
    RegisterNetEvent('aj-fuel:server:updatelocationlabels', function()
        local src = source
        local location = 0
    ...
    RegisterNetEvent('aj-fuel:server:buyStation', function(location, CitizenID)
        local src = source
    ...
    RegisterNetEvent('aj-fuel:stations:server:sellstation', function(location)
        local src = source
        local Player = AJFW.Functions.GetPlayer(src)
        ...
        if Player.Functions.AddMoney("bank", SalePrice, Lang:t("station_sold_location_payment_label")..Config.GasStations[location].label) then
            ...
            TriggerClientEvent('AJFW:Notify', src, Lang:t("station_sold_success"), 'success')

        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t("station_cannot_sell"), 'error')
        end
    end)

    RegisterNetEvent('aj-fuel:station:server:Withdraw', function(amount, location, StationBalance)
        local src = source
        local Player = AJFW.Functions.GetPlayer(src)
        ...
        if amount > StationBalance then TriggerClientEvent('AJFW:Notify', src, Lang:t("station_withdraw_too_much"), 'success') return end
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t("station_success_withdrew_1")..amount..Lang:t("station_success_withdrew_2"), 'success')
    end)

    RegisterNetEvent('aj-fuel:station:server:Deposit', function(amount, location, StationBalance)
        local src = source
        local Player = AJFW.Functions.GetPlayer(src)
        local setamount = (StationBalance + amount)
        ...
        if Player.Functions.RemoveMoney("bank", amount, Lang:t("station_deposit_payment_label")..Config.GasStations[location].label) then
            MySQL.Async.execute('UPDATE fuel_stations SET balance = ? WHERE `location` = ?', {setamount, location})
            TriggerClientEvent('AJFW:Notify', src, Lang:t("station_success_deposit_1")..amount..Lang:t("station_success_deposit_2"), 'success')
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t("station_cannot_afford_deposit")..amount.."!", 'success')
        end
    end)

    RegisterNetEvent('aj-fuel:stations:server:Shutoff', function(location)
        local src = source
        ...
        Wait(5)
        TriggerClientEvent('AJFW:Notify', src, Lang:t("station_shutoff_success"), 'success')
    ...
    RegisterNetEvent('aj-fuel:station:server:updatefuelprice', function(fuelprice, location)
        local src = source
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t("station_fuel_price_success")..fuelprice..Lang:t("station_per_liter"), 'success')
    end)

    RegisterNetEvent('aj-fuel:station:server:updatereserves', function(reason, amount, currentlevel, location)
    ...
    RegisterNetEvent('aj-fuel:station:server:updatebalance', function(reason, amount, StationBalance, location, FuelPrice)
    ...
    RegisterNetEvent('aj-fuel:stations:server:buyreserves', function(location, price, amount)
        ...
        if result then
            ...
            for k, v in pairs(result) do
                ...
                if v.fuel + amount > Config.MaxFuelReserves then
                    ...
                    TriggerClientEvent('AJFW:Notify', src, Lang:t("station_reserves_over_max"), 'error')
        ...
        if ReserveBuyPossible and Player.Functions.RemoveMoney("bank", price, "Purchased"..amount.."L of Reserves for: "..Config.GasStations[location].label.." @ $"..Config.FuelReservesPrice.." / L!") then
            if not Config.OwnersPickupFuel then
                ...
                TriggerClientEvent('aj-fuel:station:client:initiatefuelpickup', src, amount, NewAmount, location)
        ...
        elseif ReserveBuyPossible then
            TriggerClientEvent('AJFW:Notify', src, Lang:t("not_enough_money"), 'error')
        end
    end)

    RegisterNetEvent('aj-fuel:station:server:fuelpickup:failed', function(location)
        local src = source
        if location then
            if FuelPickupSent[location] then
                local cid = AJFW.Functions.GetPlayer(src).PlayerData.citizenid
                MySQL.Async.execute('UPDATE fuel_stations SET fuel = ? WHERE `location` = ?', {FuelPickupSent[location]['refuelAmount'], location})
                TriggerClientEvent('AJFW:Notify', src, Lang:t("fuel_pickup_failed"), 'success')
                ...
                if Config.FuelDebug then
                    print("`aj-fuel:station:server:fuelpickup:failed` | FuelPickupSent[location] is not valid! Location: "..location)
                end
    ...
    RegisterNetEvent('aj-fuel:station:server:fuelpickup:finished', function(location)
        local src = source
        if location then
            if FuelPickupSent[location] then
                local cid = AJFW.Functions.GetPlayer(src).PlayerData.citizenid
                MySQL.Async.execute('UPDATE fuel_stations SET fuel = ? WHERE `location` = ?', {FuelPickupSent[location].refuelAmount, location})
                TriggerClientEvent('AJFW:Notify', src, string.format(Lang:t("fuel_pickup_success"), tostring(tonumber(FuelPickupSent[location].refuelAmount))), 'success')
                -- This will print player information just in case someone figures out a way to exploit this.
                if Config.FuelDebug then
    ...
    RegisterNetEvent('aj-fuel:station:server:updatelocationname', function(newName, location)
        local src = source
        ...
        if Config.FuelDebug then print('Successfully executed the previous SQL Update!') end
        TriggerClientEvent('AJFW:Notify', src, Lang:t("station_name_change_success")..newName.."!", 'success')
        TriggerClientEvent('aj-fuel:client:updatestationlabels', -1, location, newName)
    end)

    -- Callbacks 
    AJFW.Functions.CreateCallback('aj-fuel:server:locationpurchased', function(source, cb, location)
    ...
    AJFW.Functions.CreateCallback('aj-fuel:server:doesPlayerOwnStation', function(source, cb)
        local src = source
        local Player = AJFW.Functions.GetPlayer(src)
    ...
    AJFW.Functions.CreateCallback('aj-fuel:server:isowner', function(source, cb, location)
        local src = source
        local Player = AJFW.Functions.GetPlayer(src)
        local citizenid = Player.PlayerData.citizenid
        if Config.FuelDebug then print("working on it.") end
    ...
    AJFW.Functions.CreateCallback('aj-fuel:server:fetchinfo', function(source, cb, location)
        local src = source
        local Player = AJFW.Functions.GetPlayer(src)
    ...
    AJFW.Functions.CreateCallback('aj-fuel:server:checkshutoff', function(source, cb, location)
        if Config.FuelDebug then print("Fetching Shutoff State for Location: "..location) end
        cb(Config.GasStations[location].shutoff)
	end)
    
    AJFW.Functions.CreateCallback('aj-fuel:server:fetchlabel', function(source, cb, location)
        if Config.FuelDebug then print("Fetching Shutoff State for Location: "..location) end
        MySQL.Async.fetchAll('SELECT label FROM fuel_stations WHERE location = ?', {location}, function(result)
            if result then
                cb(result)
