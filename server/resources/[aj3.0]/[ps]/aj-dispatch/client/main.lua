...
local function setupDispatch()
    local playerInfo = AJFW.Functions.GetPlayerData()
...
local function openMenu()
    ...
    local calls = lib.callback.await('aj-dispatch:callback:getCalls', false)
    if #calls == 0 then
...
local function setWaypoint()
    ...
    local data = lib.callback.await('aj-dispatch:callback:getLatestDispatch', false)
    ...
    if not waypointCooldown and lib.table.contains(data.jobs, PlayerData.job.type) then
        ...
        TriggerServerEvent('aj-dispatch:server:attach', data.id, PlayerData)
...
-- Events
RegisterNetEvent('aj-dispatch:client:notify', function(data, source)
    if data.alertTime == nil then data.alertTime = Config.AlertTime end
    local timer = data.alertTime * 1000
...
RegisterNetEvent('aj-dispatch:client:openMenu', function(data)
    if not isJobValid(PlayerData.job.type) then return end
...
-- EventHandlers
RegisterNetEvent("AJFW:Client:OnJobUpdate", setupDispatch)

AddEventHandler('AJFW:Client:OnPlayerLoaded', setupDispatch)

AddEventHandler('AJFW:Client:OnPlayerUnload', removeZones)
...
RegisterNUICallback("detachUnit", function(data, cb)
    TriggerServerEvent('aj-dispatch:server:detach', data.id, PlayerData)
    DeleteWaypoint()
    cb("ok")
end)
...
RegisterNUICallback("refreshAlerts", function(data, cb)
    ...
    local data = lib.callback.await('aj-dispatch:callback:getCalls', false)
    SendNUIMessage({ action = 'setDispatchs', data = data, })
    cb("ok")
end)
