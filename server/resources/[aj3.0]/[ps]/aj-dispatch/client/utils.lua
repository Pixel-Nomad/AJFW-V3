...
function GetIsHandcuffed()
    return AJFW.Functions.GetPlayerData()?.metadata?.ishandcuffed
end
...
local function HasPhone()
    for _, item in ipairs(Config.PhoneItems) do
        if AJFW.Functions.HasItem(item) then
            return true
        end
    end
    return false
...
function IsCallAllowed(message)
    ...
    if Config.PhoneRequired and not HasPhone() then AJFW.Functions.Notify('You need a communications device for this.', 'error', 5000) return false end
