...
CreateThread(function()
    if GetResourceState('aj-dispatch') == 'started' then
        TriggerServerEvent("aj-mdt:dispatchStatus", true)
    end
end)
...
RegisterNetEvent('AJFW:Client:OnPlayerUnload', function()
    TriggerServerEvent("aj-mdt:server:OnPlayerUnload")
    PlayerData = {}
end)
...
RegisterNetEvent("AJFW:Client:SetDuty", function(job, state)
    if AllowedJob(job) then
        TriggerServerEvent("aj-mdt:server:ToggleDuty")
	TriggerServerEvent("aj-mdt:server:ClockSystem")
...
RegisterCommand('mdt', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        ...
        AJFW.Functions.Notify("Can't do that!", "error")
    end
end, false)
...
RegisterNetEvent('mdt:client:dashboardWarrants', function()
    AJFW.Functions.TriggerCallback("mdt:server:getWarrants", function(data)
        if data then
...
RegisterNUICallback("searchProfiles", function(data, cb)
    ...
    AJFW.Functions.TriggerCallback('mdt:server:SearchProfile', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    local getProfileDataPromise = function(data)
        ...
        p = promise.new()
        AJFW.Functions.TriggerCallback('mdt:server:GetProfileData', function(result)
            p:resolve(result)
        end, data)
    ...
    for i=1,#vehicles do
        ...
        local vehData = AJFW.Shared.Vehicles[vehicle['vehicle']]
...
-- Uses the aj-base bill command to send a fine to a player
...
RegisterNUICallback("sendFine", function(data, cb)
    ...
    local p = promise.new()
    AJFW.Functions.TriggerCallback('mdt:server:GetPlayerSourceId', function(result)
        p:resolve(result)
    end, citizenId)
...
RegisterNUICallback("sendToCommunityService", function(data, cb)
    ...
    if sentence > 0 then
        TriggerServerEvent("aj-communityservice:server:StartCommunityService", targetSourceId, sentence)
    end
end)
    ...
    if not isLimited then
        ...
        for i=1, #vehicles do
            ...
            if tempModel and tempModel ~= "Unknown" then
                local vehData = AJFW.Shared.Vehicles[tempModel]
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    AJFW.Functions.TriggerCallback('mdt:server:SearchVehicles', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.impoundActive then
            ...
            if found == 0 then
                AJFW.Functions.Notify('Vehicle not found!', 'error')
...
RegisterNUICallback("searchWeapons", function(data, cb)
    ...
    AJFW.Functions.TriggerCallback('mdt:server:SearchWeapons', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        AJFW.Functions.Notify("You have set your radio frequency to "..radio..".", "success")
    else
        AJFW.Functions.Notify("Invalid Station(Please enter a number)", "error")
    end
end)
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('aj-dispatch:client:removeCallBlip', data.callid or data.id)
    cb(true)
end)
...
function GetPlayerWeaponInfos(cb)
    AJFW.Functions.TriggerCallback('getWeaponInfo', function(weaponInfos)
        cb(weaponInfos)
    end)
end

--3rd Eye Trigger Event
RegisterNetEvent('aj-mdt:client:selfregister')
AddEventHandler('aj-mdt:client:selfregister', function()
    GetPlayerWeaponInfos(function(weaponInfos)
        if weaponInfos and #weaponInfos > 0 then
            for _, weaponInfo in ipairs(weaponInfos) do
                ...
                TriggerEvent('AJFW:Notify', "Weapon " .. weaponInfo.weaponmodel .. " has been added to police database.")
...
if Config.UseWolfknightRadar == true then
    RegisterNetEvent("aj-mdt:client:trafficStop")
    AddEventHandler("aj-mdt:client:trafficStop", function()
        local plyData = AJFW.Functions.GetPlayerData()
        local currentPos = GetEntityCoords(PlayerPedId())
        local locationInfo = getStreetandZone(currentPos)
        if not IsPedInAnyPoliceVehicle(PlayerPedId()) then
            AJFW.Functions.Notify("Not in any Police Vehicle!", "error") 
            return 
        end
        ...
        if not coolDown then
            if data.veh ~= nil and data.veh ~= 0 then
                ...
                exports["aj-dispatch"]:CustomAlert({
                    coords = {
                        x = currentPos.x,
                        y = currentPos.y,
                        z = currentPos.z
            ...
            AJFW.Functions.Notify("Traffic Stop Cooldown active!", "error") 
        end
    end)
end
