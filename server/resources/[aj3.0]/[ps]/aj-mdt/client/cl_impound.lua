...
local function TakeOutImpound(vehicle)
    ...
    if coords then
        AJFW.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
...
RegisterNetEvent('aj-mdt:client:TakeOutImpound', function(data)
    ...
    if #(pos - takeDist) <= 15.0 then
        ...
        AJFW.Functions.Notify("You are too far away from the impound location!")
    end
end)