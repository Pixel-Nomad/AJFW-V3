...
-- Mugshot location  ( Position is the default AJFW Prison Interior )
	x = 1828.69
    y = 2581.72
    z = 46.3
...
local function TakeMugShot()
    AJFW.Functions.TriggerCallback('aj-mdt:server:MugShotWebhook', function(MugShotWebhook)
...
RegisterNetEvent('cqc-mugshot:client:trigger', function()
    ...
    CreateThread(function()
        ...
        if createdCamera ~= 0 then
           ...
           TriggerServerEvent('ajmdt-mugshot:server:MDTupload', playerData.citizenid, MugshotArray)
    ...
    AJFW.Functions.TriggerCallback('aj-mdt:server:MugShotWebhook', function(MugShotWebhook)
        if MugShotWebhook ~= '' then
            ...
            if sentence > 0 then
                ...
                Citizen.Wait(5000)
                -- Uses aj-policejob JailPlayer event
