...
RegisterServerEvent("aj-mdt:dispatchStatus", function(bool)
...
if Config.UseWolfknightRadar == true then
	...
	AddEventHandler("wk:onPlateScanned", function(cam, plate, index)
		...
		if bolo == true then
			TriggerClientEvent('AJFW:Notify', src, 'BOLO ID: '..boloId..' | Title: '..title..' | Registered Owner: '..vehicleOwner..' | Plate: '..plate, 'error', Config.WolfknightNotifyTime)
		end
		if warrant == true then
			TriggerClientEvent('AJFW:Notify', src, 'WANTED - INCIDENT ID: '..incidentId..' | Registered Owner: '..owner..' | Plate: '..plate, 'error', Config.WolfknightNotifyTime)
		end

		if Config.PlateScanForDriversLicense and driversLicense == false and vehicleOwner then
			TriggerClientEvent('AJFW:Notify', src, 'NO DRIVERS LICENCE | Registered Owner: '..vehicleOwner..' | Plate: '..plate, 'error', Config.WolfknightNotifyTime)
		end
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	if GetResourceState('aj-dispatch') == 'started' then
		local calls = exports['aj-dispatch']:GetDispatchCalls()
		return calls
	end
end)

RegisterNetEvent("aj-mdt:server:OnPlayerUnload", function()
...
RegisterNetEvent("aj-mdt:server:ToggleDuty", function()
    local src = source
    local player = AJFW.Functions.GetPlayer(src)
...
AJFW.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    local PlayerData = GetPlayerData(source)
    local job = PlayerData.job.name

    if not IsPoliceOrEms(job) then
        TriggerClientEvent('AJFW:Notify', source, "You don't have permission to use this command.", 'error')
        return
    end
    ...
    sendToDiscord(16753920, "MDT Leaderboard", leaderboard_message, "aj-mdt ")
    TriggerClientEvent('AJFW:Notify', source, "MDT leaderboard sent to Discord!", 'success')
end)

RegisterNetEvent("aj-mdt:server:ClockSystem", function()
    local src = source
    local PlayerData = GetPlayerData(src)
    local time = os.date("%Y-%m-%d %H:%M:%S")
    local firstName = PlayerData.charinfo.firstname:sub(1,1):upper()..PlayerData.charinfo.firstname:sub(2)
    ...
    if PlayerData.job.onduty then
        ...
        TriggerClientEvent('AJFW:Notify', source, "You're clocked-in", 'success')
		...
		sendToDiscord(65280, "MDT Clock-In", 'Player: **' ..  firstName .. " ".. lastName .. '**\n\nJob: **' .. PlayerData.job.name .. '**\n\nRank: **' .. PlayerData.job.grade.name .. '**\n\nStatus: **On Duty**', "aj-mdt ")
    else
		TriggerClientEvent('AJFW:Notify', source, "You're clocked-out", 'success')
		MySQL.query.await('UPDATE mdt_clocking SET clock_out_time = NOW(), total_time = TIMESTAMPDIFF(SECOND, clock_in_time, NOW()) WHERE user_id = @user_id ORDER BY id DESC LIMIT 1', {
			['@user_id'] = PlayerData.citizenid
		})
...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	if GetResourceState('aj-dispatch') == 'started' then
		calls = exports['aj-dispatch']:GetDispatchCalls()
	end
...
AJFW.Functions.CreateCallback('mdt:server:SearchProfile', function(source, cb, sentData)
...
AJFW.Functions.CreateCallback('aj-mdt:getDispatchCalls', function(source, cb)
    local calls = exports['aj-dispatch']:GetDispatchCalls()
    cb(calls)
end)

AJFW.Functions.CreateCallback("mdt:server:getWarrants", function(source, cb)
    local WarrantData = {}
...
AJFW.Functions.CreateCallback('mdt:server:OpenDashboard', function(source, cb)
	local PlayerData = GetPlayerData(source)
...
AJFW.Functions.CreateCallback('mdt:server:GetProfileData', function(source, cb, sentId)
	...
	if Config.UsingPsHousing and not Config.UsingDefaultAJApartments then
		...
		if propertyData and next(propertyData) then
			...
			if #apartmentList > 0 then
				...
				TriggerClientEvent("AJFW:Notify", src, 'The citizen does not have an apartment.', 'error')
				print('The citizen does not have an apartment. Set Config.UsingPsHousing to false.')
			end
		else
			TriggerClientEvent("AJFW:Notify", src, 'The citizen does not have a property.', 'error')
			print('The citizen does not have a property. Set Config.UsingPsHousing to false.')
		end	
    elseif Config.UsingDefaultAJApartments then
        ...
        if apartmentData then
            if apartmentData[1] then
                ...
                TriggerClientEvent("AJFW:Notify", src, 'The citizen does not have an apartment.', 'error')
                print('The citizen does not have an apartment. Set Config.UsingDefaultAJApartments to false.')
            end
        else
            TriggerClientEvent("AJFW:Notify", src, 'The citizen does not have an apartment.', 'error')
            print('The citizen does not have an apartment. Set Config.UsingDefaultAJApartments to false.')
...
RegisterNetEvent('ajmdt-mugshot:server:MDTupload', function(citizenid, MugShotURLs)
	...
	if id then
		...
		if Config.RemoveWeaponsPerms[Player.PlayerData.job.name] then
			if Config.RemoveWeaponsPerms[Player.PlayerData.job.name][Player.PlayerData.job.grade.level] then
				...
				local fullname = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
				TriggerClientEvent("AJFW:Notify", src, 'No Permissions to do that!', 'error')
...
AJFW.Functions.CreateCallback('mdt:server:SearchVehicles', function(source, cb, sentData)
	if not sentData then  return cb({}) end
	local src = source
	local PlayerData = GetPlayerData(src)
					...
					if impoundInfo.impoundActive then
						...
						if vehicle.impoundid ~= nil then
							...
							if result then
								...
								result.plate = plate
								TriggerClientEvent('aj-mdt:client:TakeOutImpound', src, result)
							end
...
AJFW.Functions.CreateCallback('mdt:server:SearchWeapons', function(source, cb, sentData)
	if not sentData then  return cb({}) end
	local PlayerData = GetPlayerData(source)
	if not PermCheck(source, PlayerData) then return cb({}) end
...
RegisterNetEvent('mdt:server:setCallsign', function(cid, newcallsign)
	local Player = AJFW.Functions.GetPlayerByCitizenId(cid)
                        ...
                        MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = infoResult }, function(result)
                            if result and #result > 0 then
                                ...
                                for i=1, #associated do
                                    ...
                                    sendIncidentToDiscord(3989503, "MDT Incident Report", message, "aj-mdt ", associatedData)                                
                                end
                            else
                ...
                MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = id }, function(result)
                    if result and #result > 0 then
                        ...
                        for i=1, #associated do
                            ...
                            sendIncidentToDiscord(16711680, "MDT Incident Report has been Updated", message, "aj-mdt ", associatedData)
                        end
                    else
...
RegisterNetEvent('mdt:server:setRadio', function(cid, newRadio)
	local src = source
	local targetPlayer = AJFW.Functions.GetPlayerByCitizenId(cid)
	...
	if radio ~= nil then
		...
		TriggerClientEvent("AJFW:Notify", src, targetName..' does not have a radio!', 'error')
	end
end)
...
AJFW.Functions.CreateCallback('mdt:server:GetPlayerSourceId', function(source, cb, targetCitizenId)
    local targetPlayer = AJFW.Functions.GetPlayerByCitizenId(targetCitizenId)
    if targetPlayer == nil then 
        TriggerClientEvent('AJFW:Notify', source, "Citizen seems Asleep / Missing", "error")
        return
    end
    local targetSource = targetPlayer.PlayerData.source
	...
	if Config.InventoryForWeaponsImages == "ox_inventory" then
			...
			if string.find(item.name, "WEAPON_") then
				...
				if invImage then
					weaponInfo = {
						...
						weaponmodel = AJFW.Shared.Items[string.lower(item.name)].label,
						weaponurl = invImage,
		...
		for _, item in pairs(Player.PlayerData.items) do
			if item.type == "weapon" then
				local invImage = ("https://cfx-nui-aj-framework-assets/data/inventory/images/%s"):format(item.image)
				if invImage then
					local weaponInfo = {
						serialnumber = item.info.serie,
						owner = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname,
						weaponmodel = AJFW.Shared.Items[item.name].label,
						weaponurl = invImage,
						notes = "Self Registered",
						weapClass = "Class 1",
					}
...
local function giveCitationItem(src, citizenId, fine, incidentId)
	...
	local Officer = AJFW.Functions.GetPlayer(src)
	...
	TriggerClientEvent('AJFW:Notify', src, PlayerName.." (" ..citizenId.. ") received a citation!")
	if Config.AJBankingUse then 
		exports['aj-Banking']:addAccountMoney(Officer.PlayerData.job.name, fine) 
	end
	TriggerClientEvent('inventory:client:ItemBox', Player.PlayerData.source, AJFW.Shared.Items['mdtcitation'], "add")
...
RegisterNetEvent('mdt:server:removeMoney', function(citizenId, fine, incidentId)
	...
	if not antiSpam then
		if Player.Functions.RemoveMoney('bank', fine, 'pd-fine') then
			...
			TriggerClientEvent('AJFW:Notify', Player.PlayerData.source, "Something went wrong!")
		end
		antiSpam = true
		...
		TriggerClientEvent('AJFW:Notify', src, "On cooldown!")
	end
end)
