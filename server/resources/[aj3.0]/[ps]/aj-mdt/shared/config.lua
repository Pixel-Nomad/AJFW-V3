...
Config.UsingDefaultAJApartments = true
...
-- However, if you're using a different inventory system, please refer to the "Inventory Edit | Automatic Add Weapons with images" section in aj-mdt's README.
Config.InventoryForWeaponsImages = "aj-inventory"
...
-- This is a workaround solution because the aj-menu present in aj-policejob fills in an impound location and sends it to the event. 
-- If the impound locations are modified in aj-policejob, the changes must also be implemented here to ensure consistency.
