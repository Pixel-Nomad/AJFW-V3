...
RegisterCommand("maze",function()
    exports['aj-hacks']:Maze(function(success)
        if success then
            print("success")
		else
			print("fail")
...
RegisterCommand("var", function()
    exports['aj-hacks']:VarHack(function(success)
        if success then
            print("success")
		else
			print("fail")
...
RegisterCommand("thermite", function()
    exports['aj-hacks']:Thermite(function(success)
        if success then
            print("success")
		else
			print("fail")
...
RegisterCommand("display", function()
    exports['aj-hacks']:DisplayText("Example Text", "primary") -- Colors: primary, error, success, warning, info, mint
end)

RegisterCommand("hide", function()
    exports['aj-hacks']:HideText()
end)
...
RegisterCommand("status", function()
    if not status then
        status = true
        exports['aj-hacks']:StatusShow("Area Dominance", {
            "Gang: Ballas",
            "Influence: %100",
        })
    else 
        status = false
        exports['aj-hacks']:StatusHide()
    end
end)


RegisterCommand("cmenu", function()
    exports['aj-hacks']:CreateMenu({
        {
            header = "header1",
            text = "text1",
...
RegisterCommand("input", function()
    local input = exports['aj-hacks']:Input({
        title = "Test",
        inputs = {
            {
                type = "text",
...
RegisterCommand("showimage", function()
    exports['aj-hacks']:ShowImage("https://user-images.githubusercontent.com/91661118/168956591-43462c40-e7c2-41af-8282-b2d9b6716771.png")
end)