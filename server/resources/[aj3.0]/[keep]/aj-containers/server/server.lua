...
local function init_database()
    local array = {
        [[
            CREATE TABLE IF NOT EXISTS `aj_containers` (
        ...
        [[
            CREATE TABLE IF NOT EXISTS `aj_containers_access_log` (
                `id` int(11) NOT NULL AUTO_INCREMENT,
                `random_id` varchar(50) NOT NULL,
...
local function remove_item( source, Player, item_name, amount, slot )
    ...
    TriggerClientEvent("aj-inventory:client:ItemBox", source, Core.Shared.Items[item_name], "remove")
    return res
end
...
RegisterNetEvent("aj-containers:server:create_container", function( password, position, zone_name )
    ...
    local sqlQuery = "INSERT INTO aj_containers (random_id,container_type,owner_citizenid,password,position,zone,deleted,deleted_by) VALUES (?,?,?,?,?,?,?,?)"
    local QueryData = {
        RandomID(9),
        container_type,
        citizenid,
    ...
    MySQL.Async.insert(sqlQuery, QueryData, function()
        ...
        TriggerClientEvent("aj-containers:client:update_zone", -1, zone_name)
        Notification_S(src, "Success", "success")
    end)
end)

CreateCallback("aj-containers:server:GET:ZONE:containers", function( source, cb, zone_name )
    MySQL.Async.fetchAll("SELECT random_id,position,container_type FROM aj_containers WHERE zone = ? and deleted = false", {
        zone_name
...
RegisterNetEvent("aj-containers:server:container:check_password", function( random_id, password, zone_name )
    local src = source
    MySQL.Async.fetchAll("SELECT container_type, password FROM aj_containers WHERE random_id = ?", {
        random_id
     }, function( res )
        res = res[1]
...
RegisterNetEvent("aj-containers:server:use_bolt_cutter", function( entity, random_id, zone_name )
    local src = source
    local player = Player(src)
    ...
    TriggerClientEvent("aj-containers:targets:use_bolt_cutter", src, entity, random_id, zone_name)
end)

RegisterNetEvent("aj-containers:server:open_with_bolt_cutter", function( random_id, zone_name )
    local src = source
    local player = Player(src)
    if not HasAccessToBoltCutter(src) then
        Notification_S(src, "You can't use boltcutter", "error")
    ...
    MySQL.Async.fetchAll("SELECT container_type FROM aj_containers WHERE random_id = ?", {
        random_id
     }, function( res )
        res = res[1]
        local container_type = GetContainerInfromation(res.container_type)
...
RegisterNetEvent("aj-containers:server:container:change_password", function( random_id, current_password, new_password, zone_name )
    local src = source
    local Player = Player(src)
    local citizenid = GetCitizenId(Player)
    ...
    MySQL.Async.fetchAll("SELECT id,owner_citizenid, password FROM aj_containers WHERE random_id = ?", {
        random_id
     }, function( res )
        res = res[1]
        ...
        if VerifyPassword(src, current_password, res.password, notification) == true then
            MySQL.Async.execute("UPDATE aj_containers SET password = ? WHERE id = ?", {
...
RegisterNetEvent("aj-containers:server:container:transfer_ownership", function( random_id, zone_name, new_owner )
    local src = source
    ...
    MySQL.Async.fetchAll("SELECT id,owner_citizenid FROM aj_containers WHERE random_id = ?", {
        random_id
     }, function( res )
        res = res[1]
        ...
        MySQL.Async.execute("UPDATE aj_containers SET owner_citizenid = ?, password = ? WHERE id = ?", {
            citizenid,
...
RegisterNetEvent("aj-containers:server:container:delete", function( random_id, zone_name )
    local src = source
    local player = Player(src)
    local citizenid = GetCitizenId(player)
    ...
    MySQL.Async.execute("UPDATE aj_containers SET deleted = ?, deleted_by = ? WHERE random_id = ? AND zone = ?", {
        true,
        citizenid,
        random_id,
        zone_name
...
RegisterNetEvent("aj-containers:server:container:update_position", function( random_id, zone_name, new_position )
    local src = source
    local player = Player(src)
    local citizenid = GetCitizenId(player)
    ...
    if not is_super_user(citizenid) then
        Notification_S(src, "Hmm, you can't do that!", "primary")
        TriggerClientEvent("aj-containers:client:update_zone", src, zone_name)
        return
    end

    MySQL.Async.fetchAll("SELECT id,owner_citizenid,container_type FROM aj_containers WHERE random_id = ?", {
        random_id
     }, function( res )
        res = res[1]
        MySQL.Async.execute("UPDATE aj_containers SET position = ? WHERE id = ?", {
...
if Framework == 1 then
    for k, v in pairs(GetContainerItems()) do
        Core.Functions.CreateUseableItem(k, function( source, item )
            ...
            TriggerClientEvent("aj-containers:client:container:place", source, k)
        end)
    end
elseif Framework == 2 then
    for k, v in pairs(GetContainerItems()) do
        Core.RegisterUsableItem(k, function( playerId )
            ...
            TriggerClientEvent("aj-containers:client:container:place", playerId, k)
        end)
    end
end
