...
local function Close()
    local duration = 1
    if Framework() == 1 then
        Core.Functions.Progressbar("aj_container_close", "Close", duration * 1000, false, false, {
...
RegisterNetEvent("aj-containers:client:open", function( metadata )
    if not metadata then return end
    local duration = 1

    if Framework() == 1 then
        Core.Functions.Progressbar("aj_container_opening", "Open", duration * 1000, false, false, {
            disableMovement = true,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true
