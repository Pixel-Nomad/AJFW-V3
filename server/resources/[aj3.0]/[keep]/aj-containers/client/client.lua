...
RegisterNetEvent("aj-containers:client:notification", function( msg, type ) Notification_c(msg, type) end)
...
local function ShowDrawText( text )
    if Framework == 1 then
        exports["aj-base"]:DrawText(text or "Container Depot")
    ...
    for k, v in pairs(Config.container_depots) do
        ...
        ZONE[k]:onPlayerInOut(function( isPointInside )
            if isPointInside then
                ...
                Wait(50)
                ShowDrawText(v.name)
                TriggerCallback("aj-containers:server:GET:ZONE:containers", function( containers ) for k, container in pairs(containers) do Containers:new(container) end end, current_zone)
            else
                current_zone = nil
                Containers:clean_up()
                HideDrawText()
...
RegisterNetEvent("aj-containers:client:update_zone", function( zone_name )
    local current_zone, zone = GetCurrentZone()
    if zone_name == current_zone then
        Containers:clean_up()
        TriggerCallback("aj-containers:server:GET:ZONE:containers", function( containers ) for k, container in pairs(containers) do Containers:new(container) end end, zone_name)
    end
end)
...
if Framework == 1 then
    RegisterNetEvent("AJFW:Client:OnPlayerLoaded", function() Init() end)
elseif Framework == 2 then
