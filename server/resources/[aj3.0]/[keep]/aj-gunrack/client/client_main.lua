...
local function get_closest_vehicle_helper(coords)
     -- it's a modifed aj-base's function
...
local function installing_gunrack()
     ...
     if not plate then
          AJFW.Functions.Notify(Lang:t('error.failed_to_read_plate'), "error")
          return
     end
     TriggerEvent('animations:client:EmoteCommandStart', { "mechanic4" })

     AJFW.Functions.Progressbar("aj_gunrack_install", Lang:t('progressbar.installing_gun_rack'), duration, false,
          false, {
          ...
          TriggerServerEvent('aj-gunrack:server:create_gunrack', plate)
     end)
end
...
RegisterNetEvent('aj-gunrack:client:start_installing_gunrack', function()
     installing_gunrack()
end)

RegisterNetEvent("aj-gunrack:client:open_gunrack", function(plate)
     if not plate then return end
     local duration = Config.gunrack.opening_duration
     AJFW.Functions.Progressbar("aj_gunrack_opening", Lang:t('progressbar.opening_gun_rack'), duration * 1000,
          false, false, {
               disableMovement = true,
...
AddEventHandler("aj-gunrack:client:open_gunrack_menu", function(item)
     ...
     if IsPedInAnyVehicle(player_ped, false) then
          ...
          AJFW.Functions.Notify(Lang:t('error.must_be_in_vehicle'), "error")
          return
     end
...
if Config.gunrack.use_keys_to_unlock_gunrack then
     ...
     RegisterNetEvent('aj-gunrack:client:cut_key', function()
          ...
          if inputData then
               ...
               TriggerServerEvent('aj-gunrack:server:start_cut_key', inputData.PLATE)
          end
     end)

     RegisterNetEvent('aj-gunrack:client:keycuttingmachine_animation', function(plate)
          AJFW.Functions.Progressbar("aj_gunrack_keycuttingmachine", Lang:t('progressbar.cutting_key'),
               duration, false
               ,
               false, {
                    disableMovement = true,
               ...
               TriggerServerEvent('aj-gunrack:server:cut_key', plate)
          end)
     end)
end
