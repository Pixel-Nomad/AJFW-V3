...
local function HasItem(source, Player, item_name)
     ...
     if item then
          TriggerClientEvent('AJFW:Notify', source, "You don't have a Gun Rack?!", 'error')
          return true
     end
     return false
end

AJFW.Functions.CreateUseableItem('policegunrack', function(source, item)
     local Player = AJFW.Functions.GetPlayer(source)
     if not Player then return end
     if not IsJobAllowed(Player.PlayerData.job.name) then
          TriggerClientEvent('AJFW:Notify', source, Lang:t('error.not_authorized'), "error")
          return
     end

     TriggerClientEvent('aj-gunrack:client:start_installing_gunrack', source)
end)
...
if Config.gunrack.use_keys_to_unlock_gunrack then
     ...
     AJFW.Functions.CreateUseableItem('keycuttingmachine', function(source, item)
          local src = source
          local Player = AJFW.Functions.GetPlayer(src)
          if not Player then return end
          if not IsPlayerWhitelisted(Player.PlayerData.citizenid) then
               TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_authorized'), "error")
               return
          end
          TriggerClientEvent('aj-gunrack:client:cut_key', source)
     end)
     ...
     RegisterNetEvent('aj-gunrack:server:start_cut_key', function(plate)
          local src = source
          local Player = AJFW.Functions.GetPlayer(src)
          ...
          if not key then
               TriggerClientEvent('AJFW:Notify', src, Lang:t('error.no_key_found'), "error")
               return
          end

          TriggerClientEvent('aj-gunrack:client:keycuttingmachine_animation', src, plate)
     end)

     --
     RegisterNetEvent('aj-gunrack:server:cut_key', function(plate)
          local src = source
          local Player = AJFW.Functions.GetPlayer(src)
          ...
          if key == nil then
               TriggerClientEvent('AJFW:Notify', src, Lang:t('error.no_unused_keys_found'), "error")
               return
          end
          save_key_info(Player, key, plate)
          TriggerClientEvent('AJFW:Notify', src, Lang:t('success.successful_cutting'), "success")
     end)
end
...
local function AddInitialItems(plate)
     ...
     for key, item in pairs(Config.InitialItems) do
          if AJFW.Shared.Items[item.name] then
               local i = #items + 1
               items[i] = AJFW.Shared.Items[item.name]
...
RegisterNetEvent('aj-gunrack:server:create_gunrack', function(plate)
     local src = source
     local Player = AJFW.Functions.GetPlayer(src)
     ...
     if isAlreadyInstalled(plate) then
          TriggerClientEvent('AJFW:Notify', src, Lang:t('error.already_installed'), "error")
          return
     end
     ...
     if not remove_item(src, Player, 'policegunrack', 1) then
          TriggerClientEvent('AJFW:Notify', src, Lang:t('error.failed_to_use_gunrack'), "error")
          return
     end
     ...
     TriggerClientEvent('AJFW:Notify', src, Lang:t('success.successful_installation'), "success")
...
RegisterNetEvent('aj-gunrack:server:open_gunrack', function(plate, item)
     ...
     if isAlreadyInstalled(plate) then
          ...
          TriggerClientEvent('AJFW:Notify', src, Lang:t('error.dont_have_a_gunrack'), "error")
          return
     end
end)
