...
local function init_database()
     local array = {
          [[
          CREATE TABLE IF NOT EXISTS `shared_garage` (
               `id` int(11) NOT NULL AUTO_INCREMENT,
               `id_shared_garage_categories` int(11) NOT NULL,
          ...
          [[
          CREATE TABLE IF NOT EXISTS `shared_garage_logs` (
          `id` int(11) NOT NULL AUTO_INCREMENT,
          `plate` varchar(50) DEFAULT NULL,
          ...
          CREATE TABLE IF NOT EXISTS `shared_garage_categories` (
          `id` int(11) NOT NULL AUTO_INCREMENT,
          `name` varchar(50) DEFAULT NULL,
...
RegisterNetEvent('aj-sharedgarages:server:update_out_vehicles', function(o)
     o.source = source
     if o.type == 'add' then
          out_vehicles:add(o)
     else
...
CreateCallback('aj-sharedgarages:server:doesVehicleExist', function(source, cb, plate, current_garage)
...
local function vehicle_data_logger(options, cb)
     local sqlQuery = 'INSERT INTO shared_garage_logs (plate,action,citizenid,data) VALUES (?,?,?,?)'
     local tmp = {
...
CreateCallback('aj-sharedgarages:server:get_vehicle_log', function(source, cb, data)
     local LOGS = MySQL.Sync.fetchAll('SELECT plate,action,citizenid,data,DATE_FORMAT(created,"%Y:%m:%d %h:%m:%s") AS Action_timestamp  FROM shared_garage_logs WHERE plate = ? order by Action_timestamp desc limit 15'
          , { data.plate })
     cb(LOGS)
end)
...
local function vehicle_plate_is_owned(plate)
     ...
     local result2 = MySQL.Sync.fetchScalar('SELECT plate FROM shared_garage WHERE plate = ?', { plate })
     return (result or result2)
end

CreateCallback('aj-sharedgarages:server:save_vehicle', function(source, cb, data)
     -- local functions
     ...
     local function get_category_id(Callback)
          ...
          MySQL.Async.fetchScalar('SELECT id FROM shared_garage_categories WHERE name = ? AND garage = ?', { data.category.name, data.garage }, function(id)
     ...
     local function save_in_category(category_id)
          local sqlQuery = 'INSERT INTO shared_garage (id_shared_garage_categories,citizenid,name,model,hash,mods,plate,garage,fuel,engine,body,state,permissions) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)'
          local QueryData = {
...
CreateCallback('aj-sharedgarages:server:can_we_store_this_vehicle', function(source, cb, data)
     MySQL.Async.fetchScalar('SELECT hash FROM shared_garage WHERE plate = ?', { data.VehicleProperties.plate }, function(state)
          cb(state)
     end)
end)

RegisterNetEvent('aj-sharedgarages:server:update_vehicle_name', function(new_name, plate, current_garage)
     local src = source
     ...
     MySQL.Async.execute('UPDATE shared_garage SET name = ? WHERE plate = ?', { new_name or '', plate }, function()
          Notification(src, 'success', 'success')
     end)
end)

RegisterNetEvent('aj-sharedgarages:server:update_vehicle_plate', function(new_plate, plate, current_garage)
     ...
     MySQL.Async.execute('UPDATE shared_garage SET plate = ? WHERE plate = ?', { new_plate or plate, plate }, function()
          Notification(src, 'success', 'success')
     end)
end)

RegisterNetEvent('aj-sharedgarages:server:set_is_customizable', function(state, plate, current_garage)
     local src = source
     local player = AJFW.Functions.GetPlayer(src)
     if not cidWhiteListed(player, current_garage) then
          Notification(src, 'You are not whitelisted', 'error')
     ...
     if state == 'false' then
          MySQL.Async.execute('UPDATE shared_garage SET is_customizable = ? WHERE plate = ?', { 0, plate }, function()
               Notification(src, 'success', 'success')
          end)
     else
          MySQL.Async.execute('UPDATE shared_garage SET is_customizable = ? WHERE plate = ?', { 1, plate }, function()
               Notification(src, 'success', 'success')
          end)
     end
end)

RegisterNetEvent('aj-sharedgarages:server:delete', function(plate, current_garage)
     local src = source
     local player = AJFW.Functions.GetPlayer(src)
     if not cidWhiteListed(player, current_garage) then
          Notification(src, 'You are not whitelisted', 'error')
     ...
     -- state might be no a bool value
     MySQL.Async.execute('DELETE FROM shared_garage WHERE plate = ?', { plate }, function()
          Notification(src, 'success', 'success')
     end)
end)

RegisterNetEvent("aj-sharedgarages:server:update_state", function(plate, properties)
     local src = source
     local Player = AJFW.Functions.GetPlayer(src)
     if not plate then
          print('no plate! (aj-sharedgarages:server:update_state)')
          return
     end
     local STATE = MySQL.Sync.fetchScalar('SELECT state FROM shared_garage WHERE plate = ?', { plate })
     if STATE == false then
          -- save damages too
          if properties ~= nil then
               local s = 'UPDATE shared_garage SET state = ?, garage = ? ,fuel = ? ,engine = ?, body = ?, metadata = ? WHERE plate = ?'
               MySQL.Async.execute(s, {
                    ...
                    if result == 1 then
                         ...
                         local sql = 'UPDATE shared_garage SET mods = ? WHERE plate = ? AND is_customizable = 1'
          ...
          MySQL.Async.execute('UPDATE shared_garage SET state = ? WHERE plate = ?', { 0, plate })
...
AJFW.Commands.Add('saveInsideGarage', 'Save vehicle in shared garage', {}, false, function(source, args)
     local src = source
     -- auth
     TriggerClientEvent('aj-sharedgarages:client:get_current_garage', src, 'aj-sharedgarages:server:saveInsideGarage')
end, 'user')
...
RegisterNetEvent('aj-sharedgarages:server:saveInsideGarage', function(current_garage)
     local src = source
     local function save(_type, _grades, random_plate)
          TriggerClientEvent('aj-sharedgarages:client:get_current_garage', src, 'aj-sharedgarages:server:saveInsideGarage_after', { _type, _grades, random_plate })
     end
     ...
     if garage.job then
          grades = AJFW.Shared.Jobs[garage.job[1]].grades
          save(garage.job[1], grades, random_plate)
     elseif garage.gang then
          grades = AJFW.Shared.Gangs[garage.gang[1]].grades
...
RegisterNetEvent('aj-sharedgarages:server:saveInsideGarage_after', function(current_garage, data)
     local src = source
     MySQL.Async.fetchAll('SELECT * FROM shared_garage_categories WHERE garage = ?', { current_garage }, function(res)
          TriggerClientEvent('aj-sharedgarages:client:newVehicleSetup', src, data[1], data[2], res, data[3])
     end)
end)

CreateCallback('aj-sharedgarages:server:give_keys_to_all_same_job', function(source, cb, PlayerJob)
...
RegisterNetEvent('onResourceStart', function(resourceName)
     ...
     MySQL.Async.execute('UPDATE shared_garage SET state = ? WHERE state = ?', { 1, 0 })
end)

RegisterNetEvent('aj-sharedgarages:server:Notification', function(msg, _type)
     local src = source
     Notification(src, msg, _type)
end)

-- category maker
CreateCallback('aj-sharedgarages:server:GET:category', function(source, cb, data)
     MySQL.Async.fetchAll('SELECT * FROM shared_garage_categories', {}, function(categories)
...
CreateCallback('aj-sharedgarages:server:POST:create_category', function(source, cb, inputData, currentgarage)
     local Player = AJFW.Functions.GetPlayer(source)
     ...
     MySQL.Async.insert('INSERT INTO shared_garage_categories (name , citizenid, garage, icon, type, grades) VALUES (?,?,?,?,?,?)', {
...
CreateCallback('aj-sharedgarages:server:GET:player_job_gang', function(source, cb)
     local src = source
     local Player = AJFW.Functions.GetPlayer(src)
     cb({
          ...
          extra = {
               job = AJFW.Shared.Jobs[Player.PlayerData.job.name].grades,
               gang = AJFW.Shared.Gangs[Player.PlayerData.gang.name].grades
          }
     })
end)

CreateCallback('aj-sharedgarages:server:GET:garage_categories', function(source, cb, current_garage)
     local function count_guest_vehicles(res)
          local q = ''
          local qq = 'SELECT COUNT(id_shared_garage_categories) FROM shared_garage WHERE garage = ? '
          for key, c in ipairs(res) do
               local string = 'AND id_shared_garage_categories != ' .. c.id
               if key > 1 then
                    string = ' AND id_shared_garage_categories != ' .. c.id
               end
               q = q .. string
          end
          qq = qq .. q
     ...
     MySQL.Async.fetchAll('SELECT * FROM shared_garage_categories WHERE garage = ?', { current_garage }, function(res)
          for key, c in pairs(res) do
               c.count = MySQL.Sync.fetchScalar('SELECT COUNT(id_shared_garage_categories) FROM shared_garage WHERE id_shared_garage_categories = ?', { c.id })
          end

          -- check for default category
          local default_count = MySQL.Sync.fetchScalar('SELECT COUNT(id_shared_garage_categories) FROM shared_garage WHERE id_shared_garage_categories = ?', { 0 })
...
CreateCallback('aj-sharedgarages:server:GET:vehicles_on_category', function(source, cb, category_name, current_garage)
     local src = source
     local Player = AJFW.Functions.GetPlayer(src)
     ...
     local function get_category_id(Callback)
          ...
          MySQL.Async.fetchScalar('SELECT id FROM shared_garage_categories WHERE name = ? AND garage = ?', { category_name, current_garage }, function(id)
               if not id then cb({}) return end
               Callback(id)
          end)
     end
     ...
     local function get_guest_vehicles(res)
          local q = ''
          local qq = 'SELECT * FROM shared_garage WHERE garage = ? '
          for key, c in ipairs(res) do
               local string = 'AND id_shared_garage_categories != ' .. c.id
               if key > 1 then
                    string = ' AND id_shared_garage_categories != ' .. c.id
     ...
     local function send_vehicles_list(category_id)
          local tmp = {}
          MySQL.Async.fetchAll('SELECT * FROM shared_garage WHERE id_shared_garage_categories = ? AND garage = ?', { category_id, current_garage }, function(vehicles)
...
CreateCallback('aj-sharedgarages:server:POST:edit_category', function(source, cb, _type, new_data, current_data, currentgarage)
     ...
     if _type == 'name' then
          ...
          sql = 'UPDATE shared_garage_categories SET name = ? WHERE name = ? AND garage = ?'
     elseif _type == 'icon' then
          sql = 'UPDATE shared_garage_categories SET icon = ? WHERE icon = ? AND garage = ?'
     elseif _type == 'grades' then
          sql = 'UPDATE shared_garage_categories SET grades = ? WHERE grades = ? AND garage = ?'
     else
          return
     end
...
CreateCallback('aj-sharedgarages:server:DELETE:category', function(source, cb, category_name, current_garage)
     local src = source
     ...
     local function fetch_vehicles_on_the_category(id)
          local sql = 'UPDATE shared_garage SET id_shared_garage_categories = ? WHERE id = ?'
          MySQL.Async.fetchAll('SELECT id FROM shared_garage WHERE id_shared_garage_categories = ? AND garage = ?', { id, current_garage }, function(vehicles)
     ...
     -- find id_shared_garage_categories and then check the garage name
     -- and then put them on an default category and then delete the category

     get_category_id(fetch_vehicles_on_the_category)

     MySQL.Async.execute('DELETE FROM shared_garage_categories WHERE name = ? AND garage = ?', { category_name, current_garage }, function(res)
          Notification(src, category_name .. ' is deleted!', 'success')
          cb(true)
     end)
end)

CreateCallback('aj-sharedgarages:server:UPDATE:vehicle_category', function(source, cb, plate, new_category, current_garage)
     local src = source
     ...
     local function get_category_id(Callback)
          MySQL.Async.fetchScalar('SELECT id FROM shared_garage_categories WHERE name = ? AND garage = ?', { new_category, current_garage }, function(id)
     ...
     local function fetch_vehicles_on_the_category(category_id)
          local sql = 'UPDATE shared_garage SET id_shared_garage_categories = ? WHERE id = ?'
          MySQL.Async.fetchScalar('SELECT id FROM shared_garage WHERE plate = ? AND garage = ?', { plate, current_garage }, function(id)
