...
function Open_menu()
     Config.menu = 'ajx-menu'
     if Config.menu == 'ajx-menu' then
          aj_menu:garage_menu()
          return
     end
end

AddEventHandler('aj-sharedgarages:menu:open:garage_menu', function(option)
     Open_menu()
end)
...
function aj_menu:categories()
     TriggerCallback('aj-sharedgarages:server:GET:garage_categories', function(categories)
          local Menu = {
               {
                    header = "Go Back",
                    action = function()
          ...
          for _, category in pairs(categories) do
               ...
               Menu[#Menu + 1] = {
                    ...
                    action = function()
                         aj_menu:vehicles_inside_category(category)
                    end
               }
          end
          ...
          if cidWhiteListed(GetCurrentgarage()) then
               Menu[#Menu + 1] = {
                    ...
                    action = function()
                         TriggerCallback('aj-sharedgarages:server:GET:player_job_gang', function(metadata)
                              ...
                              local inputData, reason = exports['ajx-input']:ShowInput(Input)
                              if reason == 'submit' then
                                   ...
                                   TriggerCallback('aj-sharedgarages:server:POST:create_category', function(result)
                                        Wait(50)
                                        aj_menu:categories()
...
local function get_vehicle_data(model)
     ...
     if list.allow_all then
          for key, veh in pairs(AJFW.Shared.Vehicles) do
...
function aj_menu:vehicles_inside_category(category)
     TriggerCallback('aj-sharedgarages:server:GET:vehicles_on_category', function(vehicles)
          local Menu = {
               {
                    header = "Go Back",
                    action = function()
          ...
          for _, vehicle in pairs(vehicles) do
               ...
               if veh_data then
                    Menu[#Menu + 1] = {
                         ...
                         icon = category.icon,
                         action = function()
                              aj_menu:vehicle_actions_menu(vehicle, veh_data, category)
                         end
                    }
               end
          end
                    ...
                    Menu[#Menu + 1] = {
                         ...
                         action = function()
                              TriggerCallback('aj-sharedgarages:server:GET:player_job_gang', function(metadata)
                                   ...
                                   local inputData, reason = exports['ajx-input']:ShowInput(Input)
                                   if reason == 'submit' then
                                        TriggerCallback('aj-sharedgarages:server:POST:edit_category', function(result)
                                             Wait(50)
                                             aj_menu:categories()
                              ...
                              TriggerCallback('aj-sharedgarages:server:GET:player_job_gang', function(metadata)
                                   ...
                                   if reason == 'submit' then
                                        if inputData.conf == 'Confirm' then
                                             TriggerCallback('aj-sharedgarages:server:DELETE:category', function(result)
                                                  Wait(50)
                                                  aj_menu:categories()
                                             end, category.name, GetCurrentgarage())
                                        else
                                             TriggerServerEvent('aj-sharedgarages:server:Notification', "Ok, i won't delete it!", 'error')
                                        end
                                   end
                              end)
                         end
...
function aj_menu:vehicle_actions_menu(vehicle, veh_data, category)
     ...
     if is_restricted_by_cid(vehicle) then
          if check_cids(vehicle) == false then
               TriggerServerEvent('aj-sharedgarages:server:Notification', 'This vehicle is not allowed for you', 'error')
               aj_menu:vehicles_inside_category(category)
               return
          end
     end
     ...
     if is_restricted_by_grades(vehicle) then
          if check_grades(vehicle) == false then
               TriggerServerEvent('aj-sharedgarages:server:Notification', 'This vehicle is not allowed for your rank', 'error')
               aj_menu:vehicles_inside_category(category)
               return
          end
     end
     ...
     if vehicle.state == 0 then
          TriggerServerEvent('aj-sharedgarages:server:Notification', 'Vehicle is already out!', 'error')
          aj_menu:vehicles_inside_category(category)
          return
     end

     TriggerCallback('aj-sharedgarages:server:doesVehicleExist', function(result, per)
          if result == true then
               TriggerServerEvent('aj-sharedgarages:server:Notification', 'Vehicle is already out!', 'error')
               aj_menu:vehicles_inside_category(category)
               return
          ...
          if not currentgarage or not nearspawnpoint then
               TriggerServerEvent('aj-sharedgarages:server:Notification', 'Try to find a free spot!', 'error')
               return
          end

          AJFW.Functions.SpawnVehicle(veh_data.model, function(veh)
               SetEntityAlpha(veh, 100, true)
               AJFW.Functions.SetVehicleProperties(veh, vehicle.mods)
               currentVeh = {
                    veh = veh,
                    plate = vehicle.plate
               }
               ...
               TriggerServerEvent('aj-sharedgarages:server:update_out_vehicles', {
                    type = 'add',
                    netId = NetworkGetNetworkIdFromEntity(veh),
                    plate = vehicle.plate
               })
               RecoverVehicleDamages(veh, vehicle)
               aj_menu:take_out_menu(veh_data, vehicle, veh, per, category)
...
function aj_menu:take_out_menu(data, vehicle, veh, per, category)
     local veh_data = get_vehicle_data(vehicle.model)
     ...
     local Menu = {
          ...
          },
          {
               ...
               action = function()
                    ...
                    TriggerCallback('aj-sharedgarages:server:give_keys_to_all_same_job', function(receivers)
                         for _, id in pairs(receivers) do
                              TriggerServerEvent('aj-vehiclekeys:server:GiveVehicleKeys', id, vehicle.plate)
                         end
                    end, PlayerJob)
                    TriggerServerEvent('aj-sharedgarages:server:update_state', vehicle.plate, {
                         vehicle = veh,
                         fuel = fuel,
                         engine = engine,
                         body = body,
          ...
          Menu[#Menu + 1] = {
               ...
               action = function()
                    ...
                    if reason == 'submit' then
                         ...
                         TriggerServerEvent('aj-sharedgarages:server:update_vehicle_name', inputData.vehicle_name, vehicle.plate, GetCurrentgarage())
                         Wait(50)
                         aj_menu:vehicles_inside_category(category)
                    end
               end
          ...
          Menu[#Menu + 1] = {
               ...
               action = function()
                    TriggerCallback('aj-sharedgarages:server:GET:garage_categories', function(categories)
                         ...
                         for key, _category in pairs(categories) do
                              if _category.name ~= 'default' then
                                   _Menu[#_Menu + 1] = {
                                        ...
                                        action = function()
                                             TriggerCallback('aj-sharedgarages:server:UPDATE:vehicle_category', function(result)
                                                  Wait(50)
                                                  aj_menu:vehicles_inside_category(category)
                         ...
                         exports['ajx-menu']:createMenu(_Menu)
                    end, GetCurrentgarage())
               end
          }
          ...
          Menu[#Menu + 1] = {
               ...
               action = function()
                    ...
                    if reason == 'submit' then
                         ...
                         TriggerServerEvent('aj-sharedgarages:server:set_is_customizable', inputData.customizability, vehicle.plate, GetCurrentgarage())
                         Wait(50)
                         aj_menu:vehicles_inside_category(category)
                    end
               end
               ...
               action = function()
                    ...
                    if reason == 'submit' then
                         ...
                         if inputData.delete == 'true' then
                              TriggerServerEvent('aj-sharedgarages:server:delete', vehicle.plate, GetCurrentgarage())
                              Wait(50)
                              aj_menu:vehicles_inside_category(category)
                         end
                    end
     ...
     local op = exports['ajx-menu']:createMenu(Menu)
...
function aj_menu:vehicle_parking_log(data, per)
     local Menu = {
          {
               header = 'Leave',
               leave = true
     ...
     TriggerCallback('aj-sharedgarages:server:get_vehicle_log', function(LOGS)
          local icons = {
          ...
          if not op then
               ...
               AJFW.Functions.DeleteVehicle(data.veh)
          end
     end, { plate = data.plate })
end

AddEventHandler('aj-sharedgarages:client:take_out', function(data)
     data = data[1]
     local plate = AJFW.Functions.GetPlate(data.vehicle)
     ...
     TriggerServerEvent('aj-sharedgarages:server:update_state', data.plate, data)
end)

AddEventHandler('aj-sharedgarages:client:delete_if_exist', function(plate, veh)
...
AddEventHandler('aj-sharedgarages:client:aj_put_back_to_garage', function(e)
     ...
     if IsInVehicle then
          ...
          local vehcheck, distance = AJFW.Functions.GetClosestVehicle(playercoord)
     ...
     if veh == nil then
          ...
          if CanPlayerUseGarage() and not IsPauseMenuActive() and inGarageStation then
               ...
               if IsPedInAnyVehicle(plyped, false) then
                    TriggerEvent('aj-sharedgarages:client:aj_put_back_to_garage')
                    return
               end
               Open_menu()
          end
     ...
     VehicleProperties.VehicleProperties = AJFW.Functions.GetVehicleProperties(veh)
     TriggerCallback('aj-sharedgarages:server:can_we_store_this_vehicle', function(result)
          if result ~= nil then
               local currentgarage = GetCurrentgarage()
               VehicleProperties.currentgarage = currentgarage
               TriggerServerEvent('aj-sharedgarages:server:update_state', VehicleProperties.VehicleProperties.plate, VehicleProperties)
               TriggerEvent('aj-sharedgarages:client:delete_if_exist', VehicleProperties.VehicleProperties.plate, veh)
               return
          end
          TriggerServerEvent('aj-sharedgarages:server:Notification', "This garage doesn't accept this vehicle!", 'error')
