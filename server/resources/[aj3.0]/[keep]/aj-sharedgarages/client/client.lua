...
-- Events
RegisterNetEvent('aj-sharedgarages:client:newVehicleSetup', function(job, grades, categories, random_plate)
     local function saveVehicle(d, veh)
          if not veh then return end
          local VehicleProperties = AJFW.Functions.GetVehicleProperties(veh)
          local data = {
          ...
          TriggerCallback('aj-sharedgarages:server:save_vehicle', function(res)
     ...
     if not IsPedInAnyVehicle(plyPed, false) then
          TriggerServerEvent('aj-sharedgarages:server:Notification', 'You should be inside a vehicle to use this command', 'error')
          return
     end

     local veh = GetVehiclePedIsIn(plyPed, false)
     if not inGarageStation then
          TriggerServerEvent('aj-sharedgarages:server:Notification', 'You must be in garage zone!', 'error')
          return
     end
...
local function InitGarageZone()
     for k, v in pairs(Config.Garages) do
          ...
          if v.job2 == GetJobInfo().name then
               ...
               GarageLocation[k]:onPlayerInOut(function(isPointInside)
                    if isPointInside then
                         ...
                         if CanPlayerUseGarage() then
                              ...
                              if garage and garage.type == 'job' then
                                   if radialmenu then
                                        exports['aj-radialmenu']:RemoveOption(radialmenu)
                                        radialmenu = nil
                                        Wait(50)
                                   end
                                   radialmenu = exports['aj-radialmenu']:AddOption({
                                        id = 'aj_put_back_to_garage',
                                        title = 'Parking (Job)',
                                        icon = 'car',
                                        type = 'client',
                                        event = 'aj-sharedgarages:client:aj_put_back_to_garage',
                                        shouldClose = true
                                   })
                                   exports['aj-base']:DrawText('Job Parking')
                              elseif garage and garage.type == 'gang' then
                                   if radialmenu then
                                        exports['aj-radialmenu']:RemoveOption(radialmenu)
                                        radialmenu = nil
                                   ...
                                   exports['aj-base']:DrawText('Gang Parking')
                              end
                         end
                    else
                         currentgarage = ''
...
RegisterNetEvent('aj-sharedgarages:client:get_current_garage', function(event, data)
