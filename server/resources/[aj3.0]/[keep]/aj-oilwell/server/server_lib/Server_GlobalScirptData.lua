...
local function blender_calculations(blender)
     ...
     if storage == false then
          -- failsafe
          local player = AJFW.Functions.GetPlayerByCitizenId(blender.citizenid)
          TriggerClientEvent('AJFW:Notify', player.PlayerData.source, "Fetal error could not coonect to your storage!"
               , 'error')
          TriggerClientEvent('AJFW:Notify', player.PlayerData.source, "Fail-safe triggerd shuting down!", 'primary')
...
local function CDUs_calculations(CDU)
     ...
     if CDU.metadata.temp >= 1000 then
          local player = AJFW.Functions.GetPlayerByCitizenId(CDU.citizenid)
          CDU.metadata.temp = 0
          CDU.metadata.state = false
          TriggerClientEvent('aj-oilwell:server_lib:AddExplosion', player.PlayerData.source, 'distillation')
          return
     end
     ...
     if CDU.metadata.oil_storage > 1.0 then
          ...
          if blender == false then
               ...
               TriggerClientEvent('AJFW:Notify', source, "Fetal error could not coonect to your blender!", 'error')
               TriggerClientEvent('AJFW:Notify', source, "Fail-safe triggerd shuting down!", 'primary')
               CDU.metadata.state = false
               return
          end
...
local function oilwell_calculations(oil_well)
     ...
     if oil_well.metadata.speed > 0 then
          ...
          elseif oil_well.metadata.part_info.belt <= 0 then
               ...
               local player = AJFW.Functions.GetPlayerByCitizenId(oil_well.citizenid)
               TriggerClientEvent('AJFW:Notify', player.PlayerData.source, 'Shuting down (broken belt)', 'error')
               TriggerClientEvent('aj-oilrig:client:syncSpeed', -1, oil_well.id, 0)
          end
          ...
          elseif oil_well.metadata.part_info.polish <= 0 then
               ...
               oil_well.metadata.speed            = 0
               local player                       = AJFW.Functions.GetPlayerByCitizenId(oil_well.citizenid)
               TriggerClientEvent('AJFW:Notify', player.PlayerData.source, 'Shuting down (polish value reached zero)',
                    'error')
               TriggerClientEvent('aj-oilrig:client:syncSpeed', -1, oil_well.id, 0)
          end
          ...
          elseif oil_well.metadata.part_info.clutch <= 0 then
               ...
               oil_well.metadata.speed = 0
               local player = AJFW.Functions.GetPlayerByCitizenId(oil_well.citizenid)
               TriggerClientEvent('AJFW:Notify', player.PlayerData.source, 'Shuting down (broken clutch)', 'error')
               TriggerClientEvent('aj-oilrig:client:syncSpeed', -1, oil_well.id, 0)
          end
...
function SendOilToStorage(oilrig, src, cb)
     ...
     if storage == false then
          ...
          TriggerClientEvent('AJFW:Notify', src, "Could not connect to your stroage try again!", 'error')
          cb(false)
          return
     end
     -- add to storage
     ...
     TriggerClientEvent('AJFW:Notify', src, oilrig.metadata.oil_storage .. " Gallon of Curde Oil pumped to storage")
     -- remove from oilwell
...
function SendOilFuelToStorage(player, src, cb)
     ...
     if not blender then
          TriggerClientEvent('AJFW:Notify', src, "Could not connect to your blender try again!", 'error')
          cb(false)
          return
     end
     ...
     if blender.metadata.fuel_oil == 0 then
          TriggerClientEvent('AJFW:Notify', src, "You don't have any fuel oil!", 'error')
          return
     end
     -- add to storage
     storage.metadata.fuel_oil = Round((storage.metadata.fuel_oil + blender.metadata.fuel_oil), 2)
     TriggerClientEvent('AJFW:Notify', src, blender.metadata.fuel_oil .. " Gallon of fuel oil pumped to storage")
     -- remove from oilwell
