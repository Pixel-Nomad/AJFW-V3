...
AJFW.Functions.CreateCallback('aj-oilrig:server:pump_fueloil', function(source, cb, data)
     local player = AJFW.Functions.GetPlayer(source)
     if player == nil then
          TriggerClientEvent('AJFW:Notify', source, "failed to find player!")
          cb(false)
          return
     end
...
AJFW.Functions.CreateCallback('aj-oilrig:server:PumpOilToStorageCallback', function(source, cb, data)
     local player = AJFW.Functions.GetPlayer(source)
     if player == nil then
          TriggerClientEvent('AJFW:Notify', source, "failed to find player!")
          cb(false)
     ...
     if not is_employee and not is_owner then
          TriggerClientEvent('AJFW:Notify', source, "You do no have access to this part!")
          cb(false)
          return
     end
     if oilrig.metadata.oil_storage <= 0 then
          TriggerClientEvent('AJFW:Notify', source, "Oil storage is empty!")
          cb(false)
          return
     end
...
AJFW.Functions.CreateCallback('aj-oilrig:server:getStorageData', function(source, cb)
     local player = AJFW.Functions.GetPlayer(source)
...
AJFW.Functions.CreateCallback('aj-oilrig:server:Withdraw', function(source, cb, data)
     ...
     if data.amount <= 0 then
          TriggerClientEvent('AJFW:Notify', source, "Withdraw must be more than 0!", 'error')
          return
     end
     ...
     if not data.truck then
          ...
          if data.amount > (barrel_max_size * stash_size) then
               TriggerClientEvent('AJFW:Notify', source, "Withdraw stash dont have enough space!", 'error')
               TriggerClientEvent('AJFW:Notify', source, "Maximum: " .. (barrel_max_size * stash_size) .. "/Gal",
                    'error')
               return
          end
     else
          if data.amount > 100000 then
               TriggerClientEvent('AJFW:Notify', source, "Maximum: 100,000/Gal",
                    'error')
               return
          end
     end
     ...
     if storage == false then
          ...
          TriggerClientEvent('AJFW:Notify', source, "Could not find connect to your stroage try again!", 'error')
          cb(false)
          return
     end
     ...
     if value < data.amount then
          TriggerClientEvent('AJFW:Notify', source, "You can not withdraw this much!", 'error')
          TriggerClientEvent('AJFW:Notify', source, "Requested: " .. data.amount .. " Current: " .. value, 'error')
          cb(false)
          return
     end
     ...
     TriggerClientEvent('AJFW:Notify', source, "We compeleted your withdraw request.", 'success')
     cb(true)
end)
...
local function add_oilbarell_2(Player, divide_res, barrel_type, barrel_avg_gas_octane)
     ...
     local itemInfo = AJFW.Shared.Items[item_name:lower()]
...
RegisterNetEvent('aj-oilwell:server:purgeWithdrawStash', function()
     local Player = AJFW.Functions.GetPlayer(source)
     local stash = 'Withdraw_' .. Player.PlayerData.citizenid
     MySQL.Async.execute("UPDATE stashitems SET items = '[]' WHERE stash = ?", { stash })
     TriggerClientEvent('AJFW:Notify', source, "Purge compeleted!", 'success')
end)

AJFW.Functions.CreateCallback('aj-oilrig:server:withdraw_from_queue', function(source, cb, Type)
     local player = AJFW.Functions.GetPlayer(source)
     local citizenid = player.PlayerData.citizenid
     local storage = GlobalScirptData:getDeviceByCitizenId('oilrig_storage', citizenid)
     if storage == false then
     ...
     if type(storage.metadata.queue) == "table" and next(storage.metadata.queue) == nil then
          TriggerClientEvent('AJFW:Notify', source, "You don't have anything in queue!", 'error')
          cb(false)
          return
     end
     for key, barrel in pairs(storage.metadata.queue) do
          ...
          if not barrel.truck then
               ...
               if not stashEmpty and not (size == -1) then
                    TriggerClientEvent('AJFW:Notify', source, "withdraw stash is not empty!", 'error')
                    cb(false)
                    return
               elseif not stashEmpty and size == -1 then
                    TriggerClientEvent('AJFW:Notify', source, "pls, open your withdraw stash for first time!",
                         'error')
                    cb(false)
                    return
               end
               ...
               if removemoeny then
                    ...
                    TriggerClientEvent('AJFW:Notify', source, "Request compeleted!", 'success')
                    cb({ truck = false })
                    storage.metadata.queue[key] = nil
                    return
               else
                    TriggerClientEvent('AJFW:Notify', source, "No money!", 'error')
               end
               cb(storage)
               return
     ...
     TriggerClientEvent('AJFW:Notify', source, "You don't have anything in this queue!", 'error')
     cb(false)
end)

AJFW.Functions.CreateUseableItem('oilbarell', function(source, item)
     local Player = AJFW.Functions.GetPlayer(source)
end)

AJFW.Functions.CreateUseableItem('oilwell', function(source, item)
     local Player = AJFW.Functions.GetPlayer(source)
     local RemovedItem = Player.Functions.RemoveItem('oilwell', 1)
     TriggerClientEvent('aj-inventory:client:ItemBox', source, AJFW.Shared.Items['oilwell'], "remove")

     if item.amount >= 1 and RemovedItem == true then
          TriggerClientEvent('aj-oilrig:client:spawn', source)
     end
end)
...
RegisterNetEvent('aj-oilrig:server:updateSpeed', function(inputData, id)
     local player = AJFW.Functions.GetPlayer(source)
     ...
     local is_employee, is_owner = oilrig.is_employee(player.PlayerData.citizenid)
     if not is_employee and not is_owner then
          TriggerClientEvent('AJFW:Notify', source, "You do not have access to this oilwell!", 'error')
          return
     end

     local speed = tonumber(inputData.speed)
     if not (0 <= speed and speed <= 100) then
          TriggerClientEvent('AJFW:Notify', source, 'speed must be between 0 to 100', "error")
          return
     end

     oilrig.metadata.speed = speed
     -- sync speed on other clients
     TriggerClientEvent('aj-oilrig:client:syncSpeed', -1, id, speed)
end)

AJFW.Functions.CreateCallback('aj-oilrig:server:get_CDU_Data', function(source, cb)
     local player = AJFW.Functions.GetPlayer(source)
     local citizenid = player.PlayerData.citizenid
...
AJFW.Functions.CreateCallback('aj-oilrig:server:set_CDU_temp', function(source, cb, inputData)
...
AJFW.Functions.CreateCallback('aj-oilrig:server:switchPower_of_CDU', function(source, cb)
     local player = AJFW.Functions.GetPlayer(source)
     local citizenid = player.PlayerData.citizenid
     local CDU = GlobalScirptData:getDeviceByCitizenId('oilrig_cdu', citizenid)
...
AJFW.Functions.CreateCallback('aj-oilrig:server:pumpCrudeOil_to_CDU', function(source, cb, inputData)
     ...
     if inputData.amount <= 0 then
          TriggerClientEvent('AJFW:Notify', source, "Must be more than 0", 'error')
          cb(CDU)
          return
     end

     if storage.metadata.crudeOil == 0.0 then
          TriggerClientEvent('AJFW:Notify', source, "Your storage is empty", 'error')
          cb(CDU)
          return
     end
...
AJFW.Functions.CreateCallback('aj-oilrig:server:ShowBlender', function(source, cb)
     local player = AJFW.Functions.GetPlayer(source)
     local citizenid = player.PlayerData.citizenid
...
AJFW.Functions.CreateCallback('aj-oilrig:server:toggle_blender', function(source, cb)
     local player = AJFW.Functions.GetPlayer(source)
     local citizenid = player.PlayerData.citizenid
     local blender = GlobalScirptData:getDeviceByCitizenId('oilrig_blender', citizenid)
...
AJFW.Functions.CreateCallback('aj-oilrig:server:recipe_blender', function(source, cb, inputData)
     local player = AJFW.Functions.GetPlayer(source)
     local citizenid = player.PlayerData.citizenid
     local blender = GlobalScirptData:getDeviceByCitizenId('oilrig_blender', citizenid)
     ...
     for _, value in pairs(inputData) do
          ...
          if not inRange(current_num, 0, 100) then
               TriggerClientEvent('AJFW:Notify', source, "numbers must be between 0-100", 'error')
               return
          end
...
local function remove_item(source, Player, name, slot)
     ...
     TriggerClientEvent('aj-inventory:client:ItemBox', source, AJFW.Shared.Items[name], "remove")
end

AJFW.Functions.CreateCallback('aj-oilrig:server:oil_transport:fillTransportWell', function(source, cb, amount)
     ...
     if not oil_barrel then
          TriggerClientEvent('AJFW:Notify', source, 'You do not have a oil barrel!', 'error')
          return
     end
     local current_info = oil_barrel.info

     if not current_info.type then
          TriggerClientEvent('AJFW:Notify', source, 'Failed to get oil type', 'error')
          return
     end

     if not (current_info.type == 'crudeOil' or current_info.type == 'fuel_oil' or current_info.type == 'gasoline') then
          TriggerClientEvent('AJFW:Notify', source, 'We dont export this type of oil!', 'error')
          return
     end

     if reachedMaxStock(current_info.type) then
          -- when we reached max stock
          TriggerClientEvent('AJFW:Notify', source, 'Currently we can not accept more offers pls come back later!',
               'primary')
          return
     end
     ...
     if not canWeAcceptMoreStock(current_info.type, amount) then
          ...
          TriggerClientEvent('AJFW:Notify', source, '', 'error')
          return
     end
     ...
     if current_info.gal < amount then
          ...
          TriggerClientEvent('AJFW:Notify', source, msg_string, 'error')
          cb(false)
          return
     end
     ...
     if current_info.gal > amount then
          ...
          TriggerClientEvent('AJFW:Notify', source, msg_string, 'success')

          TriggerClientEvent('aj-oilrig:client:local_mail_sender', source, {
               gender = gender,
               charinfo = charinfo,
               refund = 0,
               money = cost,
     ...
     elseif current_info.gal == amount and amount ~= 0 then
          ...
          TriggerClientEvent('AJFW:Notify', source, 'You either do not have a oil barell or its empty!', 'error')
          cb(false)
          return
     end
...
RegisterNetEvent('aj-oilrig:server:oil_transport:checkPrice', function()
     local names      = {
          crudeOil = 'Crude Oil',
          gasoline = 'Gasoline',
          fuel_oil = 'Fuel Oil'
     ...
     for key, value in pairs(current_transport_stock) do
          ...
          TriggerClientEvent('AJFW:Notify', source, s, 'primary', 7500)
     end
...
---@param cb table
AJFW.Functions.CreateCallback('aj-oilrig:server:getNetIDs', function(source, cb)
     local player = AJFW.Functions.GetPlayer(source)
     local citizenid = player.PlayerData.citizenid
     local temp = {}
...
---@param coords table
---@return 'NetId'
AJFW.Functions.CreateCallback('aj-oilrig:server:createNewOilrig', function(source, cb, coords)
...
AJFW.Functions.CreateCallback('aj-oilrig:server:regiserOilrig', function(source, cb, inputData)
     -- get player by entered cid
     local cid = tonumber(inputData.cid)
     local player = AJFW.Functions.GetPlayer(cid)
     if player ~= nil then
          ...
          TriggerClientEvent('aj-oilwell:client:force_reload', -1)
          cb(true)
     else
          TriggerClientEvent('AJFW:Notify', source, "Could not find player by it cid!")
          cb(false)
     end
end)
...
AJFW.Functions.CreateCallback('aj-oilwell:server:fix_oil_well', function(source, cb, oilrig_hash)
     local Player = AJFW.Functions.GetPlayer(source)
     ...
     TriggerClientEvent('AJFW:Notify', source, "Items used to fix oilwell.", 'primary')
     MySQL.Async.execute("UPDATE stashitems SET items = '[]' WHERE stash = ?", { stash })
     cb(true)
end)

AJFW.Functions.CreateCallback('aj-oilwell:server:is_employee', function(source, cb, oilrig_hash)
     local Player = AJFW.Functions.GetPlayer(source)
     local oil_well = GlobalScirptData:getByHash(oilrig_hash)
     if not oil_well then cb(false) return end
...
AJFW.Functions.CreateCallback('aj-oilwell:server:employees_list', function(source, cb, oilrig_hash)
     local Player = AJFW.Functions.GetPlayer(source)
     local oil_well = GlobalScirptData:getByHash(oilrig_hash)
     if not oil_well then cb(false) return end
     local is_employee, is_owner = oil_well.is_employee(Player.PlayerData.citizenid)
     if not is_employee and not is_owner then
          TriggerClientEvent('AJFW:Notify', source, "You can not see this list!", 'error')
          cb(false)
          return
     end
     ...
     for index, value in ipairs(list) do
          value.id = nil
          local Player = AJFW.Functions.GetPlayerByCitizenId(value.citizenid)
          if Player then
               value.charinfo = Player.PlayerData.charinfo
               value.online = true
          else
               Player = AJFW.Player.GetOfflinePlayer(value.citizenid)
               if not Player then
                    value.charinfo = {
                         firstname = 'deleted',
                         lastname = ''
...
RegisterNetEvent('aj-oilwell:server:add_employee', function(oilrig_hash, state_id)
     ...
     if not is_owner then
          TriggerClientEvent('AJFW:Notify', src, "You must be owner of this oilwell!", 'error')
          return
     end
     local new_employee = AJFW.Functions.GetPlayer(state_id)
     if not new_employee then
          TriggerClientEvent('AJFW:Notify', src, "Wrong state id!", 'error')
          return
     end

     if new_employee.PlayerData.citizenid == oil_well.citizenid then
          TriggerClientEvent('AJFW:Notify', src, "You can not add owner as an employee", 'error')
          return
     end
     ...
     MySQL.Async.execute(sqlQuery, QueryData, function()
          ...
          TriggerClientEvent('AJFW:Notify', src, "New employee added to the list", 'success')
     end)
end)

RegisterNetEvent('aj-oilwell:server:remove_employee', function(oilrig_hash, citizenid)
     ...
     if citizenid == oil_well.citizenid then
          TriggerClientEvent('AJFW:Notify', src, "You can not fire yourself", 'error')
          return
     end
...
AJFW.Functions.CreateCallback('aj-oilwell:server:oilwell_metadata', function(source, cb, oilrig_hash)
     local oil_well = GlobalScirptData:getByHash(oilrig_hash)
     if not oil_well then cb(false) return end
     cb(oil_well.metadata)
end)

RegisterNetEvent('aj-oilwell:server:remove_oilwell', function(oilrig_hash)
     ...
     MySQL.Async.execute(sqlQuery, { 1, oil_well.oilrig_hash }, function()
          TriggerClientEvent('AJFW:Notify', src,
...
AJFW.Commands.Add('create', 'create new oilwell', {}, false, function(source, args)
