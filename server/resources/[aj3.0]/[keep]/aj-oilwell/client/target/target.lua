...
local function makeCore()
     ...
     Citizen.CreateThread(function()
          ...
          exports['aj-target']:AddBoxZone("aj_oilwell_withdraw_truck_target", vec3_coord,
               Oilwell_config.TruckWithdraw.box.l,
               Oilwell_config.TruckWithdraw.box.w,
               {
                    name = "aj_oilwell_withdraw_truck_target",
...
AddEventHandler('aj-oilwell:client:refund_truck', function(data)
     ...
     if #(coord - spawnLocation) > 5.0 then
          AJFW.Functions.Notify('You are not close to truck refunding area', "primary")
          return
     end
     AJFW.Functions.TriggerCallback('aj-oilwell:server:refund_truck', function(result)
          if result == true then
