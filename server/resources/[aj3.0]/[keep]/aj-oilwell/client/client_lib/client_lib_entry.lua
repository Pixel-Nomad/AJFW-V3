...
function isOwner(entity)
     ...
     local is_owner = nil
     -- await didn't work!
     AJFW.Functions.TriggerCallback('aj-oilwell:server:is_employee', function(_is_employee, _is_owner)
...
function createOwnerAjTarget(hash, coord)
     exports['aj-target']:RemoveZone("oil-rig-" .. hash)
     Targets.aj_target.oilwell(coord, hash)
end

RegisterNetEvent('aj-oilwell:client:remove_oilwell', function(data)
     ...
     for i = 1, 3, 1 do
          ...
          if not inputData then
               AJFW.Functions.Notify('Canceled', "primary")
               return
          end
          if inputData.RandomHash ~= value then
               AJFW.Functions.Notify('Failed', "primary")
               return
          end
     end
     TriggerServerEvent('aj-oilwell:server:remove_oilwell', oilwell.oilrig_hash)
end)

function Add_3rd_eye(coord, Type)
     local key = Type
     if key == 'storage' then
          Targets.aj_target.storage(coord, key)
     elseif key == 'distillation' then
          Targets.aj_target.distillation(coord, key)
     elseif key == 'blender' then
          Targets.aj_target.blender(coord, key)
     elseif key == 'barrel_withdraw' then
          Targets.aj_target.barrel_withdraw(coord, key)
     elseif key == 'crude_oil_transport' then
          Targets.aj_target.crude_oil_transport(coord, key)
     elseif key == 'toggle_job' then
          Targets.aj_target.toggle_job(coord, key)
     end
end

---force remove objects in area
---@param coord table
RegisterNetEvent('aj-oilrig:client:clearArea', function(coord)
     ClearAreaOfObjects(coord.x, coord.y, coord.z, 5.0, 1)
end)

RegisterNetEvent('AJFW:Client:OnJobUpdate', function(PlayerJob)
     if CheckJob() then
          OnDuty = CheckOnduty()
     end
end)

RegisterNetEvent('aj-oilrig:client:goOnDuty', function(PlayerJob)
