...
local function showblender(data)
     ...
     openMenu[#openMenu + 1] = {
          ...
          params = {
               event = "aj-oilrig:blender_menu:recipe_blender"
          }
     }
     ...
     openMenu[#openMenu + 1] = {
          ...
          icon = start_icon,
          params = {
               event = "aj-oilrig:blender_menu:toggle_blender"
          }
     }
     ...
     openMenu[#openMenu + 1] = {
          ...
          params = {
               event = "aj-oilrig:blender_menu:pump_fueloil"
          }
     }
...
AddEventHandler('aj-oilrig:blender_menu:pump_fueloil', function()
     AJFW.Functions.TriggerCallback('aj-oilrig:server:pump_fueloil', function(result)
          showblender(result)
     end)
end)

AddEventHandler('aj-oilrig:blender_menu:ShowBlender', function()
     AJFW.Functions.TriggerCallback('aj-oilrig:server:ShowBlender', function(result)
          showblender(result)
     end)
end)

AddEventHandler('aj-oilrig:blender_menu:toggle_blender', function()
     AJFW.Functions.TriggerCallback('aj-oilrig:server:toggle_blender', function(result)
          showblender(result)
     end)
end)
...
AddEventHandler('aj-oilrig:blender_menu:recipe_blender', function()
     ...
     if inputData then
          ...
          for _, value in pairs(inputData) do
               if not inRange(tonumber(value), 0, 100) then
                    AJFW.Functions.Notify('numbers must be between 0-100', "primary")
                    return
               end
          end

          AJFW.Functions.TriggerCallback('aj-oilrig:server:recipe_blender', function(result)
               showblender(result)
          end, inputData)
     end
end)
