...
function OilRigs:update(s_res, id)
     ...
     s_res.Ajtarget = self.data_table[id].Ajtarget
     self.data_table[id] = s_res
     AJFW.Functions.TriggerCallback('aj-oilwell:server:oilwell_metadata', function(metadata)
          self:syncSpeed(self.data_table[id].entity, metadata.speed)
     end, self.data_table[id].oilrig_hash)
end

function OilRigs:startUpdate(cb)
     AJFW.Functions.TriggerCallback('aj-oilrig:server:getNetIDs', function(result)
...
--
RegisterNetEvent('aj-oilrig:client:changeRigSpeed', function(Ajtarget)
     if not CheckJob() then
          AJFW.Functions.Notify('You not a hired by oil company', "error")
          return false
     end
     ...
     local rig = OilRigs:getByEntityHandle(Ajtarget.entity)
     if not rig then
          ...
          OilRigs:startUpdate(function()
               ...
               if inputData then
                    ...
                    if not (0 <= speed and speed <= 100) then
                         AJFW.Functions.Notify('speed must be between 0 to 100', "error")
                         return
                    end
                    AJFW.Functions.Notify('oilwell speed changed to ' .. speed, "success")
                    TriggerServerEvent('aj-oilrig:server:updateSpeed', inputData, rig.id)
               end
          end)
     end, rig.oilrig_hash)
end)
...
RegisterNetEvent('aj-oilrig:client:syncSpeed', function(id, speed)
...
function spawnObjects(model, position)
     TriggerEvent('aj-oilrig:client:clearArea', position.coord)
...
RegisterNetEvent('aj-oilrig:client:spawn')
AddEventHandler('aj-oilrig:client:spawn', function()
     local coords = ChooseSpawnLocation()
     AJFW.Functions.TriggerCallback('aj-oilrig:server:createNewOilrig', function(NetId)
          if NetId ~= nil then
               local entity = NetworkGetEntityFromNetworkId(NetId)
               OBJECT = entity
               exports['aj-target']:AddEntityZone("oil-rig-" .. entity, entity, {
                    name = "oil-rig-" .. entity,
                    ...
                    options = {
                         ...
                         {
                              type = "client",
                              event = "aj-oilwell:menu:OPENMENU",
                              icon = "fa-regular fa-file-lines",
                              label = "Adjust position",
                              canInteract = function(entity)
                                   if not CheckJob() then
                                        TriggerEvent('AJFW:Notify', 'Only CEO have access to this', "error")
                                        Wait(2000)
                                        return false
                                   end
                                   if not (PlayerJob.grade.level == 4) then
...
RegisterNetEvent('aj-oilrig:client:enterInformation', function(Ajtarget)
     local inputData = exports['aj-input']:ShowInput({
     ...
     if inputData then
          ...
          local netId = NetworkGetNetworkIdFromEntity(Ajtarget.entity)

          inputData.netId = netId
          AJFW.Functions.TriggerCallback('aj-oilrig:server:regiserOilrig', function(result)
               DeleteEntity(Ajtarget.entity)
               if result == true then
                    Wait(1500)
                    AJFW.Functions.Notify('Registering oilwell to: ' .. inputData.cid, "success")
                    loadData()
               end
          end, inputData)
     end
...
RegisterNetEvent('aj-oilwell:client:force_reload', function()
     Wait(25)
     loadData()
end)
...
RegisterNetEvent('aj-oilrig:client:local_mail_sender', function(data)
...
RegisterNetEvent('aj-oilwell:server_lib:AddExplosion', function(bullding_type)
