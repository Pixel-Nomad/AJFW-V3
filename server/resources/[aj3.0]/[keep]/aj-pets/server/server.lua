...
function Pet:spawnPet(source, model, item)
    ...
    if limit == true then
        TriggerClientEvent('AJFW:Notify', source, string.format(Lang:t('error.reached_max_allowed_pet'), maxLimit),
            'error', 2500)
        return
    end
    ...
    TriggerClientEvent('aj-pets:client:callCompanion', source, model, owner, item)
end

RegisterNetEvent('aj-pets:server:despwan_not_owned_pet', function(hash)
...
function Pet:despawnPet(source, item, revive)
    ...
    TriggerClientEvent('aj-pets:client:despawn', source, item, revive)
end
...
function Pet:save_all_info(source, hash)
    ...
    if petData.info.health > 100 then
        ...
        TriggerClientEvent('aj-pets:client:forceKill', source, hash, 'hunger')
    end
...
RegisterNetEvent('aj-pets:server:setAsDespawned', function(item)
...
local function remove_item(src, Player, name, amount)
    ...
    if res then
        TriggerClientEvent('aj-inventory:client:ItemBox', src, AJFW.Shared.Items[name], "remove")
    end
    return res
end

-- food
AJFW.Functions.CreateUseableItem(core_items.food.item_name, function(source, item)
    local Player = AJFW.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('aj-pets:client:start_feeding_animation', source)
end)

RegisterNetEvent('aj-pets:server:increaseFood', function(item)
    local Player = AJFW.Functions.GetPlayer(source)
    if Player == nil or item == nil then return end
    if not remove_item(source, Player, Config.core_items.food.item_name, 1) then
        TriggerClientEvent('AJFW:Notify', source, 'Failed to remove from your inventory', 'error', 2500)
        return
    end
    local petData = Pet:findbyhash(source, item.info.hash)
    petData.info.food = petData.info.food + 50
    TriggerClientEvent('AJFW:Notify', source, 'Feeding was successful wait little bit to take effect!', 'success', 2500)
end)

-- change owenership
AJFW.Functions.CreateUseableItem(core_items.collar.item_name, function(source, item)
    TriggerClientEvent('aj-pets:client:collar_process', source)
end)

-- rename - name tag
AJFW.Functions.CreateUseableItem(core_items.nametag.item_name, function(source, item)
    local Player = AJFW.Functions.GetPlayer(source)
    if Player == nil or item == nil then return end
    TriggerClientEvent('aj-pets:client:rename_name_tag', source, item)
end)

RegisterNetEvent('aj-pets:server:rename_name_tag', function(name)
    local Player = AJFW.Functions.GetPlayer(source)
    if Player == nil then return end

    if not remove_item(source, Player, Config.core_items.nametag.item_name, 1) then
        TriggerClientEvent('AJFW:Notify', source, Lang:t('error.failed_to_remove_item_from_inventory'), 'error', 2500)
        return
    end

    TriggerClientEvent("aj-pets:client:rename_name_tagAction", source, name)
end)

-- first aid - revive
AJFW.Functions.CreateUseableItem(core_items.firstaid.item_name, function(source, item)
    local Player = AJFW.Functions.GetPlayer(source)
    if Player == nil then return end

end)

AJFW.Functions.CreateUseableItem(core_items.groomingkit.item_name, function(source, item)
    local Player = AJFW.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('aj-pets:client:start_grooming_process', source)
end)

RegisterNetEvent('aj-pets:server:grooming_process', function(item)
    ...
    TriggerClientEvent('aj-pets:client:initialization_process', source, item, information)
end)
...
local function fillwater_bottle(source, item)
    ...
    if type(item.info) ~= "table" or (type(item.info) == "table" and item.info.liter == nil) then
        ...
        TriggerClientEvent('AJFW:Notify', source, 'Washing water bottle!', 'primary', 2500)
        return
    end
    ...
    if item.info.liter > max_c then
        TriggerClientEvent('AJFW:Notify', source, 'could not do that already reached max capacity', 'error', 2500)
        return
    elseif item.info.liter == max_c then
        amount = max_c
        TriggerClientEvent('AJFW:Notify', source, 'filling already filled bottle has no effect on capacity',
            'error', 2500)
    else
    ...
    if type(amount) ~= "number" then
        TriggerClientEvent('AJFW:Notify', source, 'Failed to get amount', 'error', 2500)
        return
    end
    save_info_waterbottle(Player, item, amount)
    TriggerClientEvent('AJFW:Notify', source, 'Filled bottle', 'success', 2500)
end

AJFW.Functions.CreateUseableItem(core_items.waterbottle.item_name, function(source, item)
    local Player = AJFW.Functions.GetPlayer(source)
    local water_bottle_refill_value = Config.core_items.waterbottle.settings.water_bottle_refill_value
    if Player == nil then return end
    if not remove_item(source, Player, 'water_bottle', water_bottle_refill_value) then
        ...
        TriggerClientEvent('AJFW:Notify', source, msg, 'error', 2500)
        return
    end
    TriggerClientEvent('aj-pets:client:filling_animation', source, item)
end)

RegisterNetEvent('aj-pets:server:filling_event', function(item)
    fillwater_bottle(source, item)
end)

AJFW.Functions.CreateCallback('aj-pets:server:decrease_thirst', function(source, cb, data)
    local player = AJFW.Functions.GetPlayer(source)
    ...
    if pet_water_bottle.info == nil then
        TriggerClientEvent('AJFW:Notify', source, 'You should wash water bottle first!', 'error', 2500)
        print('issue with nill info: https://github.com/swkeep/aj-pets/issues/25')
        return
    end
    ...
    if pet_water_bottle.info.liter == nil then
        ...
        print('developer: issue with nill info -> liter: https://github.com/swkeep/aj-pets/issues/25')
        return
    end
    ...
    if pet_water_bottle.info.liter < 0 then
        TriggerClientEvent('AJFW:Notify', source, Lang:t('error.not_enough_water_in_your_bottle'), 'error', 2500)
        return
    end
    ...
    TriggerClientEvent('AJFW:Notify', source, Lang:t('success.successful_drinking'), 'success', 2500)
    save_info_waterbottle(player, pet_water_bottle, pet_water_bottle.info.liter)
end)

RegisterNetEvent('aj-pets:server:revivePet', function(item, process_type)
    local src = source
    ...
    if not petData then
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.failed_to_start_procces') .. process_type, 'primary', 2500)
        return
    end
    ...
    if petData.info.health >= pet_maxHealth then
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t('info.full_life_pet'), 'primary', 2500)
        return
    end

    if not remove_item(source, Player, Config.core_items.firstaid.item_name, 1) then
        TriggerClientEvent('AJFW:Notify', src, 'Failed to remove from your inventory', 'error', 2500)
        return
    end
    ...
    if process_type and process_type == 'Heal' then
        ...
        TriggerClientEvent('aj-pets:client:update_health_value', src, item, petData.info.health)
        TriggerClientEvent('AJFW:Notify', src, msg, 'success', 2500)
        return
    end
...
-- all pets
for key, value in pairs(Config.pets) do
    AJFW.Functions.CreateUseableItem(value.name, function(source, item)
        ...
        if type(item.info) ~= "table" or (type(item.info) == "table" and item.info.hash == nil) then
            ...
            TriggerClientEvent('AJFW:Notify', source, Lang:t('success.pet_initialization_was_successful'), 'success',
                2500)
            return
        end
        ...
        if cooldown > 0 then
            ...
            TriggerClientEvent('AJFW:Notify', source, msg, 'primary', 2500)
            return
        end
...
AJFW.Functions.CreateCallback('aj-pets:server:search_inventory', function(source, cb, cid)
    local res = search_inventory(cid)
    if not res then
        TriggerClientEvent('AJFW:Notify', source, 'K9 could not find anything!', 'error', 2500)
        cb(res)
        return
    end
    cb(res)
...
AJFW.Functions.CreateCallback('aj-pets:server:search_vehicle', function(source, cb, data)
    ...
    TriggerClientEvent('AJFW:Notify', source, 'K9 found something', 'success', 2500)
    cb(res)
end)
-- ================================================
...
function FindWhereIsItem(Player, item, source)
    if Player.PlayerData.items == nil or next(Player.PlayerData.items) == nil then
        TriggerClientEvent('AJFW:Notify', source, "no items in inventory!")
        return false
    end
    ...
    TriggerClientEvent('AJFW:Notify', source, "Could not find pet")
    return false
end

RegisterNetEvent('aj-pets:server:updateAllowedInfo', function(item, data)
...
AJFW.Functions.CreateCallback('aj-pets:server:renamePet', function(source, cb, item)
    local player = AJFW.Functions.GetPlayer(source)
    local current_pet_data = Pet:findbyhash(source, item.hash)
    ...
    if player == nil or current_pet_data == nil or current_pet_data == false or type(item.name) ~= "string" then
        ...
        TriggerClientEvent('AJFW:Notify', source, msg, 'error')
        cb(false)
        return
    end
...
AJFW.Functions.CreateCallback('aj-pets:server:updatePedData', function(source, cb, clientRes)
    local player = AJFW.Functions.GetPlayer(source)
    if player == nil then
        cb(false)
        return
...
RegisterNetEvent('aj-pets:server:onPlayerUnload', function(items)
...
RegisterNetEvent('aj-pets:server:ForceRemoveNetEntity', function(netId)
