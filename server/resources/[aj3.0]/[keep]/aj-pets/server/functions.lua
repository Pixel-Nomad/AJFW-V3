...
function initItem(source, item)
    ...
    item.info.hash = tostring(AJFW.Shared.RandomInt(2) ..
        AJFW.Shared.RandomStr(3) .. AJFW.Shared.RandomInt(1) ..
        AJFW.Shared.RandomStr(2) .. AJFW.Shared.RandomInt(3) .. AJFW.Shared.RandomStr(4))
    ...
    if Config.Settings.let_players_cutomize_their_pet_after_purchase then
        ...
        TriggerClientEvent('aj-pets:client:initialization_process', src, item, information)
    end
end
...
RegisterNetEvent('aj-pets:server:compelete_initialization_process', function(item, process_type)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if not Player then return end

    TriggerEvent('aj-pets:server:aj-pets:server:compelete_initialization_process_last_step', source, item,
        Player, process_type)
    if process_type == 'init' then return end
    Player.Functions.RemoveItem(Config.core_items.groomingkit.item_name, 1)
end)

RegisterNetEvent('aj-pets:server:aj-pets:server:compelete_initialization_process_last_step',
    function(src, item, Player, process_type)
        ...
        if process_type == Config.core_items.groomingkit.item_name then
            ...
            if Player.PlayerData.charinfo.phone ~= petData.info.owner.phone then
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_owner_of_pet'), 'error', 2500)
                return
            end
        ...
        if process_type == Config.core_items.groomingkit.item_name then
            TriggerClientEvent('AJFW:Notify', src, Lang:t('success.successful_grooming'), 'success', 2500)
...
function Update:xp(source, current_pet_data)
    ...
    if current_pet_data.info.XP > current_level_max_xp(level) then
        ...
        TriggerClientEvent('AJFW:Notify', source, msg)
    end
end
...
AJFW.Functions.CreateCallback('aj-pets:server:collar_change_owenership', function(source, cb, data)
    if type(data.new_owner_cid) == "string" then data.new_owner_cid = tonumber(data.new_owner_cid) end
    local player_owner = AJFW.Functions.GetPlayer(source)
    if player_owner == nil then return end
    local player_new_owner = AJFW.Functions.GetPlayer(data.new_owner_cid)
...
function PlayersCooldown:cleanOflinePlayers()
    local onlinePlayers = AJFW.Functions.GetPlayers()
