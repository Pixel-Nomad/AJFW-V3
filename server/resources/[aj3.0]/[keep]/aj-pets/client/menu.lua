...
local function updatePlayerJob()
    repeat
        Wait(10)
    until AJFW.Functions.GetPlayerData().job ~= nil
    ...
    [2] = {
        ...
        action = function(plyped, activePed)
            ...
            if activePed.canHunt ~= true then
                AJFW.Functions.Notify(Lang:t('menu.action_menu.error.pet_unable_to_hunt'), 'error', 5000)
                return false
            end

            if alreadyHunting.state ~= false then
                AJFW.Functions.Notify(Lang:t('menu.action_menu.error.already_hunting_something'), 'error', 5000)
                return
            end
            ...
            if activePed.itemData.info.level <= min_lvl_to_hunt then
                ...
                AJFW.Functions.Notify(msg, 'error', 5000)
                return false
            end
...
function k9SearchVehicle(veh, activePed)
    if not isModelK9(activePed.model) then
        AJFW.Functions.Notify('This pet can not do that!', "error", 1500)
        return
    end
    if not PlayerJob then return end
    if not (PlayerJob.name == 'police') then
        AJFW.Functions.Notify('You are not allowed to do this action', "error", 1500)
        return
    end

    if not PlayerJob.onduty == true then
        AJFW.Functions.Notify('You Must be on duty to do this action', "error", 1500)
        return
    end
    ...
    for key, value in pairs(coo) do
        ...
        Wait(2000)
        AJFW.Functions.TriggerCallback('aj-pets:server:search_vehicle', function(result)
            if result then
...
-- Command
AddEventHandler('aj-pets:client:actionMenuDispatcher', function(option)
        ...
        if option.type == values.TYPE then
            if values.action(plyped, activePed) == true then
                if values.triggerNotification ~= nil then
                    AJFW.Functions.Notify(replaceString(values.triggerNotification[1]), 'success', 1500)
                end
            else
                if values.triggerNotification ~= nil then
                    AJFW.Functions.Notify(replaceString(values.triggerNotification[2]))
                end
            end
        end
    end
...
AddEventHandler('aj-pets:client:main_menu', function()
    ...
    local openMenu = { {
        ...
        params = {
            event = "aj-pets:client:action_menu"
        }
    }, {
        ...
        params = {
            event = "aj-pets:client:switchControl_menu"
        }
    }, {
...
AddEventHandler('aj-pets:client:action_menu', function()
    local name = ActivePed.read().itemData.info.name
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "aj-pets:client:main_menu",
            }
        },
        {
            header = header,
    ...
    for key, value in ipairs(menu) do
        ...
        openMenu[#openMenu + 1] = {
            ...
            txt = value.desc or "",
            params = {
                event = "aj-pets:client:actionMenuDispatcher",
                args = {
                    type = value.TYPE,
                    menu = menu
                }
    ...
    openMenu[#openMenu + 1] = {
        ...
        txt = "",
        params = {
            event = "aj-pets:client:tricks_menu"
        }
    }
...
AddEventHandler('aj-pets:client:tricks_menu', function()
    local name = ActivePed.read().itemData.info.name
...
AddEventHandler('aj-pets:client:switchControl_menu', function()
    local name = ActivePed.read().itemData.info.name
    ...
    for key, value in pairs(ActivePed:petsList()) do
        openMenu[#openMenu + 1] = {
            ...
            icon = 'fa-solid fa-' .. key,
            params = {
                event = "aj-pets:client:switchControl_event",
                args = {
                    index = value.key
                }
            }
...
AddEventHandler('aj-pets:client:switchControl_event', function(option)
    if option.index < 0 then
        return
    end
    ActivePed:switchControl(option.index)
    TriggerEvent('aj-pets:client:action_menu')
end)
...
RegisterCommand('+showMenu', function()
    ...
    if ((IsDowned() and IsPoliceOrEMS()) or not IsDowned()) and not Ishandcuffed() and not IsPauseMenuActive() and
        ...
        if doesPlayerHavePet == nil then
            AJFW.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
            return
        end

        TriggerEvent('aj-pets:client:main_menu')
    end
end, false)
...
--           Customization menu
-- =======================================
RegisterNetEvent('aj-pets:client:start_grooming_process', function()
    local activePed = ActivePed:read()
    if type(activePed) ~= "table" then
        AJFW.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
        return
    end
    TriggerServerEvent('aj-pets:server:grooming_process', activePed.itemData)
end)

RegisterNetEvent('aj-pets:client:initialization_process', function(item, pet_information)
    if type(item) ~= "table" then
        AJFW.Functions.Notify(Lang:t('error.failed_to_start_procces'), 'error', 5000)
        return
    end
    if pet_information.type == 'init' then
        TriggerEvent('aj-pets:client:openMenu_customization', {
            item = item, pet_information = pet_information
        })
        return
    end
    local hasitem = AJFW.Functions.HasItem(Config.core_items.groomingkit.item_name)

    if not hasitem then AJFW.Functions.Notify('you need grooming kit', 'error', 5000) return end
    TriggerEvent('aj-pets:client:openMenu_customization', {
        item = item, pet_information = pet_information
    })
end)

AddEventHandler('aj-pets:client:openMenu_customization', function(data)
    openMenu_customization(data)
end)

AddEventHandler('aj-pets:client:openMenu_customization_rename', function(data)
...
local function rename(data)
    ...
    if inputData then
        ...
        if type(validation) == "table" and next(validation) ~= nil then
            AJFW.Functions.Notify(Lang:t('error.failed_to_validate_name'), 'error', 5000)
            if validation.reason == 'badword' then
                AJFW.Functions.Notify(Lang:t('error.badword_inside_pet_name'), 'error', 5000)
                print_table(validation.words)
                TriggerEvent('aj-pets:client:openMenu_customization_rename', data)
                return
            elseif validation.reason == 'maxCharacter' then
                AJFW.Functions.Notify(Lang:t('error.more_than_one_word_as_name'), 'error', 5000)
                TriggerEvent('aj-pets:client:openMenu_customization_rename', data)
                return
            end
            return
...
AddEventHandler('aj-pets:client:openMenu_customization_rename:rename', function(data)
    rename(data)
end)

AddEventHandler('aj-pets:client:openMenu_customization_select_variation', function(data)
    ...
    local openMenu = {
        {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = "aj-pets:client:openMenu_customization_select_variation",
                args = {
                    item = item,
...
AddEventHandler('aj-pets:client:openMenu_customization_variation:variation_menu', function(data)
...
function openMenu_customization(data)
    ...
    local openMenu = {
        ...
        },
        {
            ...
            params = {
                event = "aj-pets:client:openMenu_customization_rename",
                args = {
                    item = data.item,
        ...
        },
        {
            ...
            params = {
                event = "aj-pets:client:openMenu_customization:confirm",
                args = {
                    item = data.item,
                    pet_information = data.pet_information
                }
    ...
    local openMenu = {
        {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = "aj-pets:client:openMenu_customization",
                args = {
                    item = item,
                    pet_information = pet_information
                },
        ...
        },
        {
            ...
            icon = 'fa-regular fa-keyboard',
            params = {
                event = "aj-pets:client:openMenu_customization_rename:rename",
                args = {
                    item = item,
                    pet_information = pet_information
                },
...
function openMenu_customization_select_variation(data)
    ...
    local openMenu = {
        ...
        },
        {
            ...
            icon = 'fa-solid fa-brush',
            params = {
                event = "aj-pets:client:openMenu_customization_variation:variation_menu",
                args = {
                    item = item,
                    pet_information = pet_information
                }
...
AddEventHandler('aj-pets:client:openMenu_customization:confirm', function(data)
    TriggerServerEvent('aj-pets:server:compelete_initialization_process', data.item, data.pet_information.type)
end)
