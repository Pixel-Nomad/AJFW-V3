...
function getIntoCar()
    ...
    if not IsPedSittingInAnyVehicle(plyped) then
        AJFW.Functions.Notify(Lang:t('error.need_to_be_inside_car'), "error", 1500)
        return
    end
    if distance > 8 then
        AJFW.Functions.Notify(Lang:t('error.to_far'), "error", 1500)
        return
    end
    ...
    if seatEmpty == 6 then
        AJFW.Functions.Notify(Lang:t('error.no_empty_seat'), "error", 1500)
        return
    end
end
    ...
    while true do
        ...
        if IsControlJustReleased(0, 38) then
            ...
            if IsPedAPlayer(entity) == 1 or IsEntityAPed(entity) == false or entity == pet then
                AJFW.Functions.Notify(Lang:t('error.could_not_do_that'), "error", 1500)
                return
            end
...
function SearchLogic(plyped, activePed)
    ...
    local closestPlayer = AJFW.Functions.GetClosestPlayer(pedCoord)
    ...
    AJFW.Functions.TriggerCallback('aj-pets:server:search_inventory', function(result)
        Wait(5000)
        ...
        Wait(5000)
        if result == true then
            TriggerEvent('AJFW:Notify', 'K9 found something', 'success', 2500)
