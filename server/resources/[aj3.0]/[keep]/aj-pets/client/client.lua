local CoreName = exports['aj-base']:GetCoreObject()
...
function ActivePed:remove(index)
    ...
    TriggerServerEvent('aj-pets:server:ForceRemoveNetEntity', netId)
    self.data[index] = nil
...
function ActivePed:removeAll()
    ...
    for key, value in pairs(ActivePed:petsList()) do
        ...
        TriggerServerEvent('aj-pets:server:updateAllowedInfo', currentItem, {
            key = 'XP',
        })
    end
    TriggerServerEvent('aj-pets:server:onPlayerUnload', tmpHash)
    self.data = {}
    self.onControl = -1
end
...
RegisterNetEvent('aj-pets:client:callCompanion')
AddEventHandler('aj-pets:client:callCompanion', function(modelName, hostileTowardPlayer, item)
    ...
    CoreName.Functions.Progressbar("callCompanion", "Calling companion", Config.Settings.callCompanionDuration * 1000,
        ...
        AJFW.Functions.TriggerCallback('aj-pets:server:updatePedData', function(result)
            if hostileTowardPlayer == true then
                -- if player is not owner of pet it will attack player
                AJFW.Functions.Notify(Lang:t('error.not_owner_of_pet'), 'error', 5000)
            end
            ClearPedTasks(ped)
            ...
            if petData.hostile == true then
                TriggerServerEvent('aj-pets:server:despwan_not_owned_pet', petData.itemData.info.hash)
                return
            end
...
function request_healing_process(ped, item, process_type)
    local hasitem = AJFW.Functions.HasItem(Config.core_items.firstaid.item_name)
    if not hasitem then AJFW.Functions.Notify(Lang:t('error.not_enough_first_aid'), 'error', 5000) return end
    ...
    CoreName.Functions.Progressbar("reviveing", "Reviveing",
        ...
        TriggerServerEvent('aj-pets:server:revivePet', current_pet, process_type)
        TaskFollowTargetedPlayer(ped, plyID, false)
    end)
end

RegisterNetEvent('aj-pets:client:update_health_value', function(item, amount)
    ...
    CreateThread(function()
        ...
        while DoesEntityExist(ped) and fninished == false do
            ...
            if IsPedDeadOrDying(savedData.entity) == false and savedData.maxHealth ~= currentHealth and
                ...
                currentHealth then
                -- ped is still alive
                TriggerServerEvent('aj-pets:server:updateAllowedInfo', {
...
RegisterNetEvent('aj-pets:client:forceKill', function(hash, reason)
...
RegisterNetEvent('aj-pets:client:despawn')
AddEventHandler('aj-pets:client:despawn', function(item, revive)
    if revive ~= nil and revive == true then
        ...
        TriggerServerEvent('aj-pets:server:setAsDespawned', item)
        return
    end
    local plyPed = PlayerPedId()
...
RegisterNetEvent('aj-pets:client:start_feeding_animation', function()
    ...
    if c_health <= 100.0 or current_pet.itemData.info.health <= 100.0 then
        AJFW.Functions.Notify(Lang:t('error.your_pet_is_dead'), 'error', 5000)
        return
    end
    ...
    CoreName.Functions.Progressbar("feeding", "Feeding", Config.core_items.food.settings.duration * 1000, false, false,
        ...
        TriggerServerEvent('aj-pets:server:increaseFood', current_pet.itemData)
    end)
end)

RegisterNetEvent('aj-pets:client:', function()
...
function start_drinking_animation()
    ...
    CoreName.Functions.Progressbar("pet_drinking", "drinking", Config.core_items.waterbottle.settings.duration * 1000,
        ...
        AJFW.Functions.TriggerCallback('aj-pets:server:decrease_thirst', function(result)

        end, current_pet.itemData)
    end)
end

RegisterNetEvent('aj-pets:client:filling_animation', function(item)
    CoreName.Functions.Progressbar("filling_animation", "filling bottle",
        ...
        TriggerServerEvent('aj-pets:server:filling_event', item)
    end)
end)

RegisterNetEvent('aj-pets:client:rename_name_tag', function(item)
    if ActivePed:read() == nil then
        AJFW.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
        return
    end

    local name = exports['aj-input']:ShowInput({
    ...
    if name then
        ...
        TriggerServerEvent('aj-pets:server:rename_name_tag', name.pet_name)
    end
end)



RegisterNetEvent('aj-pets:client:rename_name_tagAction', function(name)
    ...
    CoreName.Functions.Progressbar("waitingForName", "waiting for Name",
        ...
        AJFW.Functions.TriggerCallback('aj-pets:server:renamePet', function(result)
            if type(result) == "string" then
                AJFW.Functions.Notify(Lang:t('success.pet_rename_was_successful') .. result, 'success', 5000)
            end
        end, {
...
RegisterNetEvent('aj-pets:client:collar_process', function()
    ...
    if activePed.itemData.info.hash == nil then
        AJFW.Functions.Notify(Lang:t('error.failed_to_find_pet'), 'error', 5000)
        return
    end
    ...
    if inputData then
        ...
        CoreName.Functions.Progressbar("waitingForOwenership", "waiting for new owner",
            ...
            function()
                ...
                AJFW.Functions.TriggerCallback('aj-pets:server:collar_change_owenership', function(result)
                    if result.state == false then
                        AJFW.Functions.Notify(result.msg, 'error', 5000)
                        return
                    end
                    AJFW.Functions.Notify(result.msg, 'success', 5000)
                end, {
