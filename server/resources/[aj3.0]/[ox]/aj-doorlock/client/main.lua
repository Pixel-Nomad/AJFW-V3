...
local cors = exports['aj-base']:GetCoreObject()

local PlayerDatas = {}

RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
	PlayerDatas = cors.Functions.GetPlayerData()
end)
			...
			for i = 1, num do
				...
				if door.distance < door.maxDistance then
					...
					if drawSprite and not door.hideUi then
						...
						if sprite and ((door.groups and PlayerDatas.job and (door.groups[PlayerDatas.job.name] or door.groups[PlayerDatas.gang.name])) or (door.characters and PlayerDatas.citizenid and door.characters[PlayerDatas.citizenid])) then
