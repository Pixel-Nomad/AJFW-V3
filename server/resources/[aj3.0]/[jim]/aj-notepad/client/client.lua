...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function() TriggerEvent("aj-notepad:Client:SyncNotes") end)
AddEventHandler('onResourceStart', function(r) if GetCurrentResourceName() ~= r then return end TriggerEvent("aj-notepad:Client:SyncNotes") end)

RegisterNetEvent("aj-notepad:Client:SyncNotes", function(newNotes)
	if not newNotes then
		local p = promise.new()
		AJFW.Functions.TriggerCallback('aj-notepad:Server:SyncNotes', function(cb) p:resolve(cb) end)
	...
	for k, v in pairs(Notes) do
		if not Props[k] and Notes[k] then
			...
			Targets[k] =
				exports['aj-target']:AddCircleZone(k, vec3(v.coords.x, v.coords.y, v.coords.z-1.1), 0.5, { name=k, debugPoly=Config.Debug, useZ=true, },
				{ options = { { type = "server", event = "aj-notepad:Server:ReadNote", icon = "fas fa-receipt", label = Loc[Config.Lan].targetinfo["read"], noteid = k, }, },
						distance = 1.5 })
		end
	end
	for k in pairs(Props) do if not Notes[k] then exports["aj-target"]:RemoveZone(k) destroyProp(Props[k]) end end
end)

RegisterNetEvent("aj-notepad:Client:CreateNote", function()
	...
	if Config.Menu == "ox" then
		...
		dialog = exports['aj-input']:ShowInput({
    ...
    if dialog.note or dialog[1] then
		...
		Wait(900)
        TriggerServerEvent("aj-notepad:Server:CreateNote", {
...
RegisterNetEvent("aj-notepad:Client:ReadNote", function(data)
	local notepad = {}
	notepad[#notepad+1] = {
		icon = "fas fa-receipt",
		isMenuHeader = true,
	...
	notepad[#notepad+1] = {
		...
		event = "aj-notepad:Client:DestroyNote", args = data,
		params = { event = "aj-notepad:Client:DestroyNote", args = data }
	}
	if Config.Menu == "ox" then
	...
	elseif Config.Menu == "aj" then
		exports['aj-menu']:openMenu(notepad)
	end
end)

RegisterNetEvent("aj-notepad:Client:DestroyNote", function(data)
	loadAnimDict("pickup_object")
	TaskPlayAnim(PlayerPedId(), "pickup_object", "pickup_low", 8.0, -8.0, -1, 0, 0, false, false, false)
	unloadAnimDict("pickup_object")
	Wait(900)
	TriggerServerEvent("aj-notepad:Server:SyncEffect", data.coords)
	TriggerServerEvent("aj-notepad:Server:DestroyNote", data.id)
end)

RegisterNetEvent("aj-notepad:Client:SyncEffect", function(coords)
