...
local function removeTargets() for k in pairs(Targets) do exports['aj-target']:RemoveZone(k) end Targets = {} for i = 1, #Parking do DeleteEntity(Parking[i]) Wait(10) end Parking = {}  end
...
local function syncLocations() local p = promise.new() AJFW.Functions.TriggerCallback('aj-jobgarage:server:syncLocations', function(cb) p:resolve(cb) end) Locations = Citizen.Await(p) makeTargets() end

RegisterNetEvent("aj-jobgarage:client:syncLocations", function(newLocations) Locations = newLocations makeTargets() end)
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()	AJFW.Functions.GetPlayerData(function(PlayerData) PlayerJob = PlayerData.job end)	Wait(10000) syncLocations() end)
RegisterNetEvent('AJFW:Client:OnJobUpdate', function(JobInfo) PlayerJob = JobInfo end)
AddEventHandler('onResourceStart', function(r) if GetCurrentResourceName() ~= r then return end	AJFW.Functions.GetPlayerData(function(PlayerData) PlayerJob = PlayerData.job end) Wait(10000) syncLocations() end)

local currentVeh = { out = false, current = nil }
RegisterNetEvent('aj-jobgarage:client:Garage:Menu', function(data)
	...
	if Config.Menu == "aj" then
		...
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "aj-jobgarage:client:Menu:Close" } }
	end
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = {
			...
			event = "aj-jobgarage:client:Garage:Blip",
			params = { event = "aj-jobgarage:client:Garage:Blip", },
		}
		...
		if (Config.distCheck and dist <= 30.0) or not Config.distCheck then
			vehicleMenu[#vehicleMenu+1] = {
				...
				event = "aj-jobgarage:client:RemSpawn",
				params = { event = "aj-jobgarage:client:RemSpawn" }
			}
		end
	else
		...
		for k, v in pairsByKeys(data.list) do
			...
			if showButton == true then
				...
				vehicleMenu[#vehicleMenu+1] = {
					...
					event = "aj-jobgarage:client:SpawnList", args = { spawnName = spawnName, spawncoords = data.spawncoords, list = v },
					params = { event = "aj-jobgarage:client:SpawnList", args = { spawnName = spawnName, spawncoords = data.spawncoords, list = v } }
				}
			end
		end
	end
...
RegisterNetEvent("aj-jobgarage:client:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		AJFW.Functions.SpawnVehicle(data.spawnName, function(veh)
			...
			--SetVehicleNumberPlateText(veh, "PD-"..AJFW.Functions.GetPlayerData().metadata.callsign)
			...
			if data.list.trunkItems then TriggerServerEvent("aj-jobgarage:server:addTrunkItems", AJFW.Functions.GetPlate(veh), data.list.trunkItems) end
...
RegisterNetEvent("aj-jobgarage:client:RemSpawn", function(data)
	...
	AJFW.Functions.DeleteVehicle(currentVeh.current) currentVeh = { out = false, current = nil }
end)

local markerOn = false
RegisterNetEvent("aj-jobgarage:client:Garage:Blip", function(data)
...
AddEventHandler('onResourceStop', function(r) if r ~= GetCurrentResourceName() then return end
	if GetResourceState("aj-target") == "started" or GetResourceState("ox_target") == "started" then
