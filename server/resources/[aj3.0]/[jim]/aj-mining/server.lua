RegisterServerEvent('aj-mining:Crafting:GetItem', function(ItemMake, craftable)
	local src = source
	...
	TriggerEvent("aj-mining:server:toggleItem", true, ItemMake, amount, src)
end)

RegisterServerEvent("aj-mining:Reward", function(data)
	local src = source
    local Player = AJFW.Functions.GetPlayer(src)
	local amount = 1
	if data.mine then
		TriggerEvent("aj-mining:server:toggleItem", true, "stone", math.random(1, 3), src)
	elseif data.crack then
		TriggerEvent("aj-mining:server:toggleItem", false, "stone", data.cost, src)
		for i = 1, math.random(1,3) do
			amount = math.random(1, 2)
			TriggerEvent("aj-mining:server:toggleItem", true, Config.CrackPool[math.random(1, #Config.CrackPool)], amount, src)
		end
	elseif data.wash then
		TriggerEvent("aj-mining:server:toggleItem", false, "stone", data.cost, src)
		for i = 1, math.random(1,2) do
			TriggerEvent("aj-mining:server:toggleItem", true, Config.WashPool[math.random(1, #Config.WashPool)], amount, src)
		end
	elseif data.pan then
		for i = 1, math.random(1,3) do
			TriggerEvent("aj-mining:server:toggleItem", true, Config.PanPool[math.random(1, #Config.PanPool)], amount, src)
		end
	end
end)

RegisterNetEvent("aj-mining:Selling", function(data)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.Functions.GetItemByName(data.item) ~= nil then
		...
		TriggerEvent("aj-mining:server:toggleItem", false, data.item, amount, src)
        Player.Functions.AddMoney('cash', pay)
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[data.item], 'remove', amount)
    else
		tirggerNotify(src, Loc[Config.Lan].error["dont_have"].." "..AJFW.Shared.Items[data.item].label, "error")
    end
end)
...
RegisterNetEvent('aj-mining:server:toggleItem', function(give, item, amount, newsrc)
	...
	if give == 0 or give == false then
		if HasItem(src, item, amount or 1) then -- check if you still have the item
			...
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[item], "remove", amount or 1) end
		...
		if Player.Functions.AddItem(item, amount or 1) then
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[item], "add", amount or 1)
...
AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	for k, v in pairs(Config.SellingPrices) do if not AJFW.Shared.Items[k] then print("Selling: Missing Item from AJFW.Shared.Items: '"..k.."'") end end
	for i = 1, #Config.CrackPool do if not AJFW.Shared.Items[Config.CrackPool[i]] then print("CrackPool: Missing Item from AJFW.Shared.Items: '"..Config.CrackPool[i].."'") end end
	for i = 1, #Config.WashPool do if not AJFW.Shared.Items[Config.WashPool[i]] then print("WashPool: Missing Item from AJFW.Shared.Items: '"..Config.WashPool[i].."'") end end
	for i = 1, #Config.PanPool do if not AJFW.Shared.Items[Config.PanPool[i]] then print("PanPool: Missing Item from AJFW.Shared.Items: '"..Config.PanPool[i].."'") end end
	for i = 1, #Config.Items.items do if not AJFW.Shared.Items[Config.Items.items[i].name] then print("Shop: Missing Item from AJFW.Shared.Items: '"..Config.Items.items[i].name.."'") end end
	local itemcheck = {}
	...
	for k in pairs(itemcheck) do
		if not AJFW.Shared.Items[k] then print("Crafting recipe couldn't find item '"..k.."' in the shared") end
	end
end)