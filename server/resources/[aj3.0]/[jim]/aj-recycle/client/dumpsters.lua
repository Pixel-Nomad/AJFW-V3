if Config.DumpsterDiving.Enable then if Config.Debug then print("^5Debug^7: ^2Loading^7: '^6Dumpster Diving^7'") end
    ...
    --Dumpster Third Eye
    exports['aj-target']:AddTargetModel(Config.DumpsterDiving.models,
        { options = {
            {
                action = function(entity) TriggerEvent("aj-recycle:Dumpsters:Search", { entity = entity }) end,
    ...
    RegisterNetEvent('aj-recycle:Dumpsters:Search', function(data) local Ped = PlayerPedId()
        ...
        if not searched then -- If hasn't been searched yet
            ...
            if Config.DumpsterDiving.skillcheck == "aj-lock" then
            ...
            elseif Config.DumpsterDiving.skillcheck == "aj-hacks" then
                exports['aj-hacks']:Circle(function(Skillbar)
                    if Skillbar then searchSuccess = true else searchSuccess = false end
                end, 2, 20)
            elseif Config.DumpsterDiving.skillcheck == "aj-skillbar" then
