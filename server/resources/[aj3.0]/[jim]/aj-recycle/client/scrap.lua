if Config.ScrapyardSearching.Enable then if Config.Debug then print("^5Debug^7: ^2Loading^7: '^6Scrapyard Searching^7'") end
	...
	--Dumpster Third Eye
	exports['aj-target']:AddTargetModel(Config.ScrapyardSearching.models, { options = {
        {
            action = function(entity) TriggerEvent("aj-recycle:Scrap:Search", { entity = entity }) end,
    ...
    RegisterNetEvent('aj-recycle:Scrap:Search', function(data) local Ped = PlayerPedId()
        ...
        if not searched then -- If hasn't been searched yet
            ...
            if Config.ScrapyardSearching.skillcheck == "aj-lock" then
                local Skillbar = exports['aj-lock']:StartLockPickCircle(math.random(2,4), math.random(7,10), success)
                if Skillbar then searchSuccess = true else searchSuccess = false end
            elseif Config.ScrapyardSearching.skillcheck == "aj-hacks" then
                exports['aj-hacks']:Circle(function(Skillbar)
                    if Skillbar then searchSuccess = true else searchSuccess = false end
                end, 2, 20)
            elseif Config.ScrapyardSearching.skillcheck == "aj-skillbar" then
