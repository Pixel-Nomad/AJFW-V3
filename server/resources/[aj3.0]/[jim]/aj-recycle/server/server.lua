AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	for k in pairs(Config.Prices) do if not Core.Shared.Items[k] then print("^5Debug^7: ^6Prices^7: ^2Missing Item from ^4AJFW^7.^4Shared^7.^4Items^7: '^6"..k.."^7'") end end
	if not Core.Shared.Items["recyclablematerial"] then print("^5Debug^7: ^2Missing Item from ^4AJFW^7.^4Shared^7.^4Items^7: '^6recyclablematerial^7'") end
	for _, v in pairs(Config.ScrapItems) do if not Core.Shared.Items[v] then print("^5Debug^7: ^6ScrapItems^7: ^2Missing Item from ^4AJFW^7.^4Shared^7.^4Items^7: '^6"..v.."^7'") end end
	for _, v in pairs(Config.DumpItems) do if not Core.Shared.Items[v] then print("^5Debug^7: ^2DumpItems^7: ^2Missing Item from ^4AJFW^7.^4Shared^7.^4Items^7: '^6"..v.."^7'") end end
end)

Core.Functions.CreateCallback('aj-recycle:GetCash', function(source, cb) cb(Core.Functions.GetPlayer(source).Functions.GetMoney("cash")) end)

RegisterServerEvent("aj-recycle:DoorCharge", function()
	if Config.Inv == "ox" then TriggerEvent("aj-recycle:server:toggleItem", false, "money", Config.PayAtDoor, src)
	else Core.Functions.GetPlayer(source).Functions.RemoveMoney("cash", Config.PayAtDoor) end
end)

RegisterServerEvent("aj-recycle:TradeItems", function(data)
    local src = source
	local table = {}
	...
	TriggerEvent("aj-recycle:server:toggleItem", false, "recyclablematerial", data.amount, src)
	Wait(1000)
	for i = 1, table.itemGive do
		TriggerEvent("aj-recycle:server:toggleItem", true, Config.TradeTable[math.random(1, #Config.TradeTable)], math.random(table.Min, table.Max), src)
		Wait(100)
	end
end)

RegisterNetEvent("aj-recycle:Selling:Mat", function(data)
    local src = source
	...
	if HasItem(src, data.item, amount) then
		TriggerEvent("aj-recycle:server:toggleItem", false, data.item, amount, src)
	...
	TriggerClientEvent(src, "aj-recycle:Selling:Menu", data)
end)
...
RegisterNetEvent('aj-recycle:server:toggleItem', function(give, item, amount, newsrc)
...
if Config.Inv == "ox" then
	function HasItem(src, items, amount)
		print(exports['aj-inventory']:Search(src, 'count', items), items)
		local count = exports['aj-inventory']:Search(src, 'count', items)
		if exports['aj-inventory']:Search(src, 'count', items) >= (amount or 1) then
