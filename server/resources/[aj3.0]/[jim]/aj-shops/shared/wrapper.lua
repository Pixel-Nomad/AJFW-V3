...
Items = {}
-- Load item lists from aj-base or ox_lib
if GetResourceState("aj-base") == "started" then
    Core = Core or exports['aj-base']:GetCoreObject()
    RegisterNetEvent('AJFW:Client:UpdateObject', function() Core = exports["aj-base"]:GetCoreObject() end)
...
function openMenu(Menu, data)
    ...
    elseif Config.System.Menu == "aj" then
        if data.onBack then
            table.insert(Menu, 1, { icon = "fas fa-circle-arrow-left",
                header = " ", txt = "Return",
                params = {
        ...
        if data.canClose then
            table.insert(Menu, 1, { icon = "fas fa-circle-xmark",
                ...
                params = {
                    isAction = true,
                    event = data.onExit and data.onExit or (function() exports["aj-menu"]:closeMenu() end),
                }
            })
        end
...
function createEntityTarget(entity, opts, dist)
    ...
    if Config.System.Target == "aj" then
        ...
        exports['aj-target']:AddTargetEntity(entity, options)
    end
...
AddEventHandler('onResourceStop', function(r)
    ...
    for k, v in pairs(targetEntities) do
        if Config.System.Target == "aj" then
            exports["aj-target"]:RemoveTargetEntity(v)
        end
        if Config.System.Target == "ox" then
...
function triggerNotify(title, message, type, src)
	...
	elseif Config.System.Notify == "aj" then
...
function createCallback(callbackName, funct)
    if Config.System.Callback == "aj" then
        print("Registering aj Callback", callbackName)
...
function createInput(title, opts)
    ...
    if Config.System.Menu == "aj" then
        dialog = exports['aj-input']:ShowInput({ header = title, submitText = "Pay", inputs = opts })
        return dialog
    end
