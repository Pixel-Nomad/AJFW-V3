		...
		if k == "vendingmachine" and Config.Overrides.VendOverride then
			for l, b in pairs(v["coords"]) do
				...
				elseif Config.System.Target == "ox" then
					...
					if Config.System.Debug then print("^5Debug^7: ^2Config option ^6Config.System.Target ^2should be ^7ox ^2or ^7aj") end
				end
			end
		else
			...
			for l, b in pairs(v["coords"]) do
				...
				if Config.Overrides.Peds then
					...
					Targets[label] = exports['aj-target']:AddCircleZone(label, vector3(b.x, b.y, b.z), 2.0,	{ name=label, debugPoly=Config.System.Debug, useZ=true, },
...
RegisterNetEvent('jim-shops:Charge', function(data) local dialog
	...
	local dialog = createInput(Config.System.Menu == "aj" and header or Items[data.item].label, newinputs)
	if dialog then
