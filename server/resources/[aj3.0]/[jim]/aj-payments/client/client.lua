...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function() AJFW.Functions.GetPlayerData(function(PlayerData) PlayerJob = PlayerData.job PlayerGang = PlayerData.gang end) end)
RegisterNetEvent('AJFW:Client:OnJobUpdate', function(JobInfo) PlayerJob = JobInfo onDuty = PlayerJob.onduty end)
RegisterNetEvent('AJFW:Client:SetDuty', function(duty) onDuty = duty end)
RegisterNetEvent('AJFW:Client:OnGangUpdate', function(GangInfo) PlayerGang = GangInfo end)
...
CreateThread(function()
	...
	Targets["JimBank"] =
	exports['aj-target']:AddCircleZone("JimBank", vector3(Config.CashInLocation.x, Config.CashInLocation.y, Config.CashInLocation.z), 2.0, { name="JimBank", debugPoly=Config.Debug, useZ=true, },
		{ options = {
			{ event = "aj-payments:Tickets:Menu", icon = "fas fa-receipt", label = Loc[Config.Lan].target["cashin_boss"], job = jobroles, },
			{ event = "aj-payments:Tickets:Menu", icon = "fas fa-receipt", label = Loc[Config.Lan].target["cashin_gang"], gang = gangroles, } },
		distance = 2.0 })
	...
	for k, v in pairs(Config.CustomCashRegisters) do
		for i = 1, #v do
			...
			exports['aj-target']:AddBoxZone("CustomRegister: "..k..i, v[i].coords.xyz, 0.47, 0.34, { name="CustomRegister: "..k..i, heading = v[i].coords[4], debugPoly=Config.Debug, minZ=v[i].coords.z-0.1, maxZ=v[i].coords.z+0.4 },
				{ options = { { event = "aj-payments:client:Charge", icon = "fas fa-credit-card", label = Loc[Config.Lan].target["charge"], job = job, gang = gang, img = "" }, },
					distance = 2.0 })
			if v[i].prop then
...
RegisterNetEvent('aj-payments:client:Charge', function(data, outside)
	...
	local newinputs = {} -- Begin aj-input creation here.
	if Config.List then -- If nearby player list is wanted:
		--Retrieve a list of nearby players from server
		local p = promise.new() AJFW.Functions.TriggerCallback('aj-payments:MakePlayerList', function(cb) p:resolve(cb) end)
		local onlineList = Citizen.Await(p)
		local nearbyList = {}
		--Convert list of players nearby into one aj-input understands + add distance info
		for _, v in pairs(AJFW.Functions.GetPlayersFromCoords(GetEntityCoords(PlayerPedId()), Config.PaymentRadius)) do
	...
	--Continue adding payment options to aj-input
	if Config.Menu == "aj" then
	...
	if Config.Menu == "aj" then 
		dialog = exports['aj-input']:ShowInput({ header = img..label..Loc[Config.Lan].menu["cash_reg"], submitText = Loc[Config.Lan].menu["send"], inputs = newinputs})
	elseif Config.Menu == "ox" then
	...
	if dialog then
        ...
        TriggerServerEvent('aj-payments:server:Charge', inputs.citizen, inputs.price, inputs.billtype, data.img, outside, gang)
    end
end)

RegisterNetEvent('aj-payments:client:PolCharge', function()
	...
	local dialog  
	if Config.Menu == "aj" then
		dialog = exports['aj-input']:ShowInput({ header = label..Loc[Config.Lan].menu["charge"], submitText = Loc[Config.Lan].menu["send"], inputs = newinputs})
	elseif Config.Menu == "ox" then
	...
	if dialog then
        ...
        if not inputs.citizen or not inputs.price then return end
		TriggerServerEvent('aj-payments:server:PolCharge', dialog.citizen, dialog.price)
    end
end)

RegisterNetEvent('aj-payments:Tickets:Menu', function(data)
	--Get ticket info
	local p = promise.new() AJFW.Functions.TriggerCallback('aj-payments:Ticket:Count', function(cb) p:resolve(cb) end)
		...
		if data.gang then if v.gang and k == PlayerGang.name then name = k label = PlayerGang.label sellable = true end
			...
			if sellable then -- if info is found then:
				...
				exports['aj-menu']:openMenu({
					...
					{ icon = "fas fa-circle-check", header = Loc[Config.Lan].menu["yes"], txt = "", params = { event = "aj-payments:Tickets:Sell:yes" } },
					{ icon = "fas fa-circle-xmark", header = Loc[Config.Lan].menu["no"], txt = "", params = { event = "aj-payments:Tickets:Sell:no" } },
				})
			elseif Config.Menu == "ox" then
				lib.registerContext({
					id = 'ticketmenu',
					...
					options = {
						...
						{ icon = "fas fa-circle-check", title = Loc[Config.Lan].menu["yes"], description = "",  event = "aj-payments:Tickets:Sell:yes"  },
						{ icon = "fas fa-circle-xmark", title = Loc[Config.Lan].menu["no"], description = "",  event = "aj-payments:Tickets:Sell:no"  },
					}
				})
...
RegisterNetEvent("aj-payments:client:PayPopup", function(amount, biller, billtype, img, billerjob, gang, outside)
	local img = img or ""
	if Config.Menu == "aj" then
	exports['aj-menu']:openMenu({
		...
		{ icon = "fas fa-circle-check", header = Loc[Config.Lan].menu["yes"], txt = "", params = { isServer = true, event = "aj-payments:server:PayPopup", args = { accept = true, amount = amount, biller = biller, billtype = billtype, gang = gang, outside = outside } } },
		{ icon = "fas fa-circle-xmark", header = Loc[Config.Lan].menu["no"], txt = "", params = { isServer = true, event = "aj-payments:server:PayPopup", args = { accept = false, amount = amount, biller = biller, billtype = billtype, outside = outside } } }, })
	else
		lib.registerContext({
			id = 'paypopup',
			...
			options = {
				...
				},
				{
					...
					serverEvent = "aj-payments:server:PayPopup",
					args = { 
						accept = true, 
						amount = amount, 
						biller = biller, 
...
RegisterNetEvent("aj-payments:client:PolPopup", function(amount, biller, billerjob)
	if Config.Menu == "aj" then
	exports['aj-menu']:openMenu({
		...
		{ icon = "fas fa-circle-check", header = Loc[Config.Lan].menu["yes"], txt = "", params = { isServer = true, event = "aj-payments:server:PolPopup", args = { accept = true, amount = amount, biller = biller } } },
		{ icon = "fas fa-circle-xmark", header = Loc[Config.Lan].menu["no"], txt = "", params = { isServer = true, event = "aj-payments:server:PolPopup", args = { accept = false, amount = amount, biller = biller } } }, })
	else
		lib.registerContext({
			id = 'polpopup',
			title = "🧾 "..billerjob..Loc[Config.Lan].menu["payment"],
			options = {
				...
				{ icon = "fas fa-circle-check", title = Loc[Config.Lan].menu["yes"], description = "",  serverEvent = "aj-payments:server:PolPopup", args = { accept = true, amount = amount, biller = biller  } },
				{ icon = "fas fa-circle-xmark", title = Loc[Config.Lan].menu["no"], description = "",  serverEvent = "aj-payments:server:PolPopup", args = { accept = false, amount = amount, biller = biller  } }, 
			}
		})
...
RegisterNetEvent('aj-payments:Tickets:Sell:yes', function() TriggerServerEvent('aj-payments:Tickets:Sell') end)
RegisterNetEvent('aj-payments:Tickets:Sell:no', function() exports['aj-menu']:closeMenu() end)
