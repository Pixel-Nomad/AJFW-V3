-- This script is a simple replacement for aj-Banking and aj-ATMs
-- It uses aj-Input and server callbacks to retreive info about accounts and cash
-- This requires aj-Target and has the location of every ATM, ATM prop and bank window
...
CreateThread(function()
	...
	if Config.useATM or Config.useBanks then
		for k in pairs(AJFW.Shared.Jobs) do ---Grabs the list of jobs
			if AJFW.Shared.Jobs[tostring(k)] then
				for l in pairs(AJFW.Shared.Jobs[tostring(k)].grades) do -- Grabs the list of grades
					if AJFW.Shared.Jobs[tostring(k)].grades[l].isboss == true then -- Checks the grade if is boss
		...
		for k in pairs(AJFW.Shared.Gangs) do
			if AJFW.Shared.Gangs[tostring(k)] then
				for l in pairs(AJFW.Shared.Gangs[tostring(k)].grades) do
					if tostring(k) ~= "none" then
						if AJFW.Shared.Gangs[tostring(k)].grades[l].isboss == true then
	...
	if Config.useATM then
		exports['aj-target']:AddTargetModel(Config.ATMModels, { options = { { event = "aj-payments:Client:ATM:use", icon = "fas fa-money-check-alt", label = Loc[Config.Lan].target["atm"], id = "atm" },}, distance = 1.5, })
		for k,v in pairs(Config.WallATMLocations) do
			Targets["jimwallatm"..k] =
			exports['aj-target']:AddCircleZone("jimwallatm"..k, vector3(v.x, v.y, v.z+0.2), 0.5, { name="jimwallatm"..k, debugPoly=Config.Debug, useZ=true, },
			{ options = { { event = "aj-payments:Client:ATM:use", icon = "fas fa-money-check-alt", label = Loc[Config.Lan].target["atm"], id = "atm" },
						--{ event = "aj-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = Loc[Config.Lan].target["transfer_money"], id = "transfer" },
			}, distance = 1.5 })
		end
		for k,v in pairs(Config.ATMLocations) do
			Targets["jimatm"..k] =
			exports['aj-target']:AddCircleZone("jimatm"..k, vector3(v.x, v.y, v.z+0.2), 0.5, { name="jimatm"..k, debugPoly=Config.Debug, useZ=true, },
			{ options = { { event = "aj-payments:Client:ATM:use", icon = "fas fa-money-check-alt", label = Loc[Config.Lan].target["atm"], id = "atm" },
						--{ event = "aj-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = Loc[Config.Lan].target["transfer_money"], id = "transfer" },
			}, distance = 1.5 })
		end
	...
	if Config.useBanks then
		for k,v in pairs(Config.BankLocations) do
			for l, b in pairs(v) do
				Targets["jimbank"..k..l] =
				exports['aj-target']:AddCircleZone("jimbank"..k..l, vector3(b.x, b.y, b.z+0.2), 2.0, { name="jimbank"..k..l, debugPoly=Config.Debug, useZ=true, },
				{ options = { { event = "aj-payments:Client:ATM:use", icon = "fas fa-piggy-bank", label = Loc[Config.Lan].target["bank"], id = "bank" },
							{ event = "aj-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = Loc[Config.Lan].target["transfer"], id = "transfer" },
							{ event = "aj-payments:Client:ATM:use", icon = "fas fa-money-check-dollar", label = Loc[Config.Lan].target["saving"], id = "savings" },

							{ event = "aj-payments:Client:ATM:use", icon = "fas fa-building", label = Loc[Config.Lan].target["soc_saving"], id = "society", job = bossroles },
							{ event = "aj-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = Loc[Config.Lan].target["soc_trans"], id = "societytransfer", job = bossroles },

							{ event = "aj-payments:Client:ATM:use", icon = "fas fa-building", label = Loc[Config.Lan].target["gang_acct"], id = "gang", gang = gangroles },
							{ event = "aj-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = Loc[Config.Lan].target["gang_trans"], id = "gangtransfer", gang = gangroles }, },
				distance = 2.5 })
				if Config.Peds then
...
RegisterNetEvent('aj-payments:Client:ATM:use', function(data)
	--this grabs all the info from names to savings account numbers in the databases
	local p = promise.new()
	AJFW.Functions.TriggerCallback('aj-payments:ATM:Find', function(cb) p:resolve(cb) end) local info = Citizen.Await(p)
	if Config.Banking == "aj" then
		local p = promise.new()
		AJFW.Functions.TriggerCallback('aj-bossmenu:server:GetAccount', function(cb) p:resolve(cb) end, PlayerJob.name) info.society = Citizen.Await(p)
		local p2 = promise.new()
		AJFW.Functions.TriggerCallback('aj-gangmenu:server:GetAccount', function(cb) p2:resolve(cb) end, PlayerGang.name) info.gsociety = Citizen.Await(p2)
	end
	...
	AJFW.Functions.Progressbar("accessing_atm", bartext, atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function()
		local dialog = exports['aj-input']:ShowInput({ header = setheader, txt = "test", submitText = Loc[Config.Lan].menu["transfer"], inputs = setinputs })
		if dialog then
			...
			unloadAnimDict('amb@prop_human_atm@male@enter')
			Wait(1000)
			TriggerServerEvent('aj-payments:server:ATM:use', dialog.amount, dialog.billtype, dialog.account, data.id, info.society, info.gsociety)
		end
	end, function()	triggerNotify(nil, Loc[Config.Lan].error["cancel"], "error")
	end, data.icon)
end)

RegisterNetEvent('aj-payments:client:ATM:give', function()
	local onlineList = {}
	local nearbyList = {}
	AJFW.Functions.TriggerCallback('aj-payments:MakePlayerList', function(cb) onlineList = cb if onlineList[1] == nil then Wait(200) end
		for _, id in pairs(AJFW.Functions.GetPlayersFromCoords(GetEntityCoords(PlayerPedId()), Config.PaymentRadius)) do
		...
		local dialog = exports['aj-input']:ShowInput({ header = Loc[Config.Lan].menu["give_cash"], submitText = Loc[Config.Lan].menu["give"],
		inputs = {
		...
		if dialog then
			...
			TriggerServerEvent('aj-payments:server:ATM:give', dialog.citizen, dialog.price)
		end
	end)
end)
