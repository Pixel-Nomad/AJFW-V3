...
RegisterServerEvent('aj-payments:server:ATM:use', function(amount, billtype, baccount, account, society, gsociety)
	local src = source
	...
	elseif account == "society" then
		if billtype == "withdraw" then
			...
			elseif tonumber(society) >= amount then
				...
				elseif Config.Banking == "aj-management" then exports["aj-management"]:RemoveMoney(tostring(Player.PlayerData.job.name), amount)
		...
		elseif billtype == "deposit" then
			...
			elseif bankB >= amount then
				...
				elseif Config.Banking == "aj-management" then exports["aj-management"]:AddMoney(tostring(Player.PlayerData.job.name), amount)
	...
	elseif account == "gang" then
		if billtype == "withdraw" then
			...
			elseif tonumber(gsociety) >= amount then
				...
				elseif Config.Banking == "aj-management" then exports["aj-management"]:RemoveGangMoney(tostring(Player.PlayerData.gang.name), amount)
		...
		elseif billtype == "deposit" then
			...
			elseif bankB >= amount then
				...
				elseif Config.Banking == "aj-management" then exports["aj-management"]:AddGangMoney(tostring(Player.PlayerData.gang.name), amount)
...
AJFW.Functions.CreateCallback('aj-payments:ATM:Find', function(source, cb)
	...
	-- If aj-management, grab info directly from database
	elseif not Config.Banking == "AJB" then
...
AJFW.Commands.Add("cashgive", Loc[Config.Lan].command["pay_user"], {}, false, function(source) TriggerClientEvent("aj-payments:client:ATM:give", source) end)

RegisterServerEvent("aj-payments:server:ATM:give", function(citizen, price)
    local Player = AJFW.Functions.GetPlayer(source)
