...
AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	for k in pairs(Config.Jobs) do
		if not AJFW.Shared.Jobs[k] and not AJFW.Shared.Gangs[k] then
			print("aj-payments: Config.Jobs searched for job/gang '"..k.."' and couldn't find it in the Shared")
		end
	end
end)
...
CreateThread(function()
    if Config.Enablecommand then
        AJFW.Commands.Add("cashregister", Loc[Config.Lan].command["cash_reg"], {}, false, function(source) TriggerClientEvent("aj-payments:client:Charge", source, {}, true) end)
    end
end)

AJFW.Commands.Add("polcharge", Loc[Config.Lan].command["charge"], {}, false, function(source) TriggerClientEvent("aj-payments:client:PolCharge", source) end)

RegisterServerEvent('aj-payments:Tickets:Give', function(data, biller, gang)
    local billed = AJFW.Functions.GetPlayer(source) -- This should always be from the person who accepted the payment
	...
	if biller then -- If this is found, it ISN'T a phone payment, so add money to society here
		if gang then
			if Config.Banking == "AJB" then
				exports['aj-Banking']:addAccountMoney(tostring(biller.PlayerData.gang.name), data.amount - takecomm)
				if Config.Debug then
					print("^5Debug^7: ^3aj-Banking^7(^3Gang^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.gang.name).."^7' ($^6"..exports['aj-Banking']:getAccountMoney(biller.PlayerData.gang.name).."^7)") end
			elseif Config.Banking == "aj-management" then
					exports["aj-management"]:AddGangMoney(tostring(biller.PlayerData.gang.name), data.amount - takecomm)
					if Config.Debug then print("^5Debug^7: ^3AJ-Management^7(^3Gang^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.gang.name).."^7' ($^6"..exports["aj-management"]:GetGangAccount(biller.PlayerData.gang.name).."^7)") end
			elseif Config.Banking == "aj-banking" then
				exports["aj-banking"]:AddGangMoney(tostring(biller.PlayerData.gang.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3AJ-Banking^7(^3Gang^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.gang.name).."^7' ($^6"..exports["aj-banking"]:GetGangAccount(biller.PlayerData.gang.name).."^7)") end
		...
		elseif not gang then
			if Config.Banking == "AJB"
			then exports['aj-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3aj-Banking^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..tostring(exports['aj-Banking']:getAccountMoney(biller.PlayerData.job.name)).."^7)") end
			elseif Config.Banking == "aj-management" then
				exports["aj-management"]:AddMoney(tostring(biller.PlayerData.job.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3AJ-Management^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["aj-management"]:GetAccount(biller.PlayerData.job.name).."^7)") end
			elseif Config.Banking == "aj-banking" then
				exports["aj-banking"]:AddMoney(tostring(biller.PlayerData.job.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3AJ-Banking^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6^7)") end
			elseif Config.Banking == "fd" then
	...
	if (biller.PlayerData.job.onduty or gang) and Config.TicketSystem then
		if data.amount >= Config.Jobs[data.society].MinAmountforTicket then
			if Config.TicketSystemAll then
					...
					if Player ~= nil or Player ~= billed then
						if gang then
							if Player.PlayerData.gang.name == data.society then
								if Player.Functions.AddItem('payticket', 1) then TriggerClientEvent('inventory:client:ItemBox', Player.PlayerData.source, AJFW.Shared.Items['payticket'], "add", 1) end
				...
				if biller.Functions.AddItem('payticket', 1) then TriggerClientEvent('inventory:client:ItemBox', biller.PlayerData.source, AJFW.Shared.Items['payticket'], "add", 1) end
...
RegisterServerEvent('aj-payments:Tickets:Sell', function()
...
AJFW.Functions.CreateCallback('aj-payments:Ticket:Count', function(source, cb)
	cb(AJFW.Functions.GetPlayer(source).Functions.GetItemByName('payticket'))
end)

RegisterServerEvent("aj-payments:server:Charge", function(citizen, price, billtype, img, outside, gang)
	local src = source
	...
	if amount and amount > 0 then
		...
		if Config.PhoneBank == false or gang == true or billtype == "cash" then
			TriggerClientEvent("aj-payments:client:PayPopup", billed.PlayerData.source, amount, src, billtype, img, label, gang, outside)
		else
			if Config.PhoneType == "aj" then
				MySQL.Async.insert(
...
RegisterServerEvent("aj-payments:server:PayPopup", function(data)
	local src = source
	...
	if data.accept == true then
		...
		TriggerEvent('aj-payments:Tickets:Give', newdata, biller, data.gang)
...
RegisterServerEvent("aj-payments:server:PolCharge", function(citizen, price)
	local src = source
    local biller = AJFW.Functions.GetPlayer(src)
    local billed = AJFW.Functions.GetPlayer(tonumber(citizen))
	...
	if price > 0 then
		if not Config.FineJobConfirmation then
			...
			if Config.Banking == "AJB" then
				exports['aj-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), (price - commission))
				if Config.Debug then print("^5Debug^7: ^3aj-Banking^7(^3Job^7): ^2Adding ^7$^6"..(price - commission).." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports['aj-Banking']:getAccountMoney((biller.PlayerData.job.name).."^7)")) end
			elseif Config.Banking == "aj-management" then
				exports["aj-management"]:AddMoney(tostring(biller.PlayerData.job.name), (price - commission))
				if Config.Debug then print("^5Debug^7: ^3AJ-Management^7(^3Job^7): ^2Adding ^7$^6"..(price - takecomm).." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["aj-management"]:GetAccount(biller.PlayerData.job.name).."^7)") end
			elseif Config.Banking == "aj-banking" then
				exports["aj-banking"]:AddMoney(tostring(biller.PlayerData.job.name), (price - commission))
				if Config.Debug then print("^5Debug^7: ^3AJ-Banking^7(^3Job^7): ^2Adding ^7$^6"..(price - takecomm).." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["aj-banking"]:GetAccount(biller.PlayerData.job.name).."^7)") end
			elseif Config.Banking == "fd" then
			...
			TriggerClientEvent("aj-payments:client:PolPopup", billed.PlayerData.source, price, src, biller.PlayerData.job.label)
		end
	else triggerNotify(nil, Loc[Config.Lan].error["charge_zero"], 'error', source) return end
end)

RegisterServerEvent("aj-payments:server:PolPopup", function(data)
	local src = source
    local billed = AJFW.Functions.GetPlayer(src)
    local biller = AJFW.Functions.GetPlayer(tonumber(data.biller))
	...
	if data.accept == true then
		...
		if Config.Banking == "AJB" then
			exports['aj-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), data.amount - commission)
			if Config.Debug then print("^5Debug^7: ^3aj-Banking^7(^3Job^7): ^2Adding ^7$^6"..(data.amount - commission).." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports['aj-Banking']:getAccountMoney((biller.PlayerData.job.name).."^7)")) end
		elseif Config.Banking == "aj-management" then
			exports["aj-management"]:AddMoney(tostring(biller.PlayerData.job.name), data.amount - commission)
			if Config.Debug then print("^5Debug^7: ^3AJ-Management^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["aj-management"]:GetAccount(biller.PlayerData.job.name).."^7)") end
		elseif Config.Banking == "aj-banking" then
			exports["aj-banking"]:AddMoney(tostring(biller.PlayerData.job.name), data.amount - commission)
			if Config.Debug then print("^5Debug^7: ^3AJ-Banking^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports["aj-banking"]:GetAccount(biller.PlayerData.job.name).."^7)") end
		elseif Config.Banking == "fd" then
...
AJFW.Functions.CreateCallback('aj-payments:MakePlayerList', function(source, cb)
	local onlineList = {}
	for _, v in pairs(AJFW.Functions.GetPlayers()) do
...
CreateThread(function()
	if Config.Usebzzz then
		AJFW.Functions.CreateUseableItem('terminal', function(source, item)
		TriggerClientEvent("aj-payments:client:Charge", source, {}, true)
		end)
	end
end)
