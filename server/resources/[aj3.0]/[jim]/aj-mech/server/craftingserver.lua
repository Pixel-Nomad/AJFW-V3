...
RegisterServerEvent('jim-mechanic:Crafting:GetItem', function(ItemMake, craftable)
	...
	if Config.StashCraft then
		...
		for k, v in pairs(craftable[ItemMake]) do
			...
			Player.Functions.RemoveItem(tostring(k), v, nil, nil, true)
		end
	end
	--This should give the item, while the rest removes the requirements
	Player.Functions.AddItem(ItemMake, amount, nil,nil,true)
	TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[ItemMake], "add", amount)
end)
...
AJFW.Functions.CreateCallback('jim-mechanic:Crafting:get', function(source, cb, item, craftable)
	...
	if Config.StashCraft then
		...
		for k, v in pairs(craftable[item]) do
			if AJFW.Functions.GetPlayer(source).Functions.GetItemByName(k) and AJFW.Functions.GetPlayer(source).Functions.GetItemByName(k).amount >= v then
				testtable[k] = true if Config.Debug then print("^5Debug^7: ^3Crafting^7: ^6"..k.." ^7(^2x^6"..v.."^7) ^2found^7") end
			end
		end
	end
	for k, v in pairs(testtable) do
		if not v then hasitem = false if Config.Debug then print("^5Debug^7: ^3Crafting^7: ^6"..AJFW.Shared.Items[k].label.." ^1NOT ^2found^7") end end
	end
	cb(hasitem)
end)