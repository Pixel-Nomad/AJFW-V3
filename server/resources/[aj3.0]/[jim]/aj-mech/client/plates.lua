...
RegisterNetEvent('aj-mech:client:Plates:Custom:Apply', function(index)
	...
	elseif GetVehicleNumberPlateTextIndex(vehicle) ~= tonumber(index) then
		...
		AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["plates"].installing, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			ajlog("`customplate - "..AJFW.Shared.Items["customplate"].label.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
...
RegisterNetEvent('aj-mech:client:Plates:Apply', function(data)
	...
	elseif GetVehicleMod(vehicle, tonumber(data.mod)) ~= tonumber(data.id) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			ajlog("`customplate - "..AJFW.Shared.Items["customplate"].label.." - "..modName.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
