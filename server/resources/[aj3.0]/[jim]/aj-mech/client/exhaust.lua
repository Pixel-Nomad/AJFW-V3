...
--========================================================== Exhaust
RegisterNetEvent('aj-mech:client:Exhaust:Apply', function(data)
	...
	if GetVehicleMod(vehicle, 4) == tonumber(data.id) then
		...
		TriggerEvent('aj-mech:client:Exhaust:Check')
	elseif GetVehicleMod(vehicle, 4) ~= tonumber(data.id) then
		...
		AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["common"].installing..modName.."..", time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
			...
			ajlog("`exhaust - "..AJFW.Shared.Items["exhaust"].label.." - "..modName.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
			SetVehicleMod(vehicle, 4, tonumber(data.id))
			emptyHands(PlayerPedId())
			updateCar(vehicle)
			if Config.CosmeticRemoval then toggleItem(false, "exhaust")
			else TriggerEvent('aj-mech:client:Exhaust:Check') end
...
RegisterNetEvent('aj-mech:client:Exhaust:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local ExhaustMenu = {
			...
			{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "aj-mech:client:Exhaust:Apply", args = { id = -1 } } }, }
			for k, v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 4) == v.id then icon = "fas fa-check" disabled = true end
				ExhaustMenu[#ExhaustMenu + 1] = { icon = icon, isMenuHeader = disabled,  header = k..". "..v.name, txt = v.install, params = { event = 'aj-mech:client:Exhaust:Apply', args = { id = tostring(v.id) } } }
			end
		exports['aj-menu']:openMenu(ExhaustMenu)
	end
end)