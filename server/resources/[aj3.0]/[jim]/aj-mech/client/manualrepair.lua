	...
	if Config.ManualRepairBased then
		for k, v in pairs(AJFW.Shared.Vehicles) do
			if tonumber(v.hash) == GetEntityModel(vehicle) then
				...
				if Config.Debug then print("^5Debug^7: ^2Vehicle^7: '^6"..v.hash.. "^7' (^6"..AJFW.Shared.Vehicles[k].name.."^7)") end
				cost = math.ceil((AJFW.Shared.Vehicles[k].price * percent) - math.ceil((health/100) * (AJFW.Shared.Vehicles[k].price * percent)))
			end
		end
	else
...
RegisterNetEvent('aj-mech:client:Manual:Repair', function(data)
	...
	repairing = false
	ajlog("Repair Bench used ($**"..data.cost.."**) [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
end)