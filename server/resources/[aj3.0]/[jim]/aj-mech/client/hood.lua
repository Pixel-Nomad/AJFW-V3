...
--========================================================== Hood
RegisterNetEvent('aj-mech:client:Hood:Apply', function(data)
	...
	if GetVehicleMod(vehicle, 7) == tonumber(data.id) then
		...
		TriggerEvent('aj-mech:client:Hood:Check')
	elseif GetVehicleMod(vehicle, 7) ~= tonumber(data.id) then
		...
		{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			ajlog("`hood - "..AJFW.Shared.Items["hood"].label.." - "..modName.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
			...
			else TriggerEvent('aj-mech:client:Hood:Check') end
...
RegisterNetEvent('aj-mech:client:Hood:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local HoodMenu = {
				...
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "aj-mech:client:Hood:Apply", args = { id = -1 } } }, }
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 7) == v.id then icon = "fas fa-check" disabled = true end
				HoodMenu[#HoodMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'aj-mech:client:Hood:Apply', args = { id = tostring(v.id) } } }
			end
		exports['aj-menu']:openMenu(HoodMenu)
	end
end)