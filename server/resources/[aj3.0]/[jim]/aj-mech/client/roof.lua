...
--========================================================== Roof
RegisterNetEvent('aj-mech:client:Roof:Apply', function(mod)
	...
	if GetVehicleMod(vehicle, 10) == tonumber(mod) then triggerNotify(nil, modName..Loc[Config.Lan]["common"].already, "error") TriggerEvent('aj-mech:client:Roof:Check')
	elseif GetVehicleMod(vehicle, 10) ~= tonumber(mod) then
		...
		{ task = "WORLD_HUMAN_WELDING" }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			ajlog("`roof - "..AJFW.Shared.Items["roof"].label.." - "..modName.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
			emptyHands(PlayerPedId())
			SetVehicleMod(vehicle, 10, tonumber(mod))
			updateCar(vehicle)
			if Config.CosmeticRemoval then toggleItem(false, "roof")
			else TriggerEvent('aj-mech:client:Roof:Check') end
...
RegisterNetEvent('aj-mech:client:Roof:Check', function()
	...
	local RoofMenu = {
			...
			{ icon = icon, isMenuHeader = disabled, header = Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "aj-mech:client:Roof:Apply", args = -1 } } }
		for k,v in pairs(validMods) do
			local icon = "" local disabled = false
			if GetVehicleMod(vehicle, 10) == v.id then icon = "fas fa-check" disabled = true end
			RoofMenu[#RoofMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'aj-mech:client:Roof:Apply', args = tostring(v.id) } }
		end
