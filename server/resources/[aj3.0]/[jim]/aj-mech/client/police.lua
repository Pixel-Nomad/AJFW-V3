...
Config.PoliceLocations = {
    --Add your poly zone box locations and job name for each store and it will add it to the aj-target loop above
    { coords = vector3(440.89, -986.49, 25.14), heading = 180.0, }, -- MRPD UNDERGROUND PARKING
    { coords = vector3(816.65, -1342.87, 26.12), heading = 269.0, }, -- PILL BOX GARAGE
	{ coords = vector3(1812.64, 3680.17, 33.97), heading = 300, }, -- BCSO UNDERGROUND PARKING
    { coords = vector3(375.95, -1631.57, 29.29), heading = 320, }, -- Davis GARAGE
    { coords = vector3(-487.22, 6023.54, 31.34), heading = 315, }, -- paleeto GARAGE
    { coords = vector3(341.73, -573.22, 28.8), heading = 70, }, -- paleeto GARAGE
}
...
CreateThread(function()
	for k, v in pairs(Config.PoliceLocations) do
		...
		exports['aj-target']:AddBoxZone("bench"..k, v.coords, 1.2, 4.2, { name="bench"..k, heading = v.heading, debugPoly=Config.Debug, minZ = v.coords.z-1, maxZ = v.coords.z+1.4, },
			{ options = { { event = "aj-mech:client:Police:Menu", icon = "fas fa-cogs", label = Loc[Config.Lan]["police"].userepair, job = Config.QuickJobs, }, }, distance = 5.0 })
	end
end)

RegisterNetEvent('aj-mech:client:Police:Menu', function()
	local validMods = {}
	...
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		local PoliceMenu = {}
			PoliceMenu[#PoliceMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = string.gsub(Loc[Config.Lan]["common"].close, "❌ ", ""), params = { event = "aj-mech:client:Menu:Close" } }
			PoliceMenu[#PoliceMenu+1] = { icon = "fas fa-wrench", header = Loc[Config.Lan]["police"].repair, txt = "", params = { event = "aj-mech:client:Police:Repair" }, }
			-- PoliceMenu[#PoliceMenu+1] = { icon = "fas fa-toolbox", header = Loc[Config.Lan]["police"].extras, txt = "", params = { event = "aj-mech:client:Police:Extra" }, }
			-- PoliceMenu[#PoliceMenu+1] = { icon = "fas fa-address-card", header = Loc[Config.Lan]["police"].plates, txt = "", params = { event = "aj-mech:client:Police:Plates" }, }
			if GetNumVehicleMods(vehicle, 48) > 0 or GetVehicleLiveryCount(vehicle) > -1 then
				-- PoliceMenu[#PoliceMenu+1] = { icon = "fas fa-paint-roller", header = Loc[Config.Lan]["police"].livery, txt = "", params = { event = "aj-mech:client:Police:Livery" }, } 
			end
			if GetNumVehicleMods(vehicle, 0) ~= 0 then
				-- PoliceMenu[#PoliceMenu+1] = { icon = "fas fa-car-burst", header = Loc[Config.Lan]["police"].spoiler, txt = "", params = { event = "aj-mech:client:Police:Spoiler" }, } 
			end
			-- PoliceMenu[#PoliceMenu+1] = { icon = "fas fa-brush", header = Loc[Config.Lan]["paint"].menuheader, txt = "", params = { event = "aj-mech:client:Police:Paint" }, }
			--PoliceMenu[#PoliceMenu+1] = { header = "Test", txt = "Vehicle Death Simulator", params = { event = "aj-mech:client:Police:test" }, }
		exports['aj-menu']:openMenu(PoliceMenu)
	end
end)

RegisterNetEvent('aj-mech:client:Police:Extra', function()
	local validMods = {}
	local hasMod = false
	if not IsPedInAnyVehicle(PlayerPedId(), false) then TriggerEvent('aj-mech:client:Police:Menu') return end
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	local ExtraMenu = {}
	...
	ExtraMenu[#ExtraMenu+1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Menu" }, }
	for i = 0, 20 do
		...
		ExtraMenu[#ExtraMenu+1] = { icon = icon, header = "Extra "..i, txt = "", params = { event = "aj-mech:client:Police:Extra:Apply", args = { id = i }, }, } end
	end
	if hadMod then exports['aj-menu']:openMenu(ExtraMenu) elseif not hadMod then triggerNotify(nil, Loc[Config.Lan]["common"].noOptions, "error") TriggerEvent('aj-mech:client:Police:Menu') return end
end)
RegisterNetEvent('aj-mech:client:Police:Extra:Apply', function(data)
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	local veh = {}
	...
	TriggerEvent('aj-mech:client:Police:Extra')
end)
RegisterNetEvent('aj-mech:client:Police:Repair', function()
	...
	TriggerEvent('aj-mech:client:Police:Menu')
end)
RegisterNetEvent('aj-mech:client:Police:Livery', function()
	local validMods = {}
	local vehicle = nil
	...
	if DoesEntityExist(vehicle) then
		...
		if oldlivery == true then
			...
			if GetVehicleLivery(vehicle) == 0 then stockinstall = Loc[Config.Lan]["common"].current icon = "fas fa-check" disabled = true else stockinstall = "" end
				...
				LiveryMenu[#LiveryMenu + 1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = { icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "aj-mech:client:Police:Apply", args = { id = tostring(0), old = true } } }
			for k, v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleLivery(vehicle) == v.id then icon = "fas fa-check" disabled = true end
				LiveryMenu[#LiveryMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'aj-mech:client:Police:Apply', args = { id = tostring(v.id), old = true } } }
			end
		elseif oldlivery ~= true then
			...
			if GetVehicleMod(vehicle, 48) == -1 then stockinstall = Loc[Config.Lan]["common"].current icon = "fas fa-check" disabled = true else stockinstall = "" end
				...
				LiveryMenu[#LiveryMenu + 1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = { icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "aj-mech:client:Police:Apply", args = { id = tostring(-1) } } }
			for k, v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 48) == v.id then icon = "fas fa-check" disabled = true end
				LiveryMenu[#LiveryMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'aj-mech:client:Police:Apply', args = { id = tostring(v.id) } } }
			end
		end
		exports['aj-menu']:openMenu(LiveryMenu)
	end
end)
RegisterNetEvent('aj-mech:client:Police:Apply', function(data)
	...
	if data.old then
		...
		if GetVehicleLivery(vehicle) == tonumber(data.id) then
			...
			TriggerEvent('aj-mech:client:Police:Livery')
			return
		end
	else
...
RegisterNetEvent('aj-mech:client:Police:Plates', function()
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then	vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle)
		if DoesEntityExist(vehicle) then
			local PlateMenu = {
			{ icon = "fas fa-address-card", header = searchCar(vehicle)..Loc[Config.Lan]["plates"].menuheader2, isMenuHeader = true },
			{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Menu" } } }
			for k, v in pairs(Loc[Config.Lan].vehiclePlateOptions) do
				...
				PlateMenu[#PlateMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = installed, params = { event = 'aj-mech:client:Police:Plates:Apply', args = v.id  } }
			end
			exports['aj-menu']:openMenu(PlateMenu)
		end
	end
end)
RegisterNetEvent('aj-mech:client:Police:Plates:Apply', function(index)
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then	vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle) end
	if GetVehicleNumberPlateTextIndex(vehicle) == tonumber(index) then triggerNotify(nil, Loc[Config.Lan]["plates"].already, "error") TriggerEvent('aj-mech:client:Police:Plates')
	elseif GetVehicleNumberPlateTextIndex(vehicle) ~= tonumber(index) then
		SetVehicleNumberPlateTextIndex(vehicle, index)
		TriggerEvent('aj-mech:client:Police:Plates')
	end
end)
RegisterNetEvent('aj-mech:client:Police:Spoiler', function()
	local validMods = {}
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then	vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle)
	...
	if DoesEntityExist(vehicle) then
		...
		local spoilerMenu = {
				...
				{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Menu" } },
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "aj-mech:client:Police:Spoilers:Apply", args = -1 } } }
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 0) == v.id then icon = "fas fa-check" disabled = true end
				spoilerMenu[#spoilerMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'aj-mech:client:Police:Spoilers:Apply', args = tostring(v.id) } }
			end
		exports['aj-menu']:openMenu(spoilerMenu)
	end
end)
RegisterNetEvent('aj-mech:client:Police:Spoilers:Apply', function(mod)
	if IsPedInAnyVehicle(PlayerPedId(), false) then	vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle) end
	local modName = GetLabelText(GetModTextLabel(vehicle, 0, tonumber(mod)))
	if modName == "NULL" then modName = Loc[Config.Lan]["common"].stock end
	if GetVehicleMod(vehicle, 0) == tonumber(mod) then
		triggerNotify(nil, modName..Loc[Config.Lan]["common"].already, "error")
		TriggerEvent('aj-mech:client:Police:Spoiler')
...
RegisterNetEvent('aj-mech:client:Police:Paint', function()
	local validMods = {}
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle) end
	...
	local PaintMenu = {
			...
			{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Menu", } } }
		PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].primary, txt = Loc[Config.Lan]["common"].current..": "..vehPrimaryColour, params = { event = "aj-mech:client:Police:Paints:Choose", args = Loc[Config.Lan]["paint"].primary } }
		PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].secondary, txt = Loc[Config.Lan]["common"].current..": "..vehSecondaryColour, params = { event = "aj-mech:client:Police:Paints:Choose", args = Loc[Config.Lan]["paint"].secondary } }
		PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].pearl, txt = Loc[Config.Lan]["common"].current..": "..vehPearlescentColour, params = { event = "aj-mech:client:Police:Paints:Choose", args = Loc[Config.Lan]["paint"].pearl } }
	exports['aj-menu']:openMenu(PaintMenu)
end)

RegisterNetEvent('aj-mech:client:Police:Paints:Choose', function(data)
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then	vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle) end
	if DoesEntityExist(vehicle) then
		exports['aj-menu']:openMenu({
			{ icon = "fas fa-brush", header = data..Loc[Config.Lan]["paint"].menuheader, txt = "", isMenuHeader = true },
			{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Paint" } },
			{ header = Loc[Config.Lan]["paint"].metallic, txt = "", params = { event = "aj-mech:client:Police:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metallic } } },
			{ header = Loc[Config.Lan]["paint"].matte, txt = "", params = { event = "aj-mech:client:Police:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].matte } } },
			{ header = Loc[Config.Lan]["paint"].metals, txt = "", params = { event = "aj-mech:client:Police:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metals } } },
		})
	end
end)

RegisterNetEvent('aj-mech:client:Police:Paints:Choose:Colour', function(data)
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then	vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle) end
	local vehPrimaryColour, vehSecondaryColour = GetVehicleColours(vehicle)
	local vehPearlescentColour, vehWheelColour = GetVehicleExtraColours(vehicle)
	...
	local PaintMenu = {
		...
		{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "aj-mech:client:Police:Paints:Choose", args = data.paint } } }
	local installed = nil
	if data.finish == Loc[Config.Lan]["paint"].metallic then
		for k, v in pairs(Loc[Config.Lan].vehicleResprayOptionsClassic) do
			...
			PaintMenu[#PaintMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = installed, params = { event = 'aj-mech:client:Police:Paints:Apply', args = { paint = data.paint, id = v.id, name = v.name, finish = data.finish } } } end
...
RegisterNetEvent('aj-mech:client:Police:Paints:Apply', function(data)
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then vehicle = GetVehiclePedIsIn(PlayerPedId(), false) pushVehicle(vehicle) end
	local vehPrimaryColour, vehSecondaryColour = GetVehicleColours(vehicle)
	local vehPearlescentColour, vehWheelColour = GetVehicleExtraColours(vehicle)
	...
	TriggerEvent('aj-mech:client:Police:Paints:Choose:Colour', data)
end)


RegisterNetEvent('aj-mech:client:Police:test', function(data)
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
AddEventHandler('onResourceStop', function(r)
	if r == GetCurrentResourceName() then
		for k, v in pairs(Config.PoliceLocations) do exports['aj-target']:RemoveZone("bench"..k) end
