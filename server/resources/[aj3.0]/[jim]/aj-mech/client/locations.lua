...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
    AJFW.Functions.GetPlayerData(function(PlayerData) PlayerJob = PlayerData.job
        if PlayerData.job.onduty then
			for _, v in pairs(Config.JobRoles) do if v == PlayerJob.name then havejob = true end end
			if havejob then TriggerServerEvent("AJFW:ToggleDuty") end end
    end)
end)
				...
				if isPointInside then
					if Config.RequiresJob then
						if PlayerJob.name == tostring(Config.Locations[k].job) then	injob = true
							if Config.Locations[k].autoClock.enter and not onDuty then TriggerServerEvent("AJFW:ToggleDuty") end
						end
					else injob = true end
				else
					if Config.RequiresJob then
						if PlayerJob.name == tostring(Config.Locations[k].job) then injob = false
							if Config.Locations[k].autoClock.exit and onDuty then TriggerServerEvent("AJFW:ToggleDuty") end
						end
		...
		if v.clockin then
			for l, b in pairs(v.clockin) do
				if type(b) ~= "boolean" then
					local bossrole = {}
					for grade in pairs(AJFW.Shared.Jobs[v.job].grades) do
						if AJFW.Shared.Jobs[v.job].grades[grade].isboss == true then
					...
					exports['aj-target']:AddBoxZone("MechClock: "..k..l, b.coords.xyz, (b.w or 0.45), (b.d or 0.4), { name="MechClock: "..k..l, heading = b.coords.w, debugPoly=Config.Debug, minZ=b.coords.z-0.1, maxZ=b.coords.z+0.4 },
						{ options = {
							{ type = "server", event = "AJFW:ToggleDuty", icon = "fas fa-clipboard", label = "Duty Toggle", job = v.job },
							-- { event = "aj-bossmenu:client:OpenMenu", icon = "fas fa-list", label = "Open Bossmenu", job = bossrole, },
						},
						distance = 2.0 })
				end
			end
	...
	for i = 1, #data.craftable do
		for k, v in pairs(data.craftable[i]) do
			if k ~= "amount" and k ~= "job" then
				...
				if not AJFW.Shared.Items[k] then print("^5Debug^7: ^2Item not found in server^7: '^6"..k.."^7'") else
					if data.craftable[i]["job"] and hasjob == false then else
						local text = ""
						setheader = "<img src=nui://"..Config.img..AJFW.Shared.Items[k].image.." width=30px onerror='this.onerror=null; this.remove();'> "..AJFW.Shared.Items[tostring(k)].label
...
RegisterNetEvent('aj-mech:client:NosRefill', function(data)
	...
	if cash >= Config.NosRefillCharge then
		...
		AJFW.Functions.Progressbar('making_food', "Refilling "..AJFW.Shared.Items["nos"].label, 7000, false, true, { disableMovement = true, disableCarMovement = false, disableMouse = false, disableCombat = false, },
...
RegisterNetEvent('aj-mech:client:Garage:Menu', function(data)
	...
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		...
		for k, v in pairs(data.list) do
			...
			for _, b in pairs(AJFW.Shared.Vehicles) do
...
RegisterNetEvent("aj-mech:client:Garage:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		for k, v in pairs(AJFW.Shared.Vehicles) do
			...
			name = AJFW.Shared.Vehicles[k].name break
			end
		end
		triggerNotify(nil, name..Loc[Config.Lan]["garage"].cantspawn, "error")
	else
		AJFW.Functions.SpawnVehicle(data.spawnName, function(veh)
...
Config.Locations = {
	...
	{	
		job = "mechanic",
		zones = {
			vector2(-357.03939819336, -138.75611877441),
			vector2(-349.12643432617, -149.74360656738),
			vector2(-350.21908569336, -150.67910766602),
			vector2(-361.53884887695, -156.18725585938),
			vector2(-351.84436035156, -173.09387207031),
			vector2(-345.20300292969, -171.47227478027),
			vector2(-340.10607910156, -168.43574523926),
			vector2(-329.72131347656, -166.47723388672),
			vector2(-322.79650878906, -148.76138305664),
			vector2(-307.11050415039, -103.93367767334),
			vector2(-316.62496948242, -98.01831817627),
			vector2(-313.91793823242, -91.183303833008),
			vector2(-325.5295715332, -86.95499420166),
			vector2(-328.76113891602, -93.548805236816),
			vector2(-369.31536865234, -78.909370422363),
			vector2(-371.623046875, -80.539695739746),
			vector2(-377.58810424805, -96.958518981934),
			vector2(-380.69314575195, -99.031471252441),
			vector2(-384.01599121094, -97.995788574219),
			vector2(-395.58340454102, -104.48196411133),
			vector2(-402.01123046875, -108.07752227783),
			vector2(-378.16244506836, -150.6770324707),
			vector2(-364.07302856445, -155.56346130371),
			vector2(-352.41983032227, -148.74235534668)
		},
		autoClock = { enter = false, exit = false, },
		stash = { { coords = vector4(-316.89, -129.94, 39.02, 257.21), w = 3.6, d = 0.8, }, },
		store = { { coords = vector4(-337.17, -162.16, 44.59, 180.4), w = 3.60, d = 0.8 }, },
		crafting = { { coords = vector4(-319.76, -137.3, 39.02, 254.43), w = 2.8, d = 1.5 }, },
		clockin = { { coords = vector4(-322.52, -145.27, 39.02, 271.68), prop = false }, },
		manualRepair = { { coords = vector4(-388.34, -132.71, 38.89, 299.0), prop = true, } },
		garage = { 
			spawn = vector4(-356.8, -160.01, 38.16, 28.08),
			out = vector4(-349.24, -154.46, 39.01, 121.81),
			list = { "towtruck", "panto", "slamtruck", "cheburek", "utillitruck3" },
			prop = true 
		},
		payments = { coords = vector4(-322.8, -148.0, 38.8, 251.76),
		img = "<center><p><img src=https://static.wikia.nocookie.net/gtawiki/images/b/be/BennysOriginalMotorWorks-GTAO-Logo.png width=150px></p>",
		prop = true },
		blip = vector3(-211.55, -1324.55, 30.9),
		bliplabel = "Bennys Original Motorworks",
		blipcolor = 1,
		discordlink = "",
		discordcolour = 16711680,
		discordimg = "https://static.wikia.nocookie.net/gtawiki/images/b/be/BennysOriginalMotorWorks-GTAO-Logo.png",
