	...
	local CheckMenu = {
		...
		if GetNumVehicleMods(vehicle,11) ~= 0 then -- If engine can be changed
			if GetVehicleMod(vehicle, 11) == -1 then  -- If Stock
				...
				modEngine = "<img src=nui://"..Config.img..AJFW.Shared.Items["engine"..lvl].image.." width=13px onerror='this.onerror=null; this.remove();'> "..AJFW.Shared.Items["engine"..lvl].label..": [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 11).."]"
				modEngineHead = false
			end
		...
		if GetNumVehicleMods(vehicle, 12) ~= 0 then
			if GetVehicleMod(vehicle, 12) == -1 then
				...
				modBrakes = "<img src=nui://"..Config.img..AJFW.Shared.Items["brakes"..lvl].image.." width=13px onerror='this.onerror=null; this.remove();'> "..AJFW.Shared.Items["brakes"..lvl].label..": [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 12).."]"
				modBrakesHead = false
			end
		...
		if GetNumVehicleMods(vehicle,15) ~= 0 then
			if GetVehicleMod(vehicle, 15) == -1 then
				...
				modSuspension = "<img src=nui://"..Config.img..AJFW.Shared.Items["suspension"..lvl].image.." width=13px onerror='this.onerror=null; this.remove();'> "..AJFW.Shared.Items["suspension"..lvl].label..": [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 15).."]"
		...
		if GetNumVehicleMods(vehicle,13) ~= 0 then
			if GetVehicleMod(vehicle, 13) == -1 then
				...
				modTransmission = "<img src=nui://"..Config.img..AJFW.Shared.Items["transmission"..lvl].image.." width=13px onerror='this.onerror=null; this.remove();'> "..AJFW.Shared.Items["transmission"..lvl].label.." [LVL "..lvl.." / "..GetNumVehicleMods(vehicle, 13).."]"
		...
		if GetNumVehicleMods(vehicle, 16) ~= 0 then
			if (GetVehicleMod(vehicle, 16)+1) == GetNumVehicleMods(vehicle, 16) then armicon = "car_armor"
				modArmor = "<img src=nui://"..Config.img..AJFW.Shared.Items["car_armor"].image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].reinforced
				modArmorHead = false
		...
		elseif IsToggleModOn(vehicle, 18) then turicon = "turbo"
			modTurbo = "<img src=nui://"..Config.img..AJFW.Shared.Items["turbo"].image.." width=13px onerror='this.onerror=null; this.remove();'> "..AJFW.Shared.Items["turbo"].label
			modTurboHead = false
		...
		elseif IsToggleModOn(vehicle, 22) then headicon = "headlights"
			modXenon = "<img src=nui://"..Config.img..AJFW.Shared.Items["headlights"].image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].xenoninst
			modXenonHead = false
		...
		if GetGameBuildNumber() >= 2372 then
			...
			elseif GetDriftTyresEnabled(vehicle) == 1 then drifticon = "drifttires"
				modDrift = "<img src=nui://"..Config.img..AJFW.Shared.Items["drifttires"].image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].tireinst
				modDriftHead = false
		...
		if GetVehicleTyresCanBurst(vehicle) == false and GetNumVehicleMods(vehicle,11) ~= 0 then bprooficon = "bprooftires"
			modBproof = "<img src=nui://"..Config.img..AJFW.Shared.Items["bprooftires"].image.." width=13px onerror='this.onerror=null; this.remove();'> "..Loc[Config.Lan]["check"].tireinst
			modBproofHead = false
...
RegisterNetEvent('aj-mech:client:Menu:List', function()
	...
	if mods then exports['aj-menu']:openMenu(CheckMenu) else triggerNotify(nil, Loc[Config.Lan]["common"].noOptions, "error") return end
end)
