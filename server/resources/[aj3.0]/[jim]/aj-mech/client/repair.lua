...
RegisterNetEvent('aj-mech:client:Repair:Apply', function(data)
	...
	if Config.RequiresJob and Config.StashRepair and not Config.FreeRepair then
		...
		if hasitem and countitem >= cost then
			AJFW.Functions.Progressbar("repair_part", bartext..data.part, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false,	},
		...
		AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["repair"].repairing..data.part, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
...
RegisterNetEvent('aj-mech:client:Repair:Check', function(skip)
	...
	if useMechJob() then
		if not exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), "radiator") then
		...
		if Config.Debug then
			for _, v in pairs({ "radiator", "axle", "brakes", "clutch", "fuel" }) do
				print("^5Debug^7: ^3"..v.."^7: ^6"..exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), v))
			end
		end
	end
	...
	if skip == -2 then else
		...
		if not Config.FreeRepair then
			...
			if EngineRepair ~= 0 then costEngine = Loc[Config.Lan]["repair"].cost..EngineRepair.." "..AJFW.Shared.Items[Config.RepairEngine].label end
			...
			if BodyRepair ~= 0 then costBody = Loc[Config.Lan]["repair"].cost..BodyRepair.." "..AJFW.Shared.Items[Config.RepairBody].label end
			if useMechJob() then
				RadiatorRepair = Config.RepairRadiatorCost - math.floor(Config.RepairRadiatorCost *	math.floor((exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), "radiator")+0.5) or 90) / 100)
				if RadiatorRepair ~= 0 then costRadiator = Loc[Config.Lan]["repair"].cost..RadiatorRepair.." "..AJFW.Shared.Items[Config.RepairRadiator].label end
				AxleRepair = Config.RepairAxleCost - math.floor(Config.RepairAxleCost * math.floor((exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), "axle")+0.5) or 90) / 100)
				if AxleRepair ~= 0 then costAxle = Loc[Config.Lan]["repair"].cost..AxleRepair.." "..AJFW.Shared.Items[Config.RepairAxle].label end
				BrakesRepair = Config.RepairBrakesCost - math.floor(Config.RepairBrakesCost * math.floor((exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), "brakes")+0.5) or 90) / 100)
				if BrakesRepair ~= 0 then costBrakes = Loc[Config.Lan]["repair"].cost..BrakesRepair.." "..AJFW.Shared.Items[Config.RepairBrakes].label or "" end
				ClutchRepair = Config.RepairClutchCost - math.floor(Config.RepairClutchCost * math.floor((exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), "clutch")+0.5) or 90) / 100)
				if ClutchRepair ~= 0 then costClutch = Loc[Config.Lan]["repair"].cost..ClutchRepair.." "..AJFW.Shared.Items[Config.RepairClutch].label end
				FuelRepair = Config.RepairFuelCost - math.floor(Config.RepairFuelCost * math.floor((exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), "fuel")+0.5) or 90) / 100)
				if FuelRepair ~= 0 then costFuel = Loc[Config.Lan]["repair"].cost..FuelRepair.." "..AJFW.Shared.Items[Config.RepairFuel].label end
			end
		end
	end
	...
	local RepairMenu = {
		...
		if useMechJob() then
			if math.floor((exports['aj-mechanicjob']:GetVehicleStatus(trim(GetVehicleNumberPlateText(vehicle)), "radiator"))+0.5) == 100 or skip then headerlock = true end
			if Config.StashRepair or Config.FreeRepair then
	...
	if DoesEntityExist(vehicle) then
		if prevVehicle == vehicle then
			...
			for i = 0, 5 do SetVehicleDoorOpen(vehicle, i, false, false) end
			AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["repair"].checkeng, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
			{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 16, }, {}, {}, function()
				time = math.random(3000,5000)
				AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["repair"].checkbody, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
