...
RegisterNetEvent('aj-mech:client:Paints:Apply', function(data)
	...
	local vehicle
	if not IsPedInAnyVehicle(playerPed, false) then	vehicle = getClosest(coords) pushVehicle(vehicle) lookVeh(vehicle) else	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	...
	elseif colourCheck ~= data.id then
		...
		AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["common"].installing..data.paint.." "..data.finish.." "..data.name, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
		{ task = settask, animDict = setanimDict, anim = setanim, flags = setflag, }, {}, {}, function()  SetVehicleModKit(vehicle, 0)
			ajlog("`paintcan - "..AJFW.Shared.Items["paintcan"].label.." - "..data.paint.." "..data.finish.." "..data.name.."` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
