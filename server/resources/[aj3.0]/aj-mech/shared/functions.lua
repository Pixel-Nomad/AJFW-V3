...
function useMechJob() if GetResourceState('aj-mechanicjob') == "started" then return true else return false end end
...
function searchCar(vehicle)
	if lastCar ~= vehicle then -- Check added to use old info instead of checking again
		...
		for k, v in pairs(AJFW.Shared.Vehicles) do
			...
			if Config.Debug then print("^5Debug^7: ^2Vehicle info found in^7 ^4vehicles^7.^4lua^7: ^6"..v.hash.. " ^7(^6"..AJFW.Shared.Vehicles[k].name.."^7)") end
			newName = AJFW.Shared.Vehicles[k].name.." "..AJFW.Shared.Vehicles[k].brand.."<br>"
			end
		end
...
function searchPrice(vehicle)
	...
	for k, v in pairs(AJFW.Shared.Vehicles) do
		if tonumber(v.hash) == GetEntityModel(vehicle) then
		price = cv(AJFW.Shared.Vehicles[k].price)
		end
	end
...
function updateCar(vehicle)
	updateDelay[vehicle] = { delay = Config.updateDelay or 2, mods = AJFW.Functions.GetVehicleProperties(vehicle) }
...
function ajlog(text)
	...
	TriggerServerEvent('aj-log:server:CreateLog', 'vehicleupgrades', GetCurrentResourceName() .. " - "..Player.charinfo.firstname.." "..Player.charinfo.lastname.."("..Player.source..") ["..Player.citizenid.."]", 'blue',	text)
end
...
function triggerNotify(title, message, type, src)
	...
	elseif Config.Notify == "aj" then
		if not src then	TriggerEvent("AJFW:Notify", message, "primary")
		else TriggerClientEvent("AJFW:Notify", src, message, "primary") end
...
function HasItem(items, amount)
    ...
    for _, itemData in pairs(AJFW.Functions.GetPlayerData().items) do
