...
RegisterNetEvent('jim-mechanic:client:Tires:Apply', function(data)
	...
	if r == data.R and g == data.G and b == data.B then
		...
		AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["smoke"].installing, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			ajlog("`tires - "..AJFW.Shared.Items["tires"].label.." - {"..data.R ..", "..data.G..", "..data.B.."}` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
