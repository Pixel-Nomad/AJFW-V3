...
RegisterNetEvent('jim-mechanic:client:applyDrift', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if GetDriftTyresEnabled(vehicle) ~= false then triggerNotify(nil, Loc[Config.Lan]["common"].already, "error") else
			...
			AJFW.Functions.Progressbar("drink_something", Loc[Config.Lan]["tires"].install, math.random(7000,10000), false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = true, },
