...
CreateThread(function()
    ...
    while not AJFW do Wait(25) end
    for k, _ in pairs(AJFW.Shared.Jobs) do
            ...
            if players[1] then
                for _, v in pairs(players) do
                    if v.job then
                        ...
                        if grade and AJFW.Shared.Jobs[k].grades and AJFW.Shared.Jobs[k].grades[tostring(grade)] and v.citizenid and v.charinfo and FirstName and LastName then
...
local function notifyPlayer(src, message)
    ...
    TriggerClientEvent('aj-phone:client:CustomNotification', src,
        "Employment",
        message,
...
RegisterNetEvent('aj-phone:server:fireUser', function(Job, sCID)
    local src = source
    local srcPlayer = AJFW.Functions.GetPlayer(src)
    ...
    if not AJFW.Shared.Jobs[Job].grades[grade].isboss then return end
    ...
    TriggerClientEvent("aj-phone:client:JobsHandler", -1, Job, CachedJobs[Job].employees)
    ...
    if Player.PlayerData.source then
        TriggerClientEvent('aj-phone:client:MyJobsHandler', Player.PlayerData.source, Job, nil, nil)
    end
end)

---- ** Can give any employee x amount of money out of the bank account, must be boss ** ----
RegisterNetEvent('aj-phone:server:SendEmploymentPayment', function(Job, CID, amount)
    local src = source
    ...
    local grade = tostring(CachedJobs[Job].employees[srcCID].grade)
    if not AJFW.Shared.Jobs[Job].grades[grade].isboss then return notifyPlayer(src, "You aren't a manager...") end

    local Reciever = AJFW.Functions.GetPlayerByCitizenId(CID)
    if not Reciever then return notifyPlayer(src, "Employee not found...") end

    local amt = tonumber(amount)
    if Config.ajBanking then
        if not exports['aj-Banking']:removeAccountMoney(Job, amt) then return notifyPlayer(src, "Insufficient Funds...") end
        local title = AJFW.Shared.Jobs[Job].label.." // Employee Payment"
        ...
        local trans = exports['aj-Banking']:handleTransaction(Job, title, amt, "Payment given of $"..amt.." given to "..RecieverName, BusinessName, RecieverName, "withdraw")

        ---- Player Account ----
        exports['aj-Banking']:handleTransaction(Reciever.PlayerData.citizenid, title, amt, "Payment recieved of $"..amt.." recieved from Business "..AJFW.Shared.Jobs[Job].label.. " and Manager "..BusinessName, BusinessName, RecieverName, "deposit", trans.trans_id)
    else
        if not exports['aj-management']:RemoveMoney(Job, amt) then return notifyPlayer(src, "Insufficient Funds...") end
    end
    Player.Functions.AddMoney('bank', amt)
end)

---- ** Player can hire someone aslong as they are boss within the group
RegisterNetEvent('aj-phone:server:hireUser', function(Job, id, grade)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local hiredPlayer = AJFW.Functions.GetPlayer(tonumber(id))
    ...
    if not AJFW.Shared.Jobs[Job].grades[bossGrade].isboss then return notifyPlayer(src, "You arent a manager // boss...") end
    ...
    if hiredPlayer and CachedPlayers[CID] then
        ...
        TriggerClientEvent('aj-phone:client:MyJobsHandler', hiredPlayer.PlayerData.source, Job, CachedPlayers[CID][Job], CachedJobs[Job].employees)
    end
end)

---- ** Handles the changing of someone grade within the job ** ----

RegisterNetEvent('aj-phone:server:gradesHandler', function(Job, CID, grade)
    local src = source
    local srcPlayer = AJFW.Functions.GetPlayer(src)
    ...
    if Player and CachedPlayers[CID] then
        ...
        TriggerClientEvent('aj-phone:client:MyJobsHandler', Player.PlayerData.source, Job, CachedPlayers[CID][Job], CachedJobs[Job].employees)
    end
end)


RegisterNetEvent('aj-phone:server:clockOnDuty', function(Job)
    local src = source
    ...
    if CachedPlayers[CID][Job] and CachedJobs[Job].employees[CID] then
        ...
        TriggerClientEvent('aj-phone:client:clearAppAlerts', src)
    end
end)

---- Gets the client side cache for players ----
AJFW.Functions.CreateCallback("aj-phone:server:GetMyJobs", function(source, cb)
    if FirstStart then return end
...
RegisterNetEvent('aj-phone:server:ChargeCustomer', function(id, amount, notes, job)
    ...
    TriggerEvent('aj-phone:server:CreateInvoice', tonumber(id), src, amt)
end)
    ...
    if paid then
        if amount == bills[source].amount then
            ...
            if Config.ajBanking then
                ...
                local trans = exports['aj-Banking']:handleTransaction(cid, "Personal // Invoice Transaction", amount, text, bills[source].name, name, "withdraw")

                local text2 = "A invoice issued by "..bills[source].name.." Worth $"..amount.." was paid off by "..name.." for "..bills[source].notes
                exports['aj-Banking']:handleTransaction(bills[source].job, "Business // Invoice Transaction", amount, text2, bills[source].name, name, "deposit", trans.trans_id)
                exports['aj-Banking']:addAccountMoney(bills[source].job, amount)
            else
                exports['aj-management']:AddMoney(bills[source].job, amount)
            end

            if amount >= 450 then
                local DutySrcs, count = AJFW.Functions.GetPlayersOnDuty(bills[source].job)
                if count > 0 then
                    for i  = 1, count do
                        ...
                        if #(coords - bills[source].coords) < 10.0 then
                            local tempPlayer = AJFW.Functions.GetPlayer(DutySrcs[i])
                            tempPlayer.Functions.AddItem('payticket', 1)
                            TriggerClientEvent('inventory:client:ItemBox', DutySrcs[i], AJFW.Shared.Items['payticket'], "add", 1)
                            TriggerClientEvent('AJFW:Notify', DutySrcs[i], 'Receipt received', 'success')
                        end
                    end
                end
            end
