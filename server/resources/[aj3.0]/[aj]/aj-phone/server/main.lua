...
RegisterNetEvent('aj-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
    local Target = AJFW.Functions.GetPlayerByPhone(tostring(TargetData.number))
...
RegisterNetEvent('aj-phone:server:EditContact', function(newName, newNumber, oldName, oldNumber)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:AddNewContact', function(name, number)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:AddRecentCall', function(type, data)
    ...
    local Target = AJFW.Functions.GetPlayerByPhone(data.number)
    if not Target then return end

    TriggerClientEvent('aj-phone:client:AddRecentCall', Target.PlayerData.source, {
...
RegisterNetEvent('aj-phone:server:GiveContactDetails', function(PlayerId)
    ...
    TriggerClientEvent('aj-phone:client:giveContactRequest', PlayerId, contactInfo)
end)

RegisterNetEvent('aj-phone:server:acceptContactRequest', function(contactInfo)
    ...
    if result[1] then return TriggerClientEvent('AJFW:Notify', src, 'You already have this number added!', "error") end
    ...
    TriggerClientEvent('aj-phone:client:updateContactInfo', src, contactInfo)
end)

RegisterNetEvent('aj-phone:server:CancelCall', function(ContactData)
    local Ply = AJFW.Functions.GetPlayerByPhone(tostring(ContactData.TargetData.number))
    if not Ply then return end
