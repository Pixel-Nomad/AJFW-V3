...
$(document).on("click", ".container #slider #selectcharacter", function (e) {
    ...
    if (selected != char && switching == false) {
        ...
        $.post("https://aj-multichar/switch", JSON.stringify({ char: selected, citizenid: ele.attr("data-citizenid")}), function(data) {
            if (selected >= 0) {
                setTimeout(()=>{
...
$(document).on("click", "#createchar_button", function (e) {
    ...
    $.post("https://aj-multichar/create", JSON.stringify({ char: createdCharacter})).then((res) => {
        if (res) {
            $("body").fadeOut(300);
...
$(document).on("click", "#spawnplayer_button", function (e) {
    if (!switching) {
        $.post("https://aj-multichar/select", JSON.stringify({ citizenid: $(`.container #slider #selectcharacter[data-char='${char}']`).attr('data-citizenid')}));
        $("body").fadeOut(500);
...
$(document).on("input", "#deleteplayer_input", function (e) {
    if ($(this).val() == characters[char-1]["name"]) {
        ...
        $.post("https://aj-multichar/delete", JSON.stringify({ citizenid: $(`.container #slider #selectcharacter[data-char='${char}']`).attr('data-citizenid')}))
        .then((res) => {
            if (res) {
