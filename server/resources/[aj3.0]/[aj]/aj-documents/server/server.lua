...
elseif GetResourceState("aj-base") == "started" then
  ...
  print("^8ERROR: ^3This script only supports ESX and AJFW frameworks, but non of these are present. Unfortunatelly, you cannot use this script.^7")
  return
end
...
elseif CurrentFramework == "aj" then
  AJFW = exports['aj-base']:GetCoreObject()
  RegisterCallback = function (name, fn)
    AJFW.Functions.CreateCallback(name, fn)
  end
  RegisterItem = function(itemName, fn)
    AJFW.Functions.CreateUseableItem(itemName, fn)
  end
end
...
Citizen.CreateThread(function()
    ...
    PerformHttpRequest('https://raw.githubusercontent.com/kac5a/k5_documents/master/fxmanifest.lua',function(error, result, headers)
      if not result then print('AJ Documents: ^1Couldn\'t check version.^0') end
...
RegisterCallback('aj_documents:getPlayerCopies', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

  MySQL.Async.fetchAll(
    'SELECT id, data, isCopy FROM aj_documents WHERE ownerId = @identifier and isCopy = "1"', {
...
if Config.DocumentItemName then
  RegisterItem(Config.DocumentItemName, function(source)
    local src = source
    TriggerClientEvent("aj_documents:useItem", src)
  end)
end

RegisterCallback('aj_documents:getPlayerDocuments', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

    MySQL.Async.fetchAll(
      'SELECT id, data, isCopy FROM aj_documents WHERE ownerId = @identifier and isCopy = "0"', {
      ['@identifier'] = PlayerIdentifier
    }, function(result)
      local mappedResult = {}
      for k, v in pairs(result) do
...
RegisterCallback('aj_documents:getDocumentTemplates', function(source, cb)
  local src = source  
  local PlayerJobName = GetPlayerJobName(src)

  MySQL.Async.fetchAll(
    'SELECT id, data FROM aj_document_templates WHERE job = @job', {
    ['@job'] = PlayerJobName
  }, function(result)
    local mappedResult = {}
    for k, v in pairs(result) do
...
RegisterCallback('aj_documents:createTemplate', function(source, cb, data)
  local src = source  
  local PlayerJobName = GetPlayerJobName(src)

  MySQL.Async.insert('INSERT INTO aj_document_templates (data, job) VALUES (@data, @job)', {
			['@data'] = data,
			['@job'] = PlayerJobName
		}, function(result)
      cb(result)
...
RegisterCallback('aj_documents:editTemplate', function(source, cb, data)
  local obj = json.decode(data)

  MySQL.Async.execute('UPDATE aj_document_templates SET data = @data WHERE id = @id', {
			['@data'] = data,
			['@id'] = obj.id
		}, function(result)
      cb(result)
...
RegisterCallback('aj_documents:deleteTemplate', function(source, cb, data)
  MySQL.Async.execute('DELETE FROM aj_document_templates WHERE id = @id', {
			['@id'] = data
		}, function(result)
      cb(result)
  end)
...
RegisterCallback('aj_documents:getPlayerData', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('aj_documents:createDocument', function(source, cb, data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

  MySQL.Async.insert('INSERT INTO aj_documents (data, ownerId, isCopy) VALUES (@data, @ownerId, @isCopy)', {
			['@data'] = data,
...
RegisterServerEvent('aj_documents:createServerDocument')
AddEventHandler('aj_documents:createServerDocument', function(data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('aj_documents:deleteDocument', function(source, cb, data)
  MySQL.Async.execute('DELETE FROM aj_documents WHERE id = @id', {
			['@id'] = data
		}, function(result)
      cb(result)
  end)
...
RegisterServerEvent('aj_documents:giveCopy')
AddEventHandler('aj_documents:giveCopy', function(data, targetId)
  local src = source
  local tsrc = targetId
  ...
  MySQL.Async.insert('INSERT INTO aj_documents (data, ownerId, isCopy) VALUES (@data, @ownerId, @isCopy)', {
    ...
    TriggerClientEvent("aj_documents:copyGave", src, data.name)
    TriggerClientEvent("aj_documents:copyReceived", tsrc, data.name)
  end)

end)

RegisterServerEvent("aj_documents:receiveDocument")
AddEventHandler("aj_documents:receiveDocument", function(data, targetId)
  local tsrc = targetId

  MySQL.Async.fetchAll('SELECT data FROM aj_documents WHERE id = @docId', {
    ['@docId'] = data.docId,
  }, function(result)
    TriggerClientEvent("aj_documents:viewDocument", tsrc, result[1])
  end)
end)
...
function GetPlayerIdentifier(src)
  ...
  elseif CurrentFramework == "aj" then
    PlayerIdentifier = AJFW.Functions.GetPlayer(src).PlayerData.citizenid
  end
...
function GetPlayerJobName(src)
  ...
  elseif CurrentFramework == "aj" then
    PlayerJobName = AJFW.Functions.GetPlayer(src).PlayerData.job.name
  end
