...
elseif GetResourceState("aj-base") == "started" then
  ...
  print("^8ERROR: ^3This script only supports ESX and AJFW frameworks, but non of these are not present. Unfortunatelly, you cannot use this script.^7")
  return
end
...
elseif CurrentFramework == "aj" then
  ...
  TriggerCallback = function (name, cb, ...) 
    AJFW.Functions.TriggerCallback(name, cb, ...)
  end

  Notification = function (msg) 
    AJFW.Functions.Notify(msg)
  end
end
...
RegisterNUICallback('getPlayerData', function(data, cb)
  if CurrentFramework == "esx" then
    ESX.TriggerServerCallback('aj_documents:getPlayerData', function(result)
      cb(result)
    end)
  elseif CurrentFramework == "aj" then
    local PlayerData = AJFW.Functions.GetPlayerData().charinfo
    cb({
...
RegisterNUICallback('getPlayerCopies', function(data, cb)
  TriggerCallback('aj_documents:getPlayerCopies', function(result)
    cb(result)
  end)
end)

RegisterNUICallback('getIssuedDocuments', function(data, cb)
  TriggerCallback('aj_documents:getPlayerDocuments', function(result)
    cb(result)
  end)
end)

RegisterNUICallback('createDocument', function(data, cb)
  TriggerCallback('aj_documents:createDocument',function(result)
    cb(result)
  end,
  data)
end)
...
RegisterNUICallback('createTemplate', function(data, cb)
  TriggerCallback('aj_documents:createTemplate',function(result)
    cb(result)
  end,
  data)
end)
...
RegisterNUICallback('editTemplate', function(data, cb)
  TriggerCallback('aj_documents:editTemplate',function(result)
    cb(result)
  end,
  data)
end)
...
RegisterNUICallback('deleteTemplate', function(data, cb)
  TriggerCallback('aj_documents:deleteTemplate',function(result)
    cb(result)
  end,
  data)
end)
...
RegisterNUICallback('deleteDocument', function(data, cb)
  TriggerCallback('aj_documents:deleteDocument',function(result)
    cb(result)
  end,
  data)
end)
...
RegisterNUICallback('getMyTemplates', function(data, cb)
  TriggerCallback('aj_documents:getDocumentTemplates', function(result)
    cb(result)
  end)
end)
...
RegisterNUICallback('giveCopy', function(data, cb)
  Citizen.CreateThread(function()
    ...
    if targetId == -1 then
      ...
      TriggerServerEvent("aj_documents:giveCopy", data, targetId)
    end
  end)
end)
...
RegisterNUICallback('showDocument', function(data, cb)
  Citizen.CreateThread(function()
    ...
    if targetId == -1 then
      ...
      TriggerServerEvent("aj_documents:receiveDocument", data, targetId)
    end
  end)
end)

RegisterNetEvent('aj_documents:copyGave')
AddEventHandler('aj_documents:copyGave', function(data)
	holdDocument(false)
  playAnim("mp_common", "givetake1_a", 1500)
  Notification(Config.Locale.giveNotification .. " " .. data)
end)

RegisterNetEvent("aj_documents:useItem")
AddEventHandler("aj_documents:useItem", function()
	toggleNuiFrame(true, true)
end)


RegisterNetEvent('aj_documents:copyReceived')
AddEventHandler('aj_documents:copyReceived', function(data)
  Notification(Config.Locale.receiveNotification .. " " .. data)
end)

RegisterNetEvent('aj_documents:viewDocument')
AddEventHandler('aj_documents:viewDocument', function(data)
...
function playerSelector(confirmText)
  ...
  while selectingPlayer do
    ...
    elseif CurrentFramework == "aj" then
      closestPlayer, closestPlayerDistance = AJFW.Functions.GetClosestPlayer()
    end
