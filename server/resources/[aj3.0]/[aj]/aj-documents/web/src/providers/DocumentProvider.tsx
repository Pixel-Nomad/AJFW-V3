...
type DocumentProviderValue = {
  document: AJDocument | undefined
  setDocument: (document: AJDocument | undefined) => void
}

// This should be mounted at the top level of your application, it is currently set to
// apply a CSS visibility value. If this is non-performant, this should be customized.
export const DocumentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [document, setDocument] = useState<AJDocument | undefined>()
  const [visible, setVisible] = useState<boolean>(false)

  useNuiEvent<string>('setDocument', (data) => {
    setVisible(!!data)
    setDocument(data ? JSON.parse(data) as AJDocument : undefined)
  })
