...
type Props = {
  ...
  handleCreate: (data: AJDocument) => void
  handleClose: () => void
  logoSrc: string
  isCitizen?: boolean
}
...
const CreateDocument = ({ template, handleCreate, handleClose, logoSrc, isCitizen }: Props) => {
  ...
  const { register, handleSubmit, formState: { errors }, control } = useForm<AJDocument>({
    defaultValues: {
  ...
  const handleCreateDocument: SubmitHandler<AJDocument> = (data: AJDocument) => {
