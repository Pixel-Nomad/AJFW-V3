...
function DamageRandomComponent()
	...
	local randomDamage
	local armored = GetVehicleMod(vehicle, 16)
	if armored == -1 then
		randomDamage = math.random(1,5)
	elseif armored == 0 then
		randomDamage = math.random(1,5) / 2
	elseif armored == 1 then
		randomDamage = math.random(1,5) / 4
	elseif armored == 2 then
		randomDamage = math.random(1,5) / 6
	elseif armored == 3 then
		randomDamage = math.random(1,5) / 8
	elseif armored == 4 then
		randomDamage = math.random(1,5) / 10
	elseif armored == 5 then
		randomDamage = math.random(1,5) / 12
	elseif armored == 6 then
		randomDamage = math.random(1,5) / 14
	elseif armored == 7 then
		randomDamage = math.random(1,5) / 16
	elseif armored == 8 then
		randomDamage = math.random(1,5) / 18
	elseif armored == 9 then
		randomDamage = math.random(1,5) / 20
	end
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicle), randomComponent, exports['aj-mechanicjob']:GetVehicleStatus(AJFW.Functions.GetPlate(vehicle), randomComponent) - randomDamage)
end

function DamageRandomComponentBody()
	local dmgFctr = math.random() + math.random(0, 2)
	local randomComponent = DamageComponentsBody[math.random(1, #DamageComponentsBody)]
...
local function CleanVehicle(veh)
    ...
    AJFW.Functions.Progressbar("cleaning_vehicle", Lang:t("progress.clean_veh"), math.random(10000, 20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        AJFW.Functions.Notify(Lang:t("success.cleaned_veh"))
        SetVehicleDirtLevel(veh, 0.1)
        SetVehicleUndriveable(veh, false)
        WashDecalsFromVehicle(veh, 1.0)
        TriggerServerEvent('aj-vehiclefailure:server:removewashingkit', veh)
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items["cleaningkit"], "remove")
        ClearPedTasks(ped)
    end, function() -- Cancel
        AJFW.Functions.Notify(Lang:t("error.failed_notification"), "error")
        ClearPedTasks(ped)
    end)
end
...
local function RepairVehicleFull(veh)
    ...
    AJFW.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(20000, 30000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        AJFW.Functions.Notify(Lang:t("success.repaired_veh"))
        SetVehicleEngineHealth(veh, 1000.0)
        SetVehicleEngineOn(veh, true, false)
        for i = 0, 5 do
            SetVehicleTyreFixed(veh, i)
            TriggerEvent('aj-vehiclefailure:client:TyreSync', veh, i)
        end
        if (IsBackEngine(GetEntityModel(veh))) then
        ...
        TriggerServerEvent('aj-vehiclefailure:removeItem', "advancedrepairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        AJFW.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
local function RepairVehicle(veh)
    ...
    AJFW.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(10000, 20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent('aj-vehiclefailure:removeItem', "repairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        AJFW.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
RegisterNetEvent('aj-vehiclefailure:client:RepairVehicle', function()
    local veh = AJFW.Functions.GetClosestVehicle()
    ...
    if veh ~= nil and veh ~= 0 and engineHealth < 500 then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            if #(pos - vehpos) > 4.9 then
                AJFW.Functions.Notify(Lang:t("error.out_range_veh"), "error")
            else
                AJFW.Functions.Notify(Lang:t("error.inside_veh"), "error")
            end
        end
    else
        if veh == nil or veh == 0 then
            AJFW.Functions.Notify(Lang:t("error.not_near_veh"), "error")
        else
            AJFW.Functions.Notify(Lang:t("error.healthy_veh"), "error")
        end
    end
end)

RegisterNetEvent('aj-vehiclefailure:client:SyncWash', function(veh)
    SetVehicleDirtLevel(veh, 0.1)
    SetVehicleUndriveable(veh, false)
    WashDecalsFromVehicle(veh, 1.0)
end)

RegisterNetEvent('aj-vehiclefailure:client:CleanVehicle', function()
    local veh = AJFW.Functions.GetClosestVehicle()
...
RegisterNetEvent('aj-vehiclefailure:client:RepairVehicleFull', function()
    local veh = AJFW.Functions.GetClosestVehicle()
    if veh ~= nil and veh ~= 0 then
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
...
RegisterNetEvent('aj-vehiclefailure:client:TyreSync', function(veh, tyre)
...
RegisterNetEvent('iens:repaira', function()
    ...
    AJFW.Functions.Notify(Lang:t("error.inside_veh_req"))
end)

RegisterNetEvent('iens:besked', function()
    AJFW.Functions.Notify(Lang:t("error.roadside_avail"))
end)

RegisterNetEvent('iens:notAllowed', function()
    AJFW.Functions.Notify(Lang:t("error.no_permission"))
end)
    ...
    if isPedDrivingAVehicle() then
        ...
        if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
            if GetVehicleOilLevel(vehicle) > 0 then
                ...
                AJFW.Functions.Notify(Lang:t(('fix_message_%s'):format(fixMessagePos)))
                fixMessagePos = fixMessagePos + 1
                if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
            else
                AJFW.Functions.Notify(Lang:t("error.veh_damaged"))
            end
        else
            AJFW.Functions.Notify(Lang:t(('nofix_message_%s'):format(noFixMessagePos)))
                    ...
                    if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
                        ...
                        if speed >= 1.0 then
                            ...
                            if brake > 127 then
                                ...
                                --exports['aj-vehicletuning']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicle), "brakes", exports['aj-vehicletuning']:GetVehicleStatus(AJFW.Functions.GetPlate(vehicle), "brakes") - 0.01)
								exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicle), "brakes", exports['aj-mechanicjob']:GetVehicleStatus(AJFW.Functions.GetPlate(vehicle), "brakes") - 0.01)
                            end
