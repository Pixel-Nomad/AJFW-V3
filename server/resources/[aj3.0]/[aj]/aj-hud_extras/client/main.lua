...
local function GetBuffs()
    ...
    Wait(500)
    local p = promise.new()
    AJFW.Functions.TriggerCallback('buffs:server:fetchBuffs', function(result)
        p:resolve(result)
    end)
...
local function AddBuff(buffName, time)
    local p = promise.new()
    AJFW.Functions.TriggerCallback('buffs:server:addBuff', function(result)
        p:resolve(result)
    end, buffName, time)
    return Citizen.Await(p)
...
local function StaminaBuffEffect(time, value)
    ...
    if not hasStaminaBuffActive then 
        ...
        CreateThread(function()
            ...
            while exports['aj-hud_extras']:HasBuff("stamina") do
                Wait(500)
...
local function SwimmingBuffEffect(time, value)
    ...
    if not hasSwimmingBuffActive then 
        ...
        CreateThread(function()
            ...
            while exports['aj-hud_extras']:HasBuff("swimming") do
                Wait(500)
                SetPlayerStamina(PlayerId(), GetPlayerStamina(PlayerId()) + math.random(1,10))
            end
