...
function generateDamageColor(percent) {
    // Ensure percent is within 0 to 100 range
...
$(document).on("mousedown", ".item-slot", function (event) {
    switch (event.which) {
        case 3:
            ...
            toSlot = playerslot ? toSlot + 5 : toSlot;
            if ($(this).data("item") === undefined) {
                return;
            }
            toAmount2 = $(this).data("item").amount;
            toAmount = $("#item-amount").val();
            if (toAmount2 < toAmount || toAmount <= 0) {
                toAmount = toAmount2
            }
            if (CanQuickMove()) {
                if (toSlot === null) {
...
$(document).on("click", ".item-slot", function(e){
    ...
    if ($(this).data("item") != null) {
        ...
        var elem = document.querySelector('.ply-iteminfo-container');
        var bounding = elem.getBoundingClientRect();
        if (bounding.top <= 10){
            element.css('top', ((itemOffset.top - element.height())- bounding.top)+10);
        }
        ...
        const ItemDataFrom = $(this).parent().attr("data-inventory")
        ClickedItemDataFrom = ''
        ClickedItemData = {};
        $(".attachments").hide();
        $(".btncustom").hide();
        $(".UItem").hide();
        if (ItemDataFrom !== null && ItemDataFrom !== undefined && ItemDataFrom === 'player'){
            if (ItemData !== null && ItemData !== undefined) {
                if (ItemData.name !== undefined) {
                    ClickedItemDataFrom = ItemDataFrom
...
$(document).on("click", '#item-use', function(){
    if (ClickedItemData.useable){
        if (ClickedItemData.shouldClose) {
            Inventory.Close();
        }
        $.post(
            "https://aj-inventory/UseItem",
            JSON.stringify({
                inventory: ClickedItemDataFrom,
...
$(document).on("click", '#item-give', function(){
    amount = $("#item-amount").val();
    if (amount == 0) {
        amount = ClickedItemData.amount;
    }
    $.post(
...
$(document).on("click", "#item-amount", function(e){
    $(".ply-iteminfo-container").fadeOut(100);
})
...
$(document).on("click", ".weapon-attachments-back", function (e) {
    e.preventDefault();
    $("#ajfw-inventory").css({ display: "block" });
    $("#ajfw-inventory").animate({
        left: 0 + "vw",
    },
        200
    );
    ...
    $.post(
        ...
        function (data) {
            ...
            $(".weapon-attachments-container-image").attr(
                "src",
                Config.imageResource + data.WeaponData.name + ".png"
            );
                ...
                if (data.AttachmentData.length > 0) {
                    ...
                    $.each(data.AttachmentData, function (i, attachment) {
                        ...
                        $(".weapon-attachments").append(
                            ...
                            attachment.label +
                            '</p></div> <div class="weapon-attachment-img"><img src="'+Config.imageResource +
...
$(document).on("click", "#item-attachments", function (e) {
    e.preventDefault();
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#ajfw-inventory").animate({
            ...
            200,
            function () {
                $("#ajfw-inventory").css({ display: "none" });
            }
        );
        $(".weapon-attachments-container").animate({
            left: 0 + "vw",
...
function handleDragDrop() {
    $(".item-drag").draggable({
        ...
        start: function (event, ui) {
            ...
            $(".item-slot").css("border", "0.3vh solid rgba(0, 238, 255, 0.1)");
        ...
        stop: function () {
            ...
            $(".item-slot").css("border", "0vh solid rgba(0, 238, 255, 0.0)");
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    //     }
    // }
    // fromData2 = $fromInv.find("[data-slot=" + $fromSlot + "]").data("item");
    // toData2 = $toInv.find("[data-slot=" + $toSlot + "]").data("item");
    // if (fromData2 !== undefined && toData2 !== undefined) {
    //     if (fromData2.decay !== undefined && toData2.decay === undefined ){
    //         InventoryError($fromInv, $fromSlot);
    //         return false;
    //     }
    //     if (fromData2.decay === undefined && toData2.decay !== undefined ){
    ...
    if (
        ...
        if ($fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
            $fromInv
                ...
                .html(
                    '<div class="item-slot-img"><img src="'+Config.imageResource +
                    itemData.image +
                    '" alt="' +
                    itemData.name +
    ...
    if ($toSlot < 6) {
        $toInv
            ...
            .html(
                ...
                $toSlot +
                '</p></div><div class="item-slot-img"><img src="'+Config.imageResource +
                fromData.image +
                '" alt="' +
                fromData.name +
                '" /></div><div class="item-slot-amount"><p>' +
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    toData = $toInv.find("[data-slot=" + $toSlot + "]").data("item");
    var otherinventory = otherLabel.toLowerCase();
    // if (otherinventory.split("-")[0] == "itemshop" || $fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
    //     // if (toData !== null && toData !== undefined) {
    //         InventoryError($fromInv, $fromSlot);
    //         return;
    //     // }
    ...
    //     if (fromData.decay !== undefined && toData.decay === undefined ){
    //         InventoryError($fromInv, $fromSlot);
    //         return;
    //     }
    //     if (fromData.decay === undefined && toData.decay !== undefined ){
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            if (fromData.amount == $toAmount) {
                ...
                ItemLabel =
                    '<div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div>'
                // }
                // }
                ...
                } else if ($toSlot == Config.sixthPocketSlot && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        .find("[data-slot=" + $toSlot + "]")
                        .html(
                            '<div class="item-slot-key"><p>6 </i></p></div><div class="item-slot-img"><img src="'+Config.imageResource +
                            newData.image +
                            '" alt="' +
                            newData.name +
                            '" /></div><div class="item-slot-amount"><p>' +
                ...
                var QualityColor = generateDamageColor(newData.info.quality);
                ...
                if (newData.decay || newData.type == 'weapon') {
                    $toInv
                    .find("[data-slot=" + $toSlot + "]")
                ...
                if ($fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
                    ...
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        ...
                        $fromSlot == Config.sixthPocketSlot &&
                        $fromInv.attr("data-inventory") == "player"
                    ) {
                        $fromInv
                            .find("[data-slot=" + $fromSlot + "]")
                    ...
                    var QualityColor = generateDamageColor(newDataFrom.info.quality);
                    if (fromData.info.quality !== undefined) {
                    ...
                    if (newDataFrom.decay || newDataFrom.type == 'weapon') {
                        $fromInv
                        .find("[data-slot=" + $fromSlot + "]")
                        .find(".item-slot-quality-bar")
                        .css({
            ...
            if (fromData.amount == $toAmount) {
                ...
                var QualityColor = generateDamageColor(fromData.info.quality);
                if (fromData.info.quality !== undefined) {
                ...
                if (fromData.decay || fromData.type == 'weapon') {
                    $toInv
                    .find("[data-slot=" + $toSlot + "]")
                    .find(".item-slot-quality-bar")
                    .css({
                ...
                if (toData != undefined) {
                    ...
                    var QualityColor = generateDamageColor(toData.info.quality);
                    if (toData.info.quality !== undefined) {
                    ...
                    if (toData.decay || toData.type == 'weapon') {
                        $fromInv
                        .find("[data-slot=" + $fromSlot + "]")
                        .find(".item-slot-quality-bar")
                        .css({
                    ...
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        ...
                        $fromInv
                            .find("[data-slot=" + $fromSlot + "]")
                            .html(
                                '<div class="item-slot-key"><p>6 </i></p></div><div class="item-slot-img"></div><div class="item-slot-label"><p>&nbsp;</p></div>'
                            );
                    } else {
                        $fromInv
                            .find("[data-slot=" + $fromSlot + "]")
                ...
                var QualityColor = generateDamageColor(newDataTo.info.quality);
                if (newDataTo.info.quality !== undefined) {
                ...
                if (newDataTo.decay || newDataTo.type == 'weapon') {
                    $toInv
                    .find("[data-slot=" + $toSlot + "]")
                    .find(".item-slot-quality-bar")
                    .css({
...
(() => {
    ...
    Inventory.QualityCheck = function (item, IsHotbar, IsOtherInventory) {
        ...
        var QualityColor = generateDamageColor(item.info.quality);
        if (item.info.quality !== undefined) {
        ...
        if (item.info.quality <= 0) {
            qualityLabel = "BROKEN";
            if (item.decay || item.type == 'weapon') {
                if (!IsOtherInventory) {
                    if (!IsHotbar) {
                        $(".player-inventory")
    ...
    Inventory.QualityCheck2 = function (item, IsHotbar, IsOtherInventory) {
        // if (!Inventory.IsWeaponBlocked(item.name)) {
        // if (item.name.split("_")[0] == "weapon") {
        if (item.info.quality == undefined) {
            item.info.quality = 100;
    ...
    Inventory.Open = function (data) {
        totalWeight = 0;
        totalWeightOther = 0;
        $("#player-inv-label").html(data.PlayerName+" ("+data.ServerID+")");
        $("#citizenid").html(data.CitizenID);
        $(".player-inventory").find(".item-slot").remove();
        $(".side-inventory").find(".item-slot").remove();
        ...
        for (i = 6; i < data.slots + 1; i++) {
            if (i == Config.sixthPocketSlot) {
                $(".player-inventory").append(
                    '<div class="item-slot" data-slot="' +
                    i +
                    '"><div class="item-slot-key"><p>6 </i></p></div><div class="item-slot-img"></div><div class="item-slot-label"><p>&nbsp;</p></div></div>'
                );
            } else {
                $(".player-inventory").append(
                    '<div class="item-slot" data-slot="' +
                    ...
                    if (item.slot < 6) {
                        ...
                        $(".side-inventory")
                            ...
                            .html(
                                ...
                                '" /></div><div class="item-slot-amount"><p>' +
                                item.amount +
                                '</div><div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div>'
                            );
                        $(".side-inventory")
                            .find("[data-slot=" + item.slot + "]")
                            .data("item", item);
                    } else if (item.slot == Config.sixthPocketSlot) {
                        $(".player-inventory")
                            .find("[data-slot=" + item.slot + "]")
                            .addClass("item-drag");
                        $(".player-inventory")
    ...
    Inventory.UpdateOther = function (data) {
        $.each(data.inventory, function (i, item) {
            if (item != null) {
                Inventory.QualityCheck2(item, false, true);
            }
        })
    }
    ...
    Inventory.Update = function (data) {
        ...
        for (i = 1; i < 6; i++) {
            $(".side-inventory").append(
                '<div class="item-slot" data-slot="' +
                i +
                '"><div class="item-slot-key"><p>' + i + '</p></div><div class="item-slot-img"></div><div class="item-slot-label"><p>&nbsp;</p></div></div>'
            );
        }
        for (i = 6; i < data.slots + 1; i++) {
            if (i == Config.sixthPocketSlot) {
        ...
        $.each(data.inventory, function (i, item) {
            if (item != null) {
                ...
                Inventory.QualityCheck2(item, false, false);
            }
        });
    ...
    Inventory.ToggleHotbar = function (data) {
        if (data.open) {
            ...
            var elem =
                '<div class="z-hotbar-item-slot" data-zhotbarslot="'+Config.sixthPocketSlot+'"> <div class="z-hotbar-item-slot-key"><p>6 </i></p></div><div class="z-hotbar-item-slot-img"></div><div class="z-hotbar-item-slot-label"><p>&nbsp;</p></div></div>';
            $(".z-hotbar-inventory").append(elem);
            $.each(data.items, function (i, item) {
                if (item != null) {
                    ...
                    // }
                    // }
                    if (item.slot == Config.sixthPocketSlot) {
                        $(".z-hotbar-inventory")
                            .find("[data-zhotbarslot=" + item.slot + "]")
                            .html(
                                '<div class="z-hotbar-item-slot-key"><p>6 </i></p></div><div class="z-hotbar-item-slot-img"><img src="'+Config.imageResource +
                                item.image +
                                '" alt="' +
                                item.name +
                        ...
                        $(".z-hotbar-inventory")
                            ...
                            .html(
                                '<div class="z-hotbar-item-slot-key"><p>' +
                                item.slot +
                                '</p></div><div class="z-hotbar-item-slot-img"><img src="'+Config.imageResource +
                                item.image +
                                '" alt="' +
                                item.name +
                                '" /></div><div class="z-hotbar-item-slot-amount"><p>' +
    ...
    Inventory.itemBox = function (data) {
        ...
        $itembox.html(
            ...
            type +
            '</p></div><div class="item-slot-img-itembox"><img src="'+Config.imageResource +
            data.item.image +
            '" alt="' +
            data.item.name +
            '" /></div>'
            ...
            if (!requiredItemOpen) {
                ...
                $.each(data.items, function (index, item) {
                    var element =
                        ...
                        item.label +
                        '</p></div><div id="requiredItem-image"><div class="item-slot-img"><img src="'+Config.imageResource +
                        item.image +
                        '" alt="' +
                        item.name +
                        '" /></div></div></div>';
    ...
    //         $.post("http://aj-inventory/GiveItem",
    //             JSON.stringify({
        ...
        window.addEventListener("message", function (event) {
            switch (event.data.action) {
                ...
                case "update":
                    ...
                    break;
                case "updateother":
                    Inventory.UpdateOther(event.data);
                    break;
                case "itemBox":
