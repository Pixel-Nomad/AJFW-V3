...
$(document).on('click', '#IconClassStyle', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('https://aj-spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
        ...
        if(SelectLocForSpawn == null){
            ...
            if(MainspawnType == "appartment"){
                $(".TextFoJSCode").html("Click on Spawn To Start a new journey")
            }else{
                $(".TextFoJSCode").html("Select a Location")
            }
...
$(document).on('click', '.GreenBTN', function(evt){
    ...
    if (Mainlocation !== null){
        ...
        if (MainspawnType == "appartment2") {
            $.post('https://aj-spawn/spawnplayerappartment2', JSON.stringify({
                spawnloc: Mainlocation,
                apartName: apartNames,
            }));
        } else if(MainspawnType !== "appartment"){
            $.post('https://aj-spawn/spawnplayer', JSON.stringify({
                spawnloc: Mainlocation,
                typeLoc: MainspawnType
            }));
        }else {
            $.post('https://aj-spawn/chooseAppa', JSON.stringify({
...
function setupApps(apps) {
    ...
    $(".TextFoJSCode").html("Click on Spawn To Start a new journey")
    Mainlocation = "apartment1"
    MainspawnType = "appartment"
    if(SelectLocForSpawn !== null){
        $(SelectLocForSpawn).removeClass("selected");
        SelectLocForSpawn = null
    }
...
function CloseAddCoord() {
    $.post('https://aj-spawn/CloseAddCoord', JSON.stringify({}));
