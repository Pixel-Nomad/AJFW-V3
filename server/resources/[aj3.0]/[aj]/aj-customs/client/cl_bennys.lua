...
local function saveVehicle()
    ...
    local myCar = AJFW.Functions.GetVehicleProperties(veh)
    TriggerServerEvent('aj-customs:server:updateVehicle', myCar)
end
...
local function AllowCID(restrictionData, CID)
    if type(restrictionData.CID) == "table" then
        for _, restrictedCID in ipairs(restrictionData.CID) do
            if restrictedCID == CID then return true end
        end
    else
        if restrictionData.CID == "any" or restrictionData.CID == CID or not restrictionData.CID then return true end
    end
    if Config.Debug then print('Denied for not having allowed CID. (' .. CID .. ')') end
    return false
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("aj-customs:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    AJFW.Functions.TriggerCallback('aj-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'mechanic' then
            ...
            AJFW.Functions.Notify('Some purchases are currently unavailable. Please find a mechanic.')
        end
        ...
        if not canEnter then
            AJFW.Functions.Notify('You cant do anything here!')
            ExitBennys()
            return
        end

        if Config.UseRadial and radialMenuItemId then
            exports['aj-radialmenu']:RemoveOption(radialMenuItemId)
...
function GetLocations()
    AJFW.Functions.TriggerCallback("aj-customs:server:GetLocations", function(locations)
...
function CheckForGhostVehicle()
    ...
    local closestVehicle, closestDistance = AJFW.Functions.GetClosestVehicle(GetEntityCoords(PlayerPedId()))
...
function CheckRestrictions(location)
    ...
    local allowedCID  = AllowCID(restrictions, PlayerData.citizenid)
        ...
        if PlayerData.job.name ~= 'mechanic' and Config.DisableWhenMechanicsOnline and currentMechanics >= Config.MinOnlineMechanics then
            ...
            if Config.UseRadial then
                if not radialMenuItemId then
                    radialMenuItemId = exports['aj-radialmenu']:AddOption({
                        id = 'customs',
                        title = 'Enter Customs',
                        icon = 'wrench',
                        type = 'client',
                        event = 'aj-customs:client:EnterCustoms',
                        shouldClose = true
                    })
                end
            else
...
RegisterNetEvent('aj-customs:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("aj-customs:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    AJFW.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("aj-customs:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    AJFW.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('aj-customs:client:EnterCustoms', function(override)
