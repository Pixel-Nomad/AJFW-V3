...
RegisterNUICallback("appearance_get_locales", function(_, cb)
    cb(Locales[GetConvar("aj-clothMenu:locale", "en")].UI)
end)
...
RegisterNUICallback("appearance_apply_tattoo", function(data, cb)
    local paid = not data.tattoo or not Config.ChargePerTattoo or lib.callback.await("aj-clothMenu:server:payForTattoo", false, data.tattoo)
    if paid then
