...
function Framework.HasTracker()
    return AJFW.Functions.GetPlayerData().metadata["tracker"]
end
...
function Framework.GetRankInputValues(type)
    local grades = AJFW.Shared.Jobs[client.job.name].grades
    if type == "gang" then
        grades = AJFW.Shared.Gangs[client.gang.name].grades
    end
