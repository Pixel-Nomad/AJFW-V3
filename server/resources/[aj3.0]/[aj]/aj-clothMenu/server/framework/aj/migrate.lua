...
local function MigrateAJClothing(source)
    ...
    for i=1, #allPlayerSkins, 1 do
        if not tonumber(allPlayerSkins[i].model) then
            ...
            TriggerClientEvent("aj-clothMenu:client:migration:load-aj-clothing-skin", source, allPlayerSkins[i])
...
RegisterNetEvent("aj-clothMenu:server:migrate-aj-clothing-skin", function(citizenid, appearance)
    local src = source
...
lib.addCommand("migrateskins", {
    ...
    elseif resourceName == "aj-clothing" then
        CreateThread(function()
            MigrateAJClothing(source)
        end)
    else
        lib.notify(source, {
