...
<section style="border: 1px solid #ff005e; box-shadow: 0.2vh 0.2vh 0.9vh #ff0000; display: {$MenuStore.show ? 'flex' : 'none'};" class="w-[60vw] h-[60vh] flex-col bg-[#4d0000] rounded-t-2xl shadow-lg text-white"
  ...
  <div class="flex font-semibold" style="height: calc(100% - 24px);">
    <div class="flex flex-col w-1/6 bg-[#700000]">
      <!-- Tab item List -->
      {#each tabArray as tab, i}
        {#if !tab.adminOnly || !$MenuStore.adminOnly || ($MenuStore.adminOnly && $MenuStore.isAdmin) }
          <div class="bg-[#4d0000] px-4 py-4 flex flex-row gap-3 cursor-pointer select-none" on:click={() => handleTabClick(i)}>
    ...
    {#each tabArray as tab}
      {#if !tab.adminOnly || !$MenuStore.adminOnly || ($MenuStore.adminOnly && $MenuStore.isAdmin) }
        <div style="display: {activeTab.name == tab.name ? 'flex' : 'none'};" class="flex-col w-5/6 px-5 overflow-y-scroll bg-[#4d0000]">
