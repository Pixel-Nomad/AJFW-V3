...
local injail = 0
local maxjail = 0
local crimeactive = false
local crimecooldown = false
local dev = false
local admin = false
...
local Menu = {
    ...
    isDynamicdriveModeChecked = true, -- isDynamicEngineChecked
    isDynamicnitroModeChecked = true, -- isDynamicEngineChecked
    isDynamicPurgeModeChecked = true, -- isDynamicEngineChecked
    isDynamicpursuitmodeChecked = true,
    isDynamicinjailChecked = true,
    isDynamiccrimeactiveChecked = true,
    isDynamiccrimecooldownChecked = true,
...
local function loadSettings()
    AJFW.Functions.Notify(Lang:t("notify.hud_settings_loaded"), "success")
    Wait(1000)
...
local function HandleSetupResource()
    AJFW.Functions.TriggerCallback('hud:server:getRank', function(isAdminOrGreater)
...
RegisterNetEvent("AJFW:Client:OnPlayerLoaded", function()
    ...
    if not Establishment then
        Establishment = true
        SportsModeEstablished()
    end
end)
...
local function restartHud()
    ...
    Wait(500)
    AJFW.Functions.Notify(Lang:t("notify.hud_start"), "success")
    SendNUIMessage({
        action = 'menu',
        topic = 'restart',
    })
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    AJFW.Functions.TriggerCallback('hud:server:getMenu', function(menu) loadSettings(menu); SetResourceKvp('hudSettings', json.encode(menu)) end)
end)
...
RegisterNetEvent("hud:client:LoadSpeed", function()
    if Menu.isToggleSpeedChecked == "mph" then
        config.UseMPH = true
        speedMultiplier = config.UseMPH and 2.23694 or 3.6
        AJFW.Functions.Notify('MPH Mode Activated', 'primary', 5000, 'left')
    elseif Menu.isToggleSpeedChecked == "kmh" then 
        config.UseMPH = false
        speedMultiplier = config.UseMPH and 2.23694 or 3.6
        AJFW.Functions.Notify('KMH Mode Activated', 'primary', 5000, 'left')
    end
    Wait(50)
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            AJFW.Functions.Notify(Lang:t("notify.load_square_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            AJFW.Functions.Notify(Lang:t("notify.loaded_square_map"))
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            AJFW.Functions.Notify(Lang:t("notify.load_circle_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            AJFW.Functions.Notify(Lang:t("notify.loaded_circle_map"))
        end
    end
end)
...
RegisterNUICallback('ToggleSpeed', function(data, cb)
    cb({})
    Wait(50)
...
RegisterNetEvent("hud:client:driveModeHealth", function(newdriveMode)
    driveMode = newdriveMode
end)

RegisterNetEvent("hud:client:pursuitmode", function(newpursuitmode)
    pursuitmode = newpursuitmode
end)

RegisterNetEvent("hud:client:nitroMode", function(newnitroMode, newNitroLevel)
    nitroMode = newnitroMode
    nitroLevel = newNitroLevel
end)

RegisterNetEvent("hud:client:PurgeMode", function(a, b)
    PurgeMode = a
    PurgeLevel = b
end)
...
--         AJFW.Functions.Notify(Lang:t("notify.engine_off"), 'red')
--     else
--         AJFW.Functions.Notify(Lang:t("notify.engine_on"), 'green')
--     end
...
local function NotSportsPursuitf(vehicle)
	local model = GetEntityModel(vehicle)
    local veh = true
    for i = 1, #NotSportsPursuit do
        if model == NotSportsPursuit[i] then
			veh = false
		end
    end
	return veh
...
CreateThread(function()
    ...
    while true do        
        if LocalPlayer.state.isLoggedIn then
            ...
            playerDead = IsEntityDead(player) or (PlayerData and PlayerData.metadata and PlayerData.metadata["inlaststand"]) or (PlayerData and PlayerData.metadata and  PlayerData.metadata["isdead"]) or false

            if PlayerData then
                if PlayerData.metadata then
                    injail = PlayerData.metadata["injail"]
                    maxjail = PlayerData.metadata["maxjail"]
                else
                    injail = 0
                    maxjail = 0
                end
            ...
            local vehicle = GetVehiclePedIsIn(player)
            local sendPursuit
            if NotSportsPursuitf(vehicle)then
                sendPursuit = 0
            else
                sendPursuit = pursuitmode
            end
            ...
            if IsPedInAnyVehicle(player) and not IsThisModelABicycle(vehicle) then
                ...
                if wasInVehicle then
                    wasInVehicle = false
                    mode = 'normal'
                    TriggerEvent('update:fuel:mode', 'normal')
                    TriggerEvent('hud:client:driveModeHealth', 50)
...
function SportsModeEstablished()
    CreateThread(function()
        while true do 
            sleep = 1500
            if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports[Config.FuelScript]:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        AJFW.Functions.Notify(Lang:t("notify.low_fuel"), "error")
...
local function CheckForVehiclesEV()
...
RegisterNetEvent('aj-vehicles:ForceRemove:Mode', function()
    mode = 'normal'
    TriggerEvent('update:fuel:mode', 'normal')
    TriggerEvent('hud:client:driveModeHealth', 50)
end)
...
function IsSportsMode()
    ...
    return false
end exports('IsSportsMode', IsSportsMode)

exports['aj-framework']:CreateBind('speedometerr', nil, 'Vehicle: Vehicle Mode Reverse', 'F6', function()
	if CheckForVehiclesEV() then
		local ped = PlayerPedId()
		if GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped and not IsPedInAnyPlane(ped) and not IsPedInAnyHeli(ped) and not IsPedInAnyBoat(ped) then
			if mode == 'normal' then
                mode = 'eco'
                TriggerEvent('update:fuel:mode', mode)
                TriggerEvent('hud:client:driveModeHealth', 100)
            elseif mode == 'eco' then
                mode = 'sports'
                if exports['aj-tunerchip']:GetTune(AJFW.Functions.GetPlate(GetVehiclePedIsIn(ped))) then
                    ...
                    AJFW.Functions.Notify('for engine Safety Purpose Switching to normal mode', 'error', 5000, 'left')
                    mode = 'normal'
                    TriggerEvent('update:fuel:mode', mode)
                    TriggerEvent('hud:client:driveModeHealth', 50)
                else
                    TriggerEvent('update:fuel:mode', mode)
                    TriggerEvent('hud:client:driveModeHealth', 25)
                end
...
exports['aj-framework']:CreateBind('speedometer', nil, 'Vehicle: Vehicle Mode', 'F7', function()
	if CheckForVehiclesEV() then
		local ped = PlayerPedId()
		if GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped and not IsPedInAnyPlane(ped) and not IsPedInAnyHeli(ped) and not IsPedInAnyBoat(ped) then
			if mode == 'normal' then
                if exports['aj-tunerchip']:GetTune(AJFW.Functions.GetPlate(GetVehiclePedIsIn(ped))) then
                    mode = 'eco'
                    AJFW.Functions.Notify('Vehicle is Tuned Above 50%', 'error', 5000, 'left')
                    AJFW.Functions.Notify('for engine Safety Purpose Switching to eco mode', 'error', 5000, 'left')
                    TriggerEvent('update:fuel:mode', mode)
                    TriggerEvent('hud:client:driveModeHealth', 100)
                else
                    TriggerEvent('update:fuel:mode', mode)
