...
## General Setup
General setup is quite easy, delete your OLD aj-phone
if you server havent been running previously then go ahead and run the provided SQL file in your database.

If your server has been running aj-phone previously please update your sql while being carefull and take a backup so you have no data lost.
...
1. Head over to aj-phone/server/employment.lua and change local FirstStart from false to true like shown below

```lua
    local FirstStart = true
```

2. Start the script and make sure it's fully done, it can take a while depending on your current playerbase (ensure aj-phone in console or f8)

3. Head over to aj-phone/server/employment.lua again and change the FirstStart to false
...
4. Headover to your aj-base/server/commands.lua and find the follow command 'setjob'

replace the commands with the code below:
```lua
AJFW.Commands.Add('setjob', 'Set A Players Job (Admin Only)', { { name = 'id', help = 'Player ID' }, { name = 'job', help = 'Job name' }, { name = 'grade', help = 'Grade' } }, true, function(source, args)
    ...
    if Player then
        ...
        local jobInfo = AJFW.Shared.Jobs[job]
        if jobInfo then
            if jobInfo["grades"][sgrade] then
                Player.Functions.SetJob(job, grade)
                exports['aj-phone']:hireUser(job, Player.PlayerData.citizenid, grade)
            else
                TriggerClientEvent('AJFW:Notify', source, "Not a valid grade", 'error')
            end
        else
            TriggerClientEvent('AJFW:Notify', source, "Not a valid job", 'error')
        end
    else
...
```lua
AJFW.Commands.Add('removejob', 'Removes A Players Job (Admin Only)', { { name = 'id', help = 'Player ID' }, { name = 'job', help = 'Job name' } }, true, function(source, args)
    local Player = AJFW.Functions.GetPlayer(tonumber(args[1]))
    if Player then
        if Player.PlayerData.job.name == tostring(args[2]) then
            Player.Functions.SetJob("unemployed", 0)
        end
        exports['aj-phone']:fireUser(tostring(args[2]), Player.PlayerData.citizenid)
    else
        TriggerClientEvent('AJFW:Notify', source, Lang:t('error.not_online'), 'error')
    end
end, 'admin')
...
6. If you use aj-cityhall then u have to Find ApplyJob in aj-cityhall/server/main.lua and replace with this one

```lua
RegisterNetEvent('aj-cityhall:server:ApplyJob', function(job, cityhallCoords)
    local src = source
    ...
    exports['aj-phone']:hireUser(job, Player.PlayerData.citizenid, 0)
TriggerClientEvent('AJFW:Notify', src, Lang:t('info.new_job', {job = JobInfo.label}))
end)
```
...
![AJFW Commands](https://i.gyazo.com/beb2bd18c02088c184e5e381a9f4962a.png)


## Casino Setup

1. Head over to your aj-base/config.lua
...
1. Head over to your aj-base/server/Player.lua
