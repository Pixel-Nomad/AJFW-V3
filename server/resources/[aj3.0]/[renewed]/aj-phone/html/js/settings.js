QB.Phone.Settings = {};
QB.Phone.Settings.Background = "default-AJFW";
...
QB.Phone.Functions.IsBackgroundCustom = function() {
    var retval = true;
    if (QB.Phone.Settings.Background == 'default-AJFW'){
        retval = false;
    }
    return retval
}
...
$(document).on('click', '#settings-app-submit', function(e){
    ...
    if (checkbox){
        ...
        $(".phone-background").css({"background-image":"url('/html/img/backgrounds/default-AJFW.png')"})
        $.post('https://aj-phone/SetBackground', JSON.stringify({
            background: '/html/img/backgrounds/default-AJFW.png',
        }))
    }else if (customurl){
