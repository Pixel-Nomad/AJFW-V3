...
function LoadJobCenterApp(){
    $.post('https://aj-phone/GetGroupsApp', JSON.stringify({}), function(data){
        AddDIV(data)
    });
}
...
$(document).on('click', '#jobcenter-delete-group', function(e){
    ...
    $.post('https://aj-phone/jobcenter_DeleteGroup', JSON.stringify({
        delete: Delete,
    }));
});
...
$(document).on('click', '#jobcenter-list-group', function(e){
    e.preventDefault();
    var id = $(this).data('id')
    $.post('https://aj-phone/jobcenter_CheckPlayerNames', JSON.stringify({
        id: id,
        }), function(Data){
           ClearInputNew()
