                ...
                if (PressedApplication == "twitter") {
                    if (AJ.Phone.Data.IsOpen) {
                        $.post('https://aj-phone/GetTweets', JSON.stringify({}), function(Tweets){
                            AJ.Phone.Notifications.LoadTweets(Tweets.TweetData, Tweets.hasVPN);
                        });
                    }
                    ...
                    $.post('https://aj-phone/GetJobs', JSON.stringify({}), function(data){
                    ...
                    $.post('https://aj-phone/GetChatRooms', JSON.stringify({}), function(ChatRooms){
                        AJ.Phone.Functions.HeaderTextColor("white", 100);
...
$(document).on('click', '.phone-silent-button', function(event){
    event.preventDefault();
    $.post('https://aj-phone/phone-silent-button', JSON.stringify({}),function(Data){
        if(Data){
...
AJ.Phone.Functions.Open = function(data) {
    AJ.Phone.Animations.BottomSlideUp('.container', 500, -6.6);
    AJ.Phone.Notifications.LoadTweets(data.Tweets, data.hasVPN);
...
AJ.Phone.Notifications.Custom.Add = function(icon, title, text, color, timeout, accept, deny) {
    $.post('https://aj-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
            ...
            if (!AJ.Phone.Data.IsOpen == true) {
                AJ.Phone.Animations.BottomSlideUp('.container', 150, -55);
            }

            AJ.Phone.Animations.TopSlideDown(".phone-notification-container-new", 600, 6);
            ...
            if (timeout != "NONE"){
                ...
                AJ.Phone.Notifications.Timeout = setTimeout(function(){
                    AJ.Phone.Animations.TopSlideUp(".phone-notification-container-new", 150, -8);
                    AJ.Phone.Notifications.Timeout = setTimeout(function(){
                        if (!AJ.Phone.Data.IsOpen == true) {
                        AJ.Phone.Animations.BottomSlideUp('.container', 450, -70);
                        }
                    }, 500)
    ...
    $.post('https://aj-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
            ...
            if (AJ.Phone.Notifications.Timeout == undefined || AJ.Phone.Notifications.Timeout == null) {
                ...
                if (!AJ.Phone.Data.IsOpen == true) {
                    AJ.Phone.Animations.BottomSlideUp('.container', 450, -57);
                }
                    AJ.Phone.Animations.TopSlideDown(".phone-notification-container", 600, 6);
                if (icon !== "politie") {
                ...
                AJ.Phone.Notifications.Timeout = setTimeout(function(){
                    AJ.Phone.Animations.TopSlideUp(".phone-notification-container", 600, -8);
                ...
                if (!AJ.Phone.Data.IsOpen) {
                    AJ.Phone.Animations.BottomSlideUp('.container', 300, -56);
                }
                $(".notification-icon").html('<i class="'+icon+'"></i>');
                $(".notification-title").html(title);
                $(".notification-text").html(text);
                ...
                AJ.Phone.Notifications.Timeout = setTimeout(function(){
                    AJ.Phone.Animations.TopSlideUp(".phone-notification-container", 150, -8);
                    AJ.Phone.Notifications.Timeout = setTimeout(function(){
                        if (!AJ.Phone.Data.IsOpen == true) {
                        AJ.Phone.Animations.BottomSlideUp('.container', 450, -70);
                        }
...
$(document).on('click', ".notification-accept", function() {
    $.post('https://aj-phone/AcceptNotification', JSON.stringify({})),
    AJ.Phone.Animations.TopSlideUp(".phone-notification-container-new", 150, -8);
    AJ.Phone.Notifications.Timeout = null
...
$(document).on('click', ".notification-deny", function() {
    $.post('https://aj-phone/DenyNotification', JSON.stringify({})),
    AJ.Phone.Notifications.Timeout = null
...
AJ.Phone.Functions.LoadPhoneData = function(data) {
    ...
    AJ.Phone.Data.PhoneJobs = data.PhoneJobs
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "PhoneNotification":
                AJ.Phone.Notifications.Add(event.data.PhoneNotify.icon, event.data.PhoneNotify.title, event.data.PhoneNotify.text, event.data.PhoneNotify.color, event.data.PhoneNotify.timeout, event.data.PhoneNotify.accept, event.data.PhoneNotify.deny);
                break;
            case "PhoneNotificationCustom":
                AJ.Phone.Notifications.Custom.Add(event.data.PhoneNotify.icon, event.data.PhoneNotify.title, event.data.PhoneNotify.text, event.data.PhoneNotify.color, event.data.PhoneNotify.timeout, event.data.PhoneNotify.accept, event.data.PhoneNotify.deny);
                break;
            ...
            case "IncomingCallAlert":
                ...
                break;
            case "refreshInvoice":
                    AJ.Phone.Functions.LoadBankInvoices(event.data.invoices);
                break;
            case "SetupHomeCall":
                AJ.Phone.Functions.SetupCurrentCall(event.data.CallData);
                break;
            ...
            case "UpdateCallTime":
                ...
                if (!AJ.Phone.Data.IsOpen) {
                    AJ.Phone.Animations.BottomSlideUp('.container', 150, -58);
            ...
            case "UpdateTweets":
                if (AJ.Phone.Data.currentApplication == "twitter") {
                    AJ.Phone.Notifications.LoadTweets(event.data.Tweets, event.data.hasVPN);
                }
                break;

            case "refreshDebt":
                if (AJ.Phone.Data.currentApplication == "debt") {
            ...
            case "UpdateTransactions":
                ...
                break;
            case "UpdateCrypto":
                if (AJ.Phone.Data.currentApplication == "crypto") {
                    AJ.Phone.Data.PlayerData = event.data.PlayerData;
                    LoadCryptoCoins()
                }
                break;
            ...
            case "RefreshChatRooms":
                ...
                AJ.Phone.Functions.LoadChatRooms(rooms)
                break;
            case "RefreshGroupChat":
                AJ.Phone.Functions.RefreshGroupChat(event.data.messageData)
                break;
        }
    })
});