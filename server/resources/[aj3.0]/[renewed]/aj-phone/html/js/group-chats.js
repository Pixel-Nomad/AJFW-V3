...
$('#join-room').on('click', e => {
    e.preventDefault()
    $.post("https://aj-phone/JoinGroupChat", JSON.stringify({roomID: currentRoom}), function(status) {
        if(status) {
            $('.chat-room-join').hide()
...
$('#search-message').keypress(e => {
    if(e.which == 13) {
        ...
        if(keyword !== previousSearch && keyword !== "" && keyword.length >= 3) {
            ...
            $.post("https://aj-phone/SearchGroupChatMessages", JSON.stringify({roomID: currentRoom, searchTerm: keyword}), function(messages) {
                if( ! messages) {
...
$('#pinned-messages').on('click', e => {
    ...
    if( ! pinnedOpen) {
        ...
        $.post("https://aj-phone/GetPinnedMessages", JSON.stringify({roomID: currentRoom}), function(results) {
            if( ! results) {
...
function openChatRoom(id) {
    ...
    $.post("https://aj-phone/GetGroupChatMessages", JSON.stringify({roomID: currentRoom}), function(messages) {
        RenderChatMessages(messages)  
    }) 
    $("#submit-message").prop('placeholder', 'Press [ENTER] to chat!')
...
$("#submit-message").on('keypress', (e) => {
    ...
    if(e.which === 13 && messageContent.length > 0) {
        ...
        $.post("https://aj-phone/SendGroupChatMessage", JSON.stringify(messageData), function() {
            $("#submit-message").val("")
        })
    }
})
...
$("#send-message").on('click', () => {
    ...
    if(messageContent.length > 0) {
        ...
        $.post("https://aj-phone/SendGroupChatMessage", JSON.stringify(messageData), function(status) {
            $("#submit-message").val("")
        })
    }
})
...
$(document).on('keypress', '#change-pin', function(e) {
    if(e.which == 13) {
        ...
        if(pinOk) {
            $.post("https://aj-phone/ChangeRoomPin", JSON.stringify({pinCode: pin, roomID: id}), function(status) {
                if(status) {
                    $('#change-pin').val(pin)
                    $('#owned-rooms-list-close').trigger('click')
...
$('#room-pin-input').keypress(e => {
    if(e.which == 13) {
        ...
        $.post('https://aj-phone/JoinGroupChat', JSON.stringify({roomID: id, roomPin: pin}), (status) => {
            if(status) {
...
$(document).on('click', '#remove-member', function(e){
    ...
    $.post("https://aj-phone/LeaveGroupChat", JSON.stringify({roomID: room, citizenid: cid}), function(status) {
        if(status) {
            $(this).remove();
