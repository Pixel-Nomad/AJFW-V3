...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('aj-phone:server:AddNewContact', data.ContactName, data.ContactNumber)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('aj-phone:server:EditContact', NewName, NewNumber, OldName, OldNumber)
end)
...
RegisterNUICallback('phone-silent-button', function(_, cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        AJFW.Functions.Notify("Silent Mode On", "success")
        cb(true)
    else
        CallVolume = 0.2
        AJFW.Functions.Notify("Silent Mode Off", "error")
        cb(false)
    end
end)
...
RegisterNetEvent('aj-phone:client:clearAppAlerts', function()
    Config.PhoneApplications["phone"].Alerts = 0
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNetEvent("aj-phone:client:giveContactRequest", function(contactInfo)
    local success = exports['aj-phone']:PhoneNotification("CONTACT REQUEST", contactInfo.name..' contact request', 'fas fa-phone', '#b3e0f2', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
    if success then
        TriggerServerEvent('aj-phone:server:acceptContactRequest', contactInfo)
    end
end)

RegisterNetEvent('aj-phone:client:updateContactInfo', function(contactInfo)
    PhoneData.Contacts[#PhoneData.Contacts+1] = {
