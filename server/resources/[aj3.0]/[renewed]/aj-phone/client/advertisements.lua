...
RegisterNUICallback('PostAdvert', function(data, cb)
    ...
    TriggerServerEvent('aj-phone:server:AddAdvert', data.message, url)
    cb("ok")
end)


RegisterNUICallback('FlagAdvert', function(data, cb)
    TriggerServerEvent('aj-phone:server:flagAdvert', data.number)
    cb("ok")
end)
...
RegisterNetEvent('aj-phone:client:UpdateAdverts', function(Adverts, LastAd, src)
