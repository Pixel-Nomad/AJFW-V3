    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if room.id == data.roomID then
            ...
            if not room.room_pin then
                lib.callback('aj-phone:server:GetGroupChatMessages', false, function(messages)
                    cb(messages)
                end, data.roomID)
            else
                if next(memberList) then
    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if(room.id == Room) then
            ...
            if next(memberList) then
                for _, memberData in pairs(memberList) do
                    if Player == memberData.cid or Player == room.room_owner_id then
                        lib.callback('aj-phone:server:SearchGroupChatMessages', false, function(messages)
                            cb(messages)
                        end, Room, SearchTerm)
                        break
                    end
    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if(room.id == Room) then
            ...
            if next(memberList) then
                ...
                for _, memberData in pairs(memberList) do
                    if Player == memberData.cid or Player == room.room_owner_id then
                        lib.callback('aj-phone:server:GetPinnedMessages', false, function(messages)
                            cb(messages)
                        end, Room)
                    end
                    break
...
RegisterNUICallback('JoinGroupChat', function(data, cb)
    ...
    if not room then
        ...
        if isMemberOfRoom(Player, roomID) then
            ...
            if roomPin then
                lib.callback('aj-phone:server:TryPinCode', false, function(result)
                    if result then
                        ...
                        if next(members) then
                            ...
                            lib.callback("aj-phone:server:JoinGroupChat", false, function(success)
                                if success then
...
RegisterNUICallback('ChangeRoomPin', function(data, cb)
    ...
    if not room then
        ...
        if pin then
            lib.callback('aj-phone:server:IsRoomOwner', false, function(isOwner)
                if isOwner then
                    for k, v in pairs(PhoneData.ChatRooms) do
                        if(v.id == roomID) then
                            if pin == '' then
...
RegisterNUICallback('CreateDiscordRoom', function(data, cb)
    ...
    lib.callback("aj-phone:server:PurchaseRoom", false, function(status)
        cb(status)
    end, 250, roomData)
end)