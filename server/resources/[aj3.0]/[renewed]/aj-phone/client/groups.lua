...
RegisterNetEvent('aj-phone:client:AddGroupStage', function(_, stage)
    inJob = true
    SendNUIMessage({
...
RegisterNUICallback('jobcenter_CreateJobGroup', function(data, cb) --employment
    TriggerServerEvent('aj-phone:server:jobcenter_CreateJobGroup', data)
    cb("ok")
end)

RegisterNUICallback('jobcenter_JoinTheGroup', function(data, cb) --employment
    TriggerServerEvent('aj-phone:server:jobcenter_JoinTheGroup', data)
    cb("ok")
end)

RegisterNUICallback('jobcenter_leave_grouped', function(data, cb) --employment
    if not data then return end
    local success = exports['aj-phone']:PhoneNotification("Job Center", 'Are you sure you want to leave the group?', 'fas fa-users', '#FFBF00', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
    if success then
        TriggerServerEvent('aj-phone:server:jobcenter_leave_grouped', data)
    end
    cb("ok")
end)

RegisterNUICallback('jobcenter_DeleteGroup', function(data, cb) --employment
    TriggerServerEvent('aj-phone:server:jobcenter_DeleteGroup', data)
    cb("ok")
end)
