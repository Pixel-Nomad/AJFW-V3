...
local function RemoveCrypto(src, type, amount)
    ...
    if (Crypto[type] - amount) >= 0 then
        ...
        TriggerClientEvent('aj-phone:client:UpdateCrypto', src)
        return true
    else
        return false
    end
...
-- exports['aj-phone']:hasEnough(Player, type, amount)
...
-- exports['aj-phone']:AddCrypto(Player, type, amount)
...
RegisterNetEvent('aj-phone:server:PurchaseCrypto', function(type, amount)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.money.bank >= cashAmount then
        ...
        if Config.ajBanking then
            ...
            exports['aj-Banking']:handleTransaction(cid, "Crypto Purchase", cashAmount, txt, "Los Santos Crypto", name, "withdraw")
        end
...
RegisterNetEvent('aj-phone:server:SellCrypto', function(type, amount)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if not Player or not Player.PlayerData.metadata.crypto[type] then return end -- if the crypto dosnt exist
    local v = Config.CryptoCoins[GetConfig(type)]
    ...
    if not RemoveCrypto(src, type, amount) then return end
        ...
        if Config.ajBanking then
            local cid = Player.PlayerData.citizenid
            local name = ("%s %s"):format(Player.PlayerData.charinfo.firstname, Player.PlayerData.charinfo.lastname)
            exports['aj-Banking']:handleTransaction(cid, "Sold Crypto", cryptoAmount, txt, "Los Santos Crypto", name, "deposit")
        end
end)

RegisterNetEvent('aj-phone:server:ExchangeCrypto', function(type, amount, stateid)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local Receiver = AJFW.Functions.GetPlayer(tonumber(stateid))
    if not Player or not Player.PlayerData.metadata.crypto[type] then return end -- if the crypto dosnt exist
    if not Receiver then return TriggerClientEvent("AJFW:Notify", src, 'This state id does not exists!', "error") end
    ...
    if Player.PlayerData.citizenid ~= Receiver.PlayerData.citizenid then
        if RemoveCrypto(src, type, amount) then
            ...
            TriggerClientEvent('aj-phone:client:CustomNotification', Receiver.PlayerData.source,
                "WALLET",
