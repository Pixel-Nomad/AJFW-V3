...
RegisterNetEvent('aj-phone:server:SendGroupChatMessage', function(messageData, systemMessage, roomID)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if messageData and not systemMessage then
        if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
            ...
            TriggerClientEvent('aj-phone:client:RefreshGroupChat', -1, src, messageData)
            TriggerEvent("aj-log:server:CreateLog", "discord", "Message Posted (room: ".. messageData.room_id .. ", from: ".. Player.PlayerData.citizenid ..")", "blue", messageData.message)
        else
            TriggerClientEvent('aj-phone:client:notification', src, 'Discord', 'You must be a member or room owner to send messages.')
        end
    else
        ...
        TriggerClientEvent('aj-phone:client:RefreshGroupChat', -1, src, systemMessage)
    end
end)
...
RegisterNetEvent('aj-phone:server:LeaveGroupChat', function(updatedRooms, roomID)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local members = {}
...
RegisterNetEvent('aj-phone:server:ChangeRoomPin', function(updatedRooms, roomID, pin)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:DeactivateRoom', function(updatedRooms, roomID)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
lib.callback.register('aj-phone:server:PurchaseRoom', function(source, price, roomData)
    ...
    if Player.PlayerData.money.bank >= price then
        ...
        if Config.ajBanking then
            exports['aj-Banking']:handleTransaction(cid, "Discord App", price, "Discord Channel Purchase", roomData.room_owner_name, "Discord", "withdraw")
        end
...
RegisterNetEvent('aj-phone:server:ToggleMessagePin', function(messageID, roomID)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
