...
local function NotifyGroup(group, msg, type)
    ...
    for _, v in pairs(EmploymentGroup[group].members) do
        TriggerClientEvent('AJFW:Notify', v.Player, msg, type)
    end
...
local function pNotifyGroup(group, header, msg, icon, colour, length)
    if not group or not EmploymentGroup[group] then return print("Group not found...") end
    for _, v in pairs(EmploymentGroup[group].members) do
        TriggerClientEvent('aj-phone:client:CustomNotification', v.Player,
            header or "NO HEADER",
            msg or "NO MSG",
...
local function DestroyGroup(groupID)
    ...
    TriggerEvent("aj-phone:server:GroupDeleted", groupID, members)

    EmploymentGroup[groupID] = nil
    TriggerClientEvent('aj-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
...
local function RemovePlayerFromGroup(src, groupID, disconnected)
    ...
    for k,v in pairs(g) do
        if v.Player == src then
            ...
            TriggerClientEvent('aj-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
            if not disconnected then TriggerClientEvent("AJFW:Notify", src, "You have left the group", "primary") end
...
local function setJobStatus(groupID, status, stages)
    ...
    for i=1, #m do
        if m[i] then
            TriggerClientEvent("aj-phone:client:AddGroupStage", m[i], status, stages)
        end
    end
...
local function resetJobStatus(groupID)
    ...
    for i=1, #m do
        if m[i] then
            TriggerClientEvent("aj-phone:client:AddGroupStage", m[i], EmploymentGroup[groupID].status, EmploymentGroup[groupID].stage)
            TriggerClientEvent('aj-phone:client:RefreshGroupsApp', m[i], EmploymentGroup, true)
        end
    end
...
RegisterNetEvent("aj-phone:server:employment_checkJobStauts", function ()
    local src = source
    local checkStatus = GetGroupByMembers(src)
    if checkStatus then
        TriggerClientEvent('aj-phone:client:showEmploymentPage', src)
    else
        TriggerClientEvent('aj-phone:client:showEmploymentGroupPage', src)
    end
end)

RegisterNetEvent("aj-phone:server:jobcenter_CreateJobGroup", function(data)
    local src = source
    local player = AJFW.Functions.GetPlayer(src)
    if Players[src] then TriggerClientEvent('AJFW:Notify', src, "You have already created a group", "error") return end
...
RegisterNetEvent('aj-phone:server:jobcenter_DeleteGroup', function(data)
    local src = source
...
AJFW.Functions.CreateCallback('aj-phone:server:GetGroupsApp', function(_, cb)
    cb(EmploymentGroup)
end)

RegisterNetEvent('aj-phone:server:jobcenter_JoinTheGroup', function(data)
    local src = source
    local player = AJFW.Functions.GetPlayer(src)

    if Players[src] then return TriggerClientEvent('AJFW:Notify', src, "You are already a part of a group!", "success") end
    ...
    Players[src] = true
    TriggerClientEvent('AJFW:Notify', src, "You joined the group", "success")
    TriggerClientEvent('aj-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
end)
...
AJFW.Functions.CreateCallback('aj-phone:server:getAllGroups', function(source, cb)
    local src = source
...
AJFW.Functions.CreateCallback('aj-phone:server:jobcenter_CheckPlayerNames', function(_, cb, csn)
    local Names = {}
...
RegisterNetEvent('aj-phone:server:jobcenter_leave_grouped', function(data)
    local src = source
...
local function CreateGroup(src, name, password)
	...
	id = id,
        status = "WAITING",
        GName = name,
        GPass = password or AJFW.Shared.RandomInt(7),
        Users = 1,
        leader = src,
        members = {
            {name = GetPlayerCharName(src), CID = Player.PlayerData.citizenid, Player = src}
