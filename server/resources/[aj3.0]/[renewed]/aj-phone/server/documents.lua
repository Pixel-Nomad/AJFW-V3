RegisterNetEvent("aj-phone:server:sendDocument", function(data)
    ...
    local Receiver = AJFW.Functions.GetPlayer(tonumber(data.StateID))
    ...
    if Ply.PlayerData.citizenid ~= Receiver.PlayerData.citizenid then
        ...
        TriggerClientEvent("aj-phone:client:sendingDocumentRequest", data.StateID, data, Receiver, Ply, SenderName)
    else
        TriggerClientEvent('aj-phone:client:CustomNotification', src,
            "Documents",
...
RegisterNetEvent("aj-phone:server:sendDocumentLocal", function(data, playerId)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
    local Receiver = AJFW.Functions.GetPlayer(playerId)
    local SenderName = Ply.PlayerData.charinfo.firstname..' '..Ply.PlayerData.charinfo.lastname
    ...
    TriggerClientEvent("aj-phone:client:sendingDocumentRequest", playerId, data, Receiver, Ply, SenderName)
end)
...
RegisterNetEvent('aj-phone:server:documents_Save_Note_As', function(data, Receiver)
    local src = source
    ...
    elseif data.Type == "PermSend" then
        ...
        if Note[1] then
            ...
            TriggerClientEvent('aj-phone:client:CustomNotification', tonumber(data.StateID), 'DOCUMENTS', 'New Document', 'fas fa-folder', '#d9d9d9', 5000)
        end
    end
    ...
    Wait(100)
    TriggerClientEvent('aj-phone:RefReshNotes_Free_Documents', src, Notes)
end)
