...
RegisterNetEvent('aj-phone:server:sendNewMail', function(mailData, citizenID)

    if not mailData or not mailData.sender or not mailData.subject or not mailData.message then return end
    local Player

    if citizenID then
        Player = AJFW.Functions.GetPlayerByCitizenId(citizenID)
    else
    ...
    if Player then
        ...
        TriggerClientEvent('aj-phone:client:NewMailNotify', Player.PlayerData.source, mailData)
