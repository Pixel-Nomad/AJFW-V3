...
local function openBankUI(isAtm)
    ...
    nuiHandler(true)
    lib.callback('aj-Banking:server:initalizeBanking', false, function(accounts)
...
RegisterNetEvent('aj-Banking:client:openBankUI', function(data)
...
CreateThread(function ()
    for k=1, #bankActions do
        RegisterNUICallback(bankActions[k], function(data, cb)
            local newTransaction = lib.callback.await('aj-Banking:server:'..bankActions[k], false, data)
            cb(newTransaction)
        end)
    end
...
RegisterNetEvent('aj-Banking:client:sendNotification', function(msg)
