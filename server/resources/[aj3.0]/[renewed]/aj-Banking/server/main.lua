...
lib.callback.register('aj-Banking:server:initalizeBanking', function(source)
...
lib.callback.register('aj-Banking:server:deposit', function(source, data)
    local Player = GetPlayerObject(source)
    ...
    if RemoveMoney(Player, amount, 'cash', data.comment) then
        ...
        TriggerClientEvent('aj-Banking:client:sendNotification', source, locale("not_enough_money"))
        return false
    end
end)
...
lib.callback.register('aj-Banking:server:withdraw', function(source, data)
    local Player = GetPlayerObject(source)
    local amount = tonumber(data.amount)
    if not amount or amount < 1 then
...
lib.callback.register('aj-Banking:server:transfer', function(source, data)
    local Player = GetPlayerObject(source)
    local amount = tonumber(data.amount)
    if not amount or amount < 1 then
    ...
    if cachedAccounts[data.fromAccount] then
        if cachedAccounts[data.stateid] then
            ...
            if not Player2 then
                TriggerClientEvent('aj-Banking:client:sendNotification', source, locale("fail_transfer"))
                return false
            end
            local canTransfer = RemoveAccountMoney(data.fromAccount, amount)
            if canTransfer then
...
RegisterNetEvent("aj-Banking:server:getPlayerAccounts", function()
    local Player = GetPlayerObject(source)
    local cid = GetIdentifier(Player)
    ...
    TriggerClientEvent("aj-Banking:client:accountsMenu", source, data)
end)

RegisterNetEvent("aj-Banking:server:viewMemberManagement", function(data)
    local Player = GetPlayerObject(source)
    ...
    TriggerClientEvent("aj-Banking:client:viewMemberManagement", source, retData)
end)

RegisterNetEvent('aj-Banking:server:addAccountMember', function(account, member)
    local Player = GetPlayerObject(source)
...
RegisterNetEvent('aj-Banking:server:removeAccountMember', function(data)
    local Player = GetPlayerObject(source)
...
RegisterNetEvent('aj-Banking:server:deleteAccount', function(data)
    local account = data.account
    local Player = GetPlayerObject(source)
    local cid = GetIdentifier(Player)
...
RegisterNetEvent('aj-Banking:server:changeAccountName', function(account, newName)
