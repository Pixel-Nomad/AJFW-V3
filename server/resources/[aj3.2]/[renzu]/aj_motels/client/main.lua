...
RegisterNetEvent('aj_motels:invoice')
AddEventHandler('aj_motels:invoice', function(data)
	local motels = GlobalState.Motels
    local buy = lib.alertDialog({
		header = 'Invoice',
		content = '![motel](nui://aj_motels/data/image/'..data.motel..'.png) \n ## INFO \n **Description:** '..data.description..'  \n  **Amount:** $ '..data.amount..'  \n **Payment method:** '..data.payment,
		centered = true,
		labels = {
			cancel = 'close',
			confirm = 'Pay'
	...
	if buy ~= 'cancel' then
		local success = lib.callback.await('aj_motels:payinvoice',false,data)
		if success then
...
RegisterNetEvent('aj_motels:Door', function(data)
	...
	local options = {
		{
			...
			onSelect = function()
				...
				local success = lib.callback.await('aj_motels:payrent',false,{
		...
		},
		{
			...
			icon = 'key',
			onSelect = function()
				local success = lib.callback.await('aj_motels:motelkey',false,{
					index = data.index,
					motel = data.motel,
				})
				if success then
		...
		},
		{
			...
			onSelect = function()
				...
				local success = lib.callback.await('aj_motels:removeoccupant',false,data,data.index,PlayerData.identifier)
				if success then
...
RoomList = function(data)
	local motels , time = lib.callback.await('aj_motels:getMotels',false)
	local rate = motels[data.motel].hour_rate or data.rate
	local options = {}
	--local motels = GlobalState.Motels
		...
		if occupants < data.maxoccupants and not duration then
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('aj_motels:rentaroom',false,{
						index = doorindex,
						motel = data.motel,
						duration = input[1],
						rate = rate,
...
MotelRentalMenu = function(data)
	...
	if not data.manual then
		table.insert(options,{
			...
			description = '![rent](nui://aj_motels/data/image/'..data.motel..'.png) \n Choose a room to rent \n '..data.rental_period..' Rate: $'..rate,
			icon = 'hotel',
			onSelect = function()
	...
	for player,char in pairs(players) do
		...
		table.insert(options,{
			...
			onSelect = function()
				...
				local success = lib.callback.await('aj_motels:removeoccupant',false,data,index,player)
				if success then
		...
		for i = 1, data.maxoccupants-#options do
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('aj_motels:addoccupant',false,data,index,input)
	...
	local options = {
		{
			...
			onSelect = function()
				...
				if not input then return end
				local success = lib.callback.await('aj_motels:addemployee',false,data.motel,input[1])
				if success then
    ...
    if motel and motel.employees then
       for identifier,name in pairs(motel.employees) do
          table.insert(options,{
			...
			icon = 'hotel',
			onSelect = function()
				local success = lib.callback.await('aj_motels:removeemployee',false,data.motel,identifier)
					if success then
...
MotelOwner = function(data)
	...
	if not motels[data.motel].owned then
		...
		if buy ~= 'cancel' then
			local success = lib.callback.await('aj_motels:buymotel',false,data)
			if success then
		...
		local options = {
			...
			},
			{
				...
				onSelect = function()
					...
					local success = lib.callback.await('aj_motels:sendinvoice',false,data.motel,input)
					if success then
		...
		if motels[data.motel].owned == PlayerData.identifier then
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('aj_motels:editrate',false,data.motel,input[1])
					if success then
			...
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('aj_motels:withdrawfund',false,data.motel,input[1])
					if success then
			...
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('aj_motels:transfermotel',false,data.motel,input[1])
					if success then
			...
			table.insert(options,{
				...
				onSelect = function()
					...
					if sell ~= 'cancel' then
						local success = lib.callback.await('aj_motels:sellmotel',false,data)
						if success then
...
function Teleport(x, y, z, h ,exit)
	...
	if exit then
		...
		DeleteEntity(house)
		lib.callback.await('aj_motels:SetRouting',false,data,'exit')
		shelzones = {}
...
EnterShell = function(data,login)
	...
	lib.callback.await('aj_motels:SetRouting',false,data,'enter')
	inhouse = true
	Wait(1000)
...
RegisterNetEvent('aj_motels:MessageOwner', function(data)
