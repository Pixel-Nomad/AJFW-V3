...
lib.callback.register('aj_motels:rentaroom', function(src,data)
...
lib.callback.register('aj_motels:payrent', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('aj_motels:getMotels', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	return motels, os.time()
end)

lib.callback.register('aj_motels:motelkey', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local metadata = {
		type = data.motel,
		serial = data.index,
...
lib.callback.register('aj_motels:buymotel', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	local money = xPlayer.getAccount(data.payment).money
...
lib.callback.register('aj_motels:removeoccupant', function(src,data,index,player)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('aj_motels:addoccupant', function(src,data,index,player)
	local xPlayer = GetPlayerFromId(src)
...
lib.callback.register('aj_motels:editrate', function(src,motel,rate)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('aj_motels:addemployee', function(src,motel,id)
	local xPlayer = GetPlayerFromId(src)
...
lib.callback.register('aj_motels:removeemployee', function(src,motel,identifier)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	if motels[motel].owned == xPlayer.identifier then
...
lib.callback.register('aj_motels:transfermotel', function(src,motel,id)
	local xPlayer = GetPlayerFromId(src)
	local toPlayer = GetPlayerFromId(tonumber(id))
	local motels = GlobalState.Motels
	if motels[motel].owned == xPlayer.identifier and toPlayer then
...
lib.callback.register('aj_motels:sellmotel', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('aj_motels:withdrawfund', function(src,motel,amount)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	if motels[motel].owned == xPlayer.identifier then
...
local invoices = {}
lib.callback.register('aj_motels:sendinvoice', function(src,motel,data)
	...
	if motels[motel].owned == xPlayer.identifier or motels[motel].employees[xPlayer.identifier] then
		...
		invoices[id] = data[2]
		TriggerClientEvent('aj_motels:invoice',toPlayer,{
			motel = motel,
			amount = data[2],
			description = data[3],
			id = id,
...
lib.callback.register('aj_motels:payinvoice', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
local routings = {}
lib.callback.register('aj_motels:SetRouting', function(src,data,Type)
	local xPlayer = GetPlayerFromId(src)
	if Type == 'enter' then
...
lib.callback.register('aj_motels:MessageOwner', function(src,data)
	local motels = GlobalState.Motels
	...
	if xPlayer then
		TriggerClientEvent('aj_motels:MessageOwner',xPlayer.source, data)
		return true
	end
	return false
end)

RegisterServerEvent("aj_motels:Door")
AddEventHandler('aj_motels:Door', function(data)
	local source = source
	TriggerClientEvent('aj_motels:Door', -1, data)
