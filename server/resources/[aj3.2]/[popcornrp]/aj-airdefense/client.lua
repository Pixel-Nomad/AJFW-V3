...
CreateThread(function()
    while true do
        ...
        if dist < 300 and GetEntityHeightAboveGround(ped) > 5.0 and IsPedInFlyingVehicle(ped) and (PlayerData.job.type ~= 'leo' and PlayerData.job.name ~= 'ambulance') then
            local count = 0
            ...
            while count < random do
                if not isAlarmActive then
                    AJFW.Functions.Notify('You are entering restricted airspace! Please leave in the next '..(Config.Timer / 1000)..' seconds!', 'error')     
                    isAlarmActive = true
                    Wait(Config.Timer) -- Wait for 10 seconds before explosions
                    AJFW.Functions.Notify('Air defense systems have been activated! Get out of here!', 'error')
...
CreateThread(function()
    while true do
        ...
        if dist < 700 and GetEntityHeightAboveGround(ped) > 5.0 and IsPedInFlyingVehicle(ped) and (PlayerData.job.type ~= 'leo') and (PlayerData.job.name ~= 'ambulance') then
            local count = 0
            local random = math.random(1, 5)
