...
local function RemoveStinger()
    if DoesEntityExist(closestStinger) then
        ...
        if not DoesEntityExist(closestStinger) then
            TriggerServerEvent("aj-spikes:server:pickupspikes")
        end
    end
end
...
-- events
RegisterNetEvent("aj-spikes:client:usespikes", function()
    local ped = PlayerPedId()
    local player = AJFW.Functions.GetPlayerData()
    if not IsPedInAnyVehicle(ped, false) then
        if allowed then
            DeployStinger()
            TriggerServerEvent('aj-spikes:server:removespikes')
        else
            AJFW.Functions.Notify('You are not trained to use this', 'error', 7500)
        end
    else
        AJFW.Functions.Notify('You can\'t use spikes while in a vehicle', 'error', 7500)
    end
end)

RegisterNetEvent("aj-spikes:client:removespikes", function()
    RemoveStinger()
end)
...
exports['aj-target']:AddTargetModel('p_ld_stinger_s', {
    options = {
        {
            num = 1,
            event = 'aj-spikes:client:removespikes',
