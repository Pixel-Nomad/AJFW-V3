...
Citizen.CreateThread(function()
    ...
    AJFW.Functions.TriggerCallback('jacob:get:coke', function(Coke_start, Coke_Process, Coke_boat)
		coord            = Coke_start
...
-- Citizen.CreateThread(function()
-- 	while AJFW == nil do Wait(100) end
--     AJFW.Functions.TriggerCallback('coke:startcoords', function(servercoords)
--         coord = servercoords
-- 	end)
-- end)
			...
			if dist <= 2.0 then
				...
				if IsControlJustPressed(1, 51) then
					AJFW.Functions.TriggerCallback('AJFW:HasItem', function(result)
						--AJFW.Functions.TriggerCallback('coke:pay' , function(success)
							if result then
								TriggerServerEvent('coke:paid')
								main()
							else
								AJFW.Functions.Notify("आपके पास आवश्यक सामग्री नहीं है|", "error")
							end
						--end)
					end, 'greychip')
				end
	...
	while true do
		...
		if not inUse then
			if dist <= 2.0 then
				...
				DrawText3D(coord.x, coord.y, coord.z, 'Press ~g~[ E ]~w~ to rent a Boat')
				if IsControlJustPressed(1, 51) then
					AJFW.Functions.TriggerCallback('coke:itemTake', function(HasItem, type)
						if HasItem then
							TriggerServerEvent('coke:paid')
							main()
						else
							AJFW.Functions.Notify("You do not have the necessary materials.", "error")
...
AddEventHandler('coke:onUse', function()
	if Config.useMythic then
		AJFW.Functions.Notify("You used Coke", "success")
	end
	...
	DoScreenFadeIn(2000)
	if Config.useMythic then
		AJFW.Functions.Notify("You are feeling normal now..", "success")
	end
...
function main()
	...
	if Config.useMythic then
		AJFW.Functions.Notify("Go to the Pier.", "success")
	end
...
function BoatSpawn()
    ...
    local plt = AJFW.Functions.GetPlate(ajpboat)
	SetVehicleHasBeenOwnedByPlayer(ajpboat,true)

	local plate = AJFW.Functions.GetPlate(ajpboat)
	--TriggerServerEvent('garage:addKeys', plate)
	TriggerEvent("vehiclekeys:client:SetOwner", plate)
	-- SetVehicleFuelLevel(boat, 5)
	-- exports['aj-fuel']:SetFuel(boat, 5)
	exports['aj-fuel']:SetFuel(boat, 100.0)
	...
	while checkBoat do
		...
		if DoesEntityExist(ajpboat) then
			if GetVehicleEngineHealth(ajpboat) < 0 then
				if Config.useMythic then
					AJFW.Functions.Notify("Failed, your boat was Destroyed", "error")
				end
				TriggerServerEvent('coke:updateTable', false)
				RemoveBlip(blip)
...
function delivery()
	if Config.useMythic then
		AJFW.Functions.Notify("Get in the boat and pick up the delivery marked on your GPS", "success")
	end
		...
		while delivering do
			...
			if disttocoord <= 10 then
				...
				if IsControlJustPressed(1, 51) then
					...
					AJFW.Functions.Progressbar("picking_", "Picking up the delivery..",5000, false, true, {
					...
					Citizen.Wait(2000)
					AJFW.Functions.Notify("Picked up the delivery. Return to the Pier marked on your GPS.", "success")
					Citizen.Wait(2000)
					final()
				end
			else
...
function final()
	AJFW.Functions.Notify("Deliver the boat back to a hangar", "success")
		...
		while hangar do
			...
			if IsPedInAnyBoat(PlayerPedId()) and disttocoord <= 10 then
				...
				if IsControlJustPressed(1, 51) then
					...
					AJFW.Functions.Progressbar("lockpick_vehicledoor", "Returning..", 2500, false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 3 and not isProcessing then
			...
			if IsControlJustPressed(1, 51) then
				isProcessing = true
				AJFW.Functions.TriggerCallback('coke:ingredient', function(HasItem, type)
					if HasItem then
						processing()
					else
						AJFW.Functions.Notify("You are missing something", "error")
						isProcessing = false
					end
				end)
			end
...
function processing()
	...
	AJFW.Functions.Progressbar("coke-", "Breaking down the coke..", 30000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
