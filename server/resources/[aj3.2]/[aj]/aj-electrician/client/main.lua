...
local function SetWorkBlip(d)
    ...
    TriggerEvent('aj-electrician:client:JobMarkers')
end
...
RegisterNetEvent('aj-electrician:client:VehPick', function()
    local choice = math.random(1, #Config.JobVehicles)
    ElecVeh = Config.JobVehicles[choice]
    TriggerEvent('aj-electrician:client:SpawnVehicle', ElecVeh)
end)
                        ...
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                if VehicleCheck(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                    ...
                                    AJFW.Functions.Notify('This is not an electrician vehicle!', 'error')
                                end
                            else
                                AJFW.Functions.Notify('You must be the driver to do this!', 'error')
                            end
                        else
                            TriggerEvent('aj-electrician:client:VehPick')
                        end
                    end  
                end
            end
...
RegisterNetEvent('aj-electrician:client:SpawnVehicle', function(vehicleInfo)
...
function StartJobLocations()
    ...
    AJFW.Functions.Notify("Your jobs have been added to your GPS - Go and complete your work.", "primary")
end

-- Individual Job Site Interactions
RegisterNetEvent('aj-electrician:client:JobMarkers', function(k, v)
    local inRange = false
    CompleteRepairs = 0
    while true do
        Wait(5)
                            ...
                            if IsControlJustReleased(0, 38) then
                                AJFW.Functions.Progressbar("repair_work", "Repairing Fault...", math.random(10000, 12000), false, true, {
                                    ...
                                    if CompleteRepairs <= 4 then
                                        AJFW.Functions.Notify("You've repaired the electrical fault. Head to the next job.")
                                    else
                                        AJFW.Functions.Notify("You've finished all your jobs - Return your vehicle and collect your payslip.")
                                    end
                                    RemoveBlip(v.BlipId)
                ...
                if #(pos - vector3(Config.Locations["payslip"].coords.x, Config.Locations["payslip"].coords.y, Config.Locations["payslip"].coords.z)) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if CompleteRepairs ~= 0 then
                            TriggerServerEvent('aj-electrician:server:Payslip', CompleteRepairs)
                            CompleteRepairs = 0
                            for k,v in ipairs(JobsinSession) do
                                RemoveBlip(v.BlipId)
                            end
                        else
                            AJFW.Functions.Notify("You haven't done any jobs yet!", "error")
                        end
