...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
    AJFW.Functions.TriggerCallback("aj-admin:server:getrank",function(acd2)
        if acd2 then
            rank = 'admin'
        end
    end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for i = 1, #Config.Shops do
            ...
            if Config.Shops[i].adminOnly then
                if rank ~= 'user' then
                ...
                if dist <= 5 then
                    ...
                    if dist <= 2.5 then
                        DrawText3Ds(actualShop.x, actualShop.y, actualShop.z + 0.2, '[~g~E~w~] - Browse Vehicle Catalogue')
...
RegisterNetEvent('aj-testdrive.receiveInfo')
AddEventHandler('aj-testdrive.receiveInfo', function(bank, name)
    if name then
        profileName = name
    end
    profileMoney = bank
...
RegisterNetEvent('aj-testdrive.successfulbuy')
AddEventHandler('aj-testdrive.successfulbuy', function(vehicleName,vehiclePlate,value)    
    SendNUIMessage(
        {
            type = "successful-buy",
...
RegisterNetEvent('aj-testdrive.notify')
AddEventHandler('aj-testdrive.notify', function(type, message)    
    SendNUIMessage(
        {
            type = "notify",
            typenotify = type,
...
RegisterNetEvent('aj-testdrive.vehiclesInfos')
AddEventHandler('aj-testdrive.vehiclesInfos', function() 
...
function OpenVehicleShop()
    inTheShop = true
    TriggerServerEvent("aj-testdrive.requestInfo")
    TriggerEvent('aj-testdrive.vehiclesInfos')
    Citizen.Wait(1000)
    SendNUIMessage(
        {
            data = vehiclesTable,
...
function updateSelectedVehicle(model)
  ...
  SetEntityAsMissionEntity(lastSelectedVehicleEntity, true, true)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        local vehicleProps = AJFW.Functions.GetVehicleProperties(lastSelectedVehicleEntity)
        vehicleProps.plate = newPlate

        TriggerServerEvent('aj-testdrive.CheckMoneyForVeh',data.modelcar, data.sale, data.name, vehicleProps)
...
RegisterNetEvent('aj-testdrive.spawnVehicle')
AddEventHandler('aj-testdrive.spawnVehicle', function(model, plate)    
    local hash = GetHashKey(model)
    ...
    SetEntityAsMissionEntity(vehicleBuy, true, true)
    SetPedIntoVehicle(PlayerPedId(), vehicleBuy, -1)
    SetVehicleNumberPlateText(vehicleBuy, plate)
    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicleBuy), 'body', 1000.0)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicleBuy), 'engine', 1000.0)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicleBuy), 'fuel', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicleBuy), 'clutch', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicleBuy), 'brakes', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicleBuy), 'axle', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(vehicleBuy), 'radiator', 100)
    TriggerEvent("vehiclekeys:client:SetOwner", AJFW.Functions.GetPlate(vehicleBuy))
...
RegisterNUICallback(
    ...
    function(data, cb)        
        if Config.TestDrive then
            AJFW.Functions.TriggerCallback('aj-testdrive.ghuu', function(HasItem, type)
                if HasItem then
                    startCountDown = true
                        ...
                        TriggerServerEvent('aj-testdrive.CheckMoneyForTest')
                        ...
                        exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(testDriveEntity), 'body', 1000.0)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(testDriveEntity), 'engine', 1000.0)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(testDriveEntity), 'fuel', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(testDriveEntity), 'clutch', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(testDriveEntity), 'brakes', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(testDriveEntity), 'axle', 100)
		    exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(testDriveEntity), 'radiator', 100)
                        TriggerEvent("vehiclekeys:client:SetOwner", AJFW.Functions.GetPlate(testDriveEntity))
                    ...
                    AJFW.Functions.Notify("You don't have enough money", "error")
                    CloseNui() 
