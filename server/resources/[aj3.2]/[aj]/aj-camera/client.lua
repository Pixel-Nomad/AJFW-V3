...
RegisterNetEvent("aj-camera:client:use-camera", function()
    if not active then
        active = true
                    ...
                    while active and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) and true do
                        ...
                        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
                            if not presstake then
                                presstake = true
                                AJFW.Functions.TriggerCallback("aj-camera:server:webhook", function(hook)
                                    if hook then
                                        exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                                            local image = json.decode(data)
                                            FullClose()
                                            AJFW.Functions.Progressbar("makingphoto", "Collecting Photo..",'blue', 1500, false, true, {
                                            }, {}, {}, {}, function()
                                            end)
                                            TriggerServerEvent("aj-camera:server:add-photo-item", json.encode(image.attachments[1].proxy_url))
                                        end)						
                                    end
                                end)
                            end
...
RegisterNetEvent("aj-camera:client:use-photo", function(url)
