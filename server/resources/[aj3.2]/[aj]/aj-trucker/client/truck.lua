...
Citizen.CreateThread(function()
    ...
    while true do 
        Citizen.Wait(5)
        if isLoggedIn and AJFW ~= nil then
            if PlayerJob.name == "trucker" then
                if IsControlJustReleased(0, Keys["DEL"]) then
                    if IsPedInAnyVehicle(PlayerPedId()) and isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                        getNewLocation()
                        CurrentPlate = AJFW.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId(), false))
                    end
                end
                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        AJFW.Functions.Notify('This is not the commercial vehicle!', 'error')
                                    end
                                else
                                    AJFW.Functions.Notify('You must be a driver..')
                                end
                            else
                                MenuGarage()
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locationstruck["main"].coords.x, Config.Locationstruck["main"].coords.y, Config.Locationstruck["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                ...
                                AJFW.Functions.Notify("You haven't done any work yet..", "error")
                            end
                        end
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 40.0 then
                        if not IsPedInAnyVehicle(PlayerPedId()) then
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == AJFW.Functions.GetPlate(vehicle) then
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        AJFW.Functions.Notify("You went to all the shops. Store Back the vehicle!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
