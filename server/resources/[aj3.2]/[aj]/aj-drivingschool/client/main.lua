...
function StopTheoryTest(success)
	...
	if success then
		...
		AJFW.Functions.Notify("Passed Theory Test , Start your practical test!", "success" , 5000)
		StartDriveTest()
	else
		--helptext(_U('failed_test'))
		AJFW.Functions.Notify("Failed Theory Test", "error")
	end
end
...
function StartDriveTest()
    ...
    AJFW.Functions.SpawnVehicle('blista', function(vehicle)
...
function StopDriveTest(success)
	if success then
		TriggerServerEvent('aj-drivingschool:server:GetLicense')
		AJFW.Functions.Notify("Passed Driving Test", "success")
	else
		--helptext(_U('failed_test'))
		AJFW.Functions.Notify("Failed Driving Test", "error")
	end
...
AddEventHandler('aj-drivingschool:hasEnteredMarker', function(zone)
...
AddEventHandler('aj-drivingschool:hasExitedMarker', function(zone)
	CurrentAction = nil
end)
...
-- 			TriggerEvent('aj-drivingschool:hasEnteredMarker', currentZone)
-- 		end

-- 		if not isInMarker and HasAlreadyEnteredMarker then
-- 			HasAlreadyEnteredMarker = false
-- 			TriggerEvent('aj-drivingschool:hasExitedMarker', LastZone)
-- 		end
-- 	end
-- end)
...
RegisterNetEvent('driving:test', function()
	AJFW.Functions.TriggerCallback('AJFW:HasItem', function(result1)
		if result1 then
			AJFW.Functions.Notify("You Already have License!", "error")
		else
			AJFW.Functions.Notify("You cannot close screen until and unless you give theory test", "error")
			TriggerServerEvent('aj-drivingschool:Payaj')
			StartTheoryTest()
		end
	end, 'driver_license')
end)
	...
	while true do
		...
		if CurrentTest == 'drive' then
			...
			if Config.CheckPoints[nextCheckPoint] == nil then
				...
				AJFW.Functions.Notify("Driving Test Complete", "error")
		...
		if CurrentTest == 'drive' then
			...
			if IsPedInAnyVehicle(playerPed, false) then
				...
				for k,v in pairs(Config.SpeedLimits) do
					if CurrentZoneType == k and speed > v then
						...
						if not IsAboveSpeedLimit then
							...
							AJFW.Functions.Notify("Driving Too Fast", "error")
							AJFW.Functions.Notify(" "..v.." " ,"error")
							AJFW.Functions.Notify("Mistakes - "..DriveErrors.." / "..Config.MaxErrors.." ","error")
						end
					end
				end
				...
				if health < LastVehicleHealth then
					...
					AJFW.Functions.Notify("You damaged veh", "error")
					AJFW.Functions.Notify("Mistakes - "..DriveErrors.." / "..Config.MaxErrors.." ", "error")
