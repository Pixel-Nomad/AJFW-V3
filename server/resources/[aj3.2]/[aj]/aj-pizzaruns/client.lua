...
function PullOutVehicle()
    ...
    if ownsVan == true then
        AJFW.Functions.Notify("You already have a van! Go and collect it or end your job.", "error")
    elseif ownsVan == false then
        coords = Config.Zones.Spawn.Pos
        AJFW.Functions.SpawnVehicle('blazer3', function(veh)
            ...
            plaquevehicule = AJFW.Functions.GetPlate(veh)
        end, coords, true)
        InService = true
        DrawTarget()
        AddCancelBlip()
        ownsVan = true
        TriggerServerEvent("aj-pizzaruns:TakeDeposit")
    end
end
            ...
            if dist <= 2.5 then
                ...
                if dist <= 3.0 then
                    if IsControlJustReleased(0, 38) then
                        local vehNear = AJFW.Functions.CheckFuckingCar(Config.Zones.Spawn.Pos)
                        if vehNear < 5 then
...
function DrawTarget()
    ...
    if DeliveriesCount == 4 then
        AJFW.Functions.Notify("All Pizzas have been delivered","success")
        RemoveCancelBlip()
      ...
      if pizza == 1 then
        AJFW.Functions.Notify("You have one pizza","success")
      else
        if pizza == 4 then
          pizza = 'Four'
        elseif pizza == 3 then
        ...
        AJFW.Functions.Notify("You have "..pizza.." pizzas","success")
      end
        if LastGoal == RandomPoint then
		    ...
		    AJFW.Functions.Notify("Take the Next Pizza to the customer","success")
        end
    end
end
    ...
    if not IsPedInAnyVehicle(player, false) then
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then
            ...
            if HasPizza then
                ...
                HasPizza = true
                AJFW.Functions.Notify("Take the Pizza to the customer","success")
            end
        end
    end
end
...
function DeliverPizza()
    if not PizzaDelivered then
        ...
        HasPizza = false    
        NextDelivery()
        TriggerServerEvent('aj-pizzaruns:items')
        Citizen.Wait(2500)
...
function NextDelivery()
    TriggerServerEvent('aj-pizzaruns:Payment')
    Citizen.Wait(300)
    DrawTarget()
end
-----------------
            ...
            if Delivered then
                if DistanceFromEndZone <= 2.5 then
                    ...
                    if DistanceFromEndZone <= 7 then
                        if IsControlJustReleased(0, 38) then
                            AJFW.Functions.Notify("The work is finished! Rest a while, the next orders are already waiting!", "success")
                            EndOfWork()
                        end
                    end
                end
                ...
                if DistanceFromCancelZone <= 2.5 then
                    ...
                    if DistanceFromCancelZone <= 7 then
                        if IsControlJustReleased(0, 38) then
                            AJFW.Functions.Notify("Orders Canceled!", "error")
							EndOfWork()
                        end
                    end
                end
...
function EndOfWork()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if IsVehicleModel(Van, GetHashKey('blazer3')) then
            AJFW.Functions.DeleteVehicle(Van)
            if Delivered == true then
                TriggerServerEvent("aj-pizzaruns:ReturnDeposit", 'end')
            end
            Hired = false
            InService = false
            BlipSell = nil
            ...
            AJFW.Functions.Notify("You must return to pizza blazer3!", "error")
            AJFW.Functions.Notify("If you lost the blazer3 cancel the job on foot", "error")
        end
    else
        Hired = false
        InService = false
