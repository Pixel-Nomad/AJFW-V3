...
RegisterNetEvent("aj-stancer:airsuspension")
AddEventHandler("aj-stancer:airsuspension", function(vehicle,val,coords)
...
RegisterNUICallback('setvehicleheight', function(data, cb)
    ...
    if vehicle ~= nil and vehicle ~= 0 and not busyairsus then
		print("HEIGHT")
		busyairsus = true
		TriggerServerEvent("aj-stancer:airsuspension",VehToNet(vehicle), data.val, GetEntityCoords(vehicle))
    end
	cb(true)
end)
...
RegisterNetEvent("aj-stancer:openstancer")
AddEventHandler("aj-stancer:openstancer", function(toggle)
	OpenStancer(toggle)
end)
...
function OpenStancer()
	...
	-- 	TriggerServerEvent('aj-stancer:addstancer')
