...
AddEventHandler('AJFW:Client:OnPlayerLoaded', function()
	TriggerServerEvent('aj-communityservice:server:checkIfSentenced')
end)

RegisterNetEvent('aj-communityservice:client:finishCommunityService')
AddEventHandler('aj-communityservice:client:finishCommunityService', function(source)
	communityServiceFinished = true
	isSentenced = false
	actionsRemaining = 0
end)

RegisterNetEvent('aj-communityservice:client:inCommunityService')
AddEventHandler('aj-communityservice:client:inCommunityService', function(actions_remaining)
    ...
    while actionsRemaining > 0 and communityServiceFinished ~= true do
        ...
        if #(pCoords - Config.ServiceLocation) > 45 then
            ...
            TriggerServerEvent('aj-communityservice:server:extendService')
            actionsRemaining = actionsRemaining + Config.ServiceExtensionOnEscape
            AJFW.Functions.Notify('Escape Attempted! '..Config.ServiceExtensionOnEscape..' More Months Added!', 'red')
        end
    end
    TriggerServerEvent('prison:server:GiveJailItems')
    SetEntityCoords(playerPed, Config.ReleaseLocation.x, Config.ReleaseLocation.y, Config.ReleaseLocation.z)
    TriggerServerEvent('aj-communityservice:server:finishCommunityService')
    isSentenced = false
            ...
            for i = 1, #availableActions do
                ...
                if distance < 1.5 then
                    ...
                    if (IsControlJustReleased(1, 38)) then
                        ...
                        TriggerServerEvent('aj-communityservice:server:completeService')
                        ...
                        AJFW.Functions.Progressbar('Com_Serv', 'Doing Community Service','blue', 20000, false, false, { -- Name | Label | Time | useWhileDead | canCancel
