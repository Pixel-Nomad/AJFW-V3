...
AJFW.Commands.Add('comserv', 'Send To Community Service', {{name='id', help='Target ID'}, {name='amount', help='Action Amount'}}, true, function(source, args)
    ...
    if Player.PlayerData.job.type == "leo" then
        ...
        TriggerClientEvent('AJFW:Notify', source, 'You are not a leo', 'error')
    end
end, 'leo')
...
AddEventHandler('aj-communityservice:server:checkIfSentenced', function()
    ...
    local result = MySQL.query.await('SELECT * FROM communityservice WHERE citizenid = ?', {citizenid})
...
AddEventHandler('aj-communityservice:server:completeService', function()
    local src = source
    local citizenid = AJFW.Functions.GetPlayer(src).PlayerData.citizenid
    MySQL.query('UPDATE communityservice SET actions_remaining = actions_remaining - 1 WHERE citizenid = ?', {citizenid})
end)
...
AddEventHandler('aj-communityservice:server:extendService', function()
    local src = source
    local citizenid = AJFW.Functions.GetPlayer(src).PlayerData.citizenid
    MySQL.query('UPDATE communityservice SET actions_remaining = actions_remaining + ? WHERE citizenid = ?', {citizenid, Config.ServiceExtensionOnEscape})
end)
...
AddEventHandler('aj-communityservice:server:sendToCommunityService', function(target, actions_count)
    ...
    MySQL.insert('INSERT INTO communityservice (citizenid, actions_remaining) VALUES (?, ?) ON DUPLICATE KEY UPDATE actions_remaining = ?', {citizenid, actions_count, actions_count})
...
function Release(target)
    ...
    MySQL.query('DELETE FROM communityservice WHERE citizenid = ?', {citizenid})
