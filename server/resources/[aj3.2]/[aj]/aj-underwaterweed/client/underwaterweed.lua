...
AddEventHandler("AJFW:Client:OnPlayerLoaded", function()
	...
	if GetDistanceBetweenCoords(coords, Config.CircleZones3.WeedField.coords, true) < 2000 then
		SpawnWeedPlants()
	end--]]
end)
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones3.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				AJFW.Functions.TriggerCallback('aj-underwaterweed:ingredient', function(HasItem, type)
					if HasItem then
						ProcessWeed()
					else
...
function ProcessWeed()
	...
	AJFW.Functions.Progressbar("search_register", "Processing..",'red', 15000, false, true, {
		...
		AJFW.Functions.TriggerCallback('aj-underwaterweed:ingredient', function(HasItem, type)
			if HasItem then	
				TriggerServerEvent('aj-underwaterweed:processWeed')
			else
		...
		while timeLeft > 0 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones3.WeedProcessing.coords, false) > 4 then
				TriggerServerEvent('aj-underwaterweed:cancelProcessing')
				break
			end
		end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				AJFW.Functions.Notify("रूको ज़रा सबर करो", "error", 10000)
				AJFW.Functions.Progressbar("search_register", "Picking up..",'red', 5000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					...
					TriggerServerEvent('aj-underwaterweed:pickedUpWeed')
					TriggerServerEvent('aj-underwaterweed:weed')
					ClearPedTasks(PlayerPedId())
