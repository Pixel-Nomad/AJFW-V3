...
Citizen.CreateThread(function()
    ...
    while true do 
        Citizen.Wait(1)
        if LocalPlayer.state['isLoggedIn'] and AJFW ~= nil then
            ...
            if PlayerJob.name == "reporter" then
                ...
                if #(pos - vector3(Config.Locations["stash"].coords.x, Config.Locations["stash"].coords.y, Config.Locations["stash"].coords.z)) < 10.0 then
                    ...
                    if #(pos - vector3(Config.Locations["stash"].coords.x, Config.Locations["stash"].coords.y, Config.Locations["stash"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "reporterstash_"..AJFW.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "reporterstash_"..AJFW.Functions.GetPlayerData().citizenid)
                            end
                        end
                    else
                        if showText then
