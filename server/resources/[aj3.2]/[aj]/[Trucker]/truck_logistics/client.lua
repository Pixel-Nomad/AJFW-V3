...
-- 			if not menuactive and distance <= 15.0 and isLoggedIn then
-- 				timer = 1
...
AddEventHandler('onResourceStop', function(resource)
   if resource == GetCurrentResourceName() then
	exports['aj-target']:RemoveZone("TRUCKING-UI")
   end
end)

exports['aj-target']:AddBoxZone("TRUCKING-UI", vector3(1209.27, -3115.1, 6.63), 0.4, 2.8, {
    name="hayesstash",
    heading=90,
    debugPoly=false,
    minZ=5.33,
    maxZ=5.83,
        ...
        options = {
            {
                type = "client",
                event = "truck_logistics:openEYE",
                icon = "fas fa-hammer",
                label = "OPEN TRUCKING UI",
                job = all,
            },
        },
        distance = 1.5
...
RegisterNetEvent('truck_logistics:openEYE',function()
	for k,mark in pairs(Config.empresas) do
		if not menuactive and isLoggedIn then
			empresaAtual = k
			TriggerServerEvent("truck_logistics:getData")
		end
	end
...
RegisterNUICallback('startJob', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			AJFW.Functions.Notify(Lang[Config.lang]['loading_trailer'], "primary")
			closeUI()
		end
...
RegisterNUICallback('repairTruck', function(data, cb)
	if truck and not rentTruck then
		AJFW.Functions.Notify(Lang[Config.lang]['store_truck'], "error")
		return
	end
...
RegisterNUICallback('sellTruck', function(data, cb)
	if cooldown == nil then
		...
		if truck and not rentTruck then
			AJFW.Functions.Notify(Lang[Config.lang]['store_truck_2'], "error")
			return
		end
...
RegisterNUICallback('spawnTruck', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			AJFW.Functions.Notify(Lang[Config.lang]['loading_truck'], "primary")
			closeUI()
		end
...
AddEventHandler('truck_logistics:spawnTruck', function(truck_data)
	...
	if truck then
		AJFW.Functions.Notify(Lang[Config.lang]['already_has_truck'], "error")
		return
	end
	...
	while i > 0 do
		...
		if checkPos == false then
			if i <= 1 then
				AJFW.Functions.Notify(Lang[Config.lang]['occupied_places'], "error")
				loading = false
				return
			end
		else
	...
	AJFW.Functions.Notify(Lang[Config.lang]['already_is_in_garage'], "success")
	loading = false
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(truck), 'body', 1000.0)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(truck), 'engine', 1000.0)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(truck), 'fuel', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(truck), 'clutch', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(truck), 'brakes', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(truck), 'axle', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(truck), 'radiator', 100)
	local timer = 5
	local engineH = 1000
...
AddEventHandler('truck_logistics:startContract', function(data,job_distance,reward,truck_data)
	...
	if trailer or rentTruck then
		AJFW.Functions.Notify(Lang[Config.lang]['already_has_cargo'], "error")
		return
	end
	...
	if truck and data.contract_type == 0 then
		AJFW.Functions.Notify(Lang[Config.lang]['must_store_truck'], "error")
		return
	end
	...
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(trailer), 'body', 1000.0)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(trailer), 'engine', 1000.0)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(trailer), 'fuel', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(trailer), 'clutch', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(trailer), 'brakes', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(trailer), 'axle', 100)
	exports['aj-mechanicjob']:SetVehicleStatus(AJFW.Functions.GetPlate(trailer), 'radiator', 100)
	AJFW.Functions.Notify(Lang[Config.lang]['started_job'], "success")
	loading = false
	...
	while IsEntityAVehicle(trailer) do 
		...
		if distance <= 50.0 then
			...
			if distance <= 4.0 and veh == truck and GetEntityHeading(truck) + 10 >= h and GetEntityHeading(truck) - 10 <= h and GetEntityHeading(trailer) + 10 >= h and GetEntityHeading(trailer) - 10 <= h and IsEntityAttachedToEntity(truck,trailer) then
				...
				if IsControlJustPressed(0,38) then
					...
					-- end)
					AJFW.Functions.Notify('Cargo delivered Successfully', 'success', 7000, 'left')
					trailer = nil
					trailer_blip = nil
					route_blip = nil
		...
		if bodyhealth <= 150 then
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			AJFW.Functions.Notify(Lang[Config.lang]['failed'], "error")
...
function resetLoading()
	...
	if loading == true then
		AJFW.Functions.Notify(Lang[Config.lang]['loading_fail'], "error")
		loading = false
	end
end
...
AddEventHandler('truck_logistics:vehicleClientLock', function()
	...
	if DoesEntityExist(v) and IsEntityAVehicle(v) then
		...
		if lock == 1 then
			...
			AJFW.Functions.Notify(Lang[Config.lang]['locked'], "primary", 8000)
		else
			SetVehicleDoorsLocked(v,1)
			AJFW.Functions.Notify(Lang[Config.lang]['unlocked'], "primary", 8000)
		end
