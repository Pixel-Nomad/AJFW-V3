...
Citizen.CreateThread(function()
	...
	while true do 
		...
		if contract_type == 1 then truck = nil end
			...
			MySQL.query("SELECT COUNT(driver_id) as qtd FROM trucker_drivers WHERE user_id IS NULL", function (result)
				
				local count = result[1].qtd;
				
				if count >= Config.motoristas.max_motoristas_ativos then
					MySQL.query("SELECT MIN(driver_id) as min FROM trucker_drivers WHERE user_id IS NULL", function(sql)
					local min = sql[1].min;
					
				
					MySQL.query("DELETE FROM `trucker_drivers` WHERE driver_id = @driver_id;", {['@driver_id'] = min});
					end)
				end
			end)
		MySQL.insert("INSERT INTO `trucker_drivers` (user_id, name, product_type, distance, fragile, valuable, fast, price, price_per_km, img) VALUES (NULL, @name, @product_type, @distance, @fragile, @valuable, @fast, @price, @price_per_km, @img);", {['@name'] = name, ['@product_type'] = product_type, ['@distance'] = distance, ['@fragile'] = fragile, ['@valuable'] = valuable, ['@fast'] = fast, ['@price'] = price, ['@price_per_km'] = price_per_km, ['@img'] = driver.img});
...
Citizen.CreateThread(function()
	Citizen.Wait(10000)
	while true do 
		MySQL.query("SELECT * FROM `trucker_drivers` ", function(result)

			local data = result
			for k,v in pairs(data) do
				local source = AJFW.Functions.GetPlayerByCitizenId(v.user_id)
				if Config.trabalhos.gera_dinheiro_offline or source then
					if tryGetTruckerMoney(v.user_id,v.price + v.price_per_km) then
						...
						MySQL.query("UPDATE `trucker_drivers` SET user_id = NULL WHERE driver_id = @driver_id", {['@driver_id'] = v.driver_id});
						
						MySQL.query("UPDATE `trucker_trucks` SET driver = NULL WHERE driver = @driver_id", {['@driver_id'] = v.driver_id});
						if source then
							TriggerClientEvent("AJFW:Notify", source, Lang[Config.lang]['driver_failed']:format(v.name), "error")
						end
					end
...
Citizen.CreateThread(function()
	Citizen.Wait(10000)
	while true do
		MySQL.query("SELECT * FROM trucker_loans", function(data)
			for k,v in pairs(data) do
				if v.timer + Config.emprestimos.cooldown < os.time() then
					local source = AJFW.Functions.GetPlayerByCitizenId(tonumber(v.user_id))
					if tryGetTruckerMoney(v.user_id,v.day_cost) then
						local new_loan = v.remaining_amount - v.taxes_on_day
						if new_loan > 0 then
							MySQL.query("UPDATE `trucker_loans` SET remaining_amount = @remaining_amount, timer = @timer WHERE id = @id", {['remaining_amount'] = new_loan, ['timer'] = os.time(), ['@id'] = v.id});
						else
							MySQL.query("DELETE FROM `trucker_loans` WHERE id = @id;", {['@id'] = v.id});
						end
					else
						if source then
							TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['no_loan_money'], "primary")
						else
							MySQL.query("UPDATE `trucker_users` SET loan_notify = 1 WHERE user_id = @user_id", {['@user_id'] = v.user_id});
						end
					end
					if source then
						if isOpen[source] then
...
AddEventHandler("truck_logistics:startContract",function(data)
	...
	if debug_cooldown[source] == nil then
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			MySQL.query("SELECT * FROM `trucker_available_contracts` WHERE contract_id = @id",{['@id'] = id}, function(query)
				if query and query[1] then
					MySQL.query("SELECT * FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(query_users)
										...
										if tonumber(query_users[1].fast) >= tonumber(query[1].fast) then
											if Config.habilidade_distancia[tonumber(query_users[1].distance)] >= tonumber(distance) then
												if tonumber(query[1].contract_type) == 0 then
													...
													MySQL.query("SELECT * FROM `trucker_trucks` WHERE driver = 0 AND user_id = @user_id", {['@user_id'] = user_id}, function(query_truck)
														if query_truck and query_truck[1] then
															...
															TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['own_truck'], "error")
														end
													end)
												end
											else
												TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['no_skill_1'], "error")
											end
										else
											TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['no_skill_2'], "error")
										end
									else
										TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['no_skill_3'], "error")
									end
								else
									TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['no_skill_4'], "error")
								end
							else
								TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['no_skill_5'], "error")
							end
						end
					end)
				else
					TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['job_already_started'], "error")
				end
			end)
		end
...
AddEventHandler("truck_logistics:spawnTruck",function(truck_id)
	...
	if debug_cooldown[source] == nil then
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			MySQL.query("SELECT * FROM `trucker_trucks` WHERE truck_id = @truck_id" ,{['@truck_id'] = tonumber(truck_id)}, function(result)
...
AddEventHandler("truck_logistics:upgradeSkill",function(data)
	...
	local user_id = xPlayer.PlayerData.citizenid
	if user_id then
		MySQL.query("SELECT * FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(result)
			local query = result[1];
			if query.skill_points >= (data.value - query[data.id]) then
				MySQL.query("UPDATE `trucker_users` SET "..data.id.." = @value, skill_points = @skill_points WHERE user_id = @user_id", {['@user_id'] = user_id, ['@value'] = data.value, ['@skill_points'] = (query.skill_points - (data.value - query[data.id]))});
				TriggerClientEvent("AJFW:Notify", source, Lang[Config.lang]['upgraded_skill'], "success")
				openUI(source,true)
			else
				TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['insufficient_skill_points'], "error")
			end
		end)
	end
end)
...
AddEventHandler("truck_logistics:repairTruck",function(item)
	...
	local user_id = xPlayer.PlayerData.citizenid
	if user_id then
		MySQL.query("SELECT * FROM `trucker_trucks` WHERE user_id = @user_id AND driver = 0", {['@user_id'] = user_id}, function(result)
			local query = result[1];
			if query then
				local amount = math.floor((100-(tonumber(query[item])/10)) * Config.valor_reparo[item])
				if amount > 0 then
					if tryGetTruckerMoney(user_id,amount) then
						MySQL.query("UPDATE `trucker_trucks` SET "..item.." = 1000 WHERE user_id = @user_id AND driver = 0", {['@user_id'] = user_id});
						TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['repaired'], "success")
						openUI(source,true)
					else
						TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['insufficiente_funds'], "error")
					end
				else
					TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['not_repaired'], "error")
				end
			else
				TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['have_no_truck'], "error")
			end
		end)
	end
end)
...
AddEventHandler("truck_logistics:buyTruck",function(data)
	...
	if user_id then
		if tryGetTruckerMoney(user_id,tonumber(data.price)) then
			MySQL.insert("INSERT INTO `trucker_trucks` (user_id, truck_name, driver) VALUES (@user_id, @name, NULL);", {['@user_id'] = user_id, ['@name'] = data.truck_name});
			TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['bought'], "success")
...
AddEventHandler("truck_logistics:sellTruck",function(data)
	...
	if debug_cooldown[source] == nil then
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			MySQL.query("SELECT * FROM `trucker_trucks` WHERE truck_id = @truck_id", {['@truck_id'] = data.truck_id}, function(result)
				local query = result[1];
				if query then 
					MySQL.query("DELETE FROM `trucker_trucks` WHERE truck_id = @truck_id;", {['@truck_id'] = data.truck_id});
					...
					TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['sold'], "success")
...
AddEventHandler("truck_logistics:hireDriver",function(driver_id)
	...
	local user_id = xPlayer.PlayerData.citizenid
	if user_id then
		MySQL.query("SELECT COUNT(driver_id) as qtd FROM trucker_drivers WHERE user_id = @user_id", {['@user_id'] = user_id}, function(result)
			local count = result[1].qtd;
		
			if count <= Config.motoristas.max_motoristas_por_player then
				MySQL.query("SELECT price FROM trucker_drivers WHERE driver_id = @driver_id", {['@driver_id'] = driver_id}, function(result2)
					local query = result2[1];
					if tryGetTruckerMoney(user_id,query.price) then
						MySQL.query("UPDATE `trucker_drivers` SET user_id = @user_id WHERE driver_id = @driver_id", {['@user_id'] = user_id, ['@driver_id'] = driver_id});
						TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['hired'], "success")
						openUI(source,true)
					else
						TriggerClientEvent("AJFW:Notify",source,"Dinero insuficiente", "error")
					end
				end)
			else
				TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['max_drivers'], "error")
			end
		end)
	end
end)
...
AddEventHandler("truck_logistics:fireDriver",function(driver_id)
	...
	local user_id = xPlayer.PlayerData.citizenid
	if user_id then
		MySQL.query("UPDATE `trucker_drivers` SET user_id = NULL WHERE driver_id = @driver_id", {['@driver_id'] = driver_id});
		MySQL.query("UPDATE `trucker_trucks` SET driver = NULL WHERE driver = @driver_id", {['@driver_id'] = driver_id});
		TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['fired'], "success")
		openUI(source,true)
	end
end)
...
AddEventHandler("truck_logistics:setDriver",function(data)
	...
	if user_id then
		if tonumber(data.driver_id) ~= 0 then
			MySQL.query("UPDATE `trucker_trucks` SET driver = NULL WHERE driver = @driver_id", {['@driver_id'] = data.driver_id});
		end
		MySQL.query("UPDATE `trucker_trucks` SET driver = @driver_id WHERE truck_id = @truck_id", {['@driver_id'] = data.driver_id, ['@truck_id'] = data.truck_id});
		openUI(source,true)
	end
end)
...
AddEventHandler("truck_logistics:withdrawMoney",function()
	...
	if debug_cooldown[source] == nil then
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			MySQL.query("SELECT * FROM `trucker_loans` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(sql)
				local query = sql[1];
				if not query or not query.remaining_amount or query.remaining_amount <= 0 then
					MySQL.query("SELECT money FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(sql2)
						local query = sql2[1];
						...
						if amount and amount > 0 then
							...
							exports['aj-Banking']:handleTransaction(
								...
								amount, 
								'Trucking Bank Withdraw', 
								name, 
								xPlayer.PlayerData.citizenid, 
								"deposit"
							)
							TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['money_withdrawn'], "success")
							openUI(source,true)
						else
							TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['insufficiente_money'], "error")
						end
					end)
				else
					TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['pay_loans'], "error")
				end
			end)
		end
		debug_cooldown[source] = nil
		...
		if amount and amount > 0 then
			...
			if money >= amount then
				...
				exports['aj-Banking']:handleTransaction(
					...
					"Personal Account / " .. xPlayer.PlayerData.citizenid, 
					amount, 
					'Trucking Bank Deposit', 
					xPlayer.PlayerData.citizenid, 
					name, 
					"withdraw"
				)
				giveTruckerMoney(user_id,amount)
				TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['money_deposited'], "success")
				openUI(source,true)
			else
				TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['insufficiente_money'], "error")
			end
		else
			TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['invalid_value'], "error")
		end
	end
end)
...
AddEventHandler("truck_logistics:loan",function(data)
	...
	if debug_cooldown[source] == nil then
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			MySQL.query("SELECT * FROM `trucker_loans` WHERE user_id = @user_id", {['@user_id'] = user_id}, function (result)
			local query = result
			...
			if amount_loans + Config.emprestimos.valores[data.loan_id][1] <= getMaxEmprestimo(user_id) then
				...
				MySQL.insert(sql, {['@user_id'] = user_id, ['@loan'] = Config.emprestimos.valores[data.loan_id][1], ['@remaining_amount'] = Config.emprestimos.valores[data.loan_id][1], ['@day_cost'] = Config.emprestimos.valores[data.loan_id][2], ['@taxes_on_day'] = Config.emprestimos.valores[data.loan_id][3]});
				giveTruckerMoney(user_id,Config.emprestimos.valores[data.loan_id][1])
				TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['loan'], "success")
				openUI(source,true)
			else
				TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['no_loan'], "error")
			end
		end)
		end
		debug_cooldown[source] = nil
...
AddEventHandler("truck_logistics:payLoan",function(data)
	...
	local user_id = xPlayer.PlayerData.citizenid
	if user_id then
		MySQL.query("SELECT * FROM `trucker_loans` WHERE id = @id",{['@id'] = data.loan_id}, function(result)
			local query = result[1]
				if tryGetTruckerMoney(user_id, query.remaining_amount) then
					...
					TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['loan_paid'], "success")
					openUI(source,true)
				else
					TriggerClientEvent("AJFW:Notify",source,Lang[Config.lang]['insufficiente_funds'], "error")
				end
...
AddEventHandler("truck_logistics:finishJob",function(data,distance,reward,truck_data,truck_engine,truck_body,trailer_body)
	...
	if user_id then
		...
		MySQL.query(sql, {['@reward'] = received_amount, ['@distance'] = tonumber(string.format("%.2f", distance)), ['@exp_amount'] = received_xp, ['@user_id'] = user_id});

		giveTruckerMoney(user_id,received_amount)
		TriggerClientEvent("AJFW:Notify",source, Lang[Config.lang]['reward']:format(tostring(received_amount),tostring(trailer_body*100),tostring(received_xp)), "success")
...
function tryGetTruckerMoney(user_id,amount)
	local queryMoney
	MySQL.query("SELECT money FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(result)
		...
		if tonumber(queryMoney.money) >= amount then
			...
			MySQL.query(sql, {['@amount'] = (tonumber(queryMoney.money) - amount), ['@user_id'] = user_id});
			return true
		else
			return false
		end
...
function getPlayerLevel(user_id)
	local queryLevel
	MySQL.query("SELECT exp FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function (result)
...
function openUI(source, reset)
	...
	if xPlayer then
		...
		if user_id then
			...
			users_data = MySQL.query(sql,{}, function(users_data)
            query.trucker_users = users_data[1] or nil
				if query.trucker_users == nil then
					...
					MySQL.insert(sql, {['@user_id'] = user_id});
					MySQL.query("SELECT * FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(sql2)
			...
			-- Busca os contratos
			MySQL.query("SELECT * FROM `trucker_available_contracts`", function(result)
			query.trucker_available_contracts = result
			
			-- Busca os caminhões
			MySQL.query("SELECT * FROM `trucker_trucks` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(result2);
			query.trucker_trucks = result2
			
			-- Busca os motoristas
			MySQL.query("SELECT * FROM `trucker_drivers` WHERE user_id = @user_id OR user_id IS NULL", {['@user_id'] = user_id}, function(result3)
			query.trucker_drivers = result3
			
			-- Busca os emprestimos
			MySQL.query("SELECT * FROM `trucker_loans` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(result4)
