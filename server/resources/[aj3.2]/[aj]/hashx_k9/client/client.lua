...
local function K9AttackorFollow(target)
  ...
  if target then
    ...
    following = false
    AJFW.Functions.Notify(k9_name.." is attacking!", "error", 2000)
  else
    TaskFollowToOffsetOfEntity(DOG, PlayerPedId(), 0.5, -1.0, 0.0, 5.0, -1, 1.0, true);
    following = true
    AJFW.Functions.Notify(k9_name.." is following.", "primary", 2000)
  end
end
...
local function K9Found(status, type)
  ...
  Wait(time)
  if status then
    AJFW.Functions.Notify(Lang:t("success.k9_alert"), "success", 4000)
    searching = false
    ...
    AJFW.Functions.Notify(Lang:t("error.k9_alert"), "error", 4000)
    following = true
  end
end
...
local function K9ToggleVehicle(target)
  ...
  if #(VEHICLE_COORDS - DOG_COORDS) < 5 or target then
    ...
    AJFW.Functions.Notify(k9_name..Lang:t("error.k9_toofar"), "error", 4000)
  end
end
...
local function K9SearchPerson()
    ...
    if TARGET > 0 then
      ...
      AJFW.Functions.Notify(k9_name..Lang:t("error.k9_locaterror"), "error", 4500)
    end
...
local function K9SearchVehicle()
  ...
  if VEHICLE then
    AJFW.Functions.Notify(k9_name..Lang:t("info.k9_searching"), "success", 4500)
    ...
    searching = false
      ...
      AJFW.Functions.Notify(Lang:t("error.k9_vehlocate"), "error", 4000)
  end
end
...
local function K9SearchArea()
  ...
  for i = 1, #PLAYERS do
    following = false
    AJFW.Functions.Notify(k9_name..Lang:t("info.k9_scent"), "primary", 4000)
    local DOG = NetworkGetEntityFromNetworkId(k9_id)
    local DOG_COORDS = GetEntityCoords(DOG)
    ...
    if following then
      AJFW.Functions.Notify(k9_name..Lang:t("info.k9_nofollow"), "primary", 4000)
      break
    end
    AJFW.Functions.Notify(k9_name..Lang:t("info.k9_lostscent"), "primary", 4000)
    K9AttackorFollow(false)
    Wait(2000)
  end
...
CreateThread(function()
  exports['aj-target']:AddBoxZone("K9", vector3(445.77, -999.55, 30.72), 1.5, 1.6, {
    name = "K9", 
    heading = 0, 
    debugPoly = false, 
    minZ = 29.70, 
...
RegisterNetEvent('qb-k9:client:ReturnDoggo', function() 
  if k9_id then
    ...
    AJFW.Functions.Notify(Lang:t("error.k9_returnerror"), "error", 4500)
  end
end)
...
exports['aj-framework']:CreateBind('caninecommanders', nil,  'Police: K9 COMMANDS POLICE ONLY', '', function()
  if activate_k9 then
    local k9Commands = {
      {
    ...
    exports['aj-menu']:openMenu(k9Commands)
  end
end)


exports['aj-framework']:CreateBind('cannineattackfollow', nil, 'Police: K9 COMMANDS POLICE ONLY', '', function()
  if activate_k9 then
