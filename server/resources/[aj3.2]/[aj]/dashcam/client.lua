...
exports['aj-framework']:CreateBind("dashcam", nil, "Vehicle: Vehicle Dashcam", "I", function()
...
function EnableDash()
    ...
    if DashcamConfig.RestrictVehicles then
        local active, typo = CheckVehicleRestriction() 
        if active then
...
function UpdateDashcam()
    ...
    local gameTime = month..day..hour..minute..GetClockHours()..GetClockMinutes()..GetClockSeconds()
...
function CheckVehicleRestriction()
    if DashcamConfig.RestrictionType == "custom" then
        for k,v in pairs(DashcamConfig.AllowedVehicles) do
            if GetHashKey(k) == GetEntityModel(attachedVehicle) then
                return true,v
            end
        end
        return false
