const hackerphone = Vue.createApp({
    data() {
        return {
            lang: AJHackerPhone.Lang,
            cams: AJHackerPhone.Cams,
            carlists: [],
            userlists: [],
            togglephone: false,
            togglelock: false,
            ...
            campage: true,
            hackblackout: false,
            timer: AJHackerPhone.BlackoutSeconds,
            interval: null,
            timerready : false,
            error: false,
            title: "",
    ...
    methods: {
	   ...
	   fetch(`https://${GetParentResourceName()}/${url}`, {
        ...
        blackoutButton: function(post) {
            ...
            this.interval = setInterval(() => {
                if (this.timer === 0) {
                    ...
                    this.postMessage(post)
                    this.timer = AJHackerPhone.BlackoutSeconds
        ...
        keyupHandler: function(e) {
            if (e.key == AJHackerPhone.CloseKey) {
                this.phoneanim = false
                this.postMessage('aj-hackerphone:nuicallback:escape')
                setTimeout(() => {
