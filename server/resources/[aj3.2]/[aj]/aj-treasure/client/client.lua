...
RegisterNetEvent('aj-treasurehunt:detect', function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then
      return AJFW.Functions.Notify('You cannot use the metal detector whilst in a vehicle', 'error')
    end
    if not isDetecting then
        isDetecting = true
        if inZone == 1 then
            AJFW.Functions.Progressbar('InZone', 'Searching the area...', math.random(7000), false, true, {
                ...
                AJFW.Functions.RequestAnimDict('anim@treasurehunt@hatchet@action')
                ...
                DeleteEntity(objman)
                destroyZone()
                TriggerServerEvent('aj-treasurehunt:AddItems')
            end)
        else
            AJFW.Functions.Progressbar('OutZone', 'Searching the area...', math.random(5000, 10000), false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                AJFW.Functions.Notify("You found nothing, maybe try somewhere else.", "error")
            end)
            isDetecting = false
        end
    else
        AJFW.Functions.Notify('You are already using the metal detector', 'error')
    end
end)
...
RegisterNetEvent('aj-treasurehunt:usemap', function()
    if not ZoneExists then
        AJFW.Functions.Progressbar('UseMap', 'Looking at the map....', 10000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent("aj-treasurehunt:removemap")
            ZoneExists = true
            setblip(setlocation)
        end)
    else
        AJFW.Functions.Notify('You have already used a map!', 'error')
    end
end)


RegisterNetEvent('aj-treasurehunt:sellitems', function()
    exports['aj-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = "aj-treasurehunt:SellEmerald",
                args = 1
            }
        },
        {
            ...
            params = {
                isServer = true,
                event = "aj-treasurehunt:SellDiamond",
                args = 2
            }
        },
        {
            ...
            params = {
                isServer = true,
                event = "aj-treasurehunt:SellGold",
                args = 3
            }
        }
    })
