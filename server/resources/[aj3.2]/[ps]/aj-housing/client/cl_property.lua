...
function Property:RegisterPropertyEntrance()
    ...
    local function enter()
        TriggerServerEvent("aj-housing:server:enterProperty", self.property_id)
    end

    local function raid()
        TriggerServerEvent("aj-housing:server:raidProperty", self.property_id)
    end

    local function showcase()
        TriggerServerEvent("aj-housing:server:showcaseProperty", self.property_id)
    end

    local function showData()
        local data = lib.callback.await("aj-housing:cb:getPropertyInfo", source, self.property_id)
...
function Property:RegisterGarageZone()
    ...
    TriggerEvent("aj-garages:client:addHouseGarage", self.property_id, data)
    ...
    self.garageZone = lib.zones.box({
        ...
        onEnter = function()
            TriggerEvent('aj-garages:client:setHouseGarage', self.property_id, true)
        end,
    })
end
...
function Property:UnregisterGarageZone()
    ...
    TriggerEvent("aj-garages:client:removeHouseGarage", self.property_id)
...
function Property:LeaveShell()
    ...
    TriggerServerEvent("aj-housing:server:leaveProperty", self.property_id)
...
function Property:GiveMenus()
    ...
    if self.owner or accessAndConfig then
        Framework[Config.Radial].AddRadialOption(
            ...
            "aj-housing:client:openFurnitureMenu",
    ...
    if self.owner then
        Framework[Config.Radial].AddRadialOption(
            ...
            "aj-housing:client:openManagePropertyAccessMenu",
            { propertyId = self.property_id }
        )
    end
end
...
function Property:GiveAccessMenu()
    ...
    local players = lib.callback.await("aj-housing:cb:getPlayersInProperty", source, self.property_id) or {}
    ...
    if #players > 0 then
        for i = 1, #players do
            ...
            menu.options[#menu.options + 1] = {
                ...
                onSelect = function()
                    TriggerServerEvent("aj-housing:server:addAccess", self.property_id, v.src)
                end,
            }
        end
...
function Property:RevokeAccessMenu()
    ...
    local playersWithAccess = lib.callback.await("aj-housing:cb:getPlayersWithAccess", source, self.property_id) or {}
    ...
    if #playersWithAccess > 0 then
        for i = 1, #playersWithAccess do
            ...
            alreadyAccessMenu.options[#alreadyAccessMenu.options + 1] = {
                ...
                onSelect = function()
                    TriggerServerEvent("aj-housing:server:removeAccess", self.property_id, v.citizenid)
                end,
            }
        end
...
function Property:OpenDoorbellMenu()
    ...
    for k, v in pairs(self.doorbellPool) do
        menu.options[#menu.options + 1] = {
            ...
            onSelect = function()
                TriggerServerEvent(
                    "aj-housing:server:doorbellAnswer",
...
function Property:LoadFurnitures()
    self.propertyData.furnitures = lib.callback.await('aj-housing:cb:getFurnitures', source, self.property_id) or {}
...
function Property:UpdateApartment(newApartment)
    ...
    TriggerEvent("aj-housing:client:updateApartment", oldAptName, newApartment)
end
...
RegisterNetEvent("aj-housing:client:enterProperty", function(property_id)
    local property = Property.Get(property_id)
    property:EnterShell()
end)

RegisterNetEvent("aj-housing:client:updateDoorbellPool", function(property_id, data)
    local property = Property.Get(property_id)
    property.doorbellPool = data
end)

RegisterNetEvent("aj-housing:client:updateFurniture", function(property_id, furnitures)
    local property = Property.Get(property_id)
    if not property then return end
    property:UpdateFurnitures(furnitures)
end)

RegisterNetEvent("aj-housing:client:updateProperty", function(type, property_id, data)
    local property = Property.Get(property_id)
    ...
    TriggerEvent("aj-housing:client:updatedProperty", property_id)
end)

RegisterNetEvent("aj-housing:client:openFurnitureMenu", function(data)
    Modeler:OpenMenu(data.options.propertyId)
end)

RegisterNetEvent("aj-housing:client:openManagePropertyAccessMenu", function(data)
