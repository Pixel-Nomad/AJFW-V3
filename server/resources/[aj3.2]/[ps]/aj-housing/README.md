...
- Provides support for housing and apartments and is a full replacement for aj-apartments and aj-housing
  - When a player first spawns after enabling ps-housing, they will have to choose an apartment. Once they spawn in the stashitems from their previous aj-apartment will be migrated to their new apartment stash.
...
### 1. Find the following events in `aj-multicharacter` and change in server/main.lua event to: 

`aj-multicharacter > server > main.lua`
```lua
RegisterNetEvent('aj-multicharacter:server:loadUserData', function(cData)
    local src = source
    if AJFW.Player.Login(src, cData.citizenid) then
        ...
        if Config.SkipSelection then
            ...
            TriggerClientEvent('aj-multicharacter:client:spawnLastLocation', src, coords, cData)
        else
            TriggerClientEvent('ps-housing:client:setupSpawnUI', src, cData)
        end
        TriggerEvent("aj-log:server:CreateLog", "joinleave", "Loaded", "green", "**" .. GetPlayerName(src) .. "** - " .. Name .. " (<@" .. (AJFW.Functions.GetIdentifier(src, 'discord'):gsub("discord:", "") or "unknown") .. "> | ||" .. (AJFW.Functions.GetIdentifier(src, 'ip') or 'undefined') .. "|| | " .. (AJFW.Functions.GetIdentifier(src, 'license') or 'undefined') .. " | " .. cData.citizenid .. " | " .. src .. ") loaded..")
    end
end)
```
...
`aj-multicharacter > server > main.lua`
```lua
RegisterNetEvent('aj-multicharacter:server:createCharacter', function(data)
    local src = source
    local newData = {}
    newData.cid = data.cid
    newData.charinfo = data
    if AJFW.Player.Login(src, false, newData) then
        repeat
            Wait(10)
        until hasDonePreloading[src]
        print('^2[aj-base]^7 '..GetPlayerName(src)..' has succesfully loaded!')
        AJFW.Commands.Refresh(src)
        TriggerClientEvent("aj-multicharacter:client:closeNUI", src)
        newData.citizenid = AJFW.Functions.GetPlayer(src).PlayerData.citizenid
...
```
### 2. Find the following event in `aj-multicharacter` and change in client/main.lua event to: 
`aj-multicharacter > client > main.lua`

```lua
RegisterNetEvent('aj-multicharacter:client:spawnLastLocation', function(coords, cData)
...
### 3. Find the following events in `aj-spawn` and change in client/client.lua event to: 

`aj-spawn > client.lua > line 51 > 'aj-spawn:client:setupSpawns' event`
```lua
...
`aj-spawn > client.lua > line 134 > 'chooseAppa' NUI Callback`
```lua
...
`aj-spawn > client > client.lua > line 169 'spawnplayer' NUI Callback`
```lua
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    if type == "current" then
        PreSpawnPlayer()
        AJFW.Functions.GetPlayerData(function(pd)
            ped = PlayerPedId()
...
### 4. Find the following events in `aj-garages` and change: 
`aj-garages > server > main.lua > around line 120` on event `aj-garage:server:checkOwnership`

Replace 
```lua
local hasHouseKey = exports['aj-houses']:hasKey(result[1].license, result[1].citizenid, house)
```
With
```lua
...
`aj-garages > client > main.lua > around line 451` add under event `aj-garages:client:addHouseGarage`
```lua
...
### For aj-garages v2: 
`aj-garages > server > main.lua > around line 118` on event `aj-garages:server:canDeposit`

Replace 
```lua
if type == 'house' and not exports['aj-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.HouseGarages[garage].label) then
```
With
```lua
...
`aj-garages > client > main.lua > around line 392` add under event `aj-garages:client:addHouseGarage`
```lua
RegisterNetEvent('aj-garages:client:removeHouseGarage', function(house)
...
### 6. Delete default [aj-apartments](https://github.com/AJFW-framework/aj-apartments)

### 7. Delete default [aj-houses](https://github.com/AJFW-framework/aj-houses)

### 8. Delete `aj-apartments/config.lua` references in `aj-spawn`, `aj-multicharacter` and `aj-phone` fxmanifest.lua (and any other scripts that may reference it).
...
4. [ox_target](https://github.com/overextended/ox_target) or [aj-target](https://github.com/AJFW-framework/aj-target) - Change in [Config](https://github.com/Project-Sloth/ps-housing/blob/3c0f197b6d639f13235598393c84aac8d23d5f7a/shared/config.lua#L8), default is aj-target.
...
# Migrating houses and apartments from aj-houses and aj-apartments

1. From a client run the `migratehouses` command to automatically convert all houses from aj-houses. It will print a message to the console once complete.
   **The `migratehouses` command MUST be run from a client in order to retrieve street and region data for each house**

2. From a client or server console run the `migrateapartments` command to automatically convert all apartments from aj-apartments. It will print a message to the console once complete.
...
1. Go to `aj-smallresources/server/logs.lua` and add this:
```lua
...
> This system only supports aj-base for now.
