...
lib.callback.register("aj-housing:server:requestProperties", function(source)
...
AddEventHandler("aj-housing:server:registerProperty", function (propertyData, preventEnter) -- triggered by realtor job
    ...
    TriggerClientEvent("aj-housing:client:addProperty", -1, propertyData)

    if propertyData.apartment and not preventEnter then
        local player = AJFW.Functions.GetPlayerByCitizenId(propertyData.owner)
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("aj-clothes:client:CreateFirstCharacter", src)
        ...
        TriggerEvent("aj-housing:server:createApartmentStash", propertyData.owner, id)
    end
end)

lib.callback.register("aj-housing:cb:GetOwnedApartment", function(source, cid)
    Debug("aj-housing:cb:GetOwnedApartment", source, cid)
    if cid ~= nil then
...
AddEventHandler("aj-housing:server:updateProperty", function(type, property_id, data)
...
AddEventHandler("onResourceStart", function(resourceName) -- Used for when the resource is restarted while in game
	if (GetCurrentResourceName() == resourceName) then
        ...
        TriggerClientEvent('aj-housing:client:initialiseProperties', -1, PropertiesTable)
	end 
end)

RegisterNetEvent("aj-housing:server:createNewApartment", function(aptLabel)
    local src = source
    ...
    TriggerEvent("aj-housing:server:registerProperty", propertyData)
end)

-- we show the character creator if they spawn without starting appartment and doesn't have skin set
RegisterNetEvent("AJFW:Server:OnPlayerLoaded", function()
...
-- If player has an existing apartment from aj-apartments, it will transfer the items over to the new apartment stash
RegisterNetEvent("aj-housing:server:createApartmentStash", function(citizenId, propertyId)
    ...
    TriggerEvent('aj-inventory:server:SaveStashItems', stashId, items)
end)

RegisterNetEvent('aj-apartments:returnBucket', function()
    local src = source
    SetPlayerRoutingBucket(src, 0)
    Player(src).state:set('instance', 0, true)
end)

AddEventHandler("aj-housing:server:addTenantToApartment", function (data)
    ...
    if property_id == nil then
        ...
        local targetToAdd = AJFW.Functions.GetPlayerByCitizenId(citizenid).PlayerData
        local propertyData = {
        ...
        TriggerEvent("aj-housing:server:registerProperty", propertyData, true)
    ...
    local citizenid = GetCitizenid(targetSrc, realtorSrc)
    local targetToAdd = AJFW.Functions.GetPlayerByCitizenId(citizenid)
...
function GetCitizenid(targetSrc, callerSrc)
    local Player = AJFW.Functions.GetPlayer(tonumber(targetSrc))
    if not Player then
