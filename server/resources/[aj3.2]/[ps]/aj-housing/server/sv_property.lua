...
function Property:PlayerEnter(src)
    ...
    TriggerClientEvent('aj-weathersync:client:DisableSync', src)
    TriggerClientEvent('aj-housing:client:enterProperty', src, self.property_id)

    if next(self.playersDoorbell) then
        TriggerClientEvent("aj-housing:client:updateDoorbellPool", src, self.property_id, self.playersDoorbell)
...
function Property:PlayerLeave(src)
    ...
    TriggerClientEvent('aj-weathersync:client:EnableSync', src)
...
function Property:AddToDoorbellPoolTemp(src)
    ...
    for src, _ in pairs(self.playersInside) do
        ...
        TriggerClientEvent("aj-housing:client:updateDoorbellPool", targetSrc, self.property_id, self.playersDoorbell)
    end
...
function Property:UpdateFurnitures(furnitures)
    ...
    for src, _ in pairs(self.playersInside) do
        local targetSrc = tonumber(src)
        TriggerClientEvent("aj-housing:client:updateFurniture", targetSrc, self.property_id, furnitures)
    end
end
...
function Property:UpdateDescription(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateDescription", self.property_id, description)
...
function Property:UpdatePrice(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdatePrice", self.property_id, price)
...
function Property:UpdateForSale(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateForSale", self.property_id, forsale)
...
function Property:UpdateShell(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateShell", self.property_id, shell)
...
function Property:UpdateOwner(data)
    ...
    local targetPlayer  = AJFW.Functions.GetPlayer(tonumber(targetSrc))
    ...
    --add callback 
    local targetAllow = lib.callback.await("aj-housing:cb:confirmPurchase", targetSrc, self.propertyData.price, self.propertyData.street, self.propertyData.property_id)
    ...
    local prevPlayer = AJFW.Functions.GetPlayerByCitizenId(previousOwner)
    local realtor = AJFW.Functions.GetPlayer(tonumber(realtorSrc))
    ...
    if Config.AJManagement then
        exports['aj-banking']:AddMoney(Config.RealtorJobName, totalAfterCommission)
    else
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateOwner", self.property_id, citizenid)
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateForSale", self.property_id, 0)
...
function Property:UpdateImgs(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateImgs", self.property_id, imgs)
...
function Property:UpdateDoor(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateDoor", self.property_id, newDoor, data.street, data.region)
...
function Property:UpdateHas_access(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateHas_access", self.property_id, has_access)
...
function Property:UpdateGarage(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateGarage", self.property_id, newData)
...
function Property:UpdateApartment(data)
    ...
    TriggerClientEvent("aj-housing:client:updateProperty", -1, "UpdateApartment", self.property_id, apartment)
...
function Property:DeleteProperty(data)
    ...
    TriggerClientEvent("aj-housing:client:removeProperty", -1, propertyid)
...
RegisterNetEvent('aj-housing:server:enterProperty', function (property_id)
    local src = source
    ...
    local ringDoorbellConfirmation = lib.callback.await('aj-housing:cb:ringDoorbell', src)
...
RegisterNetEvent("aj-housing:server:showcaseProperty", function(property_id)
    local src = source
    ...
    if jobName == Config.RealtorJobName and onDuty then
        local showcase = lib.callback.await('aj-housing:cb:showcase', src)
...
RegisterNetEvent('aj-housing:server:raidProperty', function(property_id)
    local src = source
    ...
    if isAllowedToRaid then
        if hasStormRam then
            if not property.raiding then
                local confirmRaid = lib.callback.await('aj-housing:cb:confirmRaid', src, (property.propertyData.street or property.propertyData.apartment) .. " " .. property.property_id, property_id)
                if confirmRaid == "confirm" then
                    ...
                    if Config.ConsumeRaidItem then
                        ...
                        if Config.Inventory == 'ox' then
                            ...
                            TriggerClientEvent("inventory:client:ItemBox", src, AJFW.Shared.Items[raidItem], "remove")
...
lib.callback.register('aj-housing:cb:getFurnitures', function(source, property_id)
    local property = Property.Get(property_id)
...
lib.callback.register('aj-housing:cb:getPlayersInProperty', function(source, property_id)
...
RegisterNetEvent('aj-housing:server:leaveProperty', function (property_id)
    local src = source
    local property = Property.Get(property_id)
...
RegisterNetEvent("aj-housing:server:doorbellAnswer", function (data) 
    local src = source
    local targetSrc = data.targetSrc
...
RegisterNetEvent("aj-housing:server:buyFurniture", function(property_id, items, price)
    local src = source
...
RegisterNetEvent("aj-housing:server:removeFurniture", function(property_id, itemid)
    local src = source
...
-- @@ VERY BAD 
-- I think its not bad anymore but if u got a better idea lmk
RegisterNetEvent("aj-housing:server:updateFurniture", function(property_id, item)
    local src = source
...
RegisterNetEvent("aj-housing:server:addAccess", function(property_id, srcToAdd)
    local src = source
...
RegisterNetEvent("aj-housing:server:removeAccess", function(property_id, citizenidToRemove)
    local src = source
    ...
    if property:CheckForAccess(citizenidToRemove) then
        ...
        local playerToAdd = AJFW.Functions.GetPlayerByCitizenId(citizenidToRemove) or AJFW.Functions.GetOfflinePlayerByCitizenId(citizenidToRemove)
...
lib.callback.register("aj-housing:cb:getPlayersWithAccess", function (source, property_id)
    local src = source
    ...
    for i = 1, #has_access do
        ...
        local Player = AJFW.Functions.GetPlayerByCitizenId(citizenid) or AJFW.Functions.GetOfflinePlayerByCitizenId(citizenid)
        if Player then
...
lib.callback.register('aj-housing:cb:getPropertyInfo', function (source, property_id)
    local src = source
    local property = Property.Get(property_id)
    ...
    if ownerCid then
        ownerPlayer = AJFW.Functions.GetPlayerByCitizenId(ownerCid) or AJFW.Functions.GetOfflinePlayerByCitizenId(ownerCid)
...
RegisterNetEvent('aj-housing:server:resetMetaData', function()
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local insideMeta = Player.PlayerData.metadata["inside"]
