...
if IsDuplicityVersion() then
    ...
    function Framework.aj.Notify(src, message, type)
        type = type == "info" and "primary" or type
        TriggerClientEvent('AJFW:Notify', src, message, type)
    end
    ...
    function Framework.aj.RegisterInventory(stash, label, stashConfig)
        -- Used for ox_inventory compat
    end

    function Framework.aj.SendLog(message)
        if Config.EnableLogs then
            TriggerEvent('aj-log:server:CreateLog', 'housing', 'Housing System', 'blue', message)
        end
    end
...
Framework.aj = {
    ...
    Notify = function(message, type)
        type = type == "info" and "primary" or type
        TriggerEvent('AJFW:Notify', message, type)
    end,
    ...
    AddApartmentEntrance = function(coords, size, heading, apartment, enter, seeAll, seeAllToRaid, targetName)
        exports['aj-target']:AddBoxZone(targetName, vector3(coords.x, coords.y, coords.z), size.x, size.y, {
            name = targetName,
            heading = heading,
            debugPoly = Config.DebugMode,
            minZ = coords.z - 1.0,
    ...
    RemoveTargetZone = function(targetName)
        exports["aj-target"]:RemoveZone(targetName)
    end,

    AddRadialOption = function(id, label, icon, _, event, options)
        exports['aj-radialmenu']:AddOption({
            id = id,
            title = label,
            icon = icon,
            type = 'client',
    ...
    RemoveRadialOption = function(id)
        exports['aj-radialmenu']:RemoveOption(id)
    end,
    ...
    RemoveTargetEntity = function (entity)
        exports["aj-target"]:RemoveTargetEntity(entity)
    end,
