...
function TrackVehicleByPlate(plate)
    AJFW.Functions.TriggerCallback('aj-garages:server:GetVehicleLocation', function(coords)
...
function GetVehicleByPlate(plate)
    local vehicles = AJFW.Functions.GetVehicles()
    for _, v in pairs(vehicles) do
        if AJFW.Functions.GetPlate(v) == plate then
            return v
        end
    end
    return nil
...
function RemoveRadialOptions()
    if MenuItemId1 ~= nil then
        exports['aj-radialmenu']:RemoveOption(MenuItemId1)
        MenuItemId1 = nil
    end
    if MenuItemId2 ~= nil then
        exports['aj-radialmenu']:RemoveOption(MenuItemId2)
        MenuItemId2 = nil
    end
end
-- Menus
local function PublicGarage(garageName, type)
    ...
    exports['aj-menu']:openMenu({{
        ...
        params = {
            event = "aj-garages:client:GarageMenu",
            args = {
                garageId = garageName,
                garage = garage,
...
local function ClearMenu()
    TriggerEvent("aj-menu:closeMenu")
end
...
local function ExitAndDeleteVehicle(vehicle)
    ...
    if Config.SpawnVehiclesServerside then
        Wait(1000)
        TriggerServerEvent('aj-garages:server:parkVehicle', plate)
    end
end
...
local function IsAuthorizedToAccessGarage(garageName)
    ...
    if garage.type == 'job' then
        ...
        elseif type(garage.job) == "table" then
            ...
            AJFW.Functions.Notify('job not defined on garage', 'error', 7500)
            return false
        end
    elseif garage.type == 'gang' then
        ...
        elseif type(garage.gang) == "table" then
            ...
            AJFW.Functions.Notify('gang not defined on garage', 'error', 7500)
            return false
        end
    end
    return true
...
local function CanParkVehicle(veh, garageName, vehLocation)
    ...
    if garage and garage.vehicleCategories and not TableContains(garage.vehicleCategories, vehCategories) then
        AJFW.Functions.Notify(Lang:t("error.not_correct_type"), "error", 4500)
        return false
    end
    ...
    if next(parkingSpots) then
        ...
        if closestDistance >= parkingDistance then
            AJFW.Functions.Notify(Lang:t("error.too_far_away"), "error", 4500)
            return false
        else
...
local function ParkOwnedVehicle(veh, garageName, vehLocation, plate)
    ...
    local properties = AJFW.Functions.GetVehicleProperties(veh)
    if not properties then
        return
    end

    TriggerServerEvent('aj-garage:server:updateVehicle', 1, totalFuel, engineDamage, bodyDamage, properties, plate,
        garageName, Config.StoreParkinglotAccuratly and closestVec3 or nil)
    ExitAndDeleteVehicle(veh)
    if plate then
        OutsideVehicles[plate] = nil
        TriggerServerEvent('aj-garages:server:UpdateOutsideVehicles', OutsideVehicles)
    end
    AJFW.Functions.Notify(Lang:t("success.vehicle_parked"), "success", 4500)
end

function ParkVehicleSpawnerVehicle(veh, garageName, vehLocation, plate)
    AJFW.Functions.TriggerCallback("aj-garage:server:CheckSpawnedVehicle", function(result)
        local canPark, _ = CanParkVehicle(veh, garageName, vehLocation)
        if result and canPark then
            TriggerServerEvent("aj-garage:server:UpdateSpawnedVehicle", plate, nil)
            ExitAndDeleteVehicle(veh)
        elseif not result then
            AJFW.Functions.Notify(Lang:t("error.not_owned"), "error", 3500)
        end
    end, plate)
end
...
local function ParkVehicle(veh, garageName, vehLocation)
    ...
    AJFW.Functions.TriggerCallback('aj-garage:server:checkOwnership', function(owned)
        if owned then
...
local function AddRadialParkingOption()
    ...
    local veh, dist = AJFW.Functions.GetClosestVehicle()
    if (veh and dist <= Config.VehicleParkDistance and Config.AllowParkingFromOutsideVehicle) or IsPedInAnyVehicle(ped) then
        MenuItemId1 = exports['aj-radialmenu']:AddOption({
            id = 'put_up_vehicle',
            title = 'Park Vehicle',
            icon = 'square-parking',
            type = 'client',
            event = 'aj-garages:client:ParkVehicle',
            shouldClose = true
        }, MenuItemId1)
    end
    MenuItemId2 = exports['aj-radialmenu']:AddOption({
        id = 'open_garage_menu',
        title = 'Open Garage',
        icon = 'warehouse',
        type = 'client',
        event = 'aj-garages:client:OpenMenu',
        shouldClose = true
    }, MenuItemId2)
end
...
local function CreateGarageZone()
    ...
    combo:onPlayerInOut(function(isPointInside, l, zone)
        if isPointInside and IsAuthorizedToAccessGarage(zone.name) then
            ...
            exports['aj-base']:DrawText(Config.Garages[CurrentGarage]['drawText'], Config.DrawTextPosition)
        else
            CurrentGarage = nil
            RemoveRadialOptions()
...
local function RegisterHousePoly(house)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            exports['aj-base']:DrawText(Config.HouseParkingDrawText, Config.DrawTextPosition)
        else
            exports['aj-base']:HideText()
            RemoveRadialOptions()
...
function JobMenuGarage(garageName)
    ...
    local jobGrade = AJFW.Functions.GetPlayerData().job.grade.level
    ...
    for index, data in pairs(vehicles) do
        ...
        if addVehicle then
            vehicleMenu[#vehicleMenu + 1] = {
                ...
                txt = "",
                params = {
                    event = "aj-garages:client:TakeOutGarage",
                    args = {
                        vehicleModel = model,
                        garage = garage,
...
function GetFreeParkingSpots(parkingSpots)
    ...
    for _, parkingSpot in ipairs(parkingSpots) do
        local veh, distance = AJFW.Functions.GetClosestVehicle(vector3(parkingSpot.x, parkingSpot.y, parkingSpot.z))
        ...
        if next(parkingSpots) then
            ...
            if Config.AllowSpawningFromAnywhere then
                ...
                if location == nil then
                    AJFW.Functions.Notify(Lang:t("error.all_occupied"), "error", 4500)
                    return
                end
                heading = location.w
            else
...
local function UpdateVehicleSpawnerSpawnedVehicle(veh, garage, heading, vehicleConf, cb)
    ...
    TriggerServerEvent("aj-garage:server:UpdateSpawnedVehicle", plate, true)
    ...
    if vehicleConf then
        if vehicleConf.extras then
            AJFW.Shared.SetDefaultVehicleExtras(veh, vehicleConf.extras)
        end
...
local function SpawnVehicleSpawnerVehicle(vehicleModel, vehicleConfig, location, heading, cb)
    ...
    if Config.SpawnVehiclesServerside then
        ...
        AJFW.Functions.SpawnVehicle(vehicleModel, function(veh)
            UpdateVehicleSpawnerSpawnedVehicle(veh, garage, heading, vehicleConfig, cb)
...
function UpdateSpawnedVehicle(spawnedVehicle, vehicleInfo, heading, garage, properties)
    local plate = AJFW.Functions.GetPlate(spawnedVehicle)
    ...
    if garage.useVehicleSpawner then
        ...
        TriggerServerEvent('aj-garage:server:updateVehicleState', 0, vehicleInfo.plate, vehicleInfo.garage)
        TriggerEvent("vehiclekeys:client:SetOwner", vehicleInfo.plate)

        AJFW.Functions.SetVehicleProperties(spawnedVehicle, properties or {})
...
RegisterNetEvent("aj-garages:client:GarageMenu", function(data)
    ...
    AJFW.Functions.TriggerCallback("aj-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            AJFW.Functions.Notify(Lang:t("error.no_vehicles"), "error", 5000)
        else
            ...
            for k, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions[#MenuGarageOptions + 1] = {
                        ...
                        params = {
                            event = "aj-garages:client:TakeOutDepot",
                            args = {
                                vehicle = v,
            ...
            exports['aj-menu']:openMenu(MenuGarageOptions)
        end
    end, garageId, type, superCategory)
end)

RegisterNetEvent('aj-garages:client:TakeOutGarage', function(data, cb)
    ...
    if garage.useVehicleSpawner then
        ...
        if Config.SpawnVehiclesServerside then
            AJFW.Functions.TriggerCallback('aj-garage:server:spawnvehicle', function(netId, properties)
            ...
            AJFW.Functions.SpawnVehicle(vehicleModel, function(veh)
                ...
                AJFW.Functions.TriggerCallback('aj-garage:server:GetVehicleProperties', function(properties)
                    UpdateSpawnedVehicle(veh, vehicle, heading, garage, properties)
                    if cb then
                        cb(veh)
                    end
...
AJFW.Functions.CreateClientCallback('aj-garages:client:GetVehicleType', function(cb, model)
    cb(GetVehicleTypeFromModelOrHash(model));
end)

RegisterNetEvent('aj-radialmenu:client:onRadialmenuOpen', function()
    UpdateRadialMenu()
end)

RegisterNetEvent('aj-garages:client:OpenMenu', function()
    ...
    elseif CurrentHouseGarage then
        TriggerEvent('aj-garages:client:OpenHouseGarage')
    end
end)

RegisterNetEvent('aj-garages:client:ParkVehicle', function()
    local ped = PlayerPedId()
    local canPark = true
    local curVeh = GetVehiclePedIsIn(ped)

    if Config.AllowParkingFromOutsideVehicle and curVeh == 0 then
        local closestVeh, dist = AJFW.Functions.GetClosestVehicle()
...
RegisterNetEvent('aj-garages:client:ParkLastVehicle', function(parkingName)
    local ped = PlayerPedId()
    ...
    if curVeh then
        ...
        AJFW.Functions.Notify(Lang:t('error.no_vehicle'), "error", 4500)
    end
end)

RegisterNetEvent('aj-garages:client:TakeOutDepot', function(data)
    local vehicle = data.vehicle
    ...
    if not vehExists then
        ...
        if PlayerData.money['cash'] >= vehicle.depotprice or PlayerData.money['bank'] >= vehicle.depotprice then
            TriggerEvent("aj-garages:client:TakeOutGarage", data, function(veh)
                if veh then
                    TriggerServerEvent("aj-garage:server:PayDepotPrice", data)
                end
            end)
        else
            AJFW.Functions.Notify(Lang:t('error.not_enough'), "error", 5000)
        end
    else
        AJFW.Functions.Notify(Lang:t('error.not_impound'), "error", 5000)
    end
end)

RegisterNetEvent('aj-garages:client:TrackVehicleByPlate', function(plate)
    TrackVehicleByPlate(plate)
end)

RegisterNetEvent('aj-garages:client:OpenHouseGarage', function()
    MenuHouseGarage()
end)

RegisterNetEvent('aj-garages:client:setHouseGarage', function(house, hasKey)
    if hasKey then
...
RegisterNetEvent('aj-garages:client:houseGarageConfig', function(garageConfig)
...
RegisterNetEvent('aj-garages:client:addHouseGarage', function(house, garageInfo)
    garageInfo.vehicleCategories = Config.HouseGarageCategories
    Config.HouseGarages[house] = garageInfo
    HouseGarages[house] = garageInfo
end)

RegisterNetEvent('aj-garages:client:removeHouseGarage', function(house)
...
AddEventHandler('AJFW:Client:OnPlayerLoaded', function()
    ...
    AJFW.Functions.TriggerCallback('aj-garage:server:GetOutsideVehicles', function(outsideVehicles)
...
AddEventHandler('onResourceStop', function(resource)
    if resource == GetCurrentResourceName() then
        ...
        for _, v in pairs(GarageZones) do
            exports['aj-target']:RemoveZone(v.name)
        end
    end
end)

RegisterNetEvent('AJFW:Client:OnGangUpdate', function(gang)
    PlayerGang = gang
end)
