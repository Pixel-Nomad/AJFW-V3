...
AJFW.Functions.CreateCallback("aj-garage:server:GetOutsideVehicle", function(source, cb, plate)
...
AJFW.Functions.CreateCallback("aj-garages:server:GetVehicleLocation", function(source, cb, plate)
    plate = string.upper(plate)
    local src = source
...
AJFW.Functions.CreateCallback("aj-garage:server:CheckSpawnedVehicle", function(source, cb, plate)
    plate = string.upper(plate)
    cb(VehicleSpawnerVehicles[plate] ~= nil and VehicleSpawnerVehicles[plate])
end)

RegisterNetEvent("aj-garage:server:UpdateSpawnedVehicle", function(plate, value)
    plate = string.upper(plate)
    VehicleSpawnerVehicles[plate] = value
end)

AJFW.Functions.CreateCallback('aj-garage:server:spawnvehicle', function (source, cb, vehInfo, coords, heading, warp)
    ...
    AJFW.Functions.TriggerClientCallback('aj-garages:client:GetVehicleType', source, function (vehicleType)
...
AJFW.Functions.CreateCallback("aj-garage:server:GetGarageVehicles", function(source, cb, garage, garageType, category)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
    ...
    elseif garageType == "depot" then    --Depot give player cars that are not in garage only
        GetDepotVehicles(pData.PlayerData.citizenid, 0, garage, function(result)
            ...
            if result[1] then
                ...
                for _, vehicle in pairs(result) do
                    if not AJFW.Shared.Vehicles[vehicle.vehicle] then
                        goto skip
                    end
                    ...
                    if category == "air" and ( AJFW.Shared.Vehicles[vehicle.vehicle].category == "helicopters" or AJFW.Shared.Vehicles[vehicle.vehicle].category == "planes" ) then
                        tosend[#tosend + 1] = vehicle
                    elseif category == "sea" and AJFW.Shared.Vehicles[vehicle.vehicle].category == "boats" then
                        tosend[#tosend + 1] = vehicle
                    elseif category == "car" and AJFW.Shared.Vehicles[vehicle.vehicle].category ~= "helicopters" and AJFW.Shared.Vehicles[vehicle.vehicle].category ~= "planes" and AJFW.Shared.Vehicles[vehicle.vehicle].category ~= "boats" then
                        tosend[#tosend + 1] = vehicle
                    end
                    ::skip::
                end
...
AJFW.Functions.CreateCallback("aj-garage:server:checkOwnership", function(source, cb, plate, garageType, garage, gang)
    plate = string.upper(plate)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback("aj-garage:server:GetVehicleProperties", function(source, cb, plate)
    local properties = {}
    local result = MySQL.query.await('SELECT mods FROM player_vehicles WHERE plate = ?', {plate})
    if result[1] then
...
RegisterNetEvent('aj-garage:server:updateVehicle', function(state, fuel, engine, body, properties, plate, garage, location, damage)
    plate = string.upper(plate)
...
RegisterNetEvent('aj-garage:server:updateVehicleState', function(state, plate, garage)
    plate = string.upper(plate)
    MySQL.update('UPDATE player_vehicles SET state = ?, garage = ?, depotprice = ? WHERE plate = ?',{state, garage, 0, plate})
end)

RegisterNetEvent('aj-garages:server:UpdateOutsideVehicles', function(Vehicles)
    local src = source
...
AJFW.Functions.CreateCallback("aj-garage:server:GetOutsideVehicles", function(source, cb)
...
RegisterNetEvent('aj-garage:server:PayDepotPrice', function(data)
    local src = source
...
RegisterNetEvent('aj-garages:server:parkVehicle', function(plate)
    plate = string.upper(plate)
...
--External Calls
--Call from aj-vehiclesales
AJFW.Functions.CreateCallback("aj-garage:server:checkVehicleOwner", function(source, cb, plate)
    plate = string.upper(plate)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
...
--Call from aj-phone
AJFW.Functions.CreateCallback('aj-garages:server:GetPlayerVehicles', function(source, cb)
    local Player = AJFW.Functions.GetPlayer(source)
    local Vehicles = {}
     ...
     MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ?', {Player.PlayerData.citizenid}, function(result)
        if result[1] then
            for k, v in pairs(result) do
                local VehicleData = AJFW.Shared.Vehicles[v.vehicle]
...
AJFW.Commands.Add("restorelostcars", "Restores cars that were parked in a grage that no longer exists in the config or is invalid (name change or removed).", {{name = "destination_garage", help = "(Optional) Garage where the cars are being sent to."}}, false,
function(source, args)
    ...
    if next(Config.Garages) ~= nil then
        ...
        if Config.Garages[destinationGarage] == nil then
            TriggerClientEvent('AJFW:Notify', src, 'Invalid garage name provided', 'error', 4500)
            return
        end
