...
local function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        AJFW.Functions.Notify(Lang:t('joined_to_radio', {channel = channel .. ' MHz'}), 'success')
    else
        AJFW.Functions.Notify(Lang:t('joined_to_radio', {channel = channel .. '.00 MHz'}), 'success')
    end
end
...
local function leaveradio()
    ...
    AJFW.Functions.Notify(Lang:t('you_leave') , 'error')
end
...
RegisterNetEvent('aj-radio:use', function()
    toggleRadio(not radioMenu)
end)

RegisterNetEvent('aj-radio:onRadioDrop', function()
            ...
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    if Config.RestrictedChannels[rchannel][PlayerData.job.name] and PlayerData.job.onduty then
                        ...
                        AJFW.Functions.Notify(Lang:t('restricted_channel_error'), 'error')
                    end
                else
                    connecttoradio(rchannel)
                end
            else
                AJFW.Functions.Notify(Lang:t('you_on_radio') , 'error')
            end
        else
            AJFW.Functions.Notify(Lang:t('invalid_radio') , 'error')
        end
    else
        AJFW.Functions.Notify(Lang:t('invalid_radio') , 'error')
    end
...
RegisterNUICallback('leaveRadio', function(_, cb)
    if RadioChannel == 0 then
        AJFW.Functions.Notify(Lang:t('not_on_radio'), 'error')
    else
        leaveradio()
    end
    cb("ok")
...
RegisterNUICallback("volumeUp", function(_, cb)
	if RadioVolume <= 95 then
		...
		AJFW.Functions.Notify(Lang:t("volume_radio", { value = RadioVolume}), "success")
		exports["pma-voice"]:setRadioVolume(RadioVolume)
	else
		AJFW.Functions.Notify(Lang:t("decrease_radio_volume"), "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("volumeDown", function(_, cb)
	if RadioVolume >= 10 then
		...
		AJFW.Functions.Notify(Lang:t("increase_radio_volume"), "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("increaseradiochannel", function(_, cb)
    ...
    AJFW.Functions.Notify(Lang:t("increase_decrease_radio_channel", {value = newChannel}), "success")
    cb("ok")
end)
