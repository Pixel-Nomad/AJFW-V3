local AJFW = nil
local DevMode = false
if GetResourceState('aj-base') == 'started' then
    AJFW = exports['aj-base']:GetCoreObject()
end

local Promise, ActiveMenu = nil, false

-- if you're not using AJFW change this where your inventory's images are
local img = "nui://aj-framework-assets/data/inventory/images/"

RegisterNUICallback("dataPost", function(data, cb)
    local id = tonumber(data.id) + 1 or nil
    -- @aj: added PlaySoundFrontend to play menu sfx
    ...
    if rData then
        ...
        if rData.action then
            -- @aj: added action to trigger a function
        ...
        if rData.event and Promise == nil then
            ...
            if AJFW and rData.AJCommand then
                if rData.unpack then
                    TriggerServerEvent('AJFW:CallCommand', rData.event, table.unpack(rData.args or {}))
                    TriggerEvent(rData.event, rData.args)
                else
                    TriggerServerEvent('AJFW:CallCommand', rData.event, rData.args)
                    TriggerEvent(rData.event, rData.args)
                end
            end
        end
...
ProcessParams = function(data)
    for _, v in pairs(data) do
        ...
        -- @aj: get images from user inventory
        if v.image then
            ...
            if i and j then
                ...
                if AJFW then
                    if AJFW.Shared.Items[tostring(v.image)] then
                        v.image = img .. AJFW.Shared.Items[tostring(v.image)].image
                    end
                else
                    v.image = img .. v.image
                end
...
RegisterNetEvent("ajx-menu:createMenu", ContextMenu)
RegisterNetEvent("ajx-menu:closeMenu", CancelMenu)
-- @aj: overlay
RegisterNetEvent("ajx-menu:Overlay", Overlay)
RegisterNetEvent("ajx-menu:closeOverlay", CloseOverlay)


local function aj(menu)
    local converted = {}
    ...
    for key, item in pairs(menu) do
        ...
        if item.params then
            ...
            if temp_btn.event == 'aj-menu:closeMenu' then
                temp_btn.event = 'ajx-menu:closeMenu'
            end
        end
