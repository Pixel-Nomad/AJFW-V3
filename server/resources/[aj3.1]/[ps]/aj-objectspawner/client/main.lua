...
local ObjectParams = {
    ["container"] = {event = "aj-objectspawner:client:containers", icon = "fas fa-question", label = "Container", SpawnRange = 200},
    ["none"] = {SpawnRange = 200},
}
...
AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        AJFW.Functions.TriggerCallback('aj-objectspawner:server:RequestObjects', function(incObjectList)
...
local function PlaceSpawnedObject(heading)
    ...
    TriggerServerEvent("aj-objectspawner:server:CreateNewObject", CurrentModel, finalCoords, CurrentObjectType, Options, CurrentObjectName)
    DeleteObject(CurrentObject)
    PlacingObject = false
    CurrentObject = nil
    CurrentObjectType = nil
...
RegisterNetEvent("aj-objectspawner:client:UpdateObjectList", function(NewObjectList)
		...
		for k, v in pairs(ObjectList) do
			...
			if dist < data["SpawnRange"] and v["IsRendered"] == nil then
                ...
                if ObjectParams[v.type] ~= nil and ObjectParams[v.type].event ~= nil then
                    exports['aj-target']:AddTargetEntity(object, {
                        --debugPoly=true,
                        options = {
                            {
