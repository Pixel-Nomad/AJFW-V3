...
RegisterNetEvent("aj-objectspawner:server:CreateNewObject", function(model, coords, objecttype, options, objectname)
    ...
    if hasperms then
        if model and coords then
            ...
            TriggerClientEvent("aj-objectspawner:client:AddObject", -1, {id = data.insertId, model = model, coords = coords, type = objecttype, name = objectname, options = options})
        else 
            print("[aj-OBJECTSPAWNER]: Object or coords was invalid")
        end
    else
        print("[aj-OBJECTSPAWNER]: You don't have permissions for this")
    end
end)
...
AJFW.Functions.CreateCallback("aj-objectspawner:server:RequestObjects", function(source, cb)
    cb(ServerObjects)
end)
...
RegisterNetEvent("aj-objectspawner:server:DeleteObject", function(objectid)
    ...
    if hasperms then
        if objectid > 0 then
            ...
            TriggerClientEvent("aj-objectspawner:client:receiveObjectDelete", -1, objectid)
        end
    else
        print("[aj-OBJECTSPAWNER]: You don't have permissions for this")
    end
