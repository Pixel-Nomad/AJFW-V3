...
local checkPackage = function()
    ...
    if AJFW.Functions.HasItem(Shared.SusPackageItem, 1) then
...
local createDropOffPed = function(coords)
	...
	exports['aj-target']:AddTargetEntity(deliveryPed, {
		options = {
			{
				type = 'client',
				event = 'aj-weedplanting:client:DeliverWeed',
...
local createNewDropOff = function()
    ...
    hasDropOff = true
    TriggerEvent('aj-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_godropoff'), 'fas fa-cannabis', '#00FF00', 8000)
	...
	dropOffArea:onPlayerInOut(function(isPointInside, point)
		if isPointInside then
			if not deliveryPed then
				TriggerEvent('aj-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_makedropoff'), 'fas fa-cannabis', '#00FF00', 8000)
...
RegisterNetEvent('aj-weedplanting:client:StartPackage', function(data)
    if waitingForPackage then return end
    local hasItem = AJFW.Functions.HasItem(Shared.PackedWeedItem, 1)
    if not hasItem then
        AJFW.Functions.Notify(_U('dont_have_anything'), 'error', 2500)
        return
    end
    ...
    AJFW.Functions.Progressbar('weedrun_pack', _U('handing_over_weed'), 4000, false, true, {
        ...
        AJFW.Functions.TriggerCallback('aj-weedplanting:server:PackageGoods', function(result)
            if not result then return end
            waitingForPackage = true
            AJFW.Functions.Notify(_U('wait_closeby'), 'primary', 2500)
            ...
            packageZone:onPlayerInOut(function(isPointInside, point)
                if not isPointInside then
                    ...
                    if waitingForPackage then
                        TriggerServerEvent('aj-weedplanting:server:DestroyWaitForPackage')
...
RegisterNetEvent('aj-weedplanting:client:PackageGoodsReceived', function()
    if waitingForPackage then
        packageZone:destroy()
        waitingForPackage = false
    end
...
RegisterNetEvent('aj-weedplanting:client:ClockIn', function()
    if delivering then return end
    delivering = true
    TriggerEvent('aj-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_waitfornew'), 'fas fa-cannabis', '#00FF00', 8000)
    Wait(math.random(Shared.DeliveryWaitTime[1], Shared.DeliveryWaitTime[2]))
    createNewDropOff()
end)

RegisterNetEvent('aj-weedplanting:client:ClockOut', function()
    ...
    AJFW.Functions.Notify(_U('weedrun_clockout'), 'primary', 2500)
end)

RegisterNetEvent('aj-weedplanting:client:DeliverWeed', function()
    if madeDeal then return end

    if not hasPackage then
        AJFW.Functions.Notify(_U('weedrun_hasnopackage'), 'error', 2500)
        return
    end
	...
	if #(GetEntityCoords(ped) - GetEntityCoords(deliveryPed)) < 5.0 then
		madeDeal = true
		exports['aj-target']:RemoveTargetEntity(deliveryPed)

		-- Alert Cops
		if math.random(100) <= Shared.CallCopsChance then
            if GetResourceState(Shared.Dispatch) == 'started' then
                exports[Shared.Dispatch]:DrugSale() -- Project-SLoth aj-dispatch
            end
        end
		...
		Wait(1000)
		TriggerServerEvent('aj-weedplanting:server:WeedrunDelivery')
		...
		dropOffArea:destroy()
		Wait(2000)
		TriggerEvent('aj-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_success'), 'fas fa-cannabis', '#00FF00', 20000)
        ClearPedTasks(ped)
...
CreateThread(function()
    ...
    exports['aj-target']:AddBoxZone('weed-run-zone', vector3(Shared.WeedRunStart.x, Shared.WeedRunStart.y, Shared.WeedRunStart.z), 0.8, 0.8, {
        name = 'weed-run-zone',
        ...
        options = {
            { -- Create Package
                type = 'client',
                event = 'aj-weedplanting:client:StartPackage',
            ...
            { -- Receive Package
                type = 'server',
                event = 'aj-weedplanting:server:CollectPackageGoods',
                icon = 'fas fa-circle-chevron-right',
            ...
            { -- Clock In for deliveries
                type = 'client',
                event = 'aj-weedplanting:client:ClockIn',
            ...
            { -- Clock out for deliveries
                type = 'client',
                event = 'aj-weedplanting:client:ClockOut',
                icon = 'fas fa-stopwatch',
