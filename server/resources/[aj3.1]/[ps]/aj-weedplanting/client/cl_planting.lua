...
RegisterNetEvent('aj-weedplanting:client:UseWeedSeed', function()
    ...
    exports['aj-base']:DrawText(_U('place_or_cancel'), 'left')
    ...
    while not planted do
        ...
        if hit == 1 then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                AJFW.Functions.Progressbar('spawn_plant', _U('place_sapling'), 2000, false, true, {
                    disableMovement = true,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() 
                    TriggerServerEvent('aj-weedplanting:server:CreateNewPlant', dest)
                    planted = false
                    seedUsed = false
                    ClearPedTasks(ped)
                    ...
                    AJFW.Functions.Notify(_U('canceled'), 'error', 2500)
                    planted = false
                    seedUsed = false
                    ClearPedTasks(ped)
                    RemoveAnimDict('amb@medic@standing@kneel@base')
...
RegisterNetEvent('aj-weedplanting:client:CheckPlant', function(data)
    local netId = NetworkGetNetworkIdFromEntity(data.entity)
    AJFW.Functions.TriggerCallback('aj-weedplanting:server:GetPlantData', function(result)
        ...
        if result.health == 0 then -- Destroy plant
            exports['aj-menu']:openMenu({
                ...
                },
                {
                    ...
                    params = {
                        event = 'aj-weedplanting:client:ClearPlant',
                        args = data.entity
                    }
                }
            })
            ...
            if PlayerJob.type == Shared.CopJob and PlayerJob.onduty then
                exports['aj-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        params = {
                            event = 'aj-weedplanting:client:HarvestPlant',
                            args = data.entity
                        }
                    },
                    {
                        ...
                        icon = 'fas fa-fire',
                        params = {
                            event = 'aj-weedplanting:client:PoliceDestroy',
                            args = data.entity
                        }
                    }
                })
            ...
            if PlayerJob.type == Shared.CopJob and PlayerJob.onduty then
                ...
                exports['aj-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        icon = 'fas fa-shower',
                        params = {
                            event = 'aj-weedplanting:client:GiveWater',
                            args = data.entity
                        }
                    },
                    {
                        ...
                        params = {
                            args = data.entity,
                            event = 'aj-weedplanting:client:GiveFertilizer',
                        }
                    },
                    {
                        ...
                        params = {
                            args = data.entity,
                            event = 'aj-weedplanting:client:AddMaleSeed',
                        }
                    }
                })
            end
...
RegisterNetEvent('aj-weedplanting:client:ClearPlant', function(entity)
    ...
    AJFW.Functions.Progressbar('clear_plant', _U('clear_plant'), 8500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function()
        TriggerServerEvent('aj-weedplanting:server:ClearPlant', netId)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    end, function()
...
RegisterNetEvent('aj-weedplanting:client:HarvestPlant', function(entity)
    local netId = NetworkGetNetworkIdFromEntity(entity)
    local ped = PlayerPedId()
    TaskTurnPedToFaceEntity(ped, entity, 1.0)
    Wait(1500)
    ...
    AJFW.Functions.Progressbar('harvest_plant', _U('harvesting_plant'), 8500, false, true, {
        disableMovement = true, 
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function()
        TriggerServerEvent('aj-weedplanting:server:HarvestPlant', netId)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    end, function()
...
RegisterNetEvent('aj-weedplanting:client:PoliceDestroy', function(entity)
    local netId = NetworkGetNetworkIdFromEntity(entity)
    local ped = PlayerPedId()
    TaskTurnPedToFaceEntity(ped, entity, 1.0)
    Wait(500)
    ClearPedTasks(ped)
    TriggerServerEvent('aj-weedplanting:server:PoliceDestroy', netId)
end)

RegisterNetEvent('aj-weedplanting:client:FireGoBrrrrrrr', function(coords)
    local ped = PlayerPedId()
...
RegisterNetEvent('aj-weedplanting:client:GiveWater', function(entity)
    if AJFW.Functions.HasItem(Shared.FullCanItem, 1) then
        local netId = NetworkGetNetworkIdFromEntity(entity)
        local ped = PlayerPedId()
        ...
        AJFW.Functions.Progressbar('weedplanting_water', _U('watering_plant'), 6000, false, false, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('aj-weedplanting:server:GiveWater', netId)
            ClearPedTasks(ped)
        ...
        AJFW.Functions.Notify(_U('missing_water'), 'error', 2500)
    end
end)

RegisterNetEvent('aj-weedplanting:client:OpenFillWaterMenu', function()
    exports['aj-menu']:openMenu({
        {
        ...
        },
        {
            ...
            params = {
                event = 'aj-weedplanting:client:FillWater',
            }
        }
    })
end)

RegisterNetEvent('aj-weedplanting:client:FillWater', function()

    local hasItem = AJFW.Functions.HasItem(Shared.WaterItem)
    
    if not hasItem then
        AJFW.Functions.Notify(_U('missing_filling_water'), 'error', 2500)
        return
    end

    AJFW.Functions.Progressbar('filling_water', _U('filling_water'), 2000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function()
        TriggerServerEvent('aj-weedplanting:server:GetFullWateringCan')
    end, function() -- Cancel
        AJFW.Functions.Notify(_U('canceled'), 'error', 2500)
    end)
end)

RegisterNetEvent('aj-weedplanting:client:GiveFertilizer', function(entity)
    if AJFW.Functions.HasItem(Shared.FertilizerItem, 1) then
        local netId = NetworkGetNetworkIdFromEntity(entity)
        local ped = PlayerPedId()
        local coords = GetEntityCoords(ped)
        ...
        AJFW.Functions.Progressbar('weedplanting_fertilizer', _U('fertilizing_plant'), 6000, false, false, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('aj-weedplanting:server:GiveFertilizer', netId)
            ClearPedTasks(ped)
            DeleteEntity(created_object)
        end, function()
            ClearPedTasks(ped)
        ...
        AJFW.Functions.Notify(_U('missing_fertilizer'), 'error', 2500)
    end
end)

RegisterNetEvent('aj-weedplanting:client:AddMaleSeed', function(entity)
    if AJFW.Functions.HasItem(Shared.MaleSeed, 1) then
        local netId = NetworkGetNetworkIdFromEntity(entity)
        local ped = PlayerPedId()
        TaskTurnPedToFaceEntity(ped, entity, 1.0)
        Wait(1500)
        ...
        AJFW.Functions.Progressbar('add_maleseed', _U('adding_male_seed'), 8500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function()
            TriggerServerEvent('aj-weedplanting:server:AddMaleSeed', netId)
            ClearPedTasks(ped)
            RemoveAnimDict('amb@medic@standing@kneel@base')
            RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
        end, function()
        ...
        AJFW.Functions.Notify(_U('missing_mseed'), 'error', 2500)
    end
end)
...
CreateThread(function()
    exports['aj-target']:AddTargetModel(Shared.WeedProps, {
        options = {
            {
                type = 'client',
                event = 'aj-weedplanting:client:CheckPlant',
