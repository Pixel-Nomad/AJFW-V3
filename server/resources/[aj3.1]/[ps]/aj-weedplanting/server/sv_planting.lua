...
local calcGrowth = function(k)
    ...
    local growth = AJFW.Shared.Round(progress * 100 / growTime, 2)
...
local calcFertilizer = function(k)
    ...
    if #WeedPlants[k].fertilizer == 0 then
        ...
        local fertilizer = AJFW.Shared.Round(100 - (time_elapsed / 60 * Shared.FertilizerDecay), 2)
...
local calcWater = function(k)
    ...
    if #WeedPlants[k].water == 0 then
        ...
        local water = AJFW.Shared.Round(100 - (time_elapsed / 60 * Shared.WaterDecay), 2)
...
local calcHealth = function(k)
    ...
    for i=1, intervals, 1 do
        ...
        if #WeedPlants[k].fertilizer == 0 then
            ...
            fertilizer_amount = math.max(AJFW.Shared.Round(100 - (time_since_fertilizer / 60 * Shared.FertilizerDecay), 2), 0.00)
        ...
        if #WeedPlants[k].water == 0 then
            ...
            water_amount = math.max(AJFW.Shared.Round(100 - (time_since_water / 60 * Shared.WaterDecay), 2), 0.00)
...
local setupPlants = function()
    ...
    for k, v in pairs(result) do
        ...
        local growth = math.min(AJFW.Shared.Round(progress * 100 / growTime, 2), 100.00)
...
RegisterNetEvent('aj-weedplanting:server:ClearPlant', function(netId)
...
RegisterNetEvent('aj-weedplanting:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    ...
    if DoesEntityExist(entity) then
        ...
        if WeedPlants[entity].gender == 'female' then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.BranchItem], 'add', 1)
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.MaleSeed], 'add', mSeeds)
            TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.FemaleSeed], 'add', fSeeds)
        end
...
RegisterNetEvent('aj-weedplanting:server:PoliceDestroy', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if DoesEntityExist(entity) then
        ...
        TriggerClientEvent('aj-weedplanting:client:FireGoBrrrrrrr', -1, WeedPlants[entity].coords)
...
RegisterNetEvent('aj-weedplanting:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.FullCanItem], 'remove', 1)
        TriggerClientEvent('AJFW:Notify', src, _U('watered_plant'), 'success', 2500)
        Player.Functions.AddItem(Shared.EmptyCanItem, 1, false)
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.EmptyCanItem], 'add', 1)
...
RegisterNetEvent('aj-weedplanting:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.FertilizerItem], 'remove', 1)
        TriggerClientEvent('AJFW:Notify', src, _U('fertilizer_added'), 'success', 2500)
...
RegisterNetEvent('aj-weedplanting:server:AddMaleSeed', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.MaleSeed, 1, false) then
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.MaleSeed], 'remove', 1)
        TriggerClientEvent('AJFW:Notify', src, _U('male_seed_added'), 'success', 2500)
...
RegisterNetEvent('aj-weedplanting:server:CreateNewPlant', function(coords)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) and Player.Functions.RemoveItem(Shared.PlantTubItem, 1) then
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.FemaleSeed], 'remove', 1)
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.PlantTubItem], 'remove', 1)
...
RegisterNetEvent('aj-weedplanting:server:GetFullWateringCan', function(netId)
   
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    
    if Player.Functions.RemoveItem(Shared.EmptyCanItem, 1, false) and Player.Functions.RemoveItem(Shared.WaterItem, 1, false) then
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.EmptyCanItem], 'remove', 1)
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.WaterItem], 'remove', 1)
        TriggerClientEvent('AJFW:Notify', src, _U('filled_can'), 'success', 2500)
        Player.Functions.AddItem(Shared.FullCanItem, 1, false)
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[Shared.FullCanItem], 'add', 1)
    end
end)

--- Callbacks

AJFW.Functions.CreateCallback('aj-weedplanting:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
...
AJFW.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    local src = source
	local Player = AJFW.Functions.GetPlayer(src)
    local tub = Player.Functions.GetItemByName(Shared.PlantTubItem)
	
    if tub ~= nil then
        TriggerClientEvent("aj-weedplanting:client:UseWeedSeed", source)
    else
        TriggerClientEvent('AJFW:Notify', src, _U('missing_tub'), 'error', 2500)
    end
end)

AJFW.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("aj-weedplanting:client:OpenFillWaterMenu", src)
end)
