    ...
    Combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
      ...
      if enteredZones then
        for i=1, #enteredZones do
          ...
          TriggerEvent("aj-zones:enter", enteredZones[i]["name"], enteredZones[i]["data"])
          TriggerServerEvent("aj-zones:enter", enteredZones[i]["name"], enteredZones[i]["data"])
        end
      end
      ...
      if leftZones then
        for i=1, #leftZones do
          ...
          TriggerEvent("aj-zones:leave", leftZones[i]["name"], leftZones[i]["data"])
          TriggerServerEvent("aj-zones:leave", leftZones[i]["name"], leftZones[i]["data"])
        end
      end
    end)
