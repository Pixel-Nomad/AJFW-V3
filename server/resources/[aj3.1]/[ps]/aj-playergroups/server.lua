...
AJFW.Functions.CreateCallback("groups:requestCreateGroup", function(source, cb)
    local src = source
    ...
    if not Players[src] then
        ...
        TriggerClientEvent("AJFW:Notify", src, "You are already in a group", "error")
        cb(false)
    end
end)

-- Get all active groups currently in the server.
AJFW.Functions.CreateCallback("groups:getActiveGroups", function(source, cb)
    local src = source
    local player = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback("groups:getGroupRequests", function(source, cb, groupID)
    local src = source
    local player = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback("groups:requestJoinGroup", function(source, cb, groupID)
    local src = source
    ...
    if not Players[src] then
        if Groups[groupID] then 
            if #Groups[groupID]["members"] < GroupLimit then
                ...
                cb(true)
                TriggerClientEvent("AJFW:Notify", lead, "Someone has requested to join the group", "success")
            else
                TriggerClientEvent("AJFW:Notify", src, "The group is full", "error")
            end
        else 
            TriggerClientEvent("AJFW:Notify", src, "That group doesn't exist", "error")
        end
        cb(true)
    else
        TriggerClientEvent("AJFW:Notify", src, "You already have a request pending", "error")
        cb(false)
    end
end)
...
RegisterNetEvent("groups:acceptRequest", function(player, groupID)
    ...
    if AddPlayerToGroup(player, groupID) then
        ...
        TriggerClientEvent("AJFW:Notify", player, "Your group join request was accepted", "success")
...
RegisterNetEvent("groups:denyRequest", function(player, groupID)
    ...
    TriggerClientEvent("AJFW:Notify", player, "Your group join request was denied", "error")
end)
...
RegisterNetEvent("groups:kickMember", function(player, groupID)
    ...
    TriggerClientEvent("AJFW:Notify", player, "You were removed from the group", "error")
end)

-- Returns all members in the specified groupID.
AJFW.Functions.CreateCallback("groups:getGroupMembers", function(source, cb, groupID)
    local src = source
    local temp = {}
        ...
        if Groups[groupID] then
            ...
            if Groups[groupID]["members"]["leader"] == player then
                if ChangeGroupLeader(groupID) then
                    Players[player] = nil
                    TriggerClientEvent("AJFW:Notify", player, "You have left the group", "primary")
                    Wait(10)
                    UpdateGroupData(groupID)
                else
                    Players[player] = nil
