...
RegisterNUICallback('group-create', function(data, cb)
    local p = promise.new()
    AJFW.Functions.TriggerCallback("groups:requestCreateGroup", function(r)
        p:resolve(r)
    end)
...
RegisterNUICallback('getActiveGroups', function(data, cb)
    ...
    AJFW.Functions.TriggerCallback("groups:getActiveGroups", function(result)
...
RegisterNUICallback('request-join', function(data, cb)
    if not requestCooldown then
        local request = promise.new()
        AJFW.Functions.TriggerCallback("groups:requestJoinGroup", function(result)
            request:resolve(result)
        end, data.groupID)
        local data = Citizen.Await(request)
        if request then 
            AJFW.Functions.Notify("Join request sent", "success")
        else 
            AJFW.Functions.Notify("You cannot do that yet", "error")
        end
        requestCooldown = true
        Wait(5000)
        requestCooldown = false
    else 
        AJFW.Functions.Notify("You need to wait before requesting again", "error")
    end
end)

RegisterNUICallback('view-requests', function(data, cb)
    local request = promise.new()
    AJFW.Functions.TriggerCallback("groups:getGroupRequests", function(result)
        request:resolve(result)
    end, data.groupID)
    local data = Citizen.Await(request)
    cb(data)
...
RegisterNUICallback('update-status', function(data)
    currentJobStage = data.status
    AJFW.Functions.Notify("Your group status changed to "..currentJobStage, "primary")
end)
