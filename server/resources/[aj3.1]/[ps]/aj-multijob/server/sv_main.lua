...
local function GetJobs(citizenid)
    ...
    MySQL.Async.fetchAll("SELECT jobdata FROM multijobs WHERE citizenid = @citizenid",{
        ...
        if jobs[1] and jobs ~= "[]" then
            ...
            local Player = AJFW.Functions.GetOfflinePlayerByCitizenId(citizenid)
            local temp = {}
...
local function UpdatePlayerJob(Player, job, grade)
    if Player.PlayerData.source ~= nil then
        ...
        local sharedJobData = AJFW.Shared.Jobs[job]
...
local function RemoveJob(citizenid, job)
    ...
    if Player == nil then
        Player = AJFW.Functions.GetOfflinePlayerByCitizenId(citizenid)
    end
...
AJFW.Commands.Add('removejob', 'Remove Multi Job (Admin Only)', { { name = 'id', help = 'ID of player' }, { name = 'job', help = 'Job Name' } }, false, function(source, args)
    local source = source
    if source ~= 0 then
        if args[1] then
...
AJFW.Functions.CreateCallback("aj-multijob:getJobs", function(source, cb)
    ...
    for i = 1, #Players, 1 do
        local xPlayer = AJFW.Functions.GetPlayer(Players[i])
    ...
    for job, grade in pairs(jobs) do
        if AJFW.Shared.Jobs[job] == nil then
            print("The job '" .. job .. "' has been removed and is not present in your AJFW jobs. Remove it from the multijob SQL or add it back to your AJFW jobs.lua.")
        else
            ...
            getjobs = {
                ...
                label = AJFW.Shared.Jobs[job].label,
                gradeLabel = AJFW.Shared.Jobs[job].grades[tostring(grade)].name,
                salary = AJFW.Shared.Jobs[job].grades[tostring(grade)].payment,
                active = online,
            }
...
RegisterNetEvent("aj-multijob:changeJob",function(cjob, cgrade)
    local source = source
    local Player = AJFW.Functions.GetPlayer(source)
...
RegisterNetEvent("aj-multijob:removeJob",function(job, grade)
    local source = source
    local Player = AJFW.Functions.GetPlayer(source)
...
RegisterNetEvent('aj-multijob:server:removeJob', function(targetCitizenId)
...
RegisterNetEvent('AJFW:Server:OnJobUpdate', function(source, newJob)
    local source = source
    local Player = AJFW.Functions.GetPlayer(source)
    local jobs = GetJobs(Player.PlayerData.citizenid)
    local amount = 0
