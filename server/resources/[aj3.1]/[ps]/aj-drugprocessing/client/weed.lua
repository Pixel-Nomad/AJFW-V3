...
local function RollJoint()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.rolling_joint"), 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function()
		TriggerServerEvent('aj-drugprocessing:rollJoint')
...
local function BagSkunk()
	...
	TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.bagging_skunk"), 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function()
		TriggerServerEvent('aj-drugprocessing:bagskunk')
		local timeLeft = Config.Delays.WeedProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
local function ProcessWeed()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('aj-drugprocessing:processCannabis')
		local timeLeft = Config.Delays.WeedProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
RegisterNetEvent("aj-drugprocessing:processWeed",function()
...
RegisterNetEvent('aj-drugprocessing:EnterWWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('aj-drugprocessing:ExitWWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent("aj-drugprocessing:pickWeed", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
				...
				spawnedWeeds -= 1
				TriggerServerEvent('aj-drugprocessing:pickedUpCannabis')
				isPickingUp = false
			end, function()
				ClearPedTasks(PlayerPedId())
				isPickingUp = false
...
RegisterNetEvent('aj-drugprocessing:client:rollJoint', function()
    AJFW.Functions.TriggerCallback('aj-drugprocessing:validate_items', function(result)
		if result.ret then
			RollJoint()
		else
...
RegisterNetEvent('aj-drugprocessing:client:bagskunk', function()
    AJFW.Functions.TriggerCallback('aj-drugprocessing:validate_items', function(result)
		if result.ret then
			BagSkunk()
		else
