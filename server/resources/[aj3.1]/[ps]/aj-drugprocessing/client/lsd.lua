...
local function Processlsd()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('aj-drugprocessing:Processlsd')
		...
		while timeLeft > 0 do
			...
			if #(GetEntityCoords(playerPed)-Config.CircleZones.lsdProcessing.coords) > 5 then
				AJFW.Functions.Notify(Lang:t("error.too_far"))
...
local function Processthionylchloride()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('aj-drugprocessing:processThionylChloride')
    ...
    while true do
        ...
        if #(coords-Config.CircleZones.lsdProcessing.coords) < 2 then
            if not isProcessing then
                ...
                AJFW.Functions.DrawText3D(pos.x, pos.y, pos.z, Lang:t("drawtext.process_lsd"))
            end
...
RegisterNetEvent('aj-drugprocessing:processingThiChlo', function()
