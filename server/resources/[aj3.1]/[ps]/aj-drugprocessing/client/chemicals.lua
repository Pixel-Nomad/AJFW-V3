...
local function createChemicalMenu()
    local chemMenu = {
        ...
        },
        {
			...
			params = {
                event = "aj-drugprocessing:hydrochloric_acid",
            }
        },
        {
            ...
            txt = Lang:t("menu.chemicals"),
			params = {
                event = "aj-drugprocessing:sodium_hydroxide",
            }
        },
        {
            ...
            txt = Lang:t("menu.chemicals"),
			params = {
                event = "aj-drugprocessing:sulfuric_acid",
            }
        },
        {
        ...
        },
        {
			...
			params = {
                event = exports['aj-menu']:closeMenu(),
            }
        },
    }
    exports['aj-menu']:openMenu(chemMenu)
end
RegisterNetEvent('aj-drugprocessing:chemicalmenu', createChemicalMenu)
...
local function process_hydrochloric_acid()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('aj-drugprocessing:processHydrochloric_acid')
...
local function process_lsa()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('aj-drugprocessing:process_lsa')
...
local function process_sulfuric_acid()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('aj-drugprocessing:processprocess_sulfuric_acid')
...
local function process_sodium_hydroxide()
	...
	AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('aj-drugprocessing:processsodium_hydroxide')
...
RegisterNetEvent("aj-drugprocessing:hydrochloric_acid", function()
    AJFW.Functions.TriggerCallback('aj-drugprocessing:validate_items', function(result)
		if result then
			process_hydrochloric_acid()
		else
			AJFW.Functions.Notify(Lang:t("error.no_chemicals"), 'error')
		end
	end, {chemicals = 1})
end)

RegisterNetEvent("aj-drugprocessing:lsa", function()
    AJFW.Functions.TriggerCallback('aj-drugprocessing:validate_items', function(result)
		if result then
			process_lsa()
		else
...
RegisterNetEvent("aj-drugprocessing:sulfuric_acid", function()
    AJFW.Functions.TriggerCallback('aj-drugprocessing:validate_items', function(result)
		if result then
...
RegisterNetEvent("aj-drugprocessing:sodium_hydroxide", function()
    AJFW.Functions.TriggerCallback('aj-drugprocessing:validate_items', function(result)
		if result then
...
RegisterNetEvent("aj-drugprocessing:pickChemicals", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		AJFW.Functions.Progressbar("search_register", Lang:t("progressbar.pickup_chemicals"), 10000, false, true, {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
			...
			TriggerServerEvent('aj-drugprocessing:pickedUpChemicals')
			isPickingUp = false
