...
local function ConvertAJ()
	...
	local ajEvidence = MySQL.query.await("SELECT stash, items FROM stashitems WHERE stash LIKE '% | Drawer%'")
	...
	if ajEvidence then
		...
		for i = 1, #ajEvidence do
			local ajStash = ajEvidence[i]
			local name = 'evidence-'..ajStash.stash:sub(12)
			local items = server.convertInventory(nil, (ajStash.items and json.decode(ajStash.items) or {}))
		...
		Print(('Creating ^3%s^0 evidence lockers from ^3%s^0 lockers by merging duplicate locker numbers'):format(count, #ajEvidence))
		if count > 0 then
			if not MySQL.transaction.await(parameters) then
