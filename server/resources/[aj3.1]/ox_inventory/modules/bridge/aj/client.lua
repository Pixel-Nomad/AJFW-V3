...
RegisterNetEvent('AJFW:Client:OnPlayerUnload', client.onLogout)

RegisterNetEvent('AJFW:Player:SetPlayerData', function(data)
...
function client.setPlayerStatus(values)
	for name, value in pairs(values) do
		...
		if name == "hunger" then
			TriggerServerEvent('consumables:server:addHunger', AJFW.Functions.GetPlayerData().metadata.hunger + value)
		elseif name == "thirst" then
			TriggerServerEvent('consumables:server:addThirst', AJFW.Functions.GetPlayerData().metadata.thirst + value)
...
AddEventHandler(('__cfx_export_aj-inventory_HasItem'), function(setCB)
	setCB(hasItem)
end)
