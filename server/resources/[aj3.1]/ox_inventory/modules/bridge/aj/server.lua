...
AddEventHandler('AJFW:Server:OnPlayerUnload', server.playerDropped)

AddEventHandler('AJFW:Server:OnJobUpdate', function(source, job)
...
AddEventHandler('AJFW:Server:OnGangUpdate', function(source, gang)
	local inventory = Inventory(source)
	if not inventory then return end
...
local function setupPlayer(Player)
	...
	AJFW.Functions.AddPlayerMethod(Player.PlayerData.source, "AddItem", function(item, amount, slot, info)
		return Inventory.AddItem(Player.PlayerData.source, item, amount, info, slot)
	end)

	AJFW.Functions.AddPlayerMethod(Player.PlayerData.source, "RemoveItem", function(item, amount, slot)
	...
	AJFW.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemByName", function(itemName)
		return setItemCompatibilityProps(Inventory.GetSlotWithItem(Player.PlayerData.source, itemName))
	end)

	AJFW.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemsByName", function(itemName)
	...
	AJFW.Functions.AddPlayerMethod(Player.PlayerData.source, "SetInventory", function()
		error('Player.Functions.SetInventory is unsupported for ox_inventory. Try ClearInventory, then add the desired items.')
	end)
end

AddEventHandler('AJFW:Server:PlayerLoaded', setupPlayer)

SetTimeout(500, function()
	AJFW = exports['aj-base']:GetCoreObject()
	server.GetPlayerFromId = AJFW.Functions.GetPlayer
	

	for _, Player in pairs(AJFW.Functions.GetAJPlayers()) do setupPlayer(Player) end
end)

function server.UseItem(source, itemName, data)
	local cb = AJFW.Functions.CanUseItem(itemName)
	return cb and cb(source, data)
end

AddEventHandler('AJFW:Server:OnMoneyChange', function(src, account, amount, changeType)
...
-- taken from qbox-core (https://github.com/Qbox-project/aj-base/blob/f4174f311aae8157181a48fa2e2bd30c8d13edb1/client/functions.lua#L25)
...
local function exportHandler(exportName, func)
    AddEventHandler(('__cfx_export_aj-inventory_%s'):format(exportName), function(setCB)
        setCB(func)
    end)
end
