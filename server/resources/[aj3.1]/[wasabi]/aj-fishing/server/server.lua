...
lib.callback.register('aj-fishing:checkItem', function(source, itemname)
...
lib.callback.register('aj-fishing:getFishData', function(source)
    local data = Config.fish[math.random(#Config.fish)]
    return data
end)

RegisterNetEvent('aj-fishing:rodBroke', function()
    RemoveItem(source, Config.fishingRod.itemName, 1)
    TriggerClientEvent('aj-fishing:interupt', source)
end)

RegisterNetEvent('aj-fishing:tryFish', function(data)
    ...
    if xPole > 0 and xBait > 0 then
        ...
        if chance <= Config.bait.loseChance then
            ...
            TriggerClientEvent('aj-fishing:notify', source, Strings.bait_lost, Strings.bait_lost_desc, 'error')
        end
	if Framework == 'esx' and not Config.oldESX then
	    local player = GetPlayer(source)
	    if player.canCarryItem(data.item, 1) then
	        AddItem(source, data.item, 1)
		TriggerClientEvent('aj-fishing:notify', source, Strings.fish_success, string.format(Strings.fish_success_desc, data.label), 'success')
	    else
                TriggerClientEvent('aj-fishing:notify', source, Strings.cannot_carry, Strings.cannot_carry_desc, 'error')
	    end
	else
            AddItem(source, data.item, 1)
            TriggerClientEvent('aj-fishing:notify', source, Strings.fish_success, string.format(Strings.fish_success_desc, data.label), 'success')
    ...
    elseif xPole > 0 and xBait < 1 then
        TriggerClientEvent('aj-fishing:interupt', source)
        TriggerClientEvent('aj-fishing:notify', source, Strings.no_bait, Strings.no_bait_desc, 'error')
    elseif xPole < 1 then
        KickPlayer(source, Strings.kicked)
    end
end)

RegisterNetEvent('aj-fishing:sellFish', function()
    ...
    for i=1, #Config.fish do
        if HasItem(source, Config.fish[i].item) > 0 then
            ...
            if rewardAmount > 0 then
                ...
                TriggerClientEvent('aj-fishing:notify', source, Strings.sold_for, (Strings.sold_for_desc):format(HasItem(source, Config.fish[i].item), Config.fish[i].label, addCommas(rewardAmount)), 'success')
...
RegisterUsableItem(Config.fishingRod.itemName, function(source)
    TriggerClientEvent('aj-fishing:startFishing', source)
end)
