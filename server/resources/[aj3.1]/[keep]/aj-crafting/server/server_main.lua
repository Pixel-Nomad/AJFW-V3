...
AJFW.Functions.CreateCallback('aj-crafting:server:get_player_information', function(source, cb)
...
AJFW.Functions.CreateCallback('aj-crafting:server:get_player_blueprints', function(source, cb)
     local Player = AJFW.Functions.GetPlayer(source)
...
local function do_player_have_materials(src, Player, item_config)
     ...
     TriggerClientEvent('AJFW:Notify', src, Lang:t('error.need_more_mat'), "error")
     return false
end

local function remove_item(src, Player, name, amount)
     Player.Functions.RemoveItem(name, amount)
     TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[name], "remove")
end
...
local function StartCraftProcess(src, data)
     ...
     if not hasBlueprint and data['blueprint_id'] then
          TriggerClientEvent('AJFW:Notify', src, "You do not own this blueprint", "error")
          return
     end
     if not item_config then
          TriggerClientEvent('AJFW:Notify', src, Lang:t('error.crafting_failed'), "error")
          return
     end

     if item_config.item_settings.level and
         not (Player.PlayerData.metadata.craftingrep >= item_config.item_settings.level) then
          TriggerClientEvent('AJFW:Notify', src, Lang:t('error.need_more_exp'), "error")
          return
     end
     ...
     if not restricted_by_job then
          TriggerClientEvent('AJFW:Notify', src, Lang:t('error.crafting_is_restricted'), "error")
          return
     end
     ...
     TriggerClientEvent("aj-crafting:client:start_crafting", src, data, item_config)
     SetTimeout(item_config.crafting.duration * 1000, function()
          ...
          TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[item_name], "add")
          TriggerClientEvent('AJFW:Notify', src, Lang:t('success.successful_crafting'), 'success')
     end)
end

RegisterServerEvent('aj-crafting:check_materials_list', function(data)
     local src = source
     local Player = AJFW.Functions.GetPlayer(src)
     ...
     if item_config then
          TriggerClientEvent('aj-crafting:client:local_mailer', src, {
               gender = gender,
               charinfo = charinfo,
...
--   EVENTS
--------------------
RegisterNetEvent('aj-crafting:server:craft_item', function(data)
     local src = source
...
local function GiveBlueprint(src, blueprint_name)
     ...
     local label = temp and temp.item_settings.label or AJFW.Shared.Items[blueprint_name].name
     return exports['aj-inventory']:AddItem(src, 'blueprint_document', 1, false, {
...
AJFW.Commands.Add("ajcrafting", "increase exp of crafting for a player", {
     {
          name = "cid",
          help = "player id"
     },
...
AJFW.Commands.Add("giveblueprint", "Give blueprint to self", {
     {
          name = "name",
          help = "blueprint name"
     },
     ...
     if not is_blueprint then
          TriggerClientEvent('AJFW:Notify', src, 'Blueprint name is not valid', 'error')
          return
     else
          GiveBlueprint(src, name)
          TriggerClientEvent('AJFW:Notify', src, ('A blueprint of [%s] added to your inventory.'):format(name), 'success')
     end
end, "dev")
