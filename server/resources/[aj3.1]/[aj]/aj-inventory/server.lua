	...
	if inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			elseif not item.weapon and server.UseItem then
				...
				-- This is used to call an external useItem function, i.e. ESX.UseItem / AJFW.Functions.CanUseItem
...
RegisterCommand('convertinventory', function(source, args)
	...
	if not convert then
		return warn('Invalid conversion argument. Valid options: esx, esxproperty, aj, linden')
	end
