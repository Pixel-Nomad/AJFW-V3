...
lib.callback.register('aj_inventory:openCraftingBench', function(source, id, index)
...
lib.callback.register('aj_inventory:craftItem', function(source, id, index, recipeId, toSlot)
	local left, bench = Inventory(source), CraftingBenches[id]
	...
	if bench then
		...
		if recipe then
			...
			local success = lib.callback.await('aj_inventory:startCrafting', source, id, recipeId)
