...
RegisterNetEvent('consumables:client:Eat', function(itemName)
    AJFW.Functions.Progressbar('eat_something', Lang:t('consumables.eat_progress'), 5000, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[itemName], 'remove')
        TriggerServerEvent('consumables:server:addHunger', AJFW.Functions.GetPlayerData().metadata.hunger + Config.Consumables.eat[itemName])
        TriggerServerEvent('hud:server:RelieveStress', math.random(2, 4))
    end)
end)

RegisterNetEvent('consumables:client:Drink', function(itemName)
    AJFW.Functions.Progressbar('drink_something', Lang:t('consumables.drink_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[itemName], 'remove')
        TriggerServerEvent('consumables:server:addThirst', AJFW.Functions.GetPlayerData().metadata.thirst + Config.Consumables.drink[itemName])
    end)
end)

RegisterNetEvent('consumables:client:DrinkAlcohol', function(itemName)
    AJFW.Functions.Progressbar('drink_alcohol', Lang:t('consumables.liqour_progress'), math.random(3000, 6000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        TriggerServerEvent('consumables:server:addThirst', AJFW.Functions.GetPlayerData().metadata.thirst + Config.Consumables.alcohol[itemName])
        TriggerServerEvent('hud:server:RelieveStress', math.random(2, 4))
        alcoholCount += 1
        AlcoholLoop()
        ...
        AJFW.Functions.Notify(Lang:t('consumables.canceled'), 'error')
    end)
end)

RegisterNetEvent('consumables:client:Custom', function(itemName)
    AJFW.Functions.TriggerCallback('consumables:itemdata', function(data)
        AJFW.Functions.Progressbar('custom_consumable', data.progress.label, data.progress.time, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true
            ...
            if data.replenish.type then
                TriggerServerEvent('consumables:server:add' .. data.replenish.type, AJFW.Functions.GetPlayerData().metadata[string.lower(data.replenish.type)] + data.replenish.replenish)
            end
...
RegisterNetEvent('consumables:client:Cokebaggy', function()
    local ped = PlayerPedId()
    AJFW.Functions.Progressbar('snort_coke', Lang:t('consumables.coke_progress'), math.random(5000, 8000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['cokebaggy'], 'remove')
...
RegisterNetEvent('consumables:client:Crackbaggy', function()
    local ped = PlayerPedId()
    AJFW.Functions.Progressbar('snort_coke', Lang:t('consumables.crack_progress'), math.random(7000, 10000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['crack_baggy'], 'remove')
...
RegisterNetEvent('consumables:client:EcstasyBaggy', function()
    AJFW.Functions.Progressbar('use_ecstasy', Lang:t('consumables.ecstasy_progress'), 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['xtcbaggy'], 'remove')
        ecstasyEffect()
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), 'mp_suicide', 'pill', 1.0)
        AJFW.Functions.Notify(Lang:t('consumables.canceled'), 'error')
...
RegisterNetEvent('consumables:client:oxy', function()
    AJFW.Functions.Progressbar('use_oxy', Lang:t('consumables.healing_progress'), 2000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['oxy'], 'remove')
...
RegisterNetEvent('consumables:client:meth', function()
    AJFW.Functions.Progressbar('snort_meth', Lang:t('consumables.meth_progress'), 1500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['meth'], 'remove')
        TriggerEvent('evidence:client:SetStatus', 'widepupils', 300)
...
RegisterNetEvent('consumables:client:UseJoint', function()
    AJFW.Functions.Progressbar('smoke_joint', Lang:t('consumables.joint_progress'), 1500, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['joint'], 'remove')
...
RegisterNetEvent('consumables:client:UseParachute', function()
    equipParachuteAnim()
    AJFW.Functions.Progressbar('use_parachute', Lang:t('consumables.use_parachute_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        local ped = PlayerPedId()
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['parachute'], 'remove')
        GiveWeaponToPed(ped, `GADGET_PARACHUTE`, 1, false, false)
        local parachuteData = {
            outfitData = { ['bag'] = { item = 7, texture = 0 } } -- Adding Parachute Clothing
        }
        TriggerEvent('aj-clothing:client:loadOutfit', parachuteData)
...
RegisterNetEvent('consumables:client:ResetParachute', function()
    if parachuteEquipped then
        equipParachuteAnim()
        AJFW.Functions.Progressbar('reset_parachute', Lang:t('consumables.pack_parachute_progress'), 40000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            local ped = PlayerPedId()
            TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['parachute'], 'add')
            local parachuteResetData = {
                outfitData = { ['bag'] = { item = 0, texture = 0 } } -- Removing Parachute Clothing
            }
            TriggerEvent('aj-clothing:client:loadOutfit', parachuteResetData)
            TaskPlayAnim(ped, 'clothingshirt', 'exit', 8.0, 1.0, -1, 49, 0, false, false, false)
            TriggerServerEvent('aj-smallpenis:server:AddParachute')
            parachuteEquipped = false
        end)
    else
        AJFW.Functions.Notify(Lang:t('consumables.no_parachute'), 'error')
    end
end)

RegisterNetEvent('consumables:client:UseArmor', function()
    if GetPedArmour(PlayerPedId()) >= 75 then
        AJFW.Functions.Notify(Lang:t('consumables.armor_full'), 'error')
        return
    end
    AJFW.Functions.Progressbar('use_armor', Lang:t('consumables.armor_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('consumables:client:UseHeavyArmor', function()
    ...
    AJFW.Functions.Progressbar('use_heavyarmor', Lang:t('consumables.heavy_armor_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('consumables:client:ResetArmor', function()
    ...
    if currVest ~= nil and currVestTexture ~= nil then
        AJFW.Functions.Progressbar('remove_armor', Lang:t('consumables.remove_armor_progress'), 2500, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            SetPedArmour(ped, 0)
            TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['heavyarmor'], 'add')
            TriggerServerEvent('consumables:server:resetArmor')
        end)
    else
        AJFW.Functions.Notify(Lang:t('consumables.armor_empty'), 'error')
    end
end)
...
--         TriggerEvent("inventory:client:Itembox", AJFW.Shared.Items["smoketrailred"], "remove")
--     else
--         AJFW.Functions.Notify("You need to have a paracute to activate smoke!", "error")
--     end
-- end)
