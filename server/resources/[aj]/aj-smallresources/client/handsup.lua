...
RegisterCommand(Config.HandsUp.command, function()
    ...
    handsUp = not handsUp
    if exports['aj-policejob']:IsHandcuffed() then return end
    if handsUp then
        TaskPlayAnim(ped, 'missminuteman_1ig_2', 'handsup_base', 8.0, 8.0, -1, 50, 0, false, false, false)
        exports['aj-smallresources']:addDisableControls(Config.HandsUp.controls)
    else
        ClearPedTasks(ped)
        exports['aj-smallresources']:removeDisableControls(Config.HandsUp.controls)
    end
end, false)
