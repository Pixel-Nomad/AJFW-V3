...
local function updatePermissionLevel()
    AJFW.Functions.TriggerCallback('aj-afkkick:server:GetPermissions', function(userGroups)
...
RegisterNetEvent('AJFW:Client:OnPermissionUpdate', function()
    updatePermissionLevel()
end)
                        ...
                        if time then
                            if time > 0 then
                                ...
                                if _type == 'minutes' then
                                    AJFW.Functions.Notify(Lang:t('afk.will_kick') .. math.ceil(time / 60) .. Lang:t('afk.time_minutes'), 'error', 10000)
                                elseif _type == 'seconds' then
                                    AJFW.Functions.Notify(Lang:t('afk.will_kick') .. time .. Lang:t('afk.time_seconds'), 'error', 10000)
                                end
                                time -= 10
                            else
