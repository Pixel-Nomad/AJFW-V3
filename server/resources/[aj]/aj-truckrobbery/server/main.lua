...
RegisterServerEvent('AttackTransport:akceptujto', function()
	...
	local xPlayer = AJFW.Functions.GetPlayer(_source)
	local accountMoney = xPlayer.PlayerData.money['bank']
	if ActiveMission == 0 then
		if accountMoney < Config.ActivationCost then
			TriggerClientEvent('AJFW:Notify', _source, 'You need ' .. Config.Currency .. '' .. Config.ActivationCost .. ' in the bank to accept the mission')
		else
		...
		TriggerClientEvent('AJFW:Notify', _source, 'Someone is already carrying out this mission')
	end
end)

RegisterServerEvent('aj-armoredtruckheist:server:callCops', function(streetLabel, coords)
	-- local place = "Armored Truck"
	-- local msg = "The Alarm has been activated from a "..place.. " at " ..streetLabel
	-- Why is this unused?
	TriggerClientEvent('aj-armoredtruckheist:client:robberyCall', -1, streetLabel, coords)
end)
...
RegisterServerEvent('AttackTransport:graczZrobilnapad', function()
	...
	TriggerClientEvent('inventory:client:ItemBox', _source, AJFW.Shared.Items['markedbills'], 'add')

	local chance = math.random(1, 100)
	TriggerClientEvent('AJFW:Notify', _source, 'You took ' .. bags .. ' bags of cash from the van')

	if chance >= 95 then
		xPlayer.Functions.AddItem('security_card_01', 1)
		TriggerClientEvent('inventory:client:ItemBox', _source, AJFW.Shared.Items['security_card_01'], 'add')
	end
	Wait(2500)
end)
