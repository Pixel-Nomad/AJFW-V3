AJFW = exports['aj-base']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
currentJob = nil
CellsBlip = nil
...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('aj-clothing:client:loadOutfit', Config.Uniforms.male)
			else
				TriggerEvent('aj-clothing:client:loadOutfit', Config.Uniforms.female)
			end
		end)
	end
end
...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
	...
	AJFW.Functions.TriggerCallback('prison:server:IsAlarmActive', function(active)
		if active then
	...
	exports['aj-target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = "client",
	...
	exports['aj-target']:AddTargetEntity(canteen_ped, {
		options = {
			{
				type = "client",
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'aj-policejob' and invokingResource ~= 'aj-ambulancejob' and invokingResource ~= GetCurrentResourceName() then
		-- Use AJFW.Debug here for a quick and easy way to print to the console to grab your attention with this message
		AJFW.Debug({ ('Player with source %s tried to execute prison:client:Enter manually or from another resource which is not authorized to call this, invokedResource: %s'):format(GetPlayerServerId(PlayerId()), invokingResource) })
		return
	end

	AJFW.Functions.Notify(Lang:t("error.injail", { Time = time }), "error")
	...
	Wait(2000)
	DoScreenFadeIn(1000)
	AJFW.Functions.Notify(Lang:t("error.do_some_work", { currentjob = Config.Jobs[currentJob] }), "error")
end)

RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		AJFW.Functions.Notify(Lang:t("info.timeleft", { JAILTIME = jailTime }))
	else
		jailTime = 0
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		AJFW.Functions.Notify(Lang:t("success.free_"))
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Wait(10)
		end
	...
	while true do
		...
		if jailTime > 0 and inJail then
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					AJFW.Functions.Notify(Lang:t("success.timesup"), "success", 10000)
				end
				TriggerServerEvent("prison:server:SetJailStatus", jailTime)
			end
		end
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['aj-base']:DrawText('[E] Check Time', 'left')
			else
				exports['aj-base']:HideText()
			end
		end)
		...
		canteen:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['aj-base']:DrawText('[E] Open Canteen', 'left')
			else
				exports['aj-base']:HideText()
			end
		end)
