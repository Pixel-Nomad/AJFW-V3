...
RegisterNetEvent('prison:server:SetJailStatus', function(jailTime)
    ...
    if jailTime > 0 then
        if Player.PlayerData.job.name ~= 'unemployed' then
            ...
            TriggerClientEvent('AJFW:Notify', src, Lang:t('info.lost_job'))
        end
    else
        GotItems[source] = nil
    end
...
RegisterNetEvent('prison:server:CheckChance', function()
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['phone'], 'add')
    TriggerClientEvent('AJFW:Notify', src, Lang:t('success.found_phone'), 'success')
    GotItems[src] = true
end)

AJFW.Functions.CreateCallback('prison:server:IsAlarmActive', function(_, cb)
    cb(AlarmActivated)
end)
