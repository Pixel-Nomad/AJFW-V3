...
AJFW.Functions.CreateCallback('aj-shops:server:SetShopInv', function(_,cb)
    local shopInvJson = LoadResourceFile(GetCurrentResourceName(), Config.ShopsInvJsonFile)
    cb(shopInvJson)
end)

RegisterNetEvent('aj-shops:server:SaveShopInv',function()
...
RegisterNetEvent('aj-shops:server:UpdateShopItems', function(shop, itemData, amount)
    if not Config.UseTruckerJob then return end
    ...
    TriggerEvent('aj-shops:server:SaveShopInv')
    TriggerClientEvent('aj-shops:client:SetShopItems', -1, shop, Config.Locations[shop].products)
end)

RegisterNetEvent('aj-shops:server:RestockShopItems', function(shop)
    ...
    TriggerEvent('aj-shops:server:SaveShopInv')
    TriggerClientEvent('aj-shops:client:RestockShopItems', -1, shop, randAmount)
end)
        ...
        for k in pairs(Player.PlayerData.items) do
            if Player.PlayerData.items[k] then
                if ItemList[Player.PlayerData.items[k].name] then
                    ...
                    AJFW.Functions.Notify(src, "You sold your chips for $" .. price)
                    TriggerEvent("aj-log:server:CreateLog", "casino", "Chips", "blue", "**" .. GetPlayerName(src) .. "** got $" .. price .. " for selling the Chips")
                end
            end
        end
    else
        AJFW.Functions.Notify(src, "You have no chips..")
    end
end)

RegisterNetEvent('aj-shops:server:SetShopList',function()
    local shoplist = {}
    local cnt = 0
    for k, v in pairs(Config.Locations) do
        cnt = cnt + 1
    ...
    TriggerClientEvent('aj-truckerjob:client:SetShopList',-1,shoplist)
end)