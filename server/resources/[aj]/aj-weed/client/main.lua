    ...
    CreateThread(function()
        for k in pairs(housePlants[currentHouse]) do
            local plantData = {
                ...
                ["plantProp"] = joaat(AJWeed.Plants[housePlants[currentHouse][k].sort].stages[housePlants[currentHouse][k].stage]),
            }
...
local function updateHousePlants(leftHouse)
    ...
    CreateThread(function()
        for k in pairs(housePlants[currentHouse]) do
            ...
            for _, stage in pairs(AJWeed.Plants[housePlants[currentHouse][k].sort]["stages"]) do
        ...
        if leftHouse then
            ...
            AJFW.Functions.TriggerCallback('aj-weed:server:getBuildingPlants', function(plants)
...
local function getPlantData()
    return {
        ...
        ["plantProp"] = GetHashKey(AJWeed.Plants[housePlants[currentHouse][closestPlant].sort]["stages"][housePlants[currentHouse][closestPlant].stage]),
        ["plantSort"] = {
            ["name"] = housePlants[currentHouse][closestPlant].sort,
            ["label"] = AJWeed.Plants[housePlants[currentHouse][closestPlant].sort]["label"],
        },
        ["plantStats"] = {
            ...
            ["highestStage"] = AJWeed.Plants[housePlants[currentHouse][closestPlant].sort]["highestStage"],
    ...
    CreateThread(function()
        while currentHouse do
            ...
            if plantSpawned and closestPlant ~= 0 then
                ...
                local plantStageLabel = Lang:t('text.stage').. ' ~b~' .. AJWeed.StageLabels[plantData["plantStats"]["stage"]] ..' ~w~' ..Lang:t('text.progress')..' ~b~'..plantData["plantStats"]["progress"]..'% ~w~ / ' ..Lang:t('text.highestStage') .. ' ~b~' .. AJWeed.StageLabels[plantData["plantStats"]["highestStage"]]
                ...
                if plantStageLabel and AJWeed.ShowStages then drawText3Ds(plantData["plantCoords"].x, plantData["plantCoords"].y, plantData["plantCoords"].z - 0.1, plantStageLabel) end
                ...
                if status > 0 and IsControlJustPressed(0, 38) then
                    ...
                    AJFW.Functions.Progressbar("remove_weed_plant", Lang:t((status == 1) and 'text.harvesting_plant' or 'text.removing_the_plant'), 8000, false, true, {
                        ...
                        if status == 1 then
                            ...
                            TriggerServerEvent('aj-weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                        else
                            TriggerServerEvent('aj-weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                        end
                    end, function() -- Cancel
                        ClearPedTasks(ped)
                        AJFW.Functions.Notify(Lang:t('error.process_canceled'), "error")
                    end)
                end
            else
                Wait(100)
...
RegisterNetEvent('aj-weed:client:getHousePlants', function(house)
    AJFW.Functions.TriggerCallback('aj-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
        housePlants[currentHouse] = plants
        spawnHousePlants()
...
RegisterNetEvent('aj-weed:client:leaveHouse', function()
    updateHousePlants(true)
end)

RegisterNetEvent('aj-weed:client:refreshHousePlants', function(house)
    if currentHouse == house or not house then
        updateHousePlants(false)
    end
end)

RegisterNetEvent('aj-weed:client:placePlant', function(type, item)
    if not currentHouse then AJFW.Functions.Notify(Lang:t('error.not_safe_here'), 'error', 3500) return end
    if closestPlant ~= 0 then AJFW.Functions.Notify(Lang:t('error.cant_place_here'), 'error', 3500) return end
    local ped = PlayerPedId()
    ...
    local plantData = {
        ...
        ["plantModel"] = AJWeed.Plants[type]["stages"]["stage-a"],
        ["plantLabel"] = AJWeed.Plants[type]["label"]
    }

    LocalPlayer.state:set("inv_busy", true, true)
    AJFW.Functions.Progressbar("plant_weed_plant", Lang:t('text.planting'), 8000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('aj-weed:server:placePlant', json.encode(plantData["plantCoords"]), type, currentHouse)
        TriggerServerEvent('aj-weed:server:removeSeed', item.slot, type)
    end, function() -- Cancel
        ClearPedTasks(ped)
        AJFW.Functions.Notify(Lang:t('error.process_canceled'), "error")
        LocalPlayer.state:set("inv_busy", false, true)
...
RegisterNetEvent('aj-weed:client:foodPlant', function()
    ...
    if closestPlant ~= 0 then
        ...
        if plantData["plantStats"]["food"] == 100 then
            AJFW.Functions.Notify(Lang:t('error.not_need_nutrition'), 'error', 3500)
        else
            LocalPlayer.state:set("inv_busy", true, true)
            AJFW.Functions.Progressbar("plant_weed_plant", Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                TriggerServerEvent('aj-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
            end, function() -- Cancel
                ClearPedTasks(ped)
                LocalPlayer.state:set("inv_busy", false, true)
                AJFW.Functions.Notify(Lang:t('error.process_canceled'), "error")
        ...
        AJFW.Functions.Notify(Lang:t('error.cant_place_here'), "error")
    end
end)
