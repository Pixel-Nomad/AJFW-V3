...
local function exploitBan(id, reason)
    ...
    TriggerEvent('aj-log:server:CreateLog', 'pawnshop', 'Player Banned', 'red',
        string.format('%s was banned by %s for %s', GetPlayerName(id), 'aj-pawnshop', reason), true)
    DropPlayer(id, 'You were permanently banned by the server for: Exploiting')
end

RegisterNetEvent('aj-pawnshop:server:sellPawnItems', function(itemName, itemAmount, itemPrice)
    local src = source
    ...
    if Player.Functions.RemoveItem(itemName, tonumber(itemAmount)) then
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t('success.sold', { value = tonumber(itemAmount), value2 = AJFW.Shared.Items[itemName].label, value3 = totalPrice }), 'success')
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[itemName], 'remove')
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.no_items'), 'error')
    end
    TriggerClientEvent('aj-pawnshop:client:openMenu', src)
end)

RegisterNetEvent('aj-pawnshop:server:meltItemRemove', function(itemName, itemAmount, item)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.Functions.RemoveItem(itemName, itemAmount) then
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[itemName], 'remove')
        local meltTime = (tonumber(itemAmount) * item.time)
        TriggerClientEvent('aj-pawnshop:client:startMelting', src, item, tonumber(itemAmount), (meltTime * 60000 / 1000))
        TriggerClientEvent('AJFW:Notify', src, Lang:t('info.melt_wait', { value = meltTime }), 'primary')
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.no_items'), 'error')
    end
end)

RegisterNetEvent('aj-pawnshop:server:pickupMelted', function(item)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local playerCoords = GetEntityCoords(GetPlayerPed(src))
    local dist
...
AJFW.Functions.CreateCallback('aj-pawnshop:server:getInv', function(source, cb)
