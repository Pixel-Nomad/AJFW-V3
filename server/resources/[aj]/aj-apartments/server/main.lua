...
RegisterNetEvent('aj-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
    ...
    if bool then
        ...
        AJFW.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData("inside", insideMeta)
        AJFW.Functions.SetPlayerBucket(src, 0)
    end
end)
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('AJFW:Notify', src, Lang:t('success.receive_apart').." ("..label..")")
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('AJFW:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
AJFW.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
AJFW.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
AJFW.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
AJFW.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
AJFW.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
AJFW.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
