    ...
    if coords then
        AJFW.Functions.TriggerCallback('AJFW:Server:SpawnVehicle', function(netId)
            ...
            AJFW.Functions.TriggerCallback('aj-garage:server:GetVehicleProperties', function(properties)
                AJFW.Functions.SetVehicleProperties(veh, properties)
        ...
        AJFW.Functions.TriggerCallback('AJFW:Server:SpawnVehicle', function(netId)
            ...
            if Config.VehicleSettings[vehicleInfo] ~= nil then
                if Config.VehicleSettings[vehicleInfo].extras ~= nil then
                    AJFW.Shared.SetDefaultVehicleExtras(veh, Config.VehicleSettings[vehicleInfo].extras)
                end
            ...
            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
            TriggerEvent('vehiclekeys:client:SetOwner', AJFW.Functions.GetPlate(veh))
            TriggerServerEvent('inventory:server:addTrunkItems', AJFW.Functions.GetPlate(veh), Config.CarItems)
...
function MenuGarage(currentSelection)
    ...
    local playerGrade = AJFW.Functions.GetPlayerData().job.grade.level
...
function MenuImpound(currentSelection)
    ...
    AJFW.Functions.TriggerCallback('police:GetImpoundedVehicles', function(result)
        local shouldContinue = false
        if result == nil then
            AJFW.Functions.Notify(Lang:t('error.no_impound'), 'error', 5000)
        else
            shouldContinue = true
            for _, v in pairs(result) do
                local enginePercent = AJFW.Shared.Round(v.engine / 10, 0)
                local currentFuel = v.fuel
                local vname = AJFW.Shared.Vehicles[v.vehicle].name
        ...
        if shouldContinue then
            ...
            exports['aj-menu']:openMenu(impoundMenu)
        end
    end)
end
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            AJFW.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerEvent('AJFW:Notify', Lang:t('success.impounded'), 'success')
                ClearPedTasks(ped)
            end, function() -- Play When Cancel
                ClearPedTasks(ped)
                TriggerEvent('AJFW:Notify', Lang:t('error.canceled'), 'error')
            end)
        end
    end
end)
    ...
    AJFW.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.type == 'leo' then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                AJFW.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            AJFW.Functions.Notify('' .. v .. '')
                        end
                    end
                end, playerId)
            else
                AJFW.Functions.Notify(Lang:t('error.none_nearby'), 'error')
            end
        end
    end)
end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function()
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['aj-input']:ShowInput({
...
RegisterNetEvent('aj-policejob:ToggleDuty', function()
    TriggerServerEvent('AJFW:ToggleDuty')
    TriggerServerEvent('police:server:UpdateCurrentCops')
    TriggerServerEvent('police:server:UpdateBlips')
end)

RegisterNetEvent('aj-police:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('aj-police:client:openArmoury', function()
    ...
    for grade = 0, playerGrade do
        if Config.Items[grade] then
            for _, item in ipairs(Config.Items[grade]) do
                local itemInfo = AJFW.Shared.Items[item.name]
                if itemInfo then
...
RegisterNetEvent('aj-police:client:spawnHelicopter', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        AJFW.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
    else
...
RegisterNetEvent('aj-police:client:openStash', function()
    TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'policestash_' .. AJFW.Functions.GetPlayerData().citizenid)
    TriggerEvent('inventory:client:SetCurrentStash', 'policestash_' .. AJFW.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('aj-police:client:openTrash', function()
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('aj-police:client:scanFingerPrint')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inArmoury and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('aj-police:client:openArmoury')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('aj-police:client:spawnHelicopter')
                    break
                end
            else
                break
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations['duty']) do
            exports['aj-target']:AddCircleZone('PoliceDuty_' .. k, vector3(v.x, v.y, v.z), 0.5, {
                name = 'PoliceDuty_' .. k,
                useZ = true,
                debugPoly = false,
            }, {
        ...
        for k, v in pairs(Config.Locations['stash']) do
            exports['aj-target']:AddCircleZone('PoliceStash_' .. k, vector3(v.x, v.y, v.z), 1.0, {
                name = 'PoliceStash_' .. k,
                useZ = true,
                debugPoly = false,
            }, {
                options = {
                    {
                        type = 'client',
                        event = 'aj-police:client:openStash',
                        icon = 'fas fa-dungeon',
        ...
        for k, v in pairs(Config.Locations['trash']) do
            exports['aj-target']:AddCircleZone('PoliceTrash_' .. k, vector3(v.x, v.y, v.z), 0.5, {
                name = 'PoliceTrash_' .. k,
                useZ = true,
                debugPoly = false,
            }, {
                options = {
                    {
                        type = 'client',
                        event = 'aj-police:client:openTrash',
                        icon = 'fas fa-trash',
        ...
        for k, v in pairs(Config.Locations['fingerprint']) do
            exports['aj-target']:AddCircleZone('PoliceFingerprint_' .. k, vector3(v.x, v.y, v.z), 0.5, {
                ...
                options = {
                    {
                        type = 'client',
                        event = 'aj-police:client:scanFingerPrint',
        ...
        for k, v in pairs(Config.Locations['armory']) do
            exports['aj-target']:AddCircleZone('PoliceArmory_' .. k, vector3(v.x, v.y, v.z), 1.0, {
                ...
                options = {
                    {
                        type = 'client',
                        event = 'aj-police:client:openArmoury',
                        icon = 'fas fa-gun',
        ...
        -- Evidence
        for k, v in pairs(Config.Locations['evidence']) do
            exports['aj-target']:AddCircleZone('PoliceEvidence_' .. k, vector3(v.x, v.y, v.z), 0.5, {
    ...
    dutyCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if not PlayerJob.onduty then
                exports['aj-base']:DrawText(Lang:t('info.on_duty'), 'left')
                dutylistener()
            else
                exports['aj-base']:DrawText(Lang:t('info.off_duty'), 'left')
                dutylistener()
            end
        else
            dutylisten = false
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['aj-base']:DrawText(Lang:t('info.stash_enter'), 'left')
                stash()
            end
        else
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['aj-base']:DrawText(Lang:t('info.trash_enter'), 'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['aj-base']:DrawText(Lang:t('info.scan_fingerprint'), 'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    armouryCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inArmoury = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['aj-base']:DrawText(Lang:t('info.enter_armory'), 'left')
                armoury()
            end
        else
            inArmoury = false
    ...
    evidenceCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inEvidence = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['aj-base']:DrawText(Lang:t('info.evidence_stash_prompt'), 'left')
                evidence()
            end
        else
            inEvidence = false
...
CreateThread(function()
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['aj-base']:HideText()
                    exports['aj-base']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['aj-base']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['aj-base']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['aj-base']:DrawText(Lang:t('info.store_veh'), 'left')
                    garage()
                else
                    local currentSelection = 0
