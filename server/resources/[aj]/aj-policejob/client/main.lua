...
AddEventHandler('AJFW:Client:OnPlayerLoaded', function()
    ...
    if player.metadata.tracker then
        ...
        TriggerEvent('aj-clothing:client:loadOutfit', trackerClothingData)
    else
        local trackerClothingData = {
            outfitData = {
                ['accessory'] = {
...
RegisterNetEvent('AJFW:Client:SetDuty', function(newDuty)
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    ...
    AJFW.Functions.Notify({ text = text, caption = street1name .. ' ' .. street2name }, 'police')
