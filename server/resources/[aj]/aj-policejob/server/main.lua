...
-- Commands
AJFW.Commands.Add('spikestrip', Lang:t('commands.place_spike'), {}, false, function(source)
    local src = source
...
AJFW.Commands.Add('grantlicense', Lang:t('commands.license_grant'), { { name = 'id', help = Lang:t('info.player_id') }, { name = 'license', help = Lang:t('info.license_type') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == 'driver' or args[2] == 'weapon' then
            local SearchedPlayer = AJFW.Functions.GetPlayer(tonumber(args[1]))
            if not SearchedPlayer then return end
            local licenseTable = SearchedPlayer.PlayerData.metadata['licences']
            if licenseTable[args[2]] then
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.license_already'), 'error')
                return
            end
            licenseTable[args[2]] = true
            SearchedPlayer.Functions.SetMetaData('licences', licenseTable)
            TriggerClientEvent('AJFW:Notify', SearchedPlayer.PlayerData.source, Lang:t('success.granted_license'), 'success')
            TriggerClientEvent('AJFW:Notify', src, Lang:t('success.grant_license'), 'success')
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.error_license_type'), 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.rank_license'), 'error')
    end
end)

AJFW.Commands.Add('revokelicense', Lang:t('commands.license_revoke'), { { name = 'id', help = Lang:t('info.player_id') }, { name = 'license', help = Lang:t('info.license_type') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == 'driver' or args[2] == 'weapon' then
            ...
            if not licenseTable[args[2]] then
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.error_license'), 'error')
                return
            end
            licenseTable[args[2]] = false
            SearchedPlayer.Functions.SetMetaData('licences', licenseTable)
            TriggerClientEvent('AJFW:Notify', SearchedPlayer.PlayerData.source, Lang:t('error.revoked_license'), 'error')
            TriggerClientEvent('AJFW:Notify', src, Lang:t('success.revoke_license'), 'success')
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.error_license'), 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.rank_revoke'), 'error')
    end
end)

AJFW.Commands.Add('pobject', Lang:t('commands.place_object'), { { name = 'type', help = Lang:t('info.poobject_object') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
    end
end)

AJFW.Commands.Add('cuff', Lang:t('commands.cuff_player'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('escort', Lang:t('commands.escort'), {}, false, function(source)
    local src = source
    TriggerClientEvent('police:client:EscortPlayer', src)
end)

AJFW.Commands.Add('callsign', Lang:t('commands.callsign'), { { name = 'name', help = Lang:t('info.callsign_name') } }, false, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    Player.Functions.SetMetaData('callsign', table.concat(args, ' '))
end)

AJFW.Commands.Add('clearcasings', Lang:t('commands.clear_casign'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('jail', Lang:t('commands.jail_player'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('unjail', Lang:t('commands.unjail_player'), { { name = 'id', help = Lang:t('info.player_id') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('clearblood', Lang:t('commands.clearblood'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('seizecash', Lang:t('commands.seizecash'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('sc', Lang:t('commands.softcuff'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('cam', Lang:t('commands.camera'), { { name = 'camid', help = Lang:t('info.camera_id') } }, false, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('flagplate', Lang:t('commands.flagplate'), { { name = 'plate', help = Lang:t('info.plate_number') }, { name = 'reason', help = Lang:t('info.flag_reason') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        local reason = {}
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t('info.vehicle_flagged', { vehicle = args[1]:upper(), reason = table.concat(reason, ' ') }))
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
    end
end)

AJFW.Commands.Add('unflagplate', Lang:t('commands.unflagplate'), { { name = 'plate', help = Lang:t('info.plate_number') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                ...
                TriggerClientEvent('AJFW:Notify', src, Lang:t('info.unflag_vehicle', { vehicle = args[1]:upper() }))
            else
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.vehicle_not_flag'), 'error')
            end
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.vehicle_not_flag'), 'error')
        end
...
AJFW.Commands.Add('plateinfo', Lang:t('commands.plateinfo'), { { name = 'plate', help = Lang:t('info.plate_number') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                TriggerClientEvent('AJFW:Notify', src, Lang:t('success.vehicle_flagged', { plate = args[1]:upper(), reason = Plates[args[1]:upper()].reason }), 'success')
            else
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.vehicle_not_flag'), 'error')
            end
        else
...
AJFW.Commands.Add('depot', Lang:t('commands.depot'), { { name = 'price', help = Lang:t('info.impound_price') } }, false, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('impound', Lang:t('commands.impound'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('paytow', Lang:t('commands.paytow'), { { name = 'id', help = Lang:t('info.player_id') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == 'tow' then
                ...
                TriggerClientEvent('AJFW:Notify', OtherPlayer.PlayerData.source, Lang:t('success.tow_paid'), 'success')
                TriggerClientEvent('AJFW:Notify', src, Lang:t('info.tow_driver_paid'))
            else
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_towdriver'), 'error')
            end
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
...
AJFW.Commands.Add('paylawyer', Lang:t('commands.paylawyer'), { { name = 'id', help = Lang:t('info.player_id') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' or Player.PlayerData.job.name == 'judge' then
        ...
        if OtherPlayer.PlayerData.job.name == 'lawyer' then
            ...
            TriggerClientEvent('AJFW:Notify', OtherPlayer.PlayerData.source, Lang:t('success.tow_paid'), 'success')
            TriggerClientEvent('AJFW:Notify', src, Lang:t('info.paid_lawyer'))
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_lawyer'), 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
    end
...
AJFW.Commands.Add('fine', Lang:t('commands.fine'), { { name = 'id', help = Lang:t('info.player_id') }, { name = 'amount', help = Lang:t('info.amount') } }, false, function(source, args)
    local biller = AJFW.Functions.GetPlayer(source)
    local billed = AJFW.Functions.GetPlayer(tonumber(args[1]))
                ...
                if amount and amount > 0 then
                    ...
                    elseif billed.Functions.RemoveMoney('cash', amount, 'paid-fine') then
                        ...
                        MySQL.Async.insert('INSERT INTO phone_invoices (citizenid, amount, society, sender, sendercitizenid) VALUES (?, ?, ?, ?, ?)', { billed.PlayerData.citizenid, amount, biller.PlayerData.job.name, biller.PlayerData.charinfo.firstname, biller.PlayerData.citizenid }, function(id)
                            if id then
                                TriggerClientEvent('aj-phone:client:AcceptorDenyInvoice', billed.PlayerData.source, id, biller.PlayerData.charinfo.firstname, biller.PlayerData.job.name, biller.PlayerData.citizenid, amount, GetInvokingResource())
                            end
                        end)
                        TriggerClientEvent('aj-phone:RefreshPhone', billed.PlayerData.source)
                    end
                else
                    TriggerClientEvent('AJFW:Notify', source, Lang:t('error.amount_higher'), 'error')
                end
            else
                TriggerClientEvent('AJFW:Notify', source, Lang:t('error.fine_yourself'), 'error')
            end
        else
            TriggerClientEvent('AJFW:Notify', source, Lang:t('error.not_online'), 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', source, Lang:t('error.on_duty_police_only'), 'error')
    end
end)

AJFW.Commands.Add('anklet', Lang:t('commands.anklet'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('ankletlocation', Lang:t('commands.ankletlocation'), { { name = 'cid', help = Lang:t('info.citizen_id') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        local citizenid = args[1]
        local Target = AJFW.Functions.GetPlayerByCitizenId(citizenid)
        ...
        if Target.PlayerData.metadata['tracker'] then
            ...
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.no_anklet'), 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
    end
...
AJFW.Commands.Add('takedrivinglicense', Lang:t('commands.drivinglicense'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
...
AJFW.Commands.Add('takedna', Lang:t('commands.takedna'), { { name = 'id', help = Lang:t('info.player_id') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local OtherPlayer = AJFW.Functions.GetPlayer(tonumber(args[1]))
    ...
    if Player.Functions.RemoveItem('empty_evidence_bag', 1) then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['filled_evidence_bag'], 'add')
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.have_evidence_bag'), 'error')
    end
end)

RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = AJFW.Functions.GetPlayer(source)
    ...
    TriggerClientEvent('aj-phone:client:addPoliceAlert', requestId, alertData)
end)

AJFW.Commands.Add('911p', Lang:t('commands.police_report'), { { name = 'message', help = Lang:t('commands.message_sent') } }, false, function(source, args)
    local src = source
    local message
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == 'leo' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent('aj-phone:client:addPoliceAlert', v.PlayerData.source, alertData)
...
-- Items
AJFW.Functions.CreateUseableItem('handcuffs', function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateUseableItem('moneybag', function(source, item)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
-- Callbacks
AJFW.Functions.CreateCallback('police:server:isPlayerDead', function(_, cb, playerId)
    local Player = AJFW.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata['isdead'])
end)

AJFW.Functions.CreateCallback('police:GetPlayerStatus', function(_, cb, playerId)
    local Player = AJFW.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
AJFW.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = AJFW.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(AJFW.Shared.Weapons[weapon]['name'])
    local retval = false
    if itemInfo then
...
AJFW.Functions.CreateCallback('police:GetDutyPlayers', function(_, cb)
    local dutyPlayers = {}
    local players = AJFW.Functions.GetAJPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == 'leo' and v.PlayerData.job.onduty then
...
AJFW.Functions.CreateCallback('police:GetImpoundedVehicles', function(_, cb)
    local vehicles = {}
...
AJFW.Functions.CreateCallback('police:IsPlateFlagged', function(_, cb, plate)
    local retval = false
...
AJFW.Functions.CreateCallback('police:GetCops', function(_, cb)
    local amount = 0
    local players = AJFW.Functions.GetAJPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == 'leo' and v.PlayerData.job.onduty then
...
AJFW.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(_, cb)
    local retval = false
    local players = AJFW.Functions.GetAJPlayers()
    for _, v in pairs(players) do
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate, garage)
    ...
    TriggerClientEvent('AJFW:Notify', src, Lang:t('success.impound_vehicle_removed'), 'success')
end)
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    local Player = AJFW.Functions.GetPlayer(src)
    local CuffedPlayer = AJFW.Functions.GetPlayer(playerId)
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    local Player = AJFW.Functions.GetPlayer(source)
    local EscortPlayer = AJFW.Functions.GetPlayer(playerId)
    ...
    if (Player.PlayerData.job.type == 'leo' or Player.PlayerData.job.name == 'ambulance') or (EscortPlayer.PlayerData.metadata['ishandcuffed'] or EscortPlayer.PlayerData.metadata['isdead'] or EscortPlayer.PlayerData.metadata['inlaststand']) then
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_cuffed_dead'), 'error')
    end
end)
...
RegisterNetEvent('police:server:SetPlayerOutVehicle', function(playerId)
    ...
    local EscortPlayer = AJFW.Functions.GetPlayer(playerId)
    if not AJFW.Functions.GetPlayer(src) or not EscortPlayer then return end
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    TriggerClientEvent('AJFW:Notify', src, Lang:t('info.sent_jail_for', { time = time }))
end)
...
RegisterNetEvent('police:server:SearchPlayer', function(playerId)
    ...
    local SearchedPlayer = AJFW.Functions.GetPlayer(playerId)
    if not AJFW.Functions.GetPlayer(src) or not SearchedPlayer then return end

    TriggerClientEvent('AJFW:Notify', src, Lang:t('info.cash_found', { cash = SearchedPlayer.PlayerData.money['cash'] }))
    TriggerClientEvent('AJFW:Notify', SearchedPlayer.PlayerData.source, Lang:t('info.being_searched'))
end)
...
RegisterNetEvent('police:server:SeizeCash', function(playerId)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['moneybag'], 'add')
    TriggerClientEvent('AJFW:Notify', SearchedPlayer.PlayerData.source, Lang:t('info.cash_confiscated'))
end)
...
RegisterNetEvent('police:server:SeizeDriverLicense', function(playerId)
    ...
    if driverLicense then
        ...
        TriggerClientEvent('AJFW:Notify', SearchedPlayer.PlayerData.source, Lang:t('info.driving_license_confiscated'))
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.no_driver_license'), 'error')
    end
end)
...
RegisterNetEvent('police:server:RobPlayer', function(playerId)
    ...
    TriggerClientEvent('AJFW:Notify', SearchedPlayer.PlayerData.source, Lang:t('info.cash_robbed', { money = money }))
    TriggerClientEvent('AJFW:Notify', Player.PlayerData.source, Lang:t('info.stolen_money', { stolen = money }))
end)
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('AJFW:Notify', src, Lang:t('info.vehicle_taken_depot', { price = price }))
        else
            MySQL.query(
                'UPDATE player_vehicles SET state = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?',
                { 2, body, engine, fuel, plate })
            TriggerClientEvent('AJFW:Notify', src, Lang:t('info.vehicle_seized'))
        end
    end
end)
...
RegisterNetEvent('police:server:SetTracker', function(targetId)
    ...
    local Target = AJFW.Functions.GetPlayer(targetId)
    if not AJFW.Functions.GetPlayer(src) or not Target then return end

    local TrackerMeta = Target.PlayerData.metadata['tracker']
    if TrackerMeta then
        Target.Functions.SetMetaData('tracker', false)
        TriggerClientEvent('AJFW:Notify', targetId, Lang:t('success.anklet_taken_off'), 'success')
        TriggerClientEvent('AJFW:Notify', src, Lang:t('success.took_anklet_from', { firstname = Target.PlayerData.charinfo.firstname, lastname = Target.PlayerData.charinfo.lastname }), 'success')
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData('tracker', true)
        TriggerClientEvent('AJFW:Notify', targetId, Lang:t('success.put_anklet'), 'success')
        TriggerClientEvent('AJFW:Notify', src, Lang:t('success.put_anklet_on', { firstname = Target.PlayerData.charinfo.firstname, lastname = Target.PlayerData.charinfo.lastname }), 'success')
