...
PlayerData = AJFW.Functions.GetPlayerData() -- Setting this for when you restart the resource in game
    ...
    if Vehicle ~= 0 then
        ...
        if not IsVehicleOnAllWheels(Vehicle) then
            VehicleMenu.items[#VehicleMenu.items + 1] = {
                ...
                icon = 'car-burst',
                type = 'client',
                event = 'aj-radialmenu:flipVehicle',
                shouldClose = true
            }
        end
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            for i = 1, AmountOfSeats do
                ...
                VehicleMenu.items[seatIndex].items[newIndex] = {
                    ...
                    icon = 'caret-up',
                    type = 'client',
                    event = 'aj-radialmenu:client:ChangeSeat',
                    shouldClose = false,
                }
            end
        end
...
local function controlToggle(bool)
    for i = 1, #controlsToToggle, 1 do
        if bool then
            exports['aj-smallresources']:addDisableControls(controlsToToggle[i])
        else
            exports['aj-smallresources']:removeDisableControls(controlsToToggle[i])
        end
    end
end
...
RegisterNetEvent('aj-radialmenu:client:noPlayers', function()
    AJFW.Functions.Notify(Lang:t("error.no_people_nearby"), 'error', 2500)
end)

RegisterNetEvent('aj-radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        if closestVehicle ~= GetVehiclePedIsIn(ped) then
            ...
            if GetVehicleDoorAngleRatio(closestVehicle, door) > 0.0 then
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('aj-radialmenu:trunk:server:Door', false, plate, door)
                else
                    SetVehicleDoorShut(closestVehicle, door, false)
                end
            else
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('aj-radialmenu:trunk:server:Door', true, plate, door)
                else
        ...
        AJFW.Functions.Notify(Lang:t("error.no_vehicle_found"), 'error', 2500)
    end
end)

RegisterNetEvent('aj-radialmenu:client:setExtra', function(data)
    local string = data.id
        ...
        if GetPedInVehicleSeat(veh, -1) == ped then
            ...
            if DoesExtraExist(veh, extra) then
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    AJFW.Functions.Notify(Lang:t("error.extra_deactivated", { extra = extra }), 'error', 2500)
                else
                    SetVehicleExtra(veh, extra, 0)
                    AJFW.Functions.Notify(Lang:t("success.extra_activated", { extra = extra }), 'success', 2500)
                end
            else
                AJFW.Functions.Notify(Lang:t("error.extra_not_present", { extra = extra }), 'error', 2500)
            end
        else
            AJFW.Functions.Notify(Lang:t("error.not_driver"), 'error', 2500)
        end
    end
end)

RegisterNetEvent('aj-radialmenu:trunk:client:Door', function(plate, door, open)
    local veh = GetVehiclePedIsIn(PlayerPedId())
    if veh ~= 0 then
        local pl = AJFW.Functions.GetPlate(veh)
        if pl == plate then
            if open then
...
RegisterNetEvent('aj-radialmenu:client:ChangeSeat', function(data)
    local Veh = GetVehiclePedIsIn(PlayerPedId())
    ...
    if not HasHarnass then
        ...
        if IsSeatFree then
            if kmh <= 100.0 then
                ...
                AJFW.Functions.Notify(Lang:t("info.switched_seats", { seat = data.title }))
            else
                AJFW.Functions.Notify(Lang:t("error.vehicle_driving_fast"), 'error')
            end
        else
            AJFW.Functions.Notify(Lang:t("error.seat_occupied"), 'error')
        end
    else
        AJFW.Functions.Notify(Lang:t("error.race_harness_on"), 'error')
    end
end)

RegisterNetEvent('aj-radialmenu:flipVehicle', function()
    AJFW.Functions.Progressbar("pick_grape", Lang:t("progress.flipping_car"), Config.Fliptime, false, true, {
...
RegisterNUICallback('selectItem', function(inData, cb)
    ...
    if data and found then
        ...
        elseif data.type == 'ajcommand' then
            TriggerServerEvent('AJFW:CallCommand', data.event, data)
        end
    end
    cb('ok')
end)
