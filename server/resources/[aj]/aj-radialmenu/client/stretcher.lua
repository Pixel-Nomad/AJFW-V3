...
local function LayOnStretcher()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('aj-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(player), "lay")
        else
            loadAnim(inBedDicts)
            if stretcherObject and #(coords - GetEntityCoords(stretcherObject)) <= 3.0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
local function attachToStretcher()
    ...
    if stretcherObject then
        if closestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('aj-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(closestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(stretcherObject)
                loadAnim("anim@heists@box_carry@")
                TaskPlayAnim(ped, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
...
RegisterNetEvent('aj-radialmenu:client:TakeStretcher', function()
    ...
    if vehicle ~= 0 then
        ...
        if obj ~= 0 then
            ...
            AJFW.Functions.Notify(Lang:t("error.obj_not_found"), 'error')
        end
    else
        AJFW.Functions.Notify(Lang:t("error.not_near_ambulance"), 'error')
    end
end)

RegisterNetEvent('aj-radialmenu:client:RemoveStretcher', function()
    local ped = PlayerPedId()
    ...
    if stretcherObject then
        ...
        if #(coords - bCoords) < 3.0 then
            if DoesEntityExist(stretcherObject) then
                ...
                ClearPedTasks(ped)
                DetachEntity(ped, false, true)
                TriggerServerEvent('aj-radialmenu:server:RemoveStretcher', coords, stretcherObject)
                isAttached = false
                stretcherObject = nil
                isLayingOnBed = false
            end
        else
            AJFW.Functions.Notify(Lang:t("error.far_away"), 'error')
        end
    end
end)

RegisterNetEvent('aj-radialmenu:client:RemoveStretcherFromArea', function(playerPos, bObject)
    local ped = PlayerPedId()
...
RegisterNetEvent('aj-radialmenu:Stretcher:client:BusyCheck', function(otherId, type)
    local ped = PlayerPedId()
    if type == "lay" then
        loadAnim("anim@gangops@morgue@table@")
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('aj-radialmenu:server:BusyResult', true, otherId, type)
        else
            TriggerServerEvent('aj-radialmenu:server:BusyResult', false, otherId, type)
        end
    else
        loadAnim('anim@heists@box_carry@')
...
RegisterNetEvent('aj-radialmenu:client:Result', function(isBusy, type)
    local ped = PlayerPedId()
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    if type == "lay" then
        if not isBusy then
            ...
            AJFW.Functions.Notify(Lang:t("error.stretcher_in_use"), "error")
            isLayingOnBed = false
        end
    else
        if not isBusy then
