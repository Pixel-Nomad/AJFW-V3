...
RegisterNetEvent('aj-kidnapping:client:SetKidnapping', function(bool)
    isKidnapping = bool
end)

RegisterNetEvent('aj-trunk:client:KidnapTrunk', function()
    local closestPlayer, distance = AJFW.Functions.GetClosestPlayer()
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            if closestVehicle ~= 0 then
                ...
                Wait(50)
                TriggerServerEvent("aj-trunk:server:KidnapTrunk", GetPlayerServerId(closestPlayer), closestVehicle)
            end
        else
            AJFW.Functions.Notify(Lang:t("error.not_kidnapped"), 'error')
        end
    end
end)

RegisterNetEvent('aj-trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
        AJFW.Functions.TriggerCallback('aj-trunk:server:getTrunkBusy', function(isBusy)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                TriggerServerEvent('aj-trunk:server:setTrunkBusy', plate, true)
                                inTrunk = true
                                Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                AJFW.Functions.Notify(Lang:t("success.entered_trunk"), 'success', 4000)
                                TrunkCam(true)
                                isKidnapped = true
                            else
                                AJFW.Functions.Notify(Lang:t("error.trunk_closed"), 'error', 2500)
                            end
                        else
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('aj-smallresources:trunk:server:setTrunkBusy', plate, nil)
                        ...
                        AJFW.Functions.Notify(Lang:t("error.someone_in_trunk"), 'error', 2500)
                    end
                else
                    AJFW.Functions.Notify(Lang:t("error.already_in_trunk"), 'error', 2500)
                end
            else
                AJFW.Functions.Notify(Lang:t("error.cant_enter_trunk"), 'error', 2500)
            end
        end, plate)
    else
        AJFW.Functions.Notify(Lang:t("error.cant_enter_trunk"), 'error', 2500)
...
RegisterNetEvent('aj-trunk:client:GetIn', function()
    local ped = PlayerPedId()
    local closestVehicle = getNearestVeh()
    if closestVehicle ~= 0 then
        local vehClass = GetVehicleClass(closestVehicle)
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ClearPedTasks(ped)
                            inTrunk = false
                            TriggerServerEvent('aj-trunk:server:setTrunkBusy', plate, false)
                            SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('aj-radialmenu:trunk:server:Door', false, plate, 5)
                            else
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('aj-radialmenu:trunk:server:Door', true, plate, 5)
                            else
