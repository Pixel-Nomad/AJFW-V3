...
RegisterNetEvent('aj-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('aj-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('aj-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('aj-admin:client:SaveCar', function()
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = AJFW.Functions.GetPlate(veh)
        local props = AJFW.Functions.GetVehicleProperties(veh)
        local hash = props.model
        local vehname = getVehicleFromVehList(hash)
        if AJFW.Shared.Vehicles[vehname] ~= nil and next(AJFW.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('aj-admin:server:SaveCar', props, AJFW.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
            AJFW.Functions.Notify(Lang:t('error.no_store_vehicle_garage'), 'error')
        end
    else
...
RegisterNetEvent('aj-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('aj-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == 'fast' then
...
RegisterNetEvent('aj-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('aj-admin:client:maxmodVehicle', function()
