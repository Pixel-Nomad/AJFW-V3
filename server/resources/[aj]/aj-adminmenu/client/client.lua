...
RegisterNetEvent('aj-admin:client:openMenu', function()
...
--[[
    AJ Core Admin Menu button functions below.
--]]
...
local function OpenPermsMenu(permsply)
    AJFW.Functions.TriggerCallback('aj-admin:server:getrank', function(rank)
        if rank then
            ...
            menu11:AddButton({
                ...
                select = function(_)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('aj-admin:server:setPermissions', permsply.id, selectedgroup)
                        AJFW.Functions.Notify(Lang:t('success.changed_perm'), 'success')
                        selectedgroup = 'Unknown'
                    else
                        AJFW.Functions.Notify(Lang:t('error.changed_perm_failed'), 'error')
                    end
                end
            })
        else
    ...
    menu10:AddButton({
        ...
        select = function(_)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('aj-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                AJFW.Functions.Notify(Lang:t('error.missing_reason'), 'error')
            end
        end
    })
end
    ...
    menu9:AddButton({
        ...
        select = function(_)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('aj-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                AJFW.Functions.Notify(Lang:t('error.invalid_reason_length_ban'), 'error')
            end
        end
    })
end
        ...
        Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= 'ban' and values ~= 'kick' and values ~= 'perms' then
                    TriggerServerEvent('aj-admin:server:' .. values, player)
...
player_management:On('select', function(_)
    menu4:ClearItems()
    AJFW.Functions.TriggerCallback('test:getplayers', function(players)
    ...
    for _, v in ipairs(elements) do
        menu8:AddButton({
            ...
            select = function(btn)
                ...
                TriggerServerEvent('aj-weathersync:server:setWeather', selection.value)
                AJFW.Functions.Notify(Lang:t('weather.weather_changed', { value = selection.label }))
            end
        })
    end
end)
...
menu5_vehicles_fix:On('Select', function(_)
    TriggerServerEvent('AJFW:CallCommand', 'fix', {})
end)
...
menu5_vehicles_remove:On('Select', function(_)
    TriggerServerEvent('AJFW:CallCommand', 'dv', {})
end)
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords2' then
        ...
        local x = AJFW.Shared.Round(coords.x, 2)
        local y = AJFW.Shared.Round(coords.y, 2)
        SendNUIMessage({
            string = string.format('vector2(%s, %s)', x, y)
        })
        AJFW.Functions.Notify(Lang:t('success.coords_copied'), 'success')
    elseif dataType == 'coords3' then
        local coords = GetEntityCoords(ped)
        local x = AJFW.Shared.Round(coords.x, 2)
        local y = AJFW.Shared.Round(coords.y, 2)
        local z = AJFW.Shared.Round(coords.z, 2)
        SendNUIMessage({
    ...
    elseif dataType == 'coords4' then
        ...
        local h = AJFW.Shared.Round(heading, 2)
        SendNUIMessage({
    ...
    elseif dataType == 'heading' then
        ...
        AJFW.Functions.Notify(Lang:t('success.heading_copied'), 'success')
    elseif dataType == 'freeaimEntity' then
        ...
        if entity then
            ...
            local x = AJFW.Shared.Round(entityCoords.x, 2)
            local y = AJFW.Shared.Round(entityCoords.y, 2)
            local z = AJFW.Shared.Round(entityCoords.z, 2)
            local rotX = AJFW.Shared.Round(entityRotation.x, 2)
            local rotY = AJFW.Shared.Round(entityRotation.y, 2)
            local rotZ = AJFW.Shared.Round(entityRotation.z, 2)
            local h = AJFW.Shared.Round(entityHeading, 2)
            SendNUIMessage({
                string = string.format('Model Name:\t%s\nModel Hash:\t%s\n\nHeading:\t%s\nCoords:\t\tvector3(%s, %s, %s)\nRotation:\tvector3(%s, %s, %s)', entityName, entityHash, h, x, y, z, rotX, rotY, rotZ)
            })
            AJFW.Functions.Notify(Lang:t('success.entity_copy'), 'success')
        else
            AJFW.Functions.Notify(Lang:t('error.failed_entity_copy'), 'error')
        end
    end
end

RegisterNetEvent('aj-admin:client:copyToClipboard', function(dataType)
...
local function ToggleShowCoordinates()
    ...
    CreateThread(function()
        while showCoords do
            ...
            local c = {}
            c.x = AJFW.Shared.Round(coords.x, 2)
            c.y = AJFW.Shared.Round(coords.y, 2)
            c.z = AJFW.Shared.Round(coords.z, 2)
            heading = AJFW.Shared.Round(heading, 2)
            Wait(0)
    ...
    CreateThread(function()
        while vehicleDevMode do
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                Draw2DText(string.format(Lang:t('info.eng_health') .. '~b~%s~s~ | ' .. Lang:t('info.body_health') .. '~b~%s~s~', AJFW.Shared.Round(eHealth, 2), AJFW.Shared.Round(bHealth, 2)), 4, { 255, 255, 255 }, 0.4, x + 0.0, y + 0.075)
            end
        end
    end)
end

RegisterNetEvent('aj-admin:client:ToggleCoords', function()
    ...
    for _, v in ipairs(elements) do
        EditDealer:AddButton({
            ...
            select = function(btn)
                local values = btn.Value
                if values == 'goto' then
                    TriggerServerEvent('AJFW:CallCommand', 'dealergoto', { dealer['name'] })
                elseif values == 'remove' then
                    TriggerServerEvent('AJFW:CallCommand', 'deletedealer', { dealer['name'] })
                    EditDealer:Close()
                    menu6:Close()
                end
            end
