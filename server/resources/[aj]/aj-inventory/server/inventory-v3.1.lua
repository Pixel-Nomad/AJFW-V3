...
local function RemoveTable(tbl, amount)
    ...
    while index > 0 do
		Wait(0)
        if amountMeta < amount then
            newData[#newData+1] = tbl[index]
            amountMeta = amountMeta + 1
        else
            break
        end
        index = index -1
	...
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			-- if Player.PlayerData.items[slot].info.quality == info.quality then
				local newData = {}
				local amountMeta = 0
				for i = 1, #Player.PlayerData.items[slot].metadata do
					newData[#newData+1] = Player.PlayerData.items[slot].metadata[i]
				end
				local index = #itemInfo['metadata']
				while index > 0 do
					Wait(0)
					amountMeta = amountMeta + 1
					if amountMeta <= amount then
						newData[#newData+1] = itemInfo['metadata'][index]
						index = index - 1 
					else
						break
					end
				...
				Player.PlayerData.items[slot].metadata = newData
	...
	if slot then
		...
		for _, _slot in pairs(slots) do
			if Player.PlayerData.items[_slot].amount > amountToRemove then
				...
				Player.PlayerData.items[_slot].metadata = newData
...
local function AddToStash(stashId, slot, otherslot, itemName, amount, info, created, metadata)
	...
	if not ItemData.unique then
		if Stashes[stashId].items[slot] and Stashes[stashId].items[slot].name == itemName then
			local newData = {}
			local amountMeta = 0
			for i = 1, #Stashes[stashId].items[slot].metadata do
			...
			Stashes[stashId].items[slot].metadata = newData
...
local function AddToTrunk(plate, slot, otherslot, itemName, amount, info, created, metadata)
	...
	if not ItemData.unique then
		if Trunks[plate].items[slot] and Trunks[plate].items[slot].name == itemName then
			local newData = {}
			local amountMeta = 0
			for i = 1, #Trunks[plate].items[slot].metadata do
			...
			Trunks[plate].items[slot].metadata = newData
...
local function AddToGlovebox(plate, slot, otherslot, itemName, amount, info, created, metadata)
	...
	if not ItemData.unique then
		if Gloveboxes[plate].items[slot] and Gloveboxes[plate].items[slot].name == itemName then
			local newData = {}
			local amountMeta = 0
			for i = 1, #Gloveboxes[plate].items[slot].metadata do
			...
			Gloveboxes[plate].items[slot].metadata = newData
...
local function AddToDrop(dropId, slot, itemName, amount, info, created, metadata)
	...
	if Drops[dropId].items[slot] and Drops[dropId].items[slot].name == itemName then
		local newData = {}
		local amountMeta = 0
		for i = 1, #Drops[dropId].items[slot].metadata do
		...
		Drops[dropId].items[slot].metadata = newData
...
local function OpenInventory(name, id, other, origin)
	...
	if name and id then
		...
		TriggerClientEvent("aj-inventory:client:closeinv", id)
...
AddEventHandler('AJFW:Server:PlayerLoaded', function(Player)
	AJFW.Functions.AddPlayerMethod(Player.PlayerData.source, "AddItem", function(item, amount, slot, info, forceUpdate)
		return AddItem(Player.PlayerData.source, item, amount, slot, info, true)
	end)
    ...
    elseif AJFW.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
        ...
        elseif AJFW.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
            if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                local imageurl = ("https://cfx-nui-aj-inventory/html/images/%s.png"):format(itemData.name)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot, true) then
			if AddItem(target, item.name, amount, false, item.info, true, item.created, item.metadata) then
				...
				AddItem(src, item.name, amount, item.slot, item.info, true, item.created, item.metadata)
