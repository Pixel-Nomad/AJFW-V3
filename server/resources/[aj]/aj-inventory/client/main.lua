...
local function ItemsToItemInfo()
    ...
    for i = 1, #Config.CraftingItems do
        ...
        local itemInfo = AJFW.Shared.Items[craftingItem.name:lower()]
        if itemInfo then
            local itemCost = {}
            for material, amount in pairs(craftingItem.costs) do
                local materialInfo = AJFW.Shared.Items[material:lower()]
...
local function SetupAttachmentItemsInfo()
    ...
    for i = 1, #Config.AttachmentCrafting do
        ...
        local itemInfo = AJFW.Shared.Items[attachmentItem.name:lower()]
        if itemInfo then
            local itemCost = {}
