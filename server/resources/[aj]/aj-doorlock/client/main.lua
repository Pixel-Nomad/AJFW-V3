...
local function lockpickFinish(success)
	if success then
		AJFW.Functions.Notify(Lang:t("success.lockpick_success"), 'success', 2500)
		...
		Wait(1800)
		TriggerServerEvent('aj-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
		AJFW.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
		if math.random(1,100) <= 17 then
			if usingAdvanced then
				TriggerServerEvent("aj-doorlock:server:removeLockpick", "advancedlockpick")
				TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items["advancedlockpick"], "remove")
			else
				TriggerServerEvent("aj-doorlock:server:removeLockpick", "lockpick")
				TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items["lockpick"], "remove")
			end
		end
	end
end
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		AJFW.Functions.TriggerCallback('aj-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	AJFW.Functions.TriggerCallback('aj-doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
...
RegisterNetEvent('aj-doorlock:client:setState', function(serverId, doorID, state, src, enableSounds, enableAnimation)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
	...
	TriggerEvent('aj-lockpick:client:openLockpick', lockpickFinish)
end)

RegisterNetEvent('aj-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	...
	if Config.DoorList[identifier] then
		AJFW.Functions.Notify((Lang:t("error.door_identifier_exists")):format(identifier), 'error')
		canContinue = true
		return
	end
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then AJFW.Functions.Notify(Lang:t("error.door_not_found"), 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then AJFW.Functions.Notify(Lang:t("error.door_registered"), 'error') canContinue = true return end
		...
		TriggerServerEvent('aj-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then AJFW.Functions.Notify(Lang:t("error.door_not_found"), 'error') return end
		if entity[1] == entity[2] then AJFW.Functions.Notify(Lang:t("error.same_entity"), 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('aj-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('aj-doorlock:client:newDoorAdded', function(data, id, creatorSrc)
	Config.DoorList[id] = data
	TriggerEvent('aj-doorlock:client:setState', creatorSrc, id, data.locked, false, true, true)
end)

RegisterNetEvent('aj-doorlock:client:ToggleDoorDebug', function()
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('aj-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
...
RegisterCommand('remotetriggerdoor', function()
	...
	TriggerServerEvent('aj-doorlock:server:updateState', nearestDoor.id, not nearestDoor.data.locked, NetworkGetNetworkIdFromEntity(playerPed), false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
