...
RegisterNetEvent('aj-tow:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('AJFW:Notify', src, Lang:t('success.paid_with_cash', { value = Config.BailPrice }), 'success')
            TriggerClientEvent('aj-tow:client:SpawnVehicle', src, vehInfo)
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            Bail[Player.PlayerData.citizenid] = Config.BailPrice
            Player.Functions.RemoveMoney('bank', Config.BailPrice, 'tow-paid-bail')
            TriggerClientEvent('AJFW:Notify', src, Lang:t('success.paid_with_bank', { value = Config.BailPrice }), 'success')
            TriggerClientEvent('aj-tow:client:SpawnVehicle', src, vehInfo)
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.no_deposit', { value = Config.BailPrice }), 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('bank', Bail[Player.PlayerData.citizenid], 'tow-bail-paid')
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('AJFW:Notify', src, Lang:t('success.refund_to_cash', { value = Config.BailPrice }), 'success')
        end
    end
end)

RegisterNetEvent('aj-tow:server:nano', function(vehNetID)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-tow:server:11101110', function(drops)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if not Player then return end
    ...
    TriggerClientEvent('AJFW:Notify', src, Lang:t('success.you_earned', { value = payment }), 'success')
end)

AJFW.Commands.Add('npc', Lang:t('info.toggle_npc'), {}, false, function(source)
    TriggerClientEvent('jobs:client:ToggleNpc', source)
end)

AJFW.Commands.Add('tow', Lang:t('info.tow'), {}, false, function(source)
    local Player = AJFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == 'tow' or Player.PlayerData.job.name == 'mechanic' then
        TriggerClientEvent('aj-tow:client:TowVehicle', source)
    end
end)
