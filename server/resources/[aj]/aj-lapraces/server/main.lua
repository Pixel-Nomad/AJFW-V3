...
local function IsWhitelisted(CitizenId)
    ...
    local Perms = AJFW.Functions.GetPermission(Player.PlayerData.source)
...
RegisterNetEvent('aj-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('aj-phone:client:RaceNotify', src, 'You have won the WR from ' .. RaceData.RaceName ..
    ...
    TriggerClientEvent('aj-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
    ...
    TriggerClientEvent('aj-phone:client:UpdateLapraces', -1)
end)

RegisterNetEvent('aj-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('aj-lapraces:client:StartRaceEditor', source, RaceName)
        else
            TriggerClientEvent('AJFW:Notify', source, 'There is already a race with this name.', 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', source, 'You have not been authorized to race\'s to create.', 'error')
    end
end)

RegisterNetEvent('aj-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('AJFW:Notify', src, 'You were the only one in the race, the race had ended', 'error')
            TriggerClientEvent('aj-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('aj-lapraces:client:JoinRace', src, Races[RaceId], AvailableRaces[AvailableKey].Laps)
    TriggerClientEvent('aj-phone:client:UpdateLapraces', -1)
    local creatorsource = AJFW.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData
                              .source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('aj-phone:client:RaceNotify', creatorsource,
            string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. Player.PlayerData.charinfo.lastname ..
                ' the race has been joined!')
    end
end)

RegisterNetEvent('aj-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('AJFW:Notify', src, 'You were the only one in the race.The race had ended.', 'error')
        TriggerClientEvent('aj-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterNetEvent('aj-lapraces:server:SetupRace', function(RaceId, Laps)
            ...
            if not Races[RaceId].Started then
                ...
                SetTimeout(5 * 60 * 1000, function()
                    if Races[RaceId].Waiting then
                        local AvailableKey = GetOpenedRaceKey(RaceId)
                        for cid, _ in pairs(Races[RaceId].Racers) do
                            local RacerData = AJFW.Functions.GetPlayerByCitizenId(cid)
                            if RacerData ~= nil then
                                TriggerClientEvent('aj-lapraces:client:LeaveRace', RacerData.PlayerData.source,
                                    Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
                ...
                TriggerClientEvent('AJFW:Notify', source, 'The race is already running', 'error')
            end
        else
            TriggerClientEvent('AJFW:Notify', source, 'The race is already running', 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', source, 'This race does not exist :(', 'error')
    end
end)

RegisterNetEvent('aj-lapraces:server:CancelRace', function(raceId)
    local src = source
    local Player = AJFW.Functions.GetPlayer(source)
    local AvailableKey = GetOpenedRaceKey(raceId)

    TriggerClientEvent('AJFW:Notify', src, 'Stopping the race: ' .. raceId, 'error')
    ...
    if AvailableKey ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == Player.PlayerData.citizenid then
            for cid, _ in pairs(Races[raceId].Racers) do
                local RacerData = AJFW.Functions.GetPlayerByCitizenId(cid)
                if RacerData ~= nil then
                    TriggerClientEvent('aj-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[raceId])
                end
            end
        ...
        TriggerClientEvent('AJFW:Notify', src, 'Race not open: ' .. raceId, 'error')
    end
end)

RegisterNetEvent('aj-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterNetEvent('aj-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('aj-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterNetEvent('aj-lapraces:server:StartRace', function(RaceId)
    local src = source
    local MyPlayer = AJFW.Functions.GetPlayer(src)
    local AvailableKey = GetOpenedRaceKey(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId, _ in pairs(Races[RaceId].Racers) do
                local Player = AJFW.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('aj-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('aj-phone:client:UpdateLapraces', -1)
        else
            TriggerClientEvent('AJFW:Notify', src, 'You are not the creator of the race..', 'error')
        end
    else
...
RegisterNetEvent('aj-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback('aj-lapraces:server:GetRacingLeaderboards', function(_, cb)
    cb(Races)
end)

AJFW.Functions.CreateCallback('aj-lapraces:server:GetRaces', function(_, cb)
    cb(AvailableRaces)
end)

AJFW.Functions.CreateCallback('aj-lapraces:server:GetListedRaces', function(_, cb)
    cb(Races)
end)

AJFW.Functions.CreateCallback('aj-lapraces:server:GetRacingData', function(_, cb, RaceId)
    cb(Races[RaceId])
end)

AJFW.Functions.CreateCallback('aj-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(AJFW.Functions.GetPlayer(source).PlayerData.citizenid))
end)

AJFW.Functions.CreateCallback('aj-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(AJFW.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)

AJFW.Functions.CreateCallback('aj-lapraces:server:CanRaceSetup', function(_, cb)
    cb(Config.RaceSetupAllowed)
end)

AJFW.Functions.CreateCallback('aj-lapraces:server:GetTrackData', function(_, cb, RaceId)
...
AJFW.Commands.Add("cancelrace", "Cancel going race..", {}, false, function(source, args)
    local Player = AJFW.Functions.GetPlayer(source)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('AJFW:Notify', source, 'This race has not started yet.', 'error')
            end
        end
    else
        TriggerClientEvent('AJFW:Notify', source, 'You have not been authorized to do this.', 'error')
    end
end)

AJFW.Commands.Add("togglesetup", "Turn on / off racing setup", {}, false, function(source, _)
    local Player = AJFW.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('AJFW:Notify', source, 'No more races can be created!', 'error')
        else
            TriggerClientEvent('AJFW:Notify', source, 'Races can be created again!', 'success')
        end
    else
        TriggerClientEvent('AJFW:Notify', source, 'You have not been authorized to do this.', 'error')
    end
