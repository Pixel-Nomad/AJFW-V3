...
local function showEntranceHeaderMenu()
    ...
    if AJFW.Functions.GetPlayerData().job and AJFW.Functions.GetPlayerData().job.name == 'realestate' then
        isOwned = true
    end
    ...
    if not isOwned then
        headerMenu[#headerMenu + 1] = {
            ...
            params = {
                event = 'aj-houses:client:ViewHouse',
                args = {}
            }
        }
    else
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu + 1] = {
                ...
                params = {
                    event = 'aj-houses:client:EnterHouse',
                    args = {}
                }
            }
            headerMenu[#headerMenu + 1] = {
                ...
                params = {
                    event = 'aj-houses:client:giveHouseKey',
                    args = {}
                }
            }
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu + 1] = {
                ...
                params = {
                    event = 'aj-houses:client:RequestRing',
                    args = {}
                }
            }
            headerMenu[#headerMenu + 1] = {
            ...
            if AJFW.Functions.GetPlayerData().job and AJFW.Functions.GetPlayerData().job.type == 'leo' then
                headerMenu[#headerMenu + 1] = {
                    ...
                    params = {
                        event = 'aj-houses:client:ResetHouse',
                        args = {}
                    }
                }
            end
    ...
    if headerMenu and next(headerMenu) then
        exports['aj-menu']:openMenu(headerMenu)
    end
end
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu + 1] = {
        ...
        params = {
            event = 'aj-houses:client:ExitOwnedHouse',
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu + 1] = {
            ...
            params = {
                event = 'aj-houses:client:FrontDoorCam',
                args = {}
            }
        }
        headerMenu[#headerMenu + 1] = {
            ...
            params = {
                event = 'aj-houses:client:AnswerDoorbell',
                args = {}
            }
        }
    end
...
local function RegisterStashTarget()
    ...
    stashTargetBox:onPlayerInOut(function(isPointInside)
        if isPointInside and not entering and isOwned then
            exports['aj-base']:DrawText(Lang:t('target.open_stash'), 'left')
        else
...
local function RegisterOutfitsTarget()
    ...
    outfitsTargetBox:onPlayerInOut(function(isPointInside)
        if isPointInside and not entering and isOwned then
            exports['aj-base']:DrawText(Lang:t('target.outfits'), 'left')
        else
            exports['aj-base']:HideText()
        end
...
local function RegisterCharactersTarget()
    ...
    charactersTargetBox:onPlayerInOut(function(isPointInside)
        if isPointInside and not entering and isOwned then
            exports['aj-base']:DrawText(Lang:t('target.change_character'), 'left')
        else
            exports['aj-base']:HideText()
        end
...
local function GetClosestPlayer()
    local closestPlayers = AJFW.Functions.GetPlayersFromCoords()
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            AJFW.Functions.TriggerCallback('aj-houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    end
    TriggerEvent('aj-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        AJFW.Functions.TriggerCallback('aj-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
    if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
        AJFW.Functions.TriggerCallback('aj-houses:server:getHouseDecorations', function(result)
...
local function RemoveHouseKey(citizenData)
    TriggerServerEvent('aj-houses:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    AJFW.Functions.TriggerCallback('aj-houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end, ClosestHouse)
...
function HouseKeysMenu()
    ...
    if holders == nil or next(holders) == nil then
        AJFW.Functions.Notify(Lang:t('error.no_key_holders'), 'error', 3500)
        CloseMenuFull()
    else
        keyholderMenu = {}
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu + 1] = {
                ...
                params = {
                    event = 'aj-houses:client:OpenClientOptions',
                    args = {
        ...
        exports['aj-menu']:openMenu(keyholderMenu)
    end
end
    ...
    keyholderOptions = {
        {
            ...
            params = {
                event = 'aj-houses:client:RevokeKey',
                args = {
        ...
        },
        {
            ...
            params = {
                event = 'aj-houses:client:removeHouseKey',
                args = {}
            }
        },
    }

    exports['aj-menu']:openMenu(keyholderOptions)
end
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['aj-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['aj-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['aj-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['aj-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['aj-interior']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['aj-interior']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['aj-interior']:CreateContainer(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['aj-interior']:CreateFurniMid(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['aj-interior']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['aj-interior']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['aj-interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['aj-interior']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['aj-interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['aj-interior']:CreateStore1(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['aj-interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['aj-interior']:CreateFurniMotelStandard(coords) -- End of free shells
    elseif Config.Houses[house].tier == 17 then
        return exports['aj-interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['aj-interior']:CreateMedium3(coords)
    elseif Config.Houses[house].tier == 19 then
        return exports['aj-interior']:CreateBanham(coords)
    elseif Config.Houses[house].tier == 20 then
        return exports['aj-interior']:CreateWestons(coords)
    elseif Config.Houses[house].tier == 21 then
        return exports['aj-interior']:CreateWestons2(coords)
    elseif Config.Houses[house].tier == 22 then
        return exports['aj-interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 23 then
        return exports['aj-interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 24 then
        return exports['aj-interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 25 then
        return exports['aj-interior']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 26 then
        return exports['aj-interior']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 27 then
        return exports['aj-interior']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 28 then
        return exports['aj-interior']:CreateHighend(coords)
    elseif Config.Houses[house].tier == 29 then
        return exports['aj-interior']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 30 then
        return exports['aj-interior']:CreateStashHouse(coords)
    elseif Config.Houses[house].tier == 31 then
        return exports['aj-interior']:CreateStashHouse2(coords)
    elseif Config.Houses[house].tier == 32 then
        return exports['aj-interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 33 then
        return exports['aj-interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 34 then
        return exports['aj-interior']:CreateOffice2(coords)
    elseif Config.Houses[house].tier == 35 then
        return exports['aj-interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 36 then
        return exports['aj-interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 37 then
        return exports['aj-interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 38 then
        return exports['aj-interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 39 then
        return exports['aj-interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 40 then
        return exports['aj-interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 41 then
        return exports['aj-interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 42 then
        return exports['aj-interior']:CreateK4Coke(coords)
    elseif Config.Houses[house].tier == 43 then
        return exports['aj-interior']:CreateK4Meth(coords)
    elseif Config.Houses[house].tier == 44 then
        return exports['aj-interior']:CreateK4Weed(coords)
    elseif Config.Houses[house].tier == 45 then
        return exports['aj-interior']:CreateContainer2(coords)
    elseif Config.Houses[house].tier == 46 then
        return exports['aj-interior']:CreateFurniStash1(coords)
    elseif Config.Houses[house].tier == 47 then
        return exports['aj-interior']:CreateFurniStash3(coords)
    elseif Config.Houses[house].tier == 48 then
        return exports['aj-interior']:CreateFurniLow(coords)
    elseif Config.Houses[house].tier == 49 then
        return exports['aj-interior']:CreateFurniMotel(coords)
    elseif Config.Houses[house].tier == 50 then
        return exports['aj-interior']:CreateFurniMotelClassic(coords)
    elseif Config.Houses[house].tier == 51 then
        return exports['aj-interior']:CreateFurniMotelHigh(coords)
    elseif Config.Houses[house].tier == 52 then
        return exports['aj-interior']:CreateFurniMotelModern2(coords)
    elseif Config.Houses[house].tier == 53 then
        return exports['aj-interior']:CreateFurniMotelModern3(coords)
    elseif Config.Houses[house].tier == 54 then
        return exports['aj-interior']:CreateCoke(coords)
    elseif Config.Houses[house].tier == 55 then
        return exports['aj-interior']:CreateCoke2(coords)
    elseif Config.Houses[house].tier == 56 then
        return exports['aj-interior']:CreateMeth(coords)
    elseif Config.Houses[house].tier == 57 then
        return exports['aj-interior']:CreateWeed(coords)
    elseif Config.Houses[house].tier == 58 then
        return exports['aj-interior']:CreateWeed2(coords)
    elseif Config.Houses[house].tier == 59 then
        return exports['aj-interior']:CreateMansion(coords)
    elseif Config.Houses[house].tier == 60 then
        return exports['aj-interior']:CreateMansion2(coords)
    elseif Config.Houses[house].tier == 61 then
        return exports['aj-interior']:CreateMansion3(coords)
    elseif Config.Houses[house].tier == 62 then
        return exports['aj-interior']:CreateHotel1(coords)
    elseif Config.Houses[house].tier == 63 then
        return exports['aj-interior']:CreateHotel2(coords)
    elseif Config.Houses[house].tier == 64 then
        return exports['aj-interior']:CreateHotel3(coords)
    elseif Config.Houses[house].tier == 65 then
        return exports['aj-interior']:CreateMotel1(coords)
    elseif Config.Houses[house].tier == 66 then
        return exports['aj-interior']:CreateMotel2(coords)
    elseif Config.Houses[house].tier == 67 then
        return exports['aj-interior']:CreateMotel3(coords)
    elseif Config.Houses[house].tier == 68 then
        return exports['aj-interior']:CreateV2Default1(coords)
    elseif Config.Houses[house].tier == 69 then
        return exports['aj-interior']:CreateV2Default2(coords)
    elseif Config.Houses[house].tier == 70 then
        return exports['aj-interior']:CreateV2Default3(coords)
    elseif Config.Houses[house].tier == 71 then
        return exports['aj-interior']:CreateV2Default4(coords)
    elseif Config.Houses[house].tier == 72 then
        return exports['aj-interior']:CreateV2Default5(coords)
    elseif Config.Houses[house].tier == 73 then
        return exports['aj-interior']:CreateV2Default6(coords)
    elseif Config.Houses[house].tier == 74 then
        return exports['aj-interior']:CreateV2Deluxe1(coords)
    elseif Config.Houses[house].tier == 75 then
        return exports['aj-interior']:CreateV2Deluxe2(coords)
    elseif Config.Houses[house].tier == 76 then
        return exports['aj-interior']:CreateV2Deluxe3(coords)
    elseif Config.Houses[house].tier == 77 then
        return exports['aj-interior']:CreateV2HighEnd1(coords)
    elseif Config.Houses[house].tier == 78 then
        return exports['aj-interior']:CreateV2HighEnd2(coords)
    elseif Config.Houses[house].tier == 79 then
        return exports['aj-interior']:CreateV2HighEnd3(coords)
    elseif Config.Houses[house].tier == 80 then
        return exports['aj-interior']:CreateV2Medium1(coords)
    elseif Config.Houses[house].tier == 81 then
        return exports['aj-interior']:CreateV2Medium2(coords)
    elseif Config.Houses[house].tier == 82 then
        return exports['aj-interior']:CreateV2Medium3(coords)
    elseif Config.Houses[house].tier == 83 then
        return exports['aj-interior']:CreateV2Modern1(coords)
    elseif Config.Houses[house].tier == 84 then
        return exports['aj-interior']:CreateV2Modern2(coords)
    elseif Config.Houses[house].tier == 85 then
        return exports['aj-interior']:CreateV2Modern3(coords)
    elseif Config.Houses[house].tier == 86 then
        return exports['aj-interior']:VineWoodHouse1(coords)
    elseif Config.Houses[house].tier == 87 then
        return exports['aj-interior']:VineWoodHouse2(coords)
    elseif Config.Houses[house].tier == 88 then
        return exports['aj-interior']:VineWoodHouse3(coords)
    elseif Config.Houses[house].tier == 89 then
        return exports['aj-interior']:CreateK4GunWarehouse(coords)
    elseif Config.Houses[house].tier == 90 then
        return exports['aj-interior']:CreateK4LuxuryHouse1(coords)
    elseif Config.Houses[house].tier == 91 then
        return exports['aj-interior']:CreateK4LuxuryHouse2(coords)
    elseif Config.Houses[house].tier == 92 then
        return exports['aj-interior']:CreateK4LuxuryHouse3(coords)
    elseif Config.Houses[house].tier == 93 then
        return exports['aj-interior']:CreateK4LuxuryHouse4(coords)
    elseif Config.Houses[house].tier == 94 then
        return exports['aj-interior']:CreateK4ManorHouse(coords)
    elseif Config.Houses[house].tier == 95 then
        return exports['aj-interior']:CreateK4Garage1(coords)
    elseif Config.Houses[house].tier == 96 then
        return exports['aj-interior']:CreateK4Garage2(coords)
    elseif Config.Houses[house].tier == 97 then
        return exports['aj-interior']:CreateK4Garage3(coords)
    elseif Config.Houses[house].tier == 98 then
        return exports['aj-interior']:CreateK4Garage4(coords)
    elseif Config.Houses[house].tier == 99 then
        return exports['aj-interior']:CreateK4Safehouse(coords)
    elseif Config.Houses[house].tier == 100 then
        return exports['aj-interior']:CreateK4Warehouse(coords)
    else
        AJFW.Functions.Notify(Lang:t('error.invalid_tier'), 'error')
    end
end
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Wait(500)
    TriggerServerEvent('aj-houses:server:SetInsideMeta', house, true)
    --TriggerEvent('aj-weathersync:client:DisableSync')
    TriggerEvent('aj-weathersync:client:EnableSync')
    TriggerEvent('aj-weed:client:getHousePlants', house)
    entering = false
    setHouseLocations()
    CloseMenuFull()
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        DoScreenFadeOut(250)
        Wait(500)
        exports['aj-interior']:DespawnInterior(houseObj, function()
            UnloadDecorations()
            TriggerEvent('aj-weathersync:client:EnableSync')
            Wait(250)
            DoScreenFadeIn(250)
            ...
            TriggerEvent('aj-weed:client:leaveHouse')
            TriggerServerEvent('aj-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
local function changeCharacter()
    ...
    if CheckDistance(logoutLoc, 1.5) then
        ...
        exports['aj-interior']:DespawnInterior(houseObj, function()
            ...
            InOwnedHouse = false
            IsInside = false
            TriggerServerEvent('aj-houses:server:LogoutLocation')
        end)
    end
end
-- Events

RegisterNetEvent('aj-houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('aj-houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('aj-houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('aj-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('aj-houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('aj-houses:server:RingDoor', ClosestHouse)
    end
end)

AddEventHandler('AJFW:Client:OnPlayerLoaded', function()
    TriggerServerEvent('aj-houses:server:setHouses')
    SetClosestHouse()
    TriggerEvent('aj-houses:client:setupHouseBlips')
    if Config.UnownedBlips then TriggerEvent('aj-houses:client:setupHouseBlips2') end
    Wait(100)
    TriggerEvent('aj-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
    TriggerServerEvent('aj-houses:server:setHouses')
end)
...
RegisterNetEvent('aj-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('aj-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub('%-', ' ')
    TriggerServerEvent('aj-houses:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('aj-houses:server:createBlip') end
end)

RegisterNetEvent('aj-houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('aj-houses:server:addGarage', ClosestHouse, coords)
    else
        AJFW.Functions.Notify(Lang:t('error.no_house'), 'error')
    end
end)

RegisterNetEvent('aj-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('aj-houses:server:lockHouse', false, ClosestHouse)
                AJFW.Functions.Notify(Lang:t('success.unlocked'), 'success', 2500)
            else
                TriggerServerEvent('aj-houses:server:lockHouse', true, ClosestHouse)
                AJFW.Functions.Notify(Lang:t('error.locked'), 'error', 2500)
            end
        else
            AJFW.Functions.Notify(Lang:t('error.no_keys'), 'error', 3500)
        end
    else
        AJFW.Functions.Notify(Lang:t('error.no_door'), 'error', 3500)
    end
end)

RegisterNetEvent('aj-houses:client:RingDoor', function(player, house)
    if ClosestHouse == house and IsInside then
        CurrentDoorBell = player
        TriggerServerEvent('InteractSound_SV:PlayOnSource', 'doorbell', 0.1)
        AJFW.Functions.Notify(Lang:t('info.door_ringing'))
    end
end)

RegisterNetEvent('aj-houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('aj-houses:server:giveHouseKey', playerId, ClosestHouse)
        else
            AJFW.Functions.Notify(Lang:t('error.no_door'), 'error')
        end
    elseif ClosestHouse == nil then
        AJFW.Functions.Notify(Lang:t('error.no_house'), 'error')
    else
        AJFW.Functions.Notify(Lang:t('error.no_one_near'), 'error')
    end
end)

RegisterNetEvent('aj-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            AJFW.Functions.TriggerCallback('aj-houses:server:getHouseOwner', function(result)
                if AJFW.Functions.GetPlayerData().citizenid == result then
                    HouseKeysMenu()
                else
                    AJFW.Functions.Notify(Lang:t('error.not_owner'), 'error')
                end
            end, ClosestHouse)
        else
            AJFW.Functions.Notify(Lang:t('error.no_door'), 'error')
...
RegisterNetEvent('aj-houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('aj-houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('aj-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('aj-houses:client:enterOwnedHouse', function(house)
...
RegisterNetEvent('aj-houses:client:LastLocationHouse', function(houseId)
    AJFW.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.metadata['injail'] == 0 then
...
RegisterNetEvent('aj-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            AJFW.Functions.TriggerCallback('aj-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('aj-houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('aj-houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('aj-houses:client:refreshBlips', function() -- Refresh unowned on buy
    for _, v in pairs(UnownedHouseBlips) do RemoveBlip(v) end
    Wait(250)
    TriggerEvent('aj-houses:client:setupHouseBlips2')
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('aj-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('aj-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('aj-houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if cData.id == 'setstash' then
                TriggerServerEvent('aj-houses:server:setLocation', coords, ClosestHouse, 1)
            elseif cData.id == 'setoutift' then
                TriggerServerEvent('aj-houses:server:setLocation', coords, ClosestHouse, 2)
            elseif cData.id == 'setlogout' then
                TriggerServerEvent('aj-houses:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            AJFW.Functions.Notify(Lang:t('error.not_owner'), 'error')
        end
    else
        AJFW.Functions.Notify(Lang:t('error.not_in_house'), 'error')
    end
end)

RegisterNetEvent('aj-houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('aj-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local Skillbar = exports['aj-skillbar']:GetSkillbarObject()
    if ClosestHouse ~= nil then
        AJFW.Functions.TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
                        ...
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('aj-houses:server:lockHouse', false, ClosestHouse)
                                    AJFW.Functions.Notify(Lang:t('success.home_invasion'), 'success')
                                    TriggerServerEvent('aj-houses:server:SetHouseRammed', true, ClosestHouse)
                                    TriggerServerEvent('aj-houses:server:SetRamState', false, ClosestHouse)
                                ...
                                RamsDone = 0
                                TriggerServerEvent('aj-houses:server:SetRamState', false, ClosestHouse)
                                AJFW.Functions.Notify(Lang:t('error.failed_invasion'), 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('aj-houses:server:SetRamState', true, ClosestHouse)
                        else
                            AJFW.Functions.Notify(Lang:t('error.inprogress_invasion'), 'error')
                        end
                    else
                        AJFW.Functions.Notify(Lang:t('error.already_open'), 'error')
                    end
                else
                    AJFW.Functions.Notify(Lang:t('error.no_house'), 'error')
                end
            else
                AJFW.Functions.Notify(Lang:t('error.no_police'), 'error')
            end
        end)
    else
        AJFW.Functions.Notify(Lang:t('error.no_house'), 'error')
...
RegisterNetEvent('aj-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('aj-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('aj-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('aj-houses:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('aj-houses:server:SetRamState', false, ClosestHouse)
        end
        if Config.Houses[ClosestHouse].IsRammed then
            ...
            TriggerServerEvent('aj-houses:server:lockHouse', true, ClosestHouse)
            RamsDone = 0
            AJFW.Functions.Notify(Lang:t('success.lock_invasion'), 'success')
        else
            AJFW.Functions.Notify(Lang:t('error.no_invasion'), 'error')
        end
    end
end)

RegisterNetEvent('aj-houses:client:ExitOwnedHouse', function()
...
RegisterNetEvent('aj-houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterNetEvent('aj-houses:client:AnswerDoorbell', function()
    if not CurrentDoorBell or CurrentDoorBell == 0 then
        AJFW.Functions.Notify(Lang:t('error.nobody_at_door'))
        return
    end
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent('aj-houses:server:OpenDoor', CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('aj-houses:client:ViewHouse', function()
...
RegisterNetEvent('aj-houses:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterNetEvent('aj-house:client:RefreshHouseTargets', function()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    AJFW.Functions.TriggerCallback('aj-houses:server:HasEnoughMoney', function(_)
        cb('ok')
...
RegisterNUICallback('buy', function(_, cb)
    ...
    if Config.UnownedBlips then TriggerEvent('aj-houses:client:refreshBlips') end
    TriggerServerEvent('aj-houses:server:buyHouse', ClosestHouse)
    cb('ok')
end)
