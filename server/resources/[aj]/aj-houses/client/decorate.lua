...
local function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('aj-anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent('aj-houses:server:savedecorations', ClosestHouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('aj-anticheat:client:ToggleDecorate', false)
end
...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		AJFW.Functions.Notify(Lang:t('info.speed') .. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('aj-houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
			...
			AJFW.Functions.Notify(Lang:t('error.no_keys'), 'error')
		end
	else
...
RegisterNUICallback('buySelectedObject', function(data, cb)
	AJFW.Functions.TriggerCallback('aj-houses:server:buyFurniture', function(isSuccess)
		if isSuccess then
			SetNuiFocus(false, false)
			cursorEnabled = false
			SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				AJFW.Functions.Notify(Lang:t('error.out_range'), 'error')
			end
		end
	end
end)
