...
CreateThread(function()
    ...
    TriggerClientEvent('aj-garages:client:houseGarageConfig', -1, HouseGarages)
    TriggerClientEvent('aj-houses:client:setHouseConfig', -1, Config.Houses)
end)
...
AJFW.Commands.Add('decorate', Lang:t('info.decorate_interior'), {}, false, function(source)
    local src = source
    TriggerClientEvent('aj-houses:client:decorate', src)
end)

AJFW.Commands.Add('createhouse', Lang:t('info.create_house'), { { name = 'price', help = Lang:t('info.price_of_house') }, { name = 'tier', help = Lang:t('info.tier_number') } }, true, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local price = tonumber(args[1])
    local tier = tonumber(args[2])
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('aj-houses:client:createHouses', src, price, tier)
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

AJFW.Commands.Add('addgarage', Lang:t('info.add_garage'), {}, false, function(source)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('aj-houses:client:addGarage', src)
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

AJFW.Commands.Add('ring', Lang:t('info.ring_doorbell'), {}, false, function(source)
    local src = source
    TriggerClientEvent('aj-houses:client:RequestRing', src)
end)

-- Item

AJFW.Functions.CreateUseableItem('police_stormram', function(source, _)
    local Player = AJFW.Functions.GetPlayer(source)
    if (Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty) then
        TriggerClientEvent('aj-houses:client:HomeInvasion', source)
    else
        TriggerClientEvent('AJFW:Notify', source, Lang:t('error.emergency_services'), 'error')
    end
end)
...
RegisterNetEvent('aj-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent('aj-houses:client:setHouseConfig', src, Config.Houses)
end)

RegisterNetEvent('aj-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent('aj-houses:client:createBlip', -1, coords)
end)

RegisterNetEvent('aj-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", '')
    ...
    TriggerClientEvent('aj-houses:client:setHouseConfig', -1, Config.Houses)
    TriggerClientEvent('AJFW:Notify', src, Lang:t('info.added_house', { value = label }))
    TriggerEvent('aj-log:server:CreateLog', 'house', Lang:t('log.house_created'), 'green', Lang:t('log.house_address', { label = label, price = price, tier = tier, agent = GetPlayerName(src) }))
end)

RegisterNetEvent('aj-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent('aj-garages:client:addHouseGarage', -1, house, garageInfo)
    TriggerClientEvent('AJFW:Notify', src, Lang:t('info.added_garage', { value = garageInfo.label }))
end)

RegisterNetEvent('aj-houses:server:viewHouse', function(house)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('aj-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes,
        pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('aj-houses:server:buyHouse', function(house)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
    ...
    if isOwned then
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.already_owned'), 'error')
        CancelEvent()
        return
    end
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('aj-houses:client:SetClosestHouse', src)
        TriggerClientEvent('aj-house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, 'bought-house') -- 21% Extra house costs
        exports['aj-banking']:AddMoney('realestate', (HousePrice / 100) * math.random(18, 25), 'House purchase')
        TriggerEvent('aj-log:server:CreateLog', 'house', Lang:t('log.house_purchased'), 'green', Lang:t('log.house_purchased_by', { house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname }))
        TriggerClientEvent('AJFW:Notify', src, Lang:t('success.house_purchased'), 'success', 5000)
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_enough_money'), 'error')
    end
end)

RegisterNetEvent('aj-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('aj-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('aj-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('aj-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('aj-houses:server:giveKey', function(house, target)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local pData = AJFW.Functions.GetPlayer(target)
    ...
    if not isHouseOwner(Player.PlayerData.license, Player.PlayerData.citizenid, house) then
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_owner'), 'error')
        return
    end
...
RegisterNetEvent('aj-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('AJFW:Notify', src, Lang:t('error.remove_key_from', { firstname = citizenData.firstname, lastname = citizenData.lastname }), 'error')
    MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
end)

RegisterNetEvent('aj-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = AJFW.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('aj-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('aj-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('aj-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('aj-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', { json.encode(decorations), house })
    TriggerClientEvent('aj-houses:server:sethousedecorations', -1, house, decorations)
end)

RegisterNetEvent('aj-houses:server:LogoutLocation', function()
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-houses:server:giveHouseKey', function(target, house)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if housekeyholders[house] then
        for _, cid in pairs(housekeyholders[house]) do
            if cid == tPlayer.PlayerData.citizenid then
                TriggerClientEvent('AJFW:Notify', src, Lang:t('error.already_keys'), 'error', 3500)
                return
            end
        end
        housekeyholders[house][#housekeyholders[house] + 1] = tPlayer.PlayerData.citizenid
        MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
        TriggerClientEvent('aj-houses:client:refreshHouse', tPlayer.PlayerData.source)

        TriggerClientEvent('AJFW:Notify', tPlayer.PlayerData.source, Lang:t('success.recieved_key', { value = Config.Houses[house].adress }), 'success', 2500)
    else
        local sourceTarget = AJFW.Functions.GetPlayer(src)
        housekeyholders[house] = {
...
RegisterNetEvent('aj-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('aj-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('aj-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('aj-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('aj-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback('aj-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money['bank']
...
AJFW.Functions.CreateCallback('aj-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
AJFW.Functions.CreateCallback('aj-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
AJFW.Functions.CreateCallback('aj-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback('aj-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

AJFW.Functions.CreateCallback('aj-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
AJFW.Functions.CreateCallback('aj-phone:server:TransferCid', function(_, cb, NewCid, house)
...
AJFW.Functions.CreateCallback('aj-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
AJFW.Functions.CreateCallback('aj-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', { house })
    if result[1] then
        retval = result[1]
...
AJFW.Functions.CreateCallback('aj-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
    if pData then
...
AJFW.Functions.CreateCallback('aj-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local MyHouses = {}
...
AJFW.Functions.CreateCallback('aj-phone:server:GetHouseKeys', function(source, cb)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local MyKeys = {}
...
AJFW.Functions.CreateCallback('aj-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then
