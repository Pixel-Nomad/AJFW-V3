...
$(document).on('click', '.bank-app-account', function(e){
    ...
    AJ.Phone.Notifications.Add("fas fa-university", "AJank", "Account number. copied!", "#badc58", 1750);
});
...
AJ.Phone.Functions.DoBankOpen = function() {
    AJ.Phone.Data.PlayerData.money.bank = (AJ.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(AJ.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&#36; "+AJ.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', AJ.Phone.Data.PlayerData.money.bank);

    $(".bank-app-loaded").css({"display":"none", "padding-left":"30vh"});
    $(".bank-app-accounts").css({"left":"30vh"});
    $(".ajank-logo").css({"left": "0vh"});
    $("#ajank-text").css({"opacity":"0.0", "left":"9vh"});
...
$(document).on('click', '.bank-app-account-actions', function(e){
    AJ.Phone.Animations.TopSlideDown(".bank-app-transfer", 400, 0);
});

$(document).on('click', '#cancel-transfer', function(e){
    e.preventDefault();

    AJ.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('https://aj-phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    AJ.Phone.Notifications.Add("fas fa-university", "AJank", "You have transfered &#36; "+amount+"!", "#badc58", 1500);
                } else {
                    AJ.Phone.Notifications.Add("fas fa-university", "AJank", "You don't have enough balance!", "#badc58", 1500);
                }
                AJ.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        AJ.Phone.Notifications.Add("fas fa-university", "AJank", "Fill out all fields!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://aj-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                AJ.Phone.Notifications.Add("fas fa-university", "AJank", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('https://aj-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        society: InvoiceData.society,
        invoiceId: InvoiceData.id,
...
AJ.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
...
AJ.Phone.Functions.LoadContactsWithNumber = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactColor = AJ.Phone.ContactColors[RandomNumber];
...
$(document).on('click', '.bank-app-my-contacts-list-back', function(e){
    ...
    AJ.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

$(document).on('click', '.bank-transfer-mycontacts-icon', function(e){
    e.preventDefault();

    AJ.Phone.Animations.TopSlideDown(".bank-app-my-contacts", 400, 0);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        AJ.Phone.Notifications.Add("fas fa-university", "AJank", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    AJ.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});