...
SetupAppstore = function(data) {
    ...
    $.each(data.StoreApps, function(i, app){
        if (data.PhoneData.InstalledApps[i] == null || data.PhoneData.InstalledApps[i] == undefined) {
            if(app.blockedjobs != AJ.Phone.Data.PlayerJob.name){
...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    AJ.Phone.Data.Applications[AppData.app] = null;

    $.post('https://aj-phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('https://aj-phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, AJ.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === AJ.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    AJ.Phone.Data.Applications[Installed.data.app] = Installed.data;
