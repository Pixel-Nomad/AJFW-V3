...
function AJPhone.AddMentionedTweet(citizenid, TweetData)
...
function AJPhone.SetPhoneAlerts(citizenid, app, alerts)
...
local function GenerateOwnerName()
    local names = {
        ...
        [15] = { name = 'Jacques Schmitt', citizenid = 'AJC091H93' },
...
local function sendNewMailToOffline(citizenid, mailData)
    ...
    if Player then
        ...
        if mailData.button == nil then
            ...
            TriggerClientEvent('aj-phone:client:NewMailNotify', src, mailData)
        else
        ...
        SetTimeout(200, function()
            ...
            TriggerClientEvent('aj-phone:client:UpdateMails', src, mails)
        end)
    else
        if mailData.button == nil then
...
AJFW.Functions.CreateCallback('aj-phone:server:GetCallState', function(_, cb, ContactData)
    local Target = AJFW.Functions.GetPlayerByPhone(ContactData.number)
    if Target ~= nil then
    ...
    if Player ~= nil then
        ...
        if invoices[1] ~= nil then
            for _, v in pairs(invoices) do
                local Ply = AJFW.Functions.GetPlayerByCitizenId(v.sender)
                if Ply ~= nil then
...
AJFW.Functions.CreateCallback('aj-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    ...
    exports['aj-phone']:sendNewMailToOffline(sendercitizenid, invoiceMailData)
    exports['aj-banking']:AddMoney(society, amount, 'Phone invoice')
...
AJFW.Functions.CreateCallback('aj-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Invoices = {}
    local Ply = AJFW.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', { invoiceId })
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', { Ply.PlayerData.citizenid })
...
AJFW.Functions.CreateCallback('aj-phone:server:GetContactPictures', function(_, cb, Chats)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetContactPicture', function(_, cb, Chat)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if result[1] ~= nil then
...
AJFW.Functions.CreateCallback('aj-phone:server:FetchResult', function(_, cb, search)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, _ in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = AJFW.Shared.Vehicles[result[k].vehicle]
...
AJFW.Functions.CreateCallback('aj-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
        ...
        TriggerClientEvent('AJFW:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)

AJFW.Functions.CreateCallback('aj-phone:server:HasPhone', function(source, cb)
...
AJFW.Functions.CreateCallback('aj-phone:server:CanTransferMoney', function(source, cb, amount, iban)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
RegisterNetEvent('aj-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('aj-phone:client:UpdateAdverts', -1, Adverts, '@' .. Player.PlayerData.charinfo.firstname .. '' .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('aj-phone:server:DeleteAdvert', function()
    local Player = AJFW.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('aj-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('aj-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('aj-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { Player.PlayerData.citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
...
RegisterNetEvent('aj-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = AJFW.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('aj-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('aj-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for _, v in pairs(AJFW.Functions.GetPlayers()) do
        local Player = AJFW.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                AJPhone.SetPhoneAlerts(Player.PlayerData.citizenid, 'twitter')
                AJPhone.AddMentionedTweet(Player.PlayerData.citizenid, TweetMessage)
                TriggerClientEvent('aj-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]['twitter'])
            else
                ...
                if result[1] ~= nil then
                    ...
                    AJPhone.SetPhoneAlerts(MentionedTarget, 'twitter')
                    AJPhone.AddMentionedTweet(MentionedTarget, TweetMessage)
                end
            end
        end
    end
...
RegisterNetEvent('aj-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
    local Target = AJFW.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('aj-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('aj-phone:server:BillingEmail', function(data, paid)
    for _, v in pairs(AJFW.Functions.GetPlayers()) do
        local target = AJFW.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. AJFW.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. AJFW.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('aj-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. AJFW.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. AJFW.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('aj-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('aj-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('aj-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('aj-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = AJFW.Functions.GetPlayer(src).citizenid
    AJPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('aj-phone:server:DeleteTweet', function(tweetId)
    local Player = AJFW.Functions.GetPlayer(source)
    local delete = false
    local TID = tweetId
    ...
    if delete then
        ...
        TriggerClientEvent('aj-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('aj-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    if Config.Linux then
        ...
        TriggerClientEvent('aj-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
    else
        MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            TweetData.citizenid,
            TweetData.firstName,
...
RegisterNetEvent('aj-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = AJFW.Functions.GetPlayer(src)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('aj-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('AJFW:Notify', src, "This account number doesn't exist!", 'error')
    end
end)

RegisterNetEvent('aj-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('aj-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
    local SenderData = AJFW.Functions.GetPlayer(src)
    local query = '%' .. ChatNumber .. '%'
    local Player = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if Player[1] ~= nil then
        local TargetData = AJFW.Functions.GetPlayerByCitizenId(Player[1].citizenid)
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('aj-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('aj-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', { SenderData.PlayerData.citizenid, ChatNumber })
            if Chat[1] ~= nil then
...
RegisterNetEvent('aj-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
    local Hour = os.date('%H')
    local Minute = os.date('%M')
    local label = Hour .. ':' .. Minute
    TriggerClientEvent('aj-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = AJFW.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('aj-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('aj-phone:server:CancelCall', function(ContactData)
    local Ply = AJFW.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('aj-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('aj-phone:server:AnswerCall', function(CallData)
    local Ply = AJFW.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('aj-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('aj-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('aj-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('aj-phone:server:AddTransaction', function(data)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('aj-phone:RefreshPhone', src)
end)

RegisterNetEvent('aj-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:getImageFromGallery', function()
    ...
    local Player = AJFW.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC', { Player.PlayerData.citizenid })
    TriggerClientEvent('aj-phone:refreshImages', src, images)
end)

RegisterNetEvent('aj-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent('AJFW:Notify', src, 'You cannot ping yourself', 'error')
    end
end)
...
AJFW.Commands.Add('bill', 'Bill A Player', { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Fine Amount' } }, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('AJFW:Notify', source, 'Invoice Successfully Sent', 'success')
                    TriggerClientEvent('AJFW:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('AJFW:Notify', source, 'Must Be A Valid Amount Above 0', 'error')
                end
            else
                TriggerClientEvent('AJFW:Notify', source, 'You Cannot Bill Yourself', 'error')
            end
        else
            TriggerClientEvent('AJFW:Notify', source, 'Player Not Online', 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', source, 'No Access', 'error')
    end
end)
