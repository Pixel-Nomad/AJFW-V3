    ...
    if Zone then
        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                AJFW.Functions.SetVehicleProperties(occasionVehicles[Zone][i].car, json.decode(occasionVehicles[Zone][i].mods))
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['aj-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = 'client',
                                event = 'aj-vehiclesales:client:OpenContract',
                                icon = 'fas fa-car',
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if oldVehicle then
            AJFW.Functions.DeleteVehicle(oldVehicle)
        end

        if EntityZones[i] and Config.UseTarget then
            exports['aj-target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
...
local function openSellContract(bool)
    local pData = AJFW.Functions.GetPlayerData()
    ...
    SendNUIMessage({
        ...
        plate = AJFW.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId()))
    })
end
...
local function sellVehicleWait(price)
    ...
    Wait(1500)
    DoScreenFadeIn(250)
    AJFW.Functions.Notify(Lang:t('success.car_up_for_sale', { value = price }), 'success')
    PlaySound(-1, 'SELECT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', 0, 0, 1)
end

local function SellData(data, model)
    AJFW.Functions.TriggerCallback('aj-vehiclesales:server:CheckModelName', function(DataReturning)
        local vehicleData = {}
        vehicleData.ent = GetVehiclePedIsUsing(PlayerPedId())
        vehicleData.model = DataReturning
        vehicleData.plate = model
        vehicleData.mods = AJFW.Functions.GetVehicleProperties(vehicleData.ent)
        vehicleData.desc = data.desc
        TriggerServerEvent('aj-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('aj-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('aj-occasions:client:MainMenu')
                        --TriggerEvent('aj-vehiclesales:client:SellVehicle')
                    else
                        AJFW.Functions.Notify(Lang:t('error.not_in_veh'), 'error', 4500)
                    end
                end
            end
            Wait(0)
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                AJFW.Functions.TriggerCallback('aj-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('sellVehicle', function(data, cb)
    local plate = AJFW.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId())) --Getting the plate and sending to the function
    SellData(data, plate)
    cb('ok')
end)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('aj-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('aj-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('aj-occasions:client:BuyFinished', function(vehdata)
    ...
    AJFW.Functions.TriggerCallback('AJFW:Server:SpawnVehicle', function(netId)
        ...
        AJFW.Functions.Notify(Lang:t('success.vehicle_bought'), 'success', 2500)
        TriggerEvent('vehiclekeys:client:SetOwner', vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        AJFW.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('aj-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        AJFW.Functions.TriggerCallback('aj-occasions:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('aj-occasions:server:sellVehicleBack', vehicleData)
                    AJFW.Functions.DeleteVehicle(vehicle)
                else
                    AJFW.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
                end
            else
                AJFW.Functions.Notify(Lang:t('error.not_your_vehicle'), 'error', 3500)
            end
...
RegisterNetEvent('aj-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    AJFW.Functions.TriggerCallback('AJFW:Server:SpawnVehicle', function(netId)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        AJFW.Functions.Notify(Lang:t('info.vehicle_returned'))
        TriggerEvent('vehiclekeys:client:SetOwner', vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        AJFW.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('aj-occasion:client:refreshVehicles', function()
    if Zone then
        AJFW.Functions.TriggerCallback('aj-occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('aj-vehiclesales:client:SellVehicle', function()
    local VehiclePlate = AJFW.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId()))
    AJFW.Functions.TriggerCallback('aj-occasions:server:checkVehicleOwner', function(owned, balance)
        if owned then
            if balance < 1 then
                AJFW.Functions.TriggerCallback('aj-occasions:server:getVehicles', function(vehicles)
                    if vehicles == nil or #vehicles < #Config.Zones[Zone].VehicleSpots then
                        ...
                        AJFW.Functions.Notify(Lang:t('error.no_space_on_lot'), 'error', 3500)
                    end
                end)
            else
                AJFW.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
...
RegisterNetEvent('aj-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        AJFW.Functions.TriggerCallback('aj-occasions:server:getSellerInformation', function(info)
            if info then
        ...
        AJFW.Functions.Notify(Lang:t('error.not_for_sale'), 'error', 7500)
    end
end)

RegisterNetEvent('aj-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'aj-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'aj-occasions:client:SellBackCar',
            }
        }
    }

    exports['aj-menu']:openMenu(MainMenu)
end)
    ...
    for k, cars in pairs(Config.Zones) do
        ...
        SpawnZone[k]:onPlayerInOut(function(isPointInside)
            if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                exports['aj-base']:DrawText(Lang:t('menu.interaction'), 'left')
                TextShown = true
                Listen4Control()
            else
                listen = false
            ...
            for k2, v in pairs(Config.Zones[k].VehicleSpots) do
                ...
                VehicleZones:onPlayerInOut(function(isPointInside)
                    if isPointInside and IsCarSpawned(k2) then
                        exports['aj-base']:DrawText(Lang:t('menu.view_contract_int'), 'left')
                        TextShown = true
                        Listen4Control(k2)
                    else
                        listen = false
