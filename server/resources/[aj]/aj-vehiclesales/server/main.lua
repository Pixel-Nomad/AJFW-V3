...
AJFW.Functions.CreateCallback('aj-occasions:server:getVehicles', function(_, cb)
...
AJFW.Functions.CreateCallback('aj-occasions:server:checkVehicleOwner', function(source, cb, plate)
    local pData = AJFW.Functions.GetPlayer(source)
...
AJFW.Functions.CreateCallback('aj-occasions:server:getSellerInformation', function(_, cb, citizenid)
...
AJFW.Functions.CreateCallback('aj-vehiclesales:server:CheckModelName', function(_, cb, plate)
    if plate then
...
RegisterNetEvent('aj-occasions:server:ReturnVehicle', function(vehicleData)
    local src = source
    ...
    if result[1] then
        if result[1].seller == Player.PlayerData.citizenid then
            ...
            TriggerClientEvent('aj-occasions:client:ReturnOwnedVehicle', src, result[1])
            TriggerClientEvent('aj-occasion:client:refreshVehicles', -1)
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_your_vehicle'), 'error', 3500)
        end
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.vehicle_does_not_exist'), 'error', 3500)
    end
end)

RegisterNetEvent('aj-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    TriggerEvent('aj-log:server:CreateLog', 'vehicleshop', 'Vehicle for Sale', 'red', '**' .. GetPlayerName(src) .. '** has a ' .. vehicleData.model .. ' priced at ' .. vehiclePrice)
    TriggerClientEvent('aj-occasion:client:refreshVehicles', -1)
end)

RegisterNetEvent('aj-occasions:server:sellVehicleBack', function(vehData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local price = 0
    ...
    TriggerClientEvent('AJFW:Notify', src, Lang:t('success.sold_car_for_price', { value = payout }), 'success', 5500)
    MySQL.query('DELETE FROM player_vehicles WHERE plate = ?', { plate })
end)

RegisterNetEvent('aj-occasions:server:buyVehicle', function(vehicleData)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM occasion_vehicles WHERE plate = ? AND occasionid = ?', { vehicleData['plate'], vehicleData['oid'] })
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            local SellerData = AJFW.Functions.GetPlayerByCitizenId(SellerCitizenId)
            ...
            TriggerEvent('aj-log:server:CreateLog', 'vehicleshop', 'bought', 'green', '**' .. GetPlayerName(src) .. '** has bought for ' .. result[1].price .. ' (' .. result[1].plate .. ') from **' .. SellerCitizenId .. '**')
            TriggerClientEvent('aj-occasions:client:BuyFinished', src, result[1])
            TriggerClientEvent('aj-occasion:client:refreshVehicles', -1)
            MySQL.query('DELETE FROM occasion_vehicles WHERE plate = ? AND occasionid = ?', { result[1].plate, result[1].occasionid })
            exports['aj-phone']:sendNewMailToOffline(SellerCitizenId, {
                sender = Lang:t('mail.sender'),
                subject = Lang:t('mail.subject'),
                message = Lang:t('mail.message', { value = NewPrice, value2 = AJFW.Shared.Vehicles[result[1].model].name })
            })
        else
            TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_enough_money'), 'error', 3500)
        end
    end
end)
