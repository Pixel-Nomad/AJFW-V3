...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('aj-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    AJFW.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('aj-diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    if currentDivingLocation.area ~= 0 then
        for k in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
            if Config.UseTarget then
                exports['aj-target']:RemoveZone(k)
            else
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['aj-target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
            ...
            zones[k]:onPlayerInOut(function(inside)
                if inside then
                    currentArea = k
                    exports['aj-base']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    currentArea = 0
...
local function sellCoral()
    ...
    AJFW.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ClearPedTasks(playerPed)
        TriggerServerEvent('aj-diving:server:SellCoral')
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['aj-base']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['aj-base']:HideText()
                end
...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
    AJFW.Functions.TriggerCallback('aj-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('aj-diving:client:NewLocations', function()
    AJFW.Functions.TriggerCallback('aj-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('aj-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('aj-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("aj-diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       oxgenlevell = Config.oxygenlevel -- oxygenlevel
       AJFW.Functions.Notify(Lang:t("success.tube_filled"), 'success')
       TriggerServerEvent('aj-diving:server:removeItemAfterFill')
    else
        AJFW.Functions.Notify(Lang:t("error.oxygenlevel", {oxygenlevel = oxgenlevell}), 'error')
    end
end)
...
RegisterNetEvent('aj-diving:client:UseGear', function()
    local ped = PlayerPedId()
    if iswearingsuit == false then
        if oxgenlevell > 0 then
            ...
            if not IsPedSwimming(ped) and not IsPedInAnyVehicle(ped) then
                gearAnim()
                AJFW.Functions.Progressbar("equip_gear", Lang:t("info.put_suit"), 5000, false, true, {}, {}, {}, {},
                    function() -- Done
                        deleteGear()
                ...
                AJFW.Functions.Notify(Lang:t("error.not_standing_up"), 'error')
            end
        else
            AJFW.Functions.Notify(Lang:t("error.need_otube"), 'error')
        end
    elseif iswearingsuit == true then
        gearAnim()
        AJFW.Functions.Progressbar("remove_gear", Lang:t("info.pullout_suit"), 5000, false, true, {}, {}, {}, {},
            function() -- Done
                SetEnableScuba(ped, false)
                ...
                ClearPedTasks(ped)
                deleteGear()
                AJFW.Functions.Notify(Lang:t("success.took_out"))
                TriggerServerEvent("InteractSound_SV:PlayOnSource", nil, 0.25)
                iswearingsuit = false
                oxgenlevell = oxgenlevell
            end)
