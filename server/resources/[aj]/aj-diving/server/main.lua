...
RegisterNetEvent('aj-diving:server:CallCops', function(coords)
    for _, Player in pairs(AJFW.Functions.GetAJPlayers()) do
        if Player then
            if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
                local msg = Lang:t("info.cop_msg")
                TriggerClientEvent('aj-diving:client:CallCops', Player.PlayerData.source, coords, msg)
                local alertData = {
...
RegisterNetEvent('aj-diving:server:SellCoral', function()
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if hasCoral(src) then
        ...
        TriggerClientEvent('AJFW:Notify', src, Lang:t("error.no_coral"), 'error')
    end
end)

RegisterNetEvent('aj-diving:server:TakeCoral', function(area, coral, bool)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if not Player then return end
    local coralType = math.random(1, #Config.CoralTypes)
    local amount = math.random(1, Config.CoralTypes[coralType].maxAmount)
    local ItemData = AJFW.Shared.Items[Config.CoralTypes[coralType].item]
    if amount > 1 then
        for _ = 1, amount, 1 do
    ...
    if (Config.CoralLocations[area].TotalCoral - 1) == 0 then
        ...
        TriggerClientEvent('aj-diving:client:NewLocations', -1)
    else
    ...
    TriggerClientEvent('aj-diving:client:UpdateCoral', -1, area, coral, bool)
end)

RegisterNetEvent('aj-diving:server:removeItemAfterFill', function()
   local src = source
   local Player = AJFW.Functions.GetPlayer(src)
   Player.Functions.RemoveItem("diving_fill", 1)
   TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["diving_fill"], "remove")
end)

-- Callbacks

AJFW.Functions.CreateCallback('aj-diving:server:GetDivingConfig', function(_, cb)
