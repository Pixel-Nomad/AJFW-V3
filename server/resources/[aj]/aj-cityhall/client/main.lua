...
local function getJobs()
    AJFW.Functions.TriggerCallback('aj-cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('aj-cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['aj-base']:DrawText('[E] Take Driving Lessons')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['aj-base']:DrawText('[E] Open Cityhall')
                            end
                        else
...
RegisterNetEvent('aj-cityhall:Client:AddCityJob', function()
    getJobs()
end)

RegisterNetEvent('aj-cityhall:client:getIds', function()
    TriggerServerEvent('aj-cityhall:server:getIDs')
end)

RegisterNetEvent('aj-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['aj-base']:DrawText('[E] Open Cityhall') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('aj-cityhall:server:requestId', id.type, closestCityhall)
        AJFW.Functions.Notify(('You have received your %s for $%s'):format(license.label, id.cost), 'success', 3500)
    else
        AJFW.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('aj-cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        AJFW.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
