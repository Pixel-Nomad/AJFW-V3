...
RegisterNetEvent('aj-scrapyard:server:LoadVehicleList', function()
    local src = source
    TriggerClientEvent("aj-scapyard:client:setNewVehicles", src, Config.CurrentVehicles)
end)


AJFW.Functions.CreateCallback('aj-scrapyard:checkOwnerVehicle', function(_, cb, plate)
...
RegisterNetEvent('aj-scrapyard:server:ScrapVehicle', function(listKey)
    local src = source
    ...
    if Config.CurrentVehicles[listKey] ~= nil then
        ...
        if Luck == Odd then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["rubber"], 'add')

        end
        Config.CurrentVehicles[listKey] = nil
        TriggerClientEvent("aj-scapyard:client:setNewVehicles", -1, Config.CurrentVehicles)
    end
end)
