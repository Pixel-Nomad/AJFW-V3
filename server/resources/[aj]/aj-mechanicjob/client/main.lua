...
local function GetVehicleService(plate)
    local retval = false
    if VehicleStatus[plate] ~= nil then 
        if VehicleStatus[plate]["radiator"] <= 80 then
            retval = true
        elseif VehicleStatus[plate]["axle"] <= 80 then
            retval = true
        elseif VehicleStatus[plate]["brakes"] <= 80 then
            retval = true
        elseif VehicleStatus[plate]["clutch"] <= 80 then
            retval = true
        elseif VehicleStatus[plate]["fuel"] <= 80 then
            retval = true
        end
    end
    return retval
...
local function DeleteTarget(id)
    if Config.UseTarget then
        exports['aj-target']:RemoveZone(id)
    else
...
local function RegisterDutyTarget()
    ...
    if Config.UseTarget then
        exports['aj-target']:AddBoxZone(dutyTargetBoxID, coords, 1.5, 1.5, {
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['aj-base']:DrawText("[E] " .. label, 'left')
            else
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['aj-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "aj-mechanicjob:client:target:OpenStash",
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['aj-base']:DrawText(Lang:t('labels.o_stash'), 'left')
            else
                exports['aj-base']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['aj-base']:DrawText(Lang:t('labels.h_vehicle'), 'left')
            else
                exports['aj-base']:DrawText(Lang:t('labels.g_vehicle'), 'left')
            end
        else
            exports['aj-base']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['aj-base']:DrawText(Lang:t('labels.o_menu'), 'left')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['aj-base']:DrawText(Lang:t('labels.work_v'), 'left')
                end
            end
        else
            exports['aj-base']:HideText()
...
local function SendStatusMessage(statusList)
    if statusList ~= nil then
        TriggerEvent('chat:addMessage', {
            ...
            args = {Lang:t('labels.veh_status'), round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..AJFW.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..AJFW.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..AJFW.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..AJFW.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..AJFW.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..AJFW.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..AJFW.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "aj-mechanicjob:client:UnattachVehicle",
            }
        }, {
            header = Lang:t('lift_menu.header_stats'),
            txt = Lang:t('lift_menu.desc_stats'),
            params = {
                event = "aj-mechanicjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "aj-mechanicjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "aj-mechanicjob:client:target:CloseMenu",
            }
        },
    }
...
local function PartsMenu()
    local plate = AJFW.Functions.GetPlate(Config.Plates[ClosestPlate].AttachedVehicle)
    if VehicleStatus[plate] ~= nil then
        local vehicleMenu = {
            {
                header = "Status",
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "aj-mechanicjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    txt = Lang:t('parts_menu.status') .. percentage .. ".0% / 100.0%",
                    params = {
                        event = "aj-mechanicjob:client:NoDamage",
                    }
                }
            end
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        {
            header = ""..partName.."",
            txt = Lang:t('parts_menu.repair_op')..AJFW.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x",
            params = {
                event = "aj-mechanicjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
    ...
    exports['aj-menu']:openMenu(TestMenu1)
end
...
local function NoDamage()
    ...
    exports['aj-menu']:openMenu(noDamage)
end
...
local function UnattachVehicle()
    ...
    TriggerServerEvent('aj-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
...
local function RepairPart(part)
    ...
    AJFW.Functions.TriggerCallback('aj-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            AJFW.Functions.Progressbar("repair_part", Lang:t('labels.progress_bar') ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                AJFW.Functions.Notify(Lang:t('notifications.rep_canceled'), "error")
            end)
        else
            AJFW.Functions.Notify(Lang:t('notifications.not_materials'), 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent("aj-mechanicjob:client:UnattachVehicle",function()
    UnattachVehicle()
end)

RegisterNetEvent("aj-mechanicjob:client:PartsMenu",function()
    PartsMenu()
end)

RegisterNetEvent("aj-mechanicjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("aj-mechanicjob:client:NoDamage",function()
    NoDamage()
end)

RegisterNetEvent("aj-mechanicjob:client:CheckStatus",function()
    CheckStatus()
end)
...
RegisterNetEvent("aj-mechanicjob:client:RepairPart",function(data)
...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
    ...
    AJFW.Functions.TriggerCallback('aj-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    AJFW.Functions.TriggerCallback('aj-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('AJFW:Client:SetDuty', function(duty)
    onDuty = duty
...
RegisterNetEvent('aj-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
...
RegisterNetEvent('aj-vehicletuning:client:RepaireeePart', function(part)
    local veh = Config.Plates[ClosestPlate].AttachedVehicle
    local plate = AJFW.Functions.GetPlate(veh)
    if part == "engine" then
    ...
    AJFW.Functions.Notify(Lang:t('notifications.partrep', {value = Config.ValuesLabels[part]}))
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then
                        ...
                        AJFW.Functions.Notify(Lang:t('notifications.uknown'), "error")
                    end
                else
                    AJFW.Functions.Notify(Lang:t('notifications.not_valid'), "error")
                end
            else
                AJFW.Functions.Notify(Lang:t('notifications.not_close'), "error")
            end
        else
            AJFW.Functions.Notify(Lang:t('notifications.veh_first'), "error")
        end
    else
        AJFW.Functions.Notify(Lang:t('notifications.outside'), "error")
    end
end)
...
RegisterNetEvent('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            AJFW.Functions.Notify(Lang:t('notifications.wrong_seat'), "error")
        end
    else
        AJFW.Functions.Notify(Lang:t('notifications.not_vehicle'), "error")
    end
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        AJFW.Functions.Progressbar("repair_advanced", Lang:t('notifications.progress_bar'), lockpickTime, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            if part == "body" then
                                ...
                                TriggerServerEvent("aj-mechanicjob:server:removePart", part, needAmount)
                                TriggerEvent("inventory:client:ItemBox", AJFW.Shared.Items[Config.RepairCost[part]], "remove")
                            elseif part ~= "engine" then
                                TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                TriggerServerEvent("aj-mechanicjob:server:removePart", part, level)
                                TriggerEvent("inventory:client:ItemBox", AJFW.Shared.Items[Config.RepairCost[part]], "remove")
                            end
                        end, function() -- Cancel
                            openingDoor = false
                            ClearPedTasks(PlayerPedId())
                            AJFW.Functions.Notify(Lang:t('notifications.process_canceled'), "error")
                        end)
                    else
                        AJFW.Functions.Notify(Lang:t('notifications.not_part'), "error")
                    end
                else
                    AJFW.Functions.Notify(Lang:t('notifications.not_valid'), "error")
                end
...
RegisterNetEvent('aj-mechanicjob:client:target:OpenStash', function ()
...
RegisterNetEvent('aj-mechanicjob:client:target:CloseMenu', function()
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])

    TriggerEvent('aj-menu:client:closeMenu')
end)
        ...
        if PlayerJob.type == 'mechanic' then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 5
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("aj-mechanicjob:client:target:OpenStash")
                    end
                end
                ...
                if isInsideVehiclePlateZone then
                    ...
                    if attachedVehicle then
                        ...
                        if IsControlJustPressed(0, 38) and IsPedInAnyVehicle(PlayerPedId()) then
                            ...
                            Wait(500)
                            DoScreenFadeIn(150)
                            TriggerServerEvent('aj-vehicletuning:server:SetAttachedVehicle', veh, ClosestPlate)
