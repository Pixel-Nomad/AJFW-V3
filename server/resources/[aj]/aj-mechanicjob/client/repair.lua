...
local function RepairPart(vehicle, plate, component)
    ...
    local hasItems = exports['aj-inventory']:HasItem(materials)
    if not hasItems then
        AJFW.Functions.Notify(Lang:t('warning.no_materials'), 'error')
        return
    end
    local componentLabel = Config.WearableParts[component].label or component
    ToggleHood(vehicle)
    AJFW.Functions.Progressbar('repairing_vehicle', string.format(Lang:t('progress.repairing'), componentLabel), 10000, false, true, {
        ...
        ToggleHood(vehicle)
        AJFW.Functions.Notify(string.format(Lang:t('success.part_repaired'), componentLabel), 'success')
        for item, amount in pairs(materials) do
            TriggerServerEvent('aj-mechanicjob:server:removeItem', item, amount)
        end
        TriggerServerEvent('aj-mechanicjob:server:repairVehicleComponent', plate, component)
    end, function()
        ToggleHood(vehicle)
    end)
end
...
RegisterNetEvent('aj-mechanicjob:client:PartsMenu', function()
    local vehicle, distance = AJFW.Functions.GetClosestVehicle()
    ...
    AJFW.Functions.TriggerCallback('aj-mechanicjob:server:getVehicleStatus', function(status)
...
RegisterNetEvent('aj-mechanicjob:client:repairVehicle', function()
    local vehicle, distance = AJFW.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    if not IsNearBone(vehicle, 'engine') then return end
    local engineHealth = GetVehicleEngineHealth(vehicle)
    local bodyHealth = GetVehicleBodyHealth(vehicle)
    ToggleHood(vehicle)
    AJFW.Functions.Progressbar('repairing_vehicle', Lang:t('progress.repair_vehicle'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ToggleHood(vehicle)
        AJFW.Functions.Notify(Lang:t('success.repaired'), 'success')
        TriggerServerEvent('aj-mechanicjob:server:removeItem', 'repairkit')
    end, function()
        ToggleHood(vehicle)
    end)
end)

RegisterNetEvent('aj-mechanicjob:client:repairVehicleFull', function()
    local vehicle, distance = AJFW.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    if not IsNearBone(vehicle, 'engine') then return end
    ToggleHood(vehicle)
    AJFW.Functions.Progressbar('repairing_vehicle', Lang:t('progress.repair_vehicle'), 10000, false, true, {
        ...
        ToggleHood(vehicle)
        AJFW.Functions.Notify(Lang:t('success.repaired'), 'success')
        TriggerServerEvent('aj-mechanicjob:server:removeItem', 'advancedrepairkit')
    end, function()
        ToggleHood(vehicle)
    end)
end)

RegisterNetEvent('aj-mechanicjob:client:repairTire', function()
    local vehicle, distance = AJFW.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    local closestWheel = GetClosestWheel(vehicle)
    if not closestWheel then return end
    AJFW.Functions.Progressbar('repairing_tire', Lang:t('progress.repair_tire'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        AJFW.Functions.Notify(Lang:t('success.tire_repaired'), 'success')
        TriggerServerEvent('aj-mechanicjob:server:removeItem', 'tirerepairkit')
    end)
end)

RegisterNetEvent('aj-mechanicjob:client:cleanVehicle', function()
    local vehicle, distance = AJFW.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    AJFW.Functions.Progressbar('cleaning_vehicle', Lang:t('progress.cleaning'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        AJFW.Functions.Notify(Lang:t('success.cleaned'), 'success')
        TriggerServerEvent('aj-mechanicjob:server:removeItem', 'cleaningkit')
    end)
end)

RegisterNetEvent('aj-mechanicjob:client:fixEverything', function()
    AJFW.Functions.TriggerCallback('aj-mechanicjob:server:hasPermission', function(hasPermission)
