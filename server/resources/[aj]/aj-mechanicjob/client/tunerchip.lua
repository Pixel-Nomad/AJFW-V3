...
RegisterNUICallback('reset', function(_, cb)
    ResetVeh()
    TriggerEvent('aj-mechanicjob:client:openChip')
    cb('ok')
end)
...
RegisterNetEvent('aj-mechanicjob:client:checkTune', function()
    local vehicle, distance = AJFW.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    local plate = AJFW.Functions.GetPlate(vehicle)
    if not plate then return end
    AJFW.Functions.TriggerCallback('aj-mechanicjob:server:checkTune', function(status)
        if status then
            AJFW.Functions.Notify(Lang:t('success.tuned'), 'success')
        else
            AJFW.Functions.Notify(Lang:t('warning.not_tuned'), 'error')
        end
    end, plate)
end)

RegisterNetEvent('aj-mechanicjob:client:openChip', function()
    ...
    if inVehicle then
        ...
        local plate = AJFW.Functions.GetPlate(vehicle)
        AJFW.Functions.Progressbar('connect_laptop', Lang:t('progress.tuner_chip'), 2000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function()
            TriggerServerEvent('aj-mechanicjob:server:tuneStatus', plate)
