...
local function SaveSkin()
    ...
    TriggerServerEvent("aj-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    TriggerServerEvent("aj-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("aj-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if AJFW.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    AJFW.Functions.TriggerCallback('aj-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('aj-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('aj-clothes:client:CreateFirstCharacter', function()
    AJFW.Functions.GetPlayerData(function(pData)
...
RegisterNetEvent("aj-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('aj-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('aj-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('aj-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if AJFW.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        AJFW.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("aj-clothing:client:adjustfacewear", function(type)
    if AJFW.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
    TriggerServerEvent("aj-clothes:loadPlayerSkin")
    PlayerData = AJFW.Functions.GetPlayerData()
    loadStores()
--    AJFW.Shared.Jobs = exports['aj-jobs']:AddJobs()
end)
RegisterNetEvent('AJFW:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)
RegisterNetEvent('AJFW:Client:OnGangUpdate', function(GangInfo)
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('aj-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('TrackerError', function(_, cb)
    AJFW.Functions.Notify(Lang:t("notify.error_bracelet"), "error")
    cb('ok')
end)
RegisterNUICallback('saveOutfit', function(data, cb)
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    TriggerServerEvent('aj-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('aj-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('aj-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    AJFW.Functions.Notify(Lang:t('notify.info_deleteOutfit', {outfit = data.outfitName}))
    cb('ok')
end)
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['aj-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not AJFW.Shared.AJJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                        local gradeLevel = PlayerData.job.grade.level
                        getOutfits(gradeLevel, Config.Outfits[v.requiredJob])
                    end
                else break end --this break is important if AJJobsStatus is true then the reste of the code in this loop does not need to run.

                exports['aj-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['aj-base']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['aj-base']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['aj-base']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        exports['aj-base']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not AJFW.Shared.AJJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(AJFW.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not AJFW.Shared.AJJobsStatus and PlayerData.job.name)
                ...
                if inZone then
                    ...
                    elseif not AJFW.Shared.AJJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not AJFW.Shared.AJJobsStatus and PlayerData.job.grade.level)
