...
local function enterRobberyHouse(house)
    ...
    local data = exports['aj-interior']:CreateHouseRobbery(coords)
...
local function lockpickFinish(success)
    ClearPedTasks(PlayerPedId())
    if success then
        TriggerServerEvent('aj-houserobbery:server:enterHouse', closestHouse)
        AJFW.Functions.Notify(Lang:t('success.worked'), 'success', 2500)
    else
        if usingAdvanced then
            if math.random(1, 100) <= Config.ChanceToBreakAdvancedLockPick then
                TriggerServerEvent('aj-houserobbery:server:removeAdvancedLockpick')
            ...
            if math.random(1, 100) <= Config.ChanceToBreakLockPick then
                TriggerServerEvent('aj-houserobbery:server:removeLockpick')
                TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items['lockpick'], 'remove')
            end
        end

        AJFW.Functions.Notify(Lang:t('error.didnt_work'), 'error', 2500)
    end
end

local function searchCabin(cabin)
    local ped = PlayerPedId()
    if math.random(1, 100) <= 85 and not AJFW.Functions.IsWearingGloves() then
    ...
    TriggerServerEvent('aj-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if succeededAttempts + 1 >= neededAttempts then
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('aj-houserobbery:server:searchFurniture', cabin, currentHouse)
            Config.Houses[currentHouse]['furniture'][cabin]['searched'] = true
            TriggerServerEvent('aj-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            succeededAttempts = 0
        ...
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('aj-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        AJFW.Functions.Notify(Lang:t('error.process_cancelled'), 'error', 3500)
        succeededAttempts = 0
        FreezeEntityPosition(ped, false)
        SetTimeout(500, function()
            IsLockpicking = false
...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
    AJFW.Functions.TriggerCallback('aj-houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('aj-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('aj-houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('aj-houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]['opened'] = state
end)

RegisterNetEvent('aj-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]['furniture'][cabin]['searched'] = state
end)

RegisterNetEvent('aj-houserobbery:client:SetBusyState', function(cabin, house, bool)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    if Config.UseClockHours then
        if GetClockHours() < Config.MinimumTime or GetClockHours() > Config.MaximumTime then
            AJFW.Functions.Notify(Lang:t('error.not_allowed_time'), 'error', 3500)
            return
        end
    end
        ...
        if CurrentCops >= Config.PoliceOnDutyRequired then
            if not Config.Houses[closestHouse]['opened'] then
                if not usingAdvanced then
                    if Config.RequireScrewdriver and not AJFW.Functions.HasItem('screwdriverset') then
                        AJFW.Functions.Notify(Lang:t('error.missing_something'), 'error', 3500)
                        return
                    end
                end
                ...
                AJFW.Functions.Notify(Lang:t('error.door_open'), 'error', 3500)
            end
        else
            AJFW.Functions.Notify(Lang:t('error.not_enough_police'), 'error', 3500)
        end
    end
end)
...
CreateThread(function()
    Wait(500)
    local requiredItems = {
        [1] = { name = AJFW.Shared.Items['advancedlockpick']['name'], image = AJFW.Shared.Items['advancedlockpick']['image'] },
        [2] = { name = AJFW.Shared.Items['screwdriverset']['name'], image = AJFW.Shared.Items['screwdriverset']['image'] },
    }
