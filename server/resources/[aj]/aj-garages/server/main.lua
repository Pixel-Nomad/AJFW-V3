...
local function filterVehiclesByCategory(vehicles, category)
    ...
    for _, vehicle in pairs(vehicles) do
        local vehicleData = AJFW.Shared.Vehicles[vehicle.vehicle]
...
AJFW.Functions.CreateCallback('aj-garages:server:getHouseGarage', function(_, cb, house)
    local houseInfo = MySQL.single.await('SELECT * FROM houselocations WHERE name = ?', { house })
    cb(houseInfo)
end)

AJFW.Functions.CreateCallback('aj-garages:server:GetGarageVehicles', function(source, cb, garage, type, category)
...
local function GetVehicleTypeByModel(model)
    local vehicleData = AJFW.Shared.Vehicles[model]
...
AJFW.Functions.CreateCallback('aj-garages:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
    local vehType = AJFW.Shared.Vehicles[vehicle] and AJFW.Shared.Vehicles[vehicle].type or GetVehicleTypeByModel(vehicle)
...
AJFW.Functions.CreateCallback('aj-garages:server:IsSpawnOk', function(_, cb, plate, type)
...
AJFW.Functions.CreateCallback('aj-garages:server:canDeposit', function(source, cb, plate, type, garage, state)
    local Player = AJFW.Functions.GetPlayer(source)
    ...
    if type == 'house' and not exports['aj-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.Garages[garage].houseName) then
        cb(false)
        return
    end
    if state == 1 then
...
RegisterNetEvent('aj-garages:server:updateVehicleStats', function(plate, fuel, engine, body)
    local src = source
...
RegisterNetEvent('aj-garages:server:updateVehicleState', function(state, plate)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('aj-garages:server:trackVehicle', function(plate)
    local src = source
    local vehicleData = OutsideVehicles[plate]
    if vehicleData and DoesEntityExist(vehicleData.entity) then
        TriggerClientEvent('aj-garages:client:trackVehicle', src, GetEntityCoords(vehicleData.entity))
        TriggerClientEvent('AJFW:Notify', src, Lang:t('success.vehicle_tracked'), 'success')
    else
        TriggerClientEvent('AJFW:Notify', src, Lang:t('error.vehicle_not_tracked'), 'error')
    end
end)

RegisterNetEvent('aj-garages:server:PayDepotPrice', function(data)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    MySQL.scalar('SELECT depotprice FROM player_vehicles WHERE plate = ?', { data.plate }, function(result)
        if result then
            ...
            if cashBalance >= depotPrice then
                ...
                TriggerClientEvent('aj-garages:client:takeOutGarage', src, data)
...
RegisterNetEvent('aj-garages:server:syncGarage', function(updatedGarages)
