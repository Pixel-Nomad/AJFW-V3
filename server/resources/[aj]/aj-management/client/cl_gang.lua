...
local PlayerGang = AJFW.Functions.GetPlayerData().gang
    ...
    if Config.UseTarget then
        for gang, zones in pairs(Config.GangMenuZones) do
            for index, data in ipairs(zones) do
                exports['aj-target']:AddBoxZone(gang .. '-GangMenu' .. index, data.coords, data.length, data.width, {
                            ...
                            if #(pos - coords) < 5.0 then
                                ...
                                if #(pos - coords) <= 1.5 then
                                    ...
                                    if not shownGangMenu then
                                        exports['aj-base']:DrawText(Lang:t('drawtextgang.label'), 'left')
                                        shownGangMenu = true
                                    end
