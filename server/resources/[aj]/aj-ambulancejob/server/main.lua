...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney('bank', Config.BillCost, 'hospital payment')
	exports['aj-Banking']:AddMoney('ambulance', Config.BillCost, 'Player treatment')
		...
		for k, v in pairs(Config.Locations['jailbeds']) do
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					...
					TriggerClientEvent('AJFW:Notify', src, Lang:t('error.possessions_taken'), 'error')
				end
				Player.Functions.RemoveMoney('bank', Config.BillCost, 'hospital payment')
				exports['aj-Banking']:AddMoney('ambulance', Config.BillCost, 'Player treatment')
				TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = AJFW.Functions.GetPlayer(src)
	local Patient = AJFW.Functions.GetPlayer(playerId)
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' then
			Player.Functions.RemoveItem('bandage', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['bandage'], 'remove')
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' or AJFW.Functions.HasItem(src, 'firstaid', 1) then
			if oldMan then
				if Player.Functions.RemoveMoney('cash', 700, 'revived-player') then
					Player.Functions.RemoveItem('firstaid', 1)
					TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['firstaid'], 'remove')
			...
			TriggerEvent('aj-log:server:CreateLog', 'ambulancejob', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), 'aj-ambulancejob', 'Trying to revive theirselves or other players'), true)
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
	...
	if not doctorCalled then
		...
		for _, v in pairs(players) do
			if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
				TriggerClientEvent('AJFW:Notify', v.PlayerData.source, Lang:t('info.dr_needed'), 'ambulance')
			end
		end
		...
		TriggerClientEvent('AJFW:Notify', src, 'Doctor has already been notified', 'error')
	end
end)
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('AJFW:Notify', helperId, Lang:t('error.cant_help'), 'error')
	end
end)
...
AJFW.Functions.CreateCallback('hospital:GetDoctors', function(_, cb)
	local amount = 0
	local players = AJFW.Functions.GetAJPlayers()
	for _, v in pairs(players) do
		if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
...
AJFW.Functions.CreateCallback('hospital:GetPlayerStatus', function(_, cb, playerId)
...
AJFW.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
AJFW.Commands.Add('911e', Lang:t('info.ems_report'), { { name = 'message', help = Lang:t('info.message_sent') } }, false, function(source, args)
	local src = source
	local message
...
AJFW.Commands.Add('status', Lang:t('info.check_health'), {}, false, function(source, _)
	local src = source
	local Player = AJFW.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
		TriggerClientEvent('hospital:client:CheckStatus', src)
	else
		TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_ems'), 'error')
	end
end)

AJFW.Commands.Add('heal', Lang:t('info.heal_player'), {}, false, function(source, _)
	local src = source
	local Player = AJFW.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
...
AJFW.Commands.Add('revivep', Lang:t('info.revive_player'), {}, false, function(source, _)
	local src = source
	local Player = AJFW.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
...
AJFW.Commands.Add('revive', Lang:t('info.revive_player_a'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		...
		if Player then
			...
			TriggerClientEvent('AJFW:Notify', src, Lang:t('error.not_online'), 'error')
		end
	else
...
AJFW.Commands.Add('setpain', Lang:t('info.pain_level'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = AJFW.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
AJFW.Commands.Add('kill', Lang:t('info.kill'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = AJFW.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
AJFW.Commands.Add('aheal', Lang:t('info.heal_player_a'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = AJFW.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
AJFW.Functions.CreateUseableItem('ifaks', function(source, item)
	local src = source
	local Player = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateUseableItem('painkillers', function(source, item)
	local src = source
	local Player = AJFW.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
AJFW.Functions.CreateUseableItem('firstaid', function(source, item)
	local src = source
	local Player = AJFW.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
