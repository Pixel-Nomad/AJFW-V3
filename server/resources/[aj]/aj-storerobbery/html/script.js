...
Padlock.Close = function () {
    ...
    $.post("https://aj-storerobbery/PadLockClose");
};
...
Keypad.Close = function (data) {
    ...
    $.post("https://aj-storerobbery/PadLockClose");
    if (data.error != null) {
        $.post("https://aj-storerobbery/CombinationFail");
    }
};
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post(
        "https://aj-storerobbery/TryCombination",
        JSON.stringify({
            combination: e.value,
        })
    );
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        if (!$(".num" + (i + 1)).hasClass("found")) {
            if (angle > (comboArr[i] - numOffset) * tickAngle && angle < (comboArr[i] + numOffset) * tickAngle) {
                ...
                // on unlock
                $.post("https://aj-storerobbery/callcops");
                if (i == comboArr.length - 1) {
                    // unlock :)
                    $.post("https://aj-storerobbery/PadLockSuccess");
                    Padlock.Close();
                }
            }
            break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            if (CurrentType == "keypad") {
                ...
                $.post("https://aj-storerobbery/exit");
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post("https://aj-storerobbery/fail");
    setTimeout(function () {
        reset();
    }, 250);
}
...
function unlock() {
    gameOver = true;
    $.post("https://aj-storerobbery/success");
    solveDeg = Math.random() * 180 - 90;
    solvePadding = 4;
    maxDistFromSolve = 45;
    pinRot = 1;
