...
RegisterNetEvent('aj-storerobbery:server:takeMoney', function(register, isDone)
    local src = source
    ...
    if isDone then
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['stickynote'], 'add')
        end
    end
end)

RegisterNetEvent('aj-storerobbery:server:setRegisterStatus', function(register)
    Config.Registers[register].robbed = true
    Config.Registers[register].time = Config.resetTime
    TriggerClientEvent('aj-storerobbery:client:setRegisterStatus', -1, register, Config.Registers[register])
end)

RegisterNetEvent('aj-storerobbery:server:setSafeStatus', function(safe)
    Config.Safes[safe].robbed = true
    TriggerClientEvent('aj-storerobbery:client:setSafeStatus', -1, safe, true)

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        Config.Safes[safe].robbed = false
        TriggerClientEvent('aj-storerobbery:client:setSafeStatus', -1, safe, false)
    end)
end)

RegisterNetEvent('aj-storerobbery:server:SafeReward', function(safe)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if luck <= 10 then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['rolex'], 'add')
        if luck == odd then
            Wait(500)
            Player.Functions.AddItem('goldbar', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items['goldbar'], 'add')
        end
    end
end)

RegisterNetEvent('aj-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId
    if type == 'safe' then
    ...
    TriggerClientEvent('aj-storerobbery:client:robberyCall', -1, type, safe, streetLabel, coords)
    TriggerClientEvent('aj-phone:client:addPoliceAlert', -1, alertData)
end)

RegisterNetEvent('aj-storerobbery:server:removeAdvancedLockpick', function()
    local Player = AJFW.Functions.GetPlayer(source)
    if not Player then return end
    Player.Functions.RemoveItem('advancedlockpick', 1)
end)

RegisterNetEvent('aj-storerobbery:server:removeLockpick', function()
    local Player = AJFW.Functions.GetPlayer(source)
    if not Player then return end
...
CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            ...
            TriggerClientEvent('aj-storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
...
AJFW.Functions.CreateCallback('aj-storerobbery:server:isCombinationRight', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

AJFW.Functions.CreateCallback('aj-storerobbery:server:getPadlockCombination', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

AJFW.Functions.CreateCallback('aj-storerobbery:server:getRegisterStatus', function(_, cb)
    cb(Config.Registers)
end)

AJFW.Functions.CreateCallback('aj-storerobbery:server:getSafeStatus', function(_, cb)
    cb(Config.Safes)
end)
