...
RegisterCommand('dance', function(source, args, raw)
    if args[1] then
        TriggerEvent("nmsh-dances:dance", tonumber(args[1]))
    else
        TriggerEvent("nmsh-dances:dance", -1)
    end
end)

RegisterNetEvent('nmsh-dances:dance', function(pDance)
    if not isCuffed then
        local noAnimations = #animations

        if pDance == -1 then
            pDance = math.random(noAnimations)
            AJFW.Functions.Notify("You randomly selected dance and it is no.( "..pDance.." ) remember this if you like", 'primary', 7000)
        end

        if pDance > noAnimations or pDance <= 0 then
            AJFW.Functions.Notify("There are only"..noAnimations.." dances, select a number inbetween or random (no input)")
            return
        end

        if animations[pDance] and animations[pDance].disabled then
            AJFW.Functions.Notify("This dance is disabled", "error", 3000)
            return
        end

        AJFW.Functions.RequestAnimDict(animations[pDance].dict)
        TaskPlayAnim(GlobalPlayerPedID, animations[pDance].dict, animations[pDance].anim, 3.0, 3.0, -1, 1, 0, 0, 0, 0)
    end
end)