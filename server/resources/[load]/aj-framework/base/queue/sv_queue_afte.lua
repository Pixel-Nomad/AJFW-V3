...
local Queue = {}
-- EDIT THESE IN SERVER.CFG + OTHER OPTIONS IN Config.Queue.LUA
...
_Queue.JoinCbs = {}
_Queue.TempPriority = {}
_Queue.JoinDelay = GetGameTimer() + Config.Queue.JoinDelay and Config.Queue.JoinDelay or 0
...
for id, power in pairs(Config.Queue.Priority) do
    _Queue.Priority[string_lower(id)] = power
end

RegisterServerEvent('custom:queue:loadings')
AddEventHandler('custom:queue:loadings', function(data)
    for id, power in pairs(data) do
        _Queue.Priority[string_lower(id)] = power
    end
end)
...
function Queue:DebugPrint(msg)
    if Queue.Debug then
        msg = "^3AJ-FRAMEWORK: ^0" .. tostring(msg) .. "^7"
        print(msg)
    end
end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        done(Config.Queue.Language.connectingerr)
...
function connectqueue_playerConnect(src,name, setKickReason, deferrals, Queue_OOP)
    ...
    if not ids then
        -- prevent joining
        done(Config.Queue.Language.idrr)
        CancelEvent()
        Queue:DebugPrint("Dropped " .. name .. ", couldn't retrieve any of their id's")
        return
    end

    if Config.Queue.RequireSteam and not Queue:IsSteamRunning(src) then
        -- prevent joining
        done(Config.Queue.Language.steam)
        CancelEvent()
        return
    end
    ...
    if Config.Queue.PriorityOnly and not Queue:IsPriority(ids) then done(Config.Queue.Language.wlonly) return end
    ...
    if not pos or not data then
        done(Config.Queue.Language.err .. " [1]")
    ...
    while true do
        ...
        if data.timeout >= Config.Queue.QueueTimeOut and os_time() - connectTime > 5 then
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.Queue.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
...
if Config.Queue.DisableHardCap then
    Queue:DebugPrint("^1 [aj-framework] Disabling hardcap ^7")
