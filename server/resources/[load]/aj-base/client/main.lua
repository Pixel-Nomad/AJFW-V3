...
function AJFW.Functions.HasItem(items, amount)
    return exports['aj-inventory']:HasItem(items, amount)
end
...
function AJFW.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function AJFW.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = AJFW.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function AJFW.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function AJFW.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function AJFW.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function AJFW.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    AJFW.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function AJFW.Functions.GetCardinalDirection(entity)
...
function AJFW.Functions.GetGroundZCoord(coords)
...
RegisterNetEvent('aj-base:client:DrawText', function(text, position)
    drawText(text, position)
end)

RegisterNetEvent('aj-base:client:ChangeText', function(text, position)
    changeText(text, position)
end)

RegisterNetEvent('aj-base:client:HideText', function()
    hideText()
end)
...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
    ...
    if not AJFW.Config.Server.PVP then return end
...
RegisterNetEvent('AJFW:Client:PvpHasToggled', function(pvp_state)
...
RegisterNetEvent('AJFW:Player:UpdatePlayerData', function()
    TriggerServerEvent('AJFW:UpdatePlayer')
end)
...
CreateThread(function()
    while true do
        local sleep = 0
        if LocalPlayer.state.isLoggedIn then
            sleep = (1000 * 60) * AJFW.Config.UpdateInterval
            TriggerServerEvent('AJFW:UpdatePlayer')
        end
        Wait(sleep)
    end
...
CreateThread(function()
    while true do
        if LocalPlayer.state.isLoggedIn then
            if (AJFW.PlayerData.metadata['hunger'] <= 0 or AJFW.PlayerData.metadata['thirst'] <= 0) and not (AJFW.PlayerData.metadata['isdead'] or AJFW.PlayerData.metadata['inlaststand']) then
                local ped = PlayerPedId()
        ...
        Wait(AJFW.Config.StatusInterval)
    end
end)
